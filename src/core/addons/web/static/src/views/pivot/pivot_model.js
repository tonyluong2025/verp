...
export class PivotModel extends Model {
    ...
    async load(searchParams) {
        ...
        const activeMeasures =
            processMeasure(searchParams.context.pivotMeasures) || this.metaData.activeMeasures;
        const metaData = this._buildMetaData({ activeMeasures });
        if (!this.reload) {
            metaData.rowGroupBys =
                searchParams.context.pivotRowGroupby ||
            ...
            metaData.rowGroupBys = searchParams.groupby.length
                ...
                : searchParams.context.pivotRowGroupby || metaData.rowGroupBys;
        }
        metaData.colGroupBys =
            searchParams.context.pivotColumnGroupby || this.metaData.colGroupBys;
    ...
    _getMeasureSpecs(config) {
        ...
        return metaData.activeMeasures.reduce((acc, measure) => {
            ...
            if (field.type === "many2one") {
                field.groupOperator = "count_distinct";
            }
            if (field.groupOperator === undefined) {
                throw new Error(
                    "No aggregate function has been provided for the measure '" + measure + "'"
                );
            }
            acc.push(measure + ":" + field.groupOperator);
            return acc;
        }, []);
    }
    /**
