...
export class GraphModel extends Model {
    ...
    async load(searchParams) {
        ...
        if (!this.initialGroupBy) {
            this.initialGroupBy = searchParams.context.graphGroupbys || this.metaData.groupby; // = arch groupby --> change that
        }
    ...
    _buildMetaData(params) {
        ...
        metaData.measure = context.graphMeasure || metaData.measure;
        metaData.mode = context.graphMode || metaData.mode;
    ...
    /**
     * Separates dataPoints coming from the readGroup(s) into different
    ...
    async _loadDataPoints(metaData) {
        ...
        if (measure !== "__count") {
            let { groupOperator, type } = fields[measure];
            if (type === "many2one") {
            ...
            measures.push(`${measure}:${groupOperator}`);
        }
            ...
            proms.push(
                this.orm
                    .webReadGroup(
                        ...
                        { fillTemporal: true, ...this.searchParams.context }
                    )
                    .then((data) => {
                        const dataPoints = [];
