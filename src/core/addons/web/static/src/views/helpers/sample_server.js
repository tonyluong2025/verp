...
export class SampleServer {
    ...
    constructor(modelName, fields) {
        ...
        // webReadGroup rpc to tweak sample data s.t. those real groups
    ...
    mockRpc(params) {
        ...
        // for all models:
        // registry.category("sampleServer").add('someRoute', () => "abcd");
        // for a specific model (e.g. 'res.partner'):
        // registry.category("sampleServer").add('res.partner/someMethod', () => 23);
        const mockFunction =
            registry.category("sampleServer").get(`${params.model}/${method}`, null) ||
            registry.category("sampleServer").get(method, null);
        if (mockFunction) {
    ...
    _generateFieldValue(modelName, fieldName, id) {
        ...
        switch (field.type) {
            ...
            case "char":
            case "text":
                if (["displayName", "name"].includes(fieldName)) {
                ...
                } else if (["label", "reference"].includes(fieldName)) {
            ...
            case "many2one":
                if (field.relation === "res.currency") {
                    /** @todo return session.companyCurrencyId */
                    return 1;
                }
            ...
            case "selection": {
                ...
                if (fieldName === "activityExceptionDecoration") {
                    return false;
                }
        ...
        for (const r of model.records) {
            ...
            for (const fieldName of fieldNames) {
                ...
                if (!field) {
                    ...
                    record[fieldName] = relRecord ? [relRecord.id, relRecord.displayName] : false;
                } else {
    ...
    /**
     * Mocks calls to the readGroup method.
     *
     * @param {Object} params
     * @param {string} params.model
    ...
    /**
     * Mocks calls to the readProgressBar method.
     * @private
     * @param {Object} params
     * @param {string} params.model
    ...
    _mockReadProgressBar(params) {
        const groupby = params.kwargs.groupby;
        const progressBar = params.kwargs.progressBar;
        ...
        for (const record of this.data[params.model].records) {
            ...
            if (groupByField.type === "many2one") {
                ...
                groupByValue = relatedRecord.displayName;
            }
    ...
    /**
     * Mocks calls to the /web/dataset/searchRead route to return sample
     * records.
     * @private
     * @param {Object} params
     * @param {string} params.model
    ...
    /**
     * Mocks calls to the webReadGroup method to return groups populated
     * with sample records. Only handles the case where the real call to
     * webReadGroup returned groups, but none of these groups contain
     * records. In this case, we keep the real groups, and populate them
     * with sample records.
     * @private
     * @param {Object} params
     * @param {Object} [result] the result of a real call to webReadGroup
    ...
    /**
     * A real (web_)readGroup call has been done, and it has returned groups,
...
SampleServer.MAIN_RECORDSET_SIZE = 6;
