...
const actionHandlersRegistry = registry.category("actionHandlers");
...
const CTX_KEY_REGEX = /^(?:(?:default_|searchDefault_|show).+|.+ViewRef|groupby|groupbyNoLeaf|activeId|activeIds|orderedBy)$/;
...
function makeActionManager(env) {
    ...
    // console.log('_getActionParams', actionParams);
    if (actionParams && typeof actionParams.actionRequest === "number") {
        const { actionRequest, options } = actionParams;
        // console.log('_loadAction', actionRequest, options.additionalContext);
    ...
    async function _loadAction(actionRequest, context = {}) {
        ...
        if (typeof actionRequest === "string" || typeof actionRequest === "number") {
            ...
            const key = `${JSON.stringify(actionRequest)},${JSON.stringify(additionalContext)}`;
            if (!actionCache[key]) {
    ...
    function _preprocessAction(action, context = {}) {
        ...
        if (action.type === "ir.actions.actwindow" || action.type === "ir.actions.client") {
            action.target = action.target || "current";
        }
        if (action.type === "ir.actions.actwindow") {
            ...
            if (target !== "inline" && !(target === "new" && action.views[0][1] === "form")) {
                // FIXME: search view arch is already sent with loadAction, so either remove it
                // from there or load all fieldviews alongside the action for the sake of consistency
                const searchViewId = action.searchViewId ? action.searchViewId[0] : false;
    ...
    function _getView(viewType) {
        ...
        if (currentController.action.type !== "ir.actions.actwindow") {
        ...
        return stack
            ...
            .map((controller) => {
                return {
                    ...
                    name: controller.title || controller.action.label || env._t("Undefined"),
                };
            });
    }
    ...
    function _getActionParams() {
        ...
        if (state.action) {
            ...
            if (actionRegistry.contains(state.action)) {
                ...
                if (state.activeId) {
                    context.activeId = state.activeId;
                }
                if (state.activeIds) {
                    context.activeIds = parseActiveIds(state.activeIds);
                } else if (state.activeId) {
                    context.activeIds = [state.activeId];
                }
            ...
            if (state.id) {
                actionRequest = {
                    ...
                    views: [[state.viewId ? state.viewId : false, "form"]],
                };
            } else if (state.viewType) {
                // This is a window action on a multi-record view => restores it from
                // the session storage
                const storedAction = browser.sessionStorage.getItem("currentAction");
                const lastAction = JSON.parse(storedAction || "{}");
                if (lastAction.resModel === state.model) {
                    if (lastAction.context) {
                        // If this method is called because of a company switch, the
                        // stored allowedCompanyIds is incorrect.
                        // (Fix will be improved in master)
                        delete lastAction.context.allowedCompanyIds;
                    }
                    actionRequest = lastAction;
                    options.viewType = state.viewType;
                }
            }
        }
        // If no action => falls back on the user default action (if any).
        if (!actionRequest && env.services.user.homeActionId) {
            actionRequest = env.services.user.homeActionId;
        }
    ...
    function _getActionInfo(action, props) {
        return {
            ...
            config: {
                ...
                displayName: action.displayName || action.label || "",
                views: action.views,
            },
        };
    }
    ...
    function _getSwitchViewParams() {
        ...
        if (state.action && !actionRegistry.contains(state.action)) {
            ...
            if (
                currentController &&
                currentController.action.type === "ir.actions.actwindow" &&
                currentActionId === state.action
            ) {
                const props = { resId: state.id || false };
                const viewType = state.viewType || currentController.view.type;
    ...
    function _getViewInfo(view, action, views, props = {}) {
        ...
        if (context.searchDisableCustomFilters) {
            viewProps.activateFavorite = false;
        }

        // view specific
        if (action.resId) {
            viewProps.resId = action.resId;
        }
    ...
    async function _updateUI(controller, options = {}) {
                ...
                if (action.target === "new") {
                    ...
                    for (const c of controllerStack) {
                        if (!nextStackActionIds.includes(c.action.jsId)) {
                            if (c.action.type === "ir.actions.actwindow") {
                    ...
                    browser.sessionStorage.setItem("currentAction", action._originalAction);
                }
                resolve();
        ...
        if (action.target === "new") {
            ...
            if (action.label) {
                actionDialogProps.title = action.label;
            }
    ...
    /**
     * Executes actions of type 'ir.actions.acturl', i.e. redirects to the
     * given url.
     *
     * @private
    ...
    /**
     * Executes an action of type 'ir.actions.actwindow'.
     *
     * @private
     * @param {ActwindowAction} action
     * @param {ActionOptions} options
    ...
    function _executeActwindowAction(action, options) {
        ...
        if (!views.length) {
            throw new Error(`No view found for actwindow action ${action.id}`);
        }
    ...
    // messages that might be shown to the user dependening on the state of htmltoPdf
    const link = '<br><br><a href="http://htmltopdf.org/" target="_blank">htmltopdf.org</a>';
    const WKHTMLTOPDF_MESSAGES = {
        broken:
            env._t(
                "Your installation of HtmltoPdf seems to be broken. The report will be shown " +
                    "in html."
            ) + link,
        install:
            env._t(
                "Unable to find HtmltoPdf on this system. The report will be shown in " + "html."
            ) + link,
        upgrade:
            env._t(
                "You should upgrade your version of HtmltoPdf to at least 0.12.0 in order to " +
                    "get a correct display of headers and footers as well as support for " +
                    "table-breaking between pages."
            ) + link,
        workers: env._t(
            "You need to start Verp with at least two workers to print a pdf version of " +
                "the reports."
        ),
    };

    // only check the htmltoPdf state once, so keep the rpc promise
    let htmltoPdfStateProm;
    ...
    function _getReportUrl(action, type) {
        let url = `/report/${type}/${action.reportName}`;
        ...
        if (action.data && JSON.stringify(action.data) !== "{}") {
            ...
            if (actionContext.activeIds) {
                url += `/${actionContext.activeIds.join(",")}`;
            }
            if (type === "html") {
    ...
    async function _triggerDownload(action, options, type) {
        ...
        try {
            await download({
                url: "/report/download",
                data: {
                    data: JSON.stringify([url, action.reportType]),
        ...
        if (action.closeOnReportDownload) {
            return doAction({ type: "ir.actions.actwindow.close" }, { onClose });
        } else if (onClose) {
            onClose();
        }
    }
    ...
    function _executeReportClientAction(action, options) {
        const props = Object.assign({}, options.props, {
            ...
            reportUrl: _getReportUrl(action, "html"),
        ...
        const controller = {
            ...
            Component: actionRegistry.get("report.clientAction"),
            action,
    ...
    async function _executeReportAction(action, options) {
        ...
        if (action.reportType === "qweb-html") {
            return _executeReportClientAction(action, options);
        } else if (action.reportType === "qweb-pdf") {
            // check the state of htmltoPdf before proceeding
            if (!htmltoPdfStateProm) {
                htmltoPdfStateProm = env.services.rpc("/report/checkHtmltoPdf");
            }
            const state = await htmltoPdfStateProm;
            // display a notification according to htmltoPdf's state
        ...
        } else if (action.reportType === "qweb-text") {
            return _triggerDownload(action, options, "text");
        } else {
            console.error(
                `The ActionManager can't handle reports of type ${action.reportType}`,
                action
            );
        }
    }
    ...
    async function _executeServerAction(action, options) {
        ...
        nextAction = nextAction || { type: "ir.actions.actwindow.close" };
     ...
     * 'ir.actions.actwindow.close' is executed.
     *
    ...
    async function doActionButton(params) {
        ...
        if (params.special) {
            action = { type: "ir.actions.actwindow.close", infos: { special: true } };
        } else if (params.type === "object") {
            // call a Javascript Object method, which may return an action to execute
            ...
            const callProm = env.services.rpc("/web/dataset/callButton", {
                args,
                kwargs: { context },
                method: params.name,
            ...
            action =
                action && typeof action === "object"
                    ? action
                    : { type: "ir.actions.actwindow.close" };
        } else if (params.type === "action") {
            // execute a given action, so load it first
            context.activeId = params.resId || null;
            context.activeIds = params.resIds;
            context.activeModel = params.resModel;
        ...
        //  - wrong default_* and searchDefault_* values won't give the expected result
        //  - wrong groupby values will fail and forbid rendering of the destination view
        let currentCtx = {};
        ...
        const activeCtx = { activeModel: params.resModel };
        if (params.resId) {
            activeCtx.activeId = params.resId;
            activeCtx.activeIds = [params.resId];
        }
        action.context = makeContext([currentCtx, params.buttonContext, activeCtx, action.context]);
        // in case an effect is returned from javascript and there is already an effect
     ...
     * stack. This action must be of type 'ir.actions.actwindow'.
     *
    ...
    async function restore(jsId) {
        ...
        if (controller.action.type === "ir.actions.actwindow") {
            Object.assign(
                controller,
    ...
    function pushState(controller) {
        ...
        if (action.context) {
            const activeId = action.context.activeId;
            if (activeId) {
                newState.activeId = activeId;
            }
            const activeIds = action.context.activeIds;
            // we don't push activeIds if it's a single element array containing
            // the activeId to make the url shorter in most cases
            if (activeIds && !(activeIds.length === 1 && activeIds[0] === activeId)) {
                newState.activeIds = activeIds.join(",");
            }
        }
        if (action.type === "ir.actions.actwindow") {
            const props = controller.props;
            newState.model = props.resModel;
            newState.viewType = props.type;
...
export const actionService = {
    ...
    start(env) {
        // console.log('actionService start call makeActionManager', env);
