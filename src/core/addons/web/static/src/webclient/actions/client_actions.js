...
export const displayNotificationAction = (env, action) => {
    ...
    let links = (params.links || []).map((link) => {
        return `<a href="${escape(link.url)}" target="_blank">${escape(link.name)}</a>`;
    });
...
registry.category("actions").add("displayNotification", displayNotificationAction);
...
InvalidAction.template = owl.tags.xml`<div class="o-invalid-action"></div>`;

registry.category("actions").add("invalidAction", InvalidAction);
