...
function computeAllowedCompanyIds(cids) {
    const { userCompanies } = session;
    let allowedCompanyIds = cids || [];
    const availableCompaniesFromSession = userCompanies.allowedCompanies;
    ...
    if (!allowedCompanyIds.length || notReallyAllowedCompanies.length) {
        allowedCompanyIds = [userCompanies.currentCompany];
    }
...
export const companyService = {
    ...
    start(env, { user, router, cookie }) {
        ...
        user.updateContext({ allowedCompanyIds: allowedCompanyIds });
        const availableCompanies = session.userCompanies.allowedCompanies;
