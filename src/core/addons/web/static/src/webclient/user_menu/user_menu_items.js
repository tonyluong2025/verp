...
function documentationItem(env) {
    const documentationURL = "https://www.theverp.com/documentation/15.0";
    return {
        type: "item",
        id: "documentation",
...
export function preferencesItem(env) {
    return {
        ...
        callback: async function () {
            const actionDescription = await env.services.orm.call("res.users", "actionGet");
            actionDescription.resId = env.services.user.userId;
...
function verpAccountItem(env) {
    return {
        type: "item",
        id: "account",
        description: env._t("My theverp.com account"),
        callback: () => {
            env.services
                ...
                .catch(() => {
                    browser.location.href = "https://accounts.theverp.com/account";
                });
        },
        sequence: 60,
    };
...
registry
    .category("userMenuitems")
    .add("documentation", documentationItem)
    .add("support", supportItem)
    .add("shortcuts", shortCutsItem)
    .add("separator", separator)
    .add("profile", preferencesItem)
    .add("verpAccount", verpAccountItem)
    .add("logOut", logOutItem);
