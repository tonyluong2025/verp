...
class ActionAdapter extends ComponentAdapter {
    setup() {
        ...
        // a legacy widget widget can pushState anytime including during its async rendering
        ...
        useEffect(
            () => {
                ...
                this.wowlEnv.bus.on("ACTION_MANAGER:UPDATE", this, () => {
                    this.env.bus.trigger("closeDialogs");
    ...
    _triggerUp(ev) {
        ...
        if (ev.name === "doAction") {
            ...
            this.onReverseBreadcrumb = payload.options && payload.options.onReverseBreadcrumb;
        ...
        } else if (ev.name === "breadcrumbClicked") {
            this.actionService.restore(payload.controllerID);
        } else if (ev.name === "pushState") {
            this.pushState(payload.state);
        } else if (ev.name === "setTitlePart") {
     ...
     */
    willUnmount() {
        if (this.__widget && this.__widget.onDetachCallback) {
            this.__widget.onDetachCallback();
        }
        super.willUnmount();
    }
    __destroy() {
...
export class ViewAdapter extends ActionAdapter {
    ...
    async loadViews(resModel, context, views) {
        return (await this.vm.loadViews({ resModel, views, context }, {})).__legacy__.fieldsViews;
    }
    ...
    async _triggerUp(ev) {
        ...
        if (ev.name === "switchView") {
            try {
                ...
                props.resId = payload.resId;
                // }
                await this.actionService.switchView(payload.viewType, props);
            } catch (e) {
        ...
        } else if (ev.name === "executeAction") {
            const buttonContext = new Context(payload.actionData.context).eval();
            const envContext = new Context(payload.env.context).eval();
            wrapSuccessOrFail(
                this.actionService.doActionButton({
                    args: payload.actionData.args,
                    buttonContext: buttonContext,
                    context: envContext,
                    close: payload.actionData.close,
                    resModel: payload.env.model,
                    name: payload.actionData.name,
                    resId: payload.env.currentId || null,
                    resIds: payload.env.resIds,
                    special: payload.actionData.special,
                    type: payload.actionData.type,
                    onClose: payload.onClosed,
                    effect: payload.actionData.effect,
                }),
                payload
            );
        } else {
