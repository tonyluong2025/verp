verp.define('web.GraphModel', function (require) {
"use strict";
 ...
 * server.  It basically just do a(some) readGroup(s) and format/normalize data.
 */
...
return AbstractModel.extend({
     ...
     * Initial loading.
     *
     * @todo All the work to fall back on the graphGroupbys keys in the context
    ...
    __load: function (params) {
        var groupBys = params.context.graphGroupbys || params.groupBys;
        ...
        this.chart = Object.assign({
            ...
            measure: params.context.graphMeasure || params.measure,
            mode: params.context.graphMode || params.mode,
            origins: [],
    ...
    __reload: function (handle, params) {
        if ('context' in params) {
            ...
            this.chart.groupby = params.context.graphGroupbys || this.chart.groupby;
            this.chart.measure = params.context.graphMeasure || this.chart.measure;
            this.chart.mode = params.context.graphMode || this.chart.mode;
        }
        if ('domain' in params) {
    ...
    /**
     * Fetch and process graph data.  It is basically a(some) readGroup(s)
     ...
     * with an aggregation function, such as myDate:week.
     *
     * @private
     * @returns {Promise}
     */
    _loadGraph: function () {
        ...
        var context = _.extend({fillTemporal: true}, this.chart.context);
    ...
    /**
     * Since readGroup is insane and returns its result on different keys
     * depending of some input, we have to normalize the result.
     * Each group coming from the readGroup produces a dataPoint
     *
     * @private
     * @param {number} originIndex
     * @param {any} rawData result from the readGroup
     */
     ...
     *
     * @private
     * @param {any} value value for the field fieldName received by the readGroup rpc
