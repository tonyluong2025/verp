/** @verp-module alias=web.PivotModel **/
 ...
 * for columns.
 *
 * The measure is the field 'salesTotal'.
 *
 * Two domains are considered: 'May 2020' and 'June 2020'.
 *
 * In the model,
 *
 *      - rowGroupBys is the list [continentId, cityId]
 *      - colGroupBys is the list [saleTeamId]
 *      - measures is the list [salesTotal]
 *      - domains is the list [d1, d2] with d1 and d2 domain expressions
 *          for say saleDate in May 2020 and June 2020, for instance
 *          d1 = [['saleDate', >=, 2020-05-01], ['saleDate', '<=', 2020-05-31]]
 ...
 *
 * For example, g = {
 *      continentId: [1, 'Europe']
 *      saleTeamId: [1, 'Sale Team 1']
 *      salesCount: 25,
 *      __count: 4
 *      __domain: [
 *                  ['saleDate', >=, 2020-05-01], ['saleDate', '<=', 2020-05-31],
 *                  ['continentId', '=', 1],
 *                  ['saleTeamId', '=', 1]
 *                ]
 * }
 ...
 *
 * Five objects contain all the data from the readGroups
 *
 ...
 *              In the example:
 *                  {
 *                      "[[], []]": [{'salesTotal': 125}, {'salesTotal': 140}]                      (total/total)
 *                      ...
 *                      "[[1, 2], [2]]": [{'salesTotal': 10}, {'salesTotal': 0}]                   (Europe/Paris/Sale Team 2)
 *                      ...
 *                  }
 *
...
var PivotModel = AbstractModel.extend({
    ...
    exportData: function () {
        ...
        function processHeader(header) {
            ...
            return {
                ...
                isBold: !!header.measure && inTotalColumn
            };
        }
        ...
        return {
            colGroupHeaders: colGroupHeaderRows,
            measureHeaders: measureRow,
            originHeaders: originRow,
            rows: tableRows,
    ...
    __load: function (params) {
        ...
        this.initialRowGroupBys = params.context.pivotRowGroupby || params.rowGroupBys;
        this.defaultGroupedBy = params.groupedBy;

        this.fields = params.fields;
        this.modelName = params.modelName;
        const measures = this._processMeasures(params.context.pivotMeasures) ||
                            params.measures.map(m => m);
        this.data = {
            expandedRowGroupBys: [],
            expandedColGroupBys: [],
            domain: this.initialDomain,
            context: _.extend({}, session.userContext, params.context),
            groupedBy: params.context.pivotRowGroupby || params.groupedBy,
            colGroupBys: params.context.pivotColumnGroupby || params.colGroupBys,
            measures,
        ...
        var defaultOrder = params.defaultOrder && params.defaultOrder.split(' ');
        if (defaultOrder) {
    ...
    __reload: function (handle, params) {
        ...
        if ('context' in params) {
            ...
            this.data.colGroupBys = params.context.pivotColumnGroupby || this.data.colGroupBys;
            this.data.groupedBy = params.context.pivotRowGroupby || this.data.groupedBy;
            this.data.measures = this._processMeasures(params.context.pivotMeasures) || this.data.measures;
     ...
     * (for readGroup or other purpose)
     *
     * @private
    ...
    /**
     * Returns a promise that returns the annotated readGroup results
    ...
    _getMeasureSpecs: function () {
        ...
        return this.data.measures.reduce(
            function (acc, measure) {
                ...
                var groupOperator = self.fields[measure].groupOperator;
                if (type === 'many2one') {
    ...
    /**
     * Extract the information in the readGroup results (groupSubdivisions)
