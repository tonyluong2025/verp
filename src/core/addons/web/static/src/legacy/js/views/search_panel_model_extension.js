/** @verp-module alias=web.searchpanelModelExtension **/
    ...
    // present by default (if not explicitly stated in the "viewTypes" attribute
    // in the arch).
    ...
    class SearchPanelModelExtension extends ActionModel.Extension {
        constructor() {
            ...
            for (const key in this.config.context) {
                const match = /^searchpanelDefault_(.*)$/.exec(key);
                if (match) {
        ...
        _createSectionsFromArch() {
            ...
            this.config.archNodes.forEach(({ attrs, tag }, index) => {
                ...
                const section = {
                    ...
                    enableCounters: !!vjUtils.vjEval(
                        attrs.enableCounters || "0"
                    ),
                    expand: !!vjUtils.vjEval(attrs.expand || "0"),
                    fieldName: attrs.name,
                    icon: attrs.icon,
                    id: nextSectionId++,
                    index,
                    limit: vjUtils.vjEval(attrs.limit || String(DEFAULT_LIMIT)),
                    type,
                    values: new Map(),
                };
                if (type === "category") {
                    ...
                    section.hierarchize = !!vjUtils.vjEval(
                        attrs.hierarchize || "1"
                    );
                    section.values.set(false, {
                        childrenIds: [],
                        displayName: this.env._t("All"),
                        id: false,
                        bold: true,
                        parentId: false,
                    });
        ...
        async _fetchFilters(filters) {
            ...
            await Promise.all(filters.map(async (filter) => {
                const result = await this.env.services.rpc({
                    method: "searchPanelSelectMultiRange",
                    model: this.config.modelName,
                    args: [filter.fieldName],
                    kwargs: {
                        categoryDomain: categoryDomain,
                        comodelDomain: Domain.prototype.stringToArray(
                            filter.domain,
                            evalContext
                        ),
        ...
        _getCategoryDomain(excludedCategoryId) {
            ...
            for (const category of this.categories) {
                ...
                const operator =
                    field.type === "many2one" && category.parentField ? "childOf" : "=";
                domain.push([
                    category.fieldName,
                    operator,
                    category.activeValueId,
        ...
        static extractArchInfo(archs, viewType) {
            ...
            if (spNode) {
                ...
                const { viewTypes } = spNode.attrs;
                const _viewTypes = viewTypes ?
                    viewTypes.split(",") :
                    DEFAULT_VIEW_TYPES;
                if (_viewTypes.includes(actualType)) {
                    return {
                        attrs: spNode.attrs,
