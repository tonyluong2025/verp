/** @verp-module alias=web.AbstractController **/

/**
 * The Controller class is the class coordinating the model and the renderer.
 * It is the C in MVC, and is what was formerly known in Verp as a View.
 *
...
var AbstractController = mvc.Controller.extend(ActionMixin, {
    customEvents: _.extend({}, ActionMixin.customEvents, {
    ...
    start: async function () {
        this.$el.addClass('o-view-controller');
        this.renderButtons();
        ...
        if (this.withSearchPanel) {
            ...
            content.classList.add('o-controller-with-searchpanel');
        ...
        this.updateButtons();
        this.el.classList.toggle('o-view-sample-data', this.model.isInSampleMode());
    },
    /**
     * @override
     */
    ...
    onAttachCallback: function () {
        ActionMixin.onAttachCallback.call(this);
    ...
    onDetachCallback: function () {
        ActionMixin.onDetachCallback.call(this);
     ...
     *   'o-view-sample-data' (may be async). This allows to reload and/or
    ...
    async _removeSampleData(callback) {
        ...
        this.el.classList.remove('o-view-sample-data');
    },
    /**
     * Renders the html provided by the route specified by the
     * bannerRoute attribute on the controller (bannerRoute in the template).
     * Renders it before the view output and add a css class 'o-has-banner' to it.
    ...
    _renderBanner: async function () {
        if (this.bannerRoute !== undefined) {
            const response = await this._rpc({
                ...
                params: {context: session.userContext},
            });
            if (!response.html) {
                this.$el.removeClass('o-has-banner');
                return Promise.resolve();
            }
            this.$el.addClass('o-has-banner');
            ...
            $banner.insertBefore(this.$('> .o-content'));
    ...
    _startRenderer: function () {
        if (this.renderer instanceof owl.Component) {
            return this.renderer.mount(this.$('.o-content')[0]);
        }
        return this.renderer.appendTo(this.$('.o-content'));
    },
    /**
    ...
    /**
     * Can be used to update the key 'cpContent'. This method is called in start and _update methods.
     *
     * @private
     * @param {Object} state the state given by the model
     */
     _updateControlPanelProps(state) {
        if (!this.controlPanelProps.cpContent) {
     ...
     *   closed.
     * * if the link has a label attribute, invoke the action with that
    ...
    _onActionClicked: function (ev) { // FIXME: maybe this should also work on <button> tags?
        ...
        if (data.method !== undefined && data.model !== undefined) {
            ...
            this.dp.add(this._rpc({
                ...
                if (action !== undefined) {
                    self.doAction(action, options);
                }
            });
        } else if ($target.attr('name')) {
            this.doAction(
                $target.attr('name'),
                data.context && {additionalContext: data.context}
            );
        } else {
            this.doAction({
                label: $target.attr('title') || _.str.strip($target.text()),
                type: 'ir.actions.actwindow',
                resModel: data.model || this.modelName,
                resId: data.resId,
                target: 'current', // TODO: make customisable?
                views: data.views || (data.resId ? [[false, 'form']] : [[false, 'list'], [false, 'form']]),
                domain: data.domain || [],
            }, {
                additionalContext: _.extend({}, data.context)
            });
        }
    },
    /**
    ...
    /**
     * When an Verp event arrives requesting a record to be opened, this method
     * gets the resId, and request a switch view in the appropriate mode
     *
     * Note: this method seems wrong, it relies on the model being a basic model,
     * to get the resId.  It should receive the resId in the event data
    ...
    /**
     * Intercepts the 'switchView' event to add the controllerID into the data,
