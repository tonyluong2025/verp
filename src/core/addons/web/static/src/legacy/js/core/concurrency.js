verp.define('web.concurrency', function (require) {
"use strict";
 ...
 * everything concurrency related in Verp.
 *
 * The basic concurrency primitives in Verp JS are the callback, and the
...
return {
    ...
    /**
     * A (Verp) mutex is a primitive for serializing computations.  This is
     ...
     * rejected
     *
     * @param {Promise} [targetDef] the promise to potentially reject
     * @param {Promise} [referenceDef] the reference target
     * @returns {Promise}
     */
    rejectAfter: function (targetDef, referenceDef) {
        return new Promise(function (resolve, reject) {
            targetDef.then(resolve).guardedCatch(reject);
            referenceDef.then(reject).guardedCatch(reject);
        });
    }
};
