verp.define('web.ajax', function (require) {
"use strict";
...
function _genericJsonRpc (fctName, params, settings, fct) {
    ...
    if (!shadow) {
        core.bus.trigger('rpcRequest', data.id);
    }
    ...
    var promise = new Promise(function (resolve, reject) {
        ...
        result.then(function (result) {
            if (!shadow) {
                core.bus.trigger('rpcResponse', data.id);
            }
            resolve(result);
        }, function (reason) {
            ...
            if (type === "server") {
                ...
                if (error.code === 100) {
                    core.bus.trigger('invalidateSession');
                }
                reject({message: error, event: $.Event()});
            } else {
                if (!shadow) {
                    core.bus.trigger('rpcResponseFailed', data.id);
                }
                var nerror = {
                    code: -32098,
    ...
    promise.guardedCatch(function (reason) { // Allow promise user to disable rpcError call in case of failure
        setTimeout(function () {
            ...
            if (!reason.event.isDefaultPrevented()) {
                core.bus.trigger('rpcError', reason.message, reason.event);
            }
        }, 0);
    });
    return promise;
...
function jsonRpc(url, fctName, params, settings) {
    settings = settings || {};
    return _genericJsonRpc(fctName, params, settings, function(data) {
        return $.ajax(url, _.extend({}, settings, {
            url: url,
            dataType: 'json',
            type: 'POST',
            data: JSON.stringify(data, time.dateToUtc),
        ...
        var scriptLoadedPromise = new Promise(function (resolve, reject) {
            if (alreadyRequired) {
                ...
                script.onload = script.onreadystatechange = function() {
                    if ((script.readyState && script.readyState !== "loaded" && script.readyState !== "complete") || script.onloadDone) {
                        return;
                    }
...
function getFile(options) {
    ...
    if (core.csrfToken) {
        data.append('csrfToken', core.csrfToken);
    }
    ...
    xhr.onload = function () {
        ...
        decoder.onload = function () {
            ...
            try { // Case of a serialized Verp Exception: It is Json Parsable
                var node = nodes[1] || nodes[0];
                err = JSON.parse(node.textContent);
            } catch (e) { // Arbitrary uncaught javascript side exception
                err = {
...
function post (controllerUrl, data) {
    ...
    if (core.csrfToken) {
        postData.append('csrfToken', core.csrfToken);
    }
    ...
    return function (url, qweb) {
        function _load() {
            ...
            if (loadingsData.length) {
                ...
                loadingData.qweb.addTemplate(loadingData.url, function () {
 ...
 * @param {string} [tplRoute='/web/dataset/callKw']
...
var loadAsset = (function () {
    ...
    var load = function loadAsset(xmlid, context, tplRoute = '/web/dataset/callKw') {
        if (cache[xmlid]) {
            return cache[xmlid];
        }
        context = _.extend({}, session.userContext, context);
        const params = {
            args: [xmlid, {
        ...
        if (tplRoute === '/web/dataset/callKw') {
