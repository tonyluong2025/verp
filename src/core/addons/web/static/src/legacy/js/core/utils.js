verp.define('web.utils', function (require) {
"use strict";
 ...
 * }
 * qweb.render("someTemplate", { message: h });
 *
...
var utils = Object.assign({
     ...
     * @returns {boolean}
     */
    floatIsZero: function (value, decimals) {
        var epsilon = Math.pow(10, -decimals);
        return Math.abs(utils.roundPrecision(value, epsilon)) < epsilon;
    },
    /**
     ...
     * @returns {string}
     */
    humanNumber: function (number, decimals, minDigits, formatterCallback) {
        number = Math.round(number);
        decimals = decimals | 0;
        minDigits = minDigits || 1;
        formatterCallback = formatterCallback || utils.insertThousandSeps;
     ...
     * @returns {String}
     */
    insertThousandSeps: function (num) {
        var negative = num[0] === '-';
        num = (negative ? num.slice(1) : num);
        return (negative ? '-' : '') + utils.intersperse(
            num, _t.database.parameters.grouping, _t.database.parameters.thousandsSep);
    },
    /**
    ...
    /**
     * @param {any} node
     * @param {any} humanReadable
     * @param {any} indent
     * @returns {string}
     */
    jsonNodeToXml: function (node, humanReadable, indent) {
        // For debugging purpose, this function will convert a json node back to xml
        indent = indent || 0;
        var sindent = (humanReadable ? (new Array(indent + 1).join('\t')) : ''),
            r = sindent + '<' + node.tag,
            cr = humanReadable ? '\n' : '';
        ...
        if (node.children && node.children.length) {
            ...
            for (var i = 0, ii = node.children.length; i < ii; i++) {
                childs.push(utils.jsonNodeToXml(node.children[i], humanReadable, indent + 1));
            }
            r += childs.join(cr);
     ...
     * See the corresponding floatRound() in server/tools/float_utils.js for more info
     * @param {Number} value the value to be rounded
     * @param {Number} decimals the number of decimals. eg: roundDecimals(3.141592,2) -> 3.14
     */
    roundDecimals: function (value, decimals) {
         ...
         *
         * Such errors will propagate in roundPrecision and lead to inconsistencies between Javascript
         * and JavaScript. To avoid this, we parse the scientific notation.
         */
        return utils.roundPrecision(value, parseFloat('1e' + -decimals));
    },
    /**
    ...
    roundPrecision: function (value, precision) {
        ...
        var normalizedValue = value / precision;
        var epsilonMagnitude = Math.log(Math.abs(normalizedValue))/Math.log(2);
        var epsilon = Math.pow(2, epsilonMagnitude - 52);
         ...
         * Javascript, the method is symmetric. For example:
         * - In JS, Math.round(-0.5) is equal to -0.
         * - In Javascript, round(-0.5) is equal to -1.
         * We want to keep the Javascript behavior for consistency.
         */
        var sign = normalizedValue < 0 ? -1.0 : 1.0;
        var roundedValue = sign * Math.round(Math.abs(normalizedValue));
     ...
     * @returns boolean
     */
    isEmail: function (value, allowMailto) {
     ...
     * @param {any} f
     */
    traverseRecords: function (data, f) {
        if (data.type === 'record') {
            f(data);
        } else if (data.data) {
            for (var i = 0; i < data.data.length; i++) {
                utils.traverseRecords(data.data[i], f);
            }
        }
    },
    /**
    ...
    /**
     * @param {any} node
     * @param {any} stripWhitespace
     * @returns
     */
    xmlToJson: function (node, stripWhitespace) {
        switch (node.nodeType) {
            case 9:
                return utils.xmlToJson(node.documentElement, stripWhitespace);
            case 3:
            case 4:
                return (stripWhitespace && node.data.trim() === '') ? undefined : node.data;
            case 1:
                ...
                return {
                    ...
                    children: _.compact(_.map(node.childNodes, function (node) {
                        return utils.xmlToJson(node, stripWhitespace);
                    })),
                };
        }
    },
    ...
    assetsDomain: function () {
        return [
            ...
            '|',
            '|',
            ['label', '=like', '%.assets\_%.css'],
            ['label', '=like', '%.assets\_%.js'],
            ['label', '=like', '%.report_assets\_%.css'],
        ];
    },
}, cookieUtils);
