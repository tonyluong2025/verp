verp.define('web.BasicController', function (require) {
"use strict";
...
var BasicController = AbstractController.extend(FieldManagerMixin, {
    events: Object.assign({}, AbstractController.prototype.events, {
        'click .o-content': '_onContentClicked',
    }),
    customEvents: _.extend({}, AbstractController.prototype.customEvents, FieldManagerMixin.customEvents, {
        discardChanges: '_onDiscardChanges',
        pagerChanged: '_onPagerChanged',
        reload: '_onReload',
        resequenceRecords: '_onResequenceRecords',
        setDirty: '_onSetDirty',
        loadOptionalFields: '_onLoadOptionalFields',
        saveOptionalFields: '_onSaveOptionalFields',
    ...
    init: function (parent, model, renderer, params) {
        ...
        // record before leaving Verp (see @_urgentSave).
        this.pendingChanges = [];
    },
    /**
     * @override
    ...
    start: async function () {
        ...
        this.$el.toggleClass('o-cannot-create', !this.activeActions.create);
     ...
     * haven't been sent to the model yet, just in case we would leave Verp
     ...
     * @param {VerpEvent} e - the event that triggered the change
     * @returns {Promise}
     */
    _confirmChange: function (id, fields, e) {
        if (e.name === 'discardChanges' && e.target.reset) {
    ...
    _confirmSaveNewRecord: function () {
        ...
        var def = new Promise(function (resolve, reject) {
            ...
            var dialog = Dialog.confirm(self, message, {
                title: _t("Warning"),
                confirmCallback: resolve.bind(self, true),
    ...
    _deleteRecords: function (ids) {
        ...
        if (this.confirmOnDelete) {
            ...
            dialog = Dialog.confirm(this, message, { confirmCallback: confirmCallback });
        } else {
            doIt();
        }
    },
     ...
     *        the webclient/actionManager's hashchange mechanism is improved.
     ...
     *    or the action's id in database, or xmlid
     ...
     *     - that will be used as activeIds to load an action
    ...
    async _executeButtonAction(actionData, recordData) {
        const prom = new Promise((resolve, reject) => {
            this.triggerUp('executeAction', {
                ...
                env: recordData,
                onClosed: () => this.isDestroyed() ? Promise.resolve() : this.reload(),
                onSuccess: resolve,
                onFail: () => this.update({}, { reload: false }).then(reject).guardedCatch(reject)
            });
        });
    ...
    /**
     * To be called **only** when Verp is about to be closed, and we want to
    ...
    _onContentClicked(ev) {
        if (this.$buttons && this._shouldBounceOnClick(ev.target)) {
            this.$buttons.find('.btn-primary:visible:first').verpBounce();
        }
    },
    /**
    ...
    _onFieldChanged: function (ev) {
        if (this.mode === 'readonly' && !('forceSave' in ev.data)) {
     ...
     *
     * @todo: rename dbId into handle
     *
     * @param {VerpEvent} ev
     * @param {Object} ev.data
     * @param {string} [ev.data.dbId] handle of the data to reload and
    ...
    _onReload: function (ev) {
        ...
        var data = ev && ev.data || {};
        var handle = data.dbId;
        var prom;
        if (handle) {
            // reload the relational field given its dbId
            prom = this.model.reload(handle).then(this._confirmSave.bind(this, handle));
        } else {
            // no dbId given, so reload the main record
            prom = this.reload({
        ...
        this.triggerUp('mutexify', {
            action: async () => {
                ...
                const resIds = ev.data.recordIds
                    .map(recordId => state.data.find(d => d.id === recordId).resId);
                const options = {
    ...
    _onTranslate: async function (ev) {
        ...
        var resId = record.resId || record.resIds[0];
        var result = await this._rpc({
            route: '/web/dataset/callButton',
            params: {
                model: 'ir.translation',
                method: 'translateFields',
                args: [record.model, resId, ev.data.fieldName],
        ...
        this.translationDialog = new TranslationDialog(this, {
            ...
            searchName: result.context.searchDefault_label,
            ...
            isText: result.context.translationType === 'text',
            showSrc: result.context.translationShowSrc,
        });
