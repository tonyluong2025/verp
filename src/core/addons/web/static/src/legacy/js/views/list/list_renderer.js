/** @verp-module alias=web.ListRenderer **/
...
var FIELD_CLASSES = {
    char: 'o-list-char',
    float: 'o-list-number',
    integer: 'o-list-number',
    monetary: 'o-list-number',
    text: 'o-list-text',
    many2one: 'o-list-many2one',
};

var ListRenderer = BasicRenderer.extend({
    className: 'o-list-view',
    events: {
        "mousedown": "_onMouseDown",
        "click .o-optional-columns-dropdown .dropdown-item": "_onToggleOptionalColumn",
        "click .o-optional-columns-dropdown-toggle": "_onToggleOptionalColumnDropdown",
        'click tbody tr': '_onRowClicked',
        'change tbody .o-list-record-selector': '_onSelectRecord',
        'click thead th.o-column-sortable': '_onSortColumn',
        'click .o-list-record-selector': '_onToggleCheckbox',
        'click .o-group-header': '_onToggleGroup',
        'change thead .o-list-record-selector input': '_onToggleSelection',
    ...
    sampleDataTargets: [
        ...
        '.o-group-header',
        '.o-list-table > tfoot',
        '.o-list-table > thead .o-list-record-selector',
    ],
    /**
     * @constructor
     * @param {Widget} parent
    ...
    init: function (parent, state, params) {
        ...
        this.noOpen = params.noOpen;
    },
    /**
     * Compute columns visilibity. This can't be done earlier as we need the
     * controller to respond to the loadOptionalFields call of processColumns.
     *
     * @override
     */
    willStart: function () {
    ...
    _computeAggregates: function () {
        ...
        if (this.selection.length) {
            utils.traverseRecords(this.state, function (record) {
    ...
    _extractDecorationAttrs: function (node) {
        ...
        for (const [key, expr] of Object.entries(node.attrs)) {
            if (DECORATIONS.includes(key)) {
                ...
                decorations[cssClass] = expr; //vj.parse(vj.tokenize(expr)); Tony
            }
        }
        return decorations;
    },
    ...
    _findConnectedCell: function ($cell, direction, colIndex) {
        ...
        var $connectedCell;
        if ($connectedRow.hasClass('o-group-header')) {
            $connectedCell = $connectedRow.children();
            this.currentColIndex = colIndex;
        } else if ($connectedRow.has('td.o-group-field-row-add').length) {
            $connectedCell = $connectedRow.find('.o-group-field-row-add');
            this.currentColIndex = colIndex;
        } else {
    ...
    _getSelectableRecordCheckboxes: function () {
        return this.$('tbody .o-list-record-selector input:visible:not(:disabled)');
    },
    /**
     ...
     * "buttonGroup" nodes, with a single "buttonGroup" node for adjacent
     * "button" nodes. A "buttonGroup" node has a "children" attribute
     ...
     *   - detects oe-read-only/oe-edit-only classnames
    ...
    _preprocessColumns: function () {
        ...
        this.arch.children.filter(c => c.tag === 'buttonGroup').forEach(c => {
    ...
    _processColumns: function (columnInvisibleFields) {
        ...
        this.triggerUp('loadOptionalFields', {
        ...
        _.each(this.arch.children, function (c) {
            if (c.tag !== 'control' && c.tag !== 'groupby' && c.tag !== 'header') {
                var reject = c.attrs.modifiers.columnInvisible;
                // If there is an evaluated domain for the field we override the node
                // attribute to have the evaluated modifier value.
                if (c.tag === "buttonGroup") {
                    // FIXME: 'columnInvisible' attribute is available for fields *and* buttons,
    ...
    /**
     * Classnames "oe-edit-only" and "oe-read-only" aim to only display the cell
    ...
    _processModeClassNames: function () {
        this.arch.children.forEach(c => {
            if (c.attrs.class) {
                c.attrs.editOnly = /\boe-edit-only\b/.test(c.attrs.class);
                c.attrs.readOnly = /\boe-read-only\b/.test(c.attrs.class);
            }
        });
    },
    /**
    ...
    _renderAggregateCells: function (aggregateValues) {
        ...
        return _.map(this.columns, function (column) {
            ...
            if (column.attrs.editOnly) {
                $cell.addClass('oe-edit-only');
            }
            if (column.attrs.readOnly) {
                $cell.addClass('oe-read-only');
            }
            if (column.attrs.name in aggregateValues) {
                ...
                var formatFunc = fieldUtils.format[column.attrs.widget];
                if (!formatFunc) {
                    formatFunc = fieldUtils.format[field.type];
                }
                var formattedValue = formatFunc(value, field, {
                    escape: true,
                    digits: column.attrs.digits ? JSON.parse(column.attrs.digits) : undefined,
                });
                $cell.addClass('o-list-number').attr('title', help).html(formattedValue);
            }
            return $cell;
        });
    },
    ...
    _renderBodyCell: function (record, node, colIndex, options) {
        var tdClassName = 'o-data-cell';
        if (node.tag === 'buttonGroup') {
            tdClassName += ' o-list-button';
        } else if (node.tag === 'field') {
            tdClassName += ' o-field-cell';
            var typeClass = FIELD_CLASSES[this.state.fields[node.attrs.name].type];
            if (typeClass) {
                tdClassName += (' ' + typeClass);
            }
            if (node.attrs.widget) {
                tdClassName += (' o-' + node.attrs.widget.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase() + '-cell');
            }
        }
        if (node.attrs.editOnly) {
            tdClassName += ' oe-edit-only';
        }
        if (node.attrs.readOnly) {
            tdClassName += ' oe-read-only';
        }
    ...
    _renderButton: function (record, node) {
        ...
        if (node.attrs.icon) {
            ...
            if (!btnStyleRegex.test(nodeWithoutWidth.attrs.class)) {
                extraClass = 'btn-link o-icon-button';
            }
        }
    ...
    _renderGroupButton: function (list, node) {
        var $button = viewUtils.renderButtonFromNode(node, {
            extraClass: node.attrs.icon ? 'o-icon-button' : undefined,
        ...
        if (group.isOpen && this.groupbys[groupby]) {
            ...
            if ($buttons.length) {
                var $buttonSection = $('<div>', {
                    class: 'o-group-buttons',
                }).append($buttons);
                $th.append($buttonSection);
            }
        }
        return $('<tr>')
            .addClass('o-group-header')
            .toggleClass('o-group-open', group.isOpen)
            .toggleClass('o-group-has-content', group.count > 0)
            .data('group', group)
            .append(cells);
    },
    /**
    ...
    _renderHeaderCell: function (node) {
        ...
        if (node.attrs.editOnly) {
            $th.addClass('oe-edit-only');
        }
        if (node.attrs.readOnly) {
            $th.addClass('oe-read-only');
        }
        if (node.tag === 'buttonGroup') {
            $th.addClass('o-list-button');
        }
        ...
        if (node.attrs.widget) {
            $th.addClass(' o-' + node.attrs.widget.replace(/_/g, '-').replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase() + '-cell');
        ...
        $th.text(description)
            ...
            .addClass((field.sortable || this.state.fieldsInfo.list[name].options.allowOrder || false) && 'o-column-sortable');
    ...
    _renderRow: function (record) {
        ...
        var $tr = $('<tr/>', { class: 'o-data-row' })
            .attr('data-id', record.id)
            .append($cells);
        if (this.hasSelectors) {
            $tr.prepend(this._renderSelector('td', !record.resId));
        }
        if (this.noOpen && this.mode === "readonly") {
            $tr.addClass('o-list-no-open');
        }
    ...
    _renderOptionalColumnsDropdown: function () {
        ...
        var $optionalColumnsDropdown = $('<div>', {
            class: 'o-optional-columns text-center dropdown',
        });
        var $a = $("<a>", {
        ...
        var $dropdown = $("<div>", {
            class: 'dropdown-menu o-optional-columns-dropdown ' + dropdownMenuClass,
            role: 'menu',
        });
    ...
    _renderSelector: function (tag, disableInput) {
        ...
        return $('<' + tag + '>')
            .addClass('o-list-record-selector')
            .append($content);
    },
    /**
    ...
    async _renderView() {
        ...
        if (this.state.count > 0 || !this.noContentHelp) {
            ...
            const $table = $(
                '<table class="o-list-table table table-sm table-hover table-striped"/>'
            );
            $table.toggleClass('o-list-table-grouped', this.isGrouped);
            $table.toggleClass('o-list-table-ungrouped', !this.isGrouped);
            const defs = [];
            this.defs = defs;
            if (this.isGrouped) {
        ...
        this.el.innerHTML = "";
        this.el.classList.remove('o-list-optional-columns');
        ...
        if (tableWrapper) {
            ...
            if (document.body.contains(this.el)) {
                this.pagers.forEach(pager => pager.onAttachCallback());
            }
            if (this._shouldRenderOptionalColumnsDropdown()) {
                this.el.classList.add('o-list-optional-columns');
                this.$('table').append(
                    $('<i class="o-optional-columns-dropdown-toggle fa fa-ellipsis-v"/>')
                );
                this.$el.append(this._renderOptionalColumnsDropdown());
            }
            if (this.selection.length) {
                const $checkedRows = this.$('tr').filter(
                    (i, el) => this.selection.includes(el.dataset.id)
                );
                $checkedRows.find('.o-list-record-selector input').prop('checked', true);
                if ($checkedRows.length === this.$('.o-data-row').length) { // all rows are checked
                    this.$('thead .o-list-record-selector input').prop('checked', true);
                }
            }
        }
     ...
     * decoration-X attributes with a javascript expression as value. Then, for each
     * record, we evaluate the javascript expression, and conditionnaly add the
     ...
     *   'font-weight-bold') and values are the javascript expressions to evaluate
    ...
    _setDecorationClasses: function ($el, decorations, record) {
        for (const [cssClass, expr] of Object.entries(decorations)) {
            $el.toggleClass(cssClass, vj.VJ_isTrue(vj.evaluate(expr, record.evalContext)));
        }
    },
    /**
     * @private
    ...
    _updateSelection: function () {
        ...
        this.$('thead .o-list-record-selector input').prop('checked', allChecked);
        if (JSON.stringify(this.selection) !== previousSelection) {
            this.triggerUp('selectionchanged', { allChecked, selection: this.selection });
        }
        this._updateFooter();
    },
    ...
    _onGroupButtonClicked: function (record, node, ev) {
        ...
        if (node.attrs.type === 'edit') {
            this.triggerUp('groupEditButtonClicked', {
                record: record,
            });
        } else {
    ...
    _onToggleOptionalColumn: function (ev) {
        ...
        this.triggerUp('saveOptionalFields', {
            keyParts: this._getOptionalColumnsStorageKeyParts(),
    ...
    _onToggleOptionalColumnDropdown: function (ev) {
        ...
        ev.stopPropagation();
        this.$('.o-optional-columns .dropdown-toggle').dropdown('toggle');
        ...
        if (_t.database.parameters.direction === 'rtl') {
            position = position - this.$('.o-optional-columns .o-optional-columns-dropdown').width();
            direction = "right";
        }
        this.$('.o-optional-columns').css(direction, position);
    },
    /**
    ...
    _onKeyDown: function (ev) {
        ...
        switch (ev.keyCode) {
            case $.ui.keyCode.LEFT:
                ev.preventDefault();
                $tr = $cell.closest('tr');
                $tr.closest('tbody').addClass('o-keyboard-navigation');
                if ($tr.hasClass('o-group-header') && $tr.hasClass('o-group-open')) {
            ...
            case $.ui.keyCode.UP:
                ev.preventDefault();
                $cell.closest('tbody').addClass('o-keyboard-navigation');
            ...
            case $.ui.keyCode.ENTER:
                ev.preventDefault();
                $tr = $cell.closest('tr');
                if ($tr.hasClass('o-group-header')) {
                    this._onToggleGroup(ev);
                } else {
                    var id = $tr.data('id');
                    if (id) {
                        this.triggerUp('openRecord', { id: id, target: ev.target });
                    }
                }
                break;
        }
        if ($futureCell) {
            ...
            var isInGroupHeader = $futureCell.closest('tr').hasClass('o-group-header');
    ...
    _onMouseDown: function () {
        $('.o-keyboard-navigation').removeClass('o-keyboard-navigation');
    },
    /**
     * @private
     * @param {OwlEvent} ev
    ...
    _onRowClicked: function (ev) {
        // The specialclick property explicitely allow events to bubble all
        // the way up to bootstrap's level rather than being stopped earlier.
        if (!ev.target.closest('.o-list-record-selector') && !$(ev.target).prop('specialclick') && !this.noOpen) {
    ...
    _onSortColumn: function (ev) {
        ...
        this.triggerUp('toggleColumnOrder', { id: this.state.id, name: name });
    },
    /**
            ...
            this.triggerUp('togglegroup', {
                ...
                onSuccess: () => {
                    ...
                    if (document.activeElement.tagName === 'BODY') {
                        var groupHeaders = $('tr.o-group-header:data("group")');
                        var header = groupHeaders.filter(function () {
                            return $(this).data('group').id === group.id;
                        });
                        header.find('.o-group-name').focus();
                    }
                },
            });
        }
    ...
    _onToggleSelection: function (ev) {
        ...
        this.$('tbody .o-list-record-selector input:not(":disabled")').prop('checked', checked);
        this._updateSelection();
    },
});
