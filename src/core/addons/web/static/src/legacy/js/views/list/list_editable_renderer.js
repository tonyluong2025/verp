/** @verp-module alias=web.EditableListRenderer **/
 ...
 *
 * Unlike Verp v10 and before, this list renderer is independant from the form
...
ListRenderer.include({
    RESIZE_DELAY: 200,
    customEvents: _.extend({}, ListRenderer.prototype.customEvents, {
        navigationMove: '_onNavigationMove',
    }),
    events: _.extend({}, ListRenderer.prototype.events, {
        'click .o-field-x2many-list-row-add a': '_onAddRecord',
        'click .o-group-field-row-add a': '_onAddRecordToGroup',
        'keydown .o-field-x2many-list-row-add a': '_onKeyDownAddRecord',
        'click tbody td.o-data-cell': '_onCellClick',
        'click tbody tr:not(.o-data-row)': '_onEmptyRowClick',
        'click tfoot': '_onFooterClick',
        'click tr .o-list-record-remove': '_onRemoveIconClick',
    }),
    /**
     * @override
     * @param {Object} params
    ...
    start: function () {
        ...
        core.bus.on('domUpdated', this, () => this._freezeColumnWidths());
        return this._super();
    },
    /**
    ...
    canBeSaved: function (recordId) {
        ...
        this.$('.o-selected-row .o-data-cell').removeClass('o-invalid-cell');
        this.$('.o-selected-row .o-data-cell:has(> .o-field-invalid)').addClass('o-invalid-cell');
        return fieldNames;
    },
    /**
    ...
    confirmUpdate: function (state, id, fields, ev) {
        ...
        return this.confirmChange(state, id, fields, ev).then(function () {
            ...
            return Promise.all(defs).then(function () {
                ...
                $editedRow.nextAll('.o-data-row').remove();
                $editedRow.prevAll('.o-data-row').remove();
                var $newRow = $newBody.find('.o-data-row[data-id="' + id + '"]');
                $newRow.prevAll('.o-data-row').get().reverse().forEach(function (row) {
                    $(row).insertBefore($editedRow);
                });
                $newRow.nextAll('.o-data-row').get().reverse().forEach(function (row) {
                    $(row).insertAfter($editedRow);
                });

                // Call onAttachCallback methods on widgets that implement it
                if (self._isInDom) {
                    for (const handle in self.allFieldWidgets) {
                        if (handle !== id) {
                            self.allFieldWidgets[handle].forEach(widget => {
                                if (widget.onAttachCallback) {
    ...
    focusCell: function (recordId, column) {
        ...
        $row.find('.o-data-cell')[cellIndex].focus();
    },
    /**
    ...
    removeLine: function (state, recordId) {
        ...
        if (this.state.count >= 4) {
            ...
            const $lastDataRow = this.$('.o-data-row:last');
    ...
    setRowMode: function (recordId, mode) {
        ...
        var $tds = $row.children('.o-data-cell');
        ...
        $row.toggleClass('o-selected-row', editMode);
        if (editMode) {
        ...
        return Promise.all(defs).then(function () {
            ...
            WidgetAdapterMixin.onAttachCallback.call(self);
            ...
            if (options.canDiscard && record.isNew()) {
                ...
                if (!record.isDirty()) {
                    this.triggerUp('mutexify', {
                        action: () => {
                            return this.triggerUp('discardChanges', {
                                recordId: recordId,
                                onSuccess: resolve,
                                onFailure: reject,
                            });
    ...
    _computeDefaultWidths: function () {
        ...
        if (isListEmpty) {
            ...
            const trashHeader = this.el.getElementsByClassName('o-list-record-remove-header')[0];
            if (trashHeader) {
    ...
    _disableRecordSelectors: function () {
        this.$('.o-list-record-selector input').attr('disabled', 'disabled');
    },
    /**
     * @private
     */
    _enableRecordSelectors: function () {
        this.$('.o-list-record-selector input').attr('disabled', false);
    },
    /**
    ...
    _freezeColumnWidths: function () {
        ...
        const table = this.el.getElementsByClassName('o-list-table')[0];
    ...
    _getBorderRow: function (side) {
        let $borderDataRow = this.$(`.o-data-row:${side}`);
    ...
    _getNearestEditableRow: function ($row, next) {
        ...
        if (this.editable) {
            ...
            if (!$nearestRow.hasClass('o-data-row')) {
                var $nextBody = $row.closest('tbody')[direction]();
                while ($nextBody.length && !$nextBody.find('.o-data-row').length) {
                    $nextBody = $nextBody[direction]();
                }
                $nearestRow = $nextBody.find(`.o-data-row:${next ? 'first' : 'last'}`);
            }
        } else {
    ...
    _getRecord: function (recordId) {
        var record;
        utils.traverseRecords(this.state, function (r) {
            if (r.id === recordId) {
                record = r;
            }
        });
    ...
    _getRecordID: function (rowIndex) {
        var $tr = this.$('table.o-list-table > tbody > tr').eq(rowIndex);
        return $tr.data('id');
    },
    /**
    ...
    _getRow: function (recordId) {
        return this.$('.o-data-row[data-id="' + recordId + '"]');
    },
    /**
        ...
        this.commitChanges(recordId).then(() => {
            ...
            if (!this.isGrouped) {
                ...
                if ($nextRow.length) {
                    ...
                    this.triggerUp('discardChanges', {
                        recordId: recordId,
                        onSuccess: this.triggerUp.bind(this, 'activateNextWidget', { side: side }),
                    });
                    return;
                }
            } else {
                ...
                if (next && this.editable === "bottom" && $directNextRow.hasClass('o-add-record-row')) {
            ...
            if (nextRowIndex !== null) {
                ...
                this.unselectRow().then(this.triggerUp.bind(this, 'addRecord', {
                    groupId: groupId,
    ...
    _renderGroup: function (group) {
        ...
        if (!group.groupedBy.length && this.addCreateLineInGroups) {
            ...
            var $td = $('<td>')
                        ...
                        .addClass('o-group-field-row-add')
                        .attr('tabindex', -1)
                        .append($a);
            var $tr = $('<tr>', {class: 'o-add-record-row'})
                        .attr('data-group-id', group.id)
                        .append($td);
    ...
    _renderHeader: function () {
        ...
        if (this.addTrashIcon) {
            $thead.find('tr').append($('<th>', {class: 'o-list-record-remove-header'}));
        }
        return $thead;
    },
    /**
    ...
    _renderHeaderCell: function () {
        ...
        if ($th[0].innerHTML.length && this._hasVisibleRecords(this.state)) {
            ...
            resizeHandle.classList = 'o-resize';
    ...
    _renderRow: function (record, index) {
        ...
        if (this.addTrashIcon) {
            ...
            var $td = $('<td>', {class: 'o-list-record-remove'}).append($icon);
            $row.append($td);
        }
        return $row;
    },
    ...
    _renderRows: function () {
        ...
        if (this.addCreateLine) {
            ...
            var $td = $('<td>')
                ...
                .addClass('o-field-x2many-list-row-add');
            $tr.append($td);
            $rows.push($tr);
    ...
    _renderView: function () {
        ...
        return this._super.apply(this, arguments).then(() => {
            const table = this.el.getElementsByClassName('o-list-table')[0];
            if (table) {
                table.classList.toggle('o-empty-list', !this._hasVisibleRecords(this.state));
                this._freezeColumnWidths();
            }
        });
    },
    ...
    _squeezeTable: function () {
        ...
        table.classList.add('o-list-computing-widths');
        ...
        const sortedThs = [...thead.querySelectorAll('th:not(.o-list-button)')]
        ...
        table.classList.remove('o-list-computing-widths');
    ...
    _onAddRecord: function (ev) {
        ...
        var self = this;
        this.unselectRow().then(function () {
            self.triggerUp('addRecord', {context: ev.currentTarget.dataset.context && [ev.currentTarget.dataset.context]}); // TODO write a test, the promise was not considered
        });
    },
    /**
    ...
    _onCellClick: function (event) {
        ...
        if (!this._isRecordEditable($tr.data('id')) || $(event.target).prop('specialclick')) {
            return;
        }
        var fieldIndex = Math.max($tr.find('.o-field-cell').index($td), 0);
    ...
    _onKeyDown: function (ev) {
        ...
        if (recordEditable && ev.keyCode === $.ui.keyCode.ENTER && $tr.hasClass('o-selected-row')) {
            // enter on a textarea for example, let it bubble
            return;
        }

        if (recordEditable && ev.keyCode === $.ui.keyCode.ENTER &&
            !$tr.hasClass('o-selected-row') && !$tr.hasClass('o-group-header')) {
            ev.stopPropagation();
            ev.preventDefault();
            if ($target.closest('td').hasClass('o-group-field-row-add')) {
     ...
     * - nextLine: move to leftmost activable cell on the row below.
     *
                ...
                this.triggerUp('discardChanges', {
                    ...
                    onSuccess: function () {
                        ...
                        if (recordId) {
                            ...
                            self.$('.o-field-x2many-list-row-add a:first').focus(); // FIXME
                        }
                    }
                });
                break;
    ...
    _onRemoveIconClick: function (event) {
        ...
        var id = $row.data('id');
        if ($row.hasClass('o-selected-row')) {
            this.triggerUp('listRecordRemove', {id: id});
        } else {
            var self = this;
            this.unselectRow().then(function () {
                self.triggerUp('listRecordRemove', {id: id});
            });
        }
    },
    /**
    ...
    _onStartResize: function (ev) {
        ...
        const optionalDropdown = this.el.getElementsByClassName('o-optional-columns')[0];
        ...
        table.classList.add('o-resizing');
        resizingColumnElements.forEach(el => el.classList.add('o-column-resizing'));
        ...
        const stopResize = ev => {
            ...
            table.classList.remove('o-resizing');
            resizingColumnElements.forEach(el => el.classList.remove('o-column-resizing'));
    ...
    _onWindowClicked: function (event) {
        ...
        // listview again
        if ($(event.target).hasClass('o-facet-remove')) {
            return;
        }
        ...
        this.unselectRow({
            ...
            // it is non dirty
            canDiscard: ![...event.target.classList].includes('o-list-button-save')
        });
    },
});
