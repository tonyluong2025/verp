verp.define('web.CustomFilterItem', function (require) {
    "use strict";
     ...
     *                            [
     *                                [field, operator, (operatorValue|inputValue)]
     *                            ]
     *
    ...
    class CustomFilterItem extends Component {
        constructor() {
            ...
            this.DECIMAL_POINT = this.env._t.database.parameters.decimalPoint;
            ...
            const preFilters = this.conditions.map(condition => {
                ...
                if ('value' in operator) {
                    ...
                    domainValue = condition.value.map(
                        val => fieldUtils.parse[type](val, { type }, { timezone: true })
                    );
                    const dateValue = condition.value.map(
                        val => fieldUtils.format[type](val, { type }, { timezone: false })
                    );
                    descriptionArray.push(`"${dateValue.join(" " + this.env._t("and") + " ")}"`);
                } else if (type === "selection") {
                    domainValue = [condition.value];
                    const formattedValue = fieldUtils.format[type](condition.value, field);
        ...
        onValueChange(condition, ev) {
            ...
            if (FIELD_TYPES[type] === "number") {
                try {
                    ...
                    condition.value = fieldUtils.parse[type](ev.target.value);
