verp.define('web.data', function (require) {
"use strict";
...
var Query = Class.extend({
    ...
    _execute: function (options) {
        var self = this;
        options = options || {};
        return session.rpc('/web/dataset/searchRead', {
            model: this._model.name,
            fields: this._fields || false,
            domain: vjUtils.eval('domains',
                    [this._model.domain(this._filter)]),
            context: vjUtils.eval('contexts',
     ...
     * @returns {jQuery.Deferred<Array<verp.web.QueryGroup>> | null}
     */
    groupby: function (grouping) {
        var ctx = vjUtils.eval(
            'context', this._model.context(this._context));
        ...
        if (_.isEmpty(grouping) && !ctx.groupbyNoLeaf) {
            return null;
        }
        ...
        var self = this;
        return this._model.call('readGroup', {
            groupby: grouping,
            ...
            return _(results).map(function (result) {
                ...
                result.__context.groupby = result.__context.groupby || [];
     ...
     * query's own.
     *
     * @param {Boolean} lazy indicates if the readGroup should return only the
    ...
    orderby: function (fields) {
        ...
        return this.clone({orderby: fields});
    }
});

var QueryGroup = Class.extend({
    init: function (model, grouping_fields, read_group_group) {
        // In cases where groupbyNoLeaf and no groupby, the result of
        // readGroup has aggregate fields but no __context or __domain.
        // Create default (empty) values for those so that things don't break
        var fixed_group = _.extend(
            {__context: {groupby: []}, __domain: []},
            read_group_group);
        ...
        var leaf_group = fixed_group.__context.groupby.length === 0;
        ...
        this.attributes = {
            ...
            // if terminal group (or no group) and groupbyNoLeaf => use group.__count
            length: group_size,
            value: value,
            // A group is open-able if it's not a leaf in groupbyNoLeaf mode
            has_children: !(leaf_group && fixed_group.__context.groupbyNoLeaf),
    ...
    subgroups: function () {
        return this.model.query().groupby(this.model.context().groupby);
    },
    query: function () {
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of VERP records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the VERP model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // VERP ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // VERP ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
    ...
    read_ids: function (ids, fields, options) {
        ...
        var method = 'read';
        var ids_arg = ids;
        var context = this.getContext(options.context);
        if (options.check_access_rule === true){
            method = 'searchRead';
            ids_arg = [['id', 'in', ids]];
            context = new Context(context, {activeTest: false});
        }
     ...
     * @returns {Promise}
     */
    defaultGet: function (fields, options) {
        options = options || {};
        return this._model.call('defaultGet',
            [fields], {context: this.getContext(options.context)});
    },
    /**
     ...
     * @returns {Promise}
     */
    callButton: function (method, args) {
        return this._model.callButton(method, args);
    },
    /**
     ...
     * @returns {Promise}
     */
    nameGet: function (ids) {
        return this._model.call('nameGet', [ids], {context: this.getContext()});
    },
    /**
     *
     * @param {String} name name to perform a search for/on
     * @param {Array} [domain=[]] filters for the objects returned, VERP domain
     ...
     * @param {Function} callback function to call with nameSearch result
     * @returns {Promise}
     */
    nameSearch: function (name, domain, operator, limit) {
        return this._model.call('nameSearch', {
            name: name || '',
            args: domain || false,
     ...
     * @param name
     */
    nameCreate: function (name, context) {
        return this._model.call('nameCreate', [name], {context: this.getContext(context)});
    },
    getContext: function (request_context) {
     ...
     * an SQL-like form usable by VERP's ORM.
     *
     ...
     * Verp's ORM (such as 'name desc', 'productId', 'order_date asc')
     */
...
var DataSetSearch = DataSet.extend({
    ...
    read_slice: function (fields, options) {
        ...
        var q = this._model.query(fields || false)
            ...
            .offset(options.offset || 0)
            .limit(options.limit || false);
        q = q.orderby.apply(q, this._sort);
...
var Model = Class.extend({
    /**
    new verp.web.Model(name[, context[, domain]])

    @constructs instance.web.Model
    @extends instance.web.Class

    @param {String} name name of the VERP model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
     ...
     */
    context: function (context) {
        return new Context(session.userContext, this._context, context || {});
    },
    /**
     * Call a method (over RPC) on the bound VERP model.
     *
    ...
    call: function (method, args, kwargs, options) {
        ...
        vjUtils.ensure_evaluated(args, kwargs);
        var callKw = '/web/dataset/callKw/' + this.name + '/' + method;
        return session.rpc(callKw, {
            model: this.name,
            method: method,
            args: args,
            kwargs: kwargs
    ...
    callButton: function (method, args) {
        vjUtils.ensure_evaluated(args, {});
        // context should be the last argument
        var context = (args || []).length > 0 ? args.pop() : {};
        return session.rpc('/web/dataset/callButton', {
            model: this.name,
            method: method,
            args: args || [],
