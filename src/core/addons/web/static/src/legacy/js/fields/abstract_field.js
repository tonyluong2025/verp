verp.define('web.AbstractField', function (require) {
"use strict";
 ...
 *   triggering events (with triggerUp).  These events bubble up and are interpreted
...
var AbstractField = Widget.extend({
    ...
    /**
     * List of selectors matching elements that must never trigger the 'quickEdit'
    ...
    */
    ...
    init: function (parent, name, record, options) {
        ...
        // this is the resId for the record in database.  Obviously, it is
        // readonly.  Also, when the user is creating a new record, there is
        // no resId.  When the record will be created, the field widget will
        // be destroyed (when the form view switches to readonly mode) and a new
        // widget with a resId in mode readonly will be created.
        ...
        this.formatType = this.attrs.widget in fieldUtils.format ?
                            this.attrs.widget :
                            this.field.type;
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            ...
            self.$el.addClass('o-field-widget');
            self.$el.toggleClass('o-quick-editable', self._canQuickEdit);
     ...
     *
     * @param {any} record
     * @param {VerpEvent} [event] an event that triggered the reset action. It
    ...
    removeInvalidClass: function () {
        this.$el.removeClass('o-field-invalid');
    ...
    setInvalidClass: function () {
        this.$el.addClass('o-field-invalid');
    ...
    _applyDecorations: function () {
        ...
        this.attrs.decorations.forEach(function (dec) {
            var isToggled = vj.VJ_isTrue(
            vj.evaluate(dec.expression, self.record.evalContext)
            );
    ...
    _formatValue: function (value, formatType) {
        ...
        return fieldUtils.format[formatType](value, this.field, options);
    },
    /**
    ...
    _parseValue: function (value) {
        return fieldUtils.parse[this.formatType](value, this.field, this.parseOptions);
    },
    /**
     ...
     * @private
     * @param {any} record
     * @param {VerpEvent} event the event that triggered the change
     */
    ...
    _setValue: function (value, options) {
        ...
        try {
            ...
            this._isValid = false;
            this.triggerUp('setDirty', {dataPointID: this.dataPointID});
     ...
     * abstract field does not trigger the navigationMove event a second time.
    ...
    _onKeydown: function (ev) {
        switch (ev.which) {
            case $.ui.keyCode.TAB:
                var event = this.triggerUp('navigationMove', {
            ...
            case $.ui.keyCode.ENTER:
                ...
                ev.preventDefault();
                ev.stopPropagation();
                this.triggerUp('navigationMove', {direction: 'nextLine'});
                break;
            case $.ui.keyCode.ESCAPE:
                this.triggerUp('navigationMove', {direction: 'cancel', originalEvent: ev});
                break;
            case $.ui.keyCode.UP:
                ev.stopPropagation();
            ...
            case $.ui.keyCode.RIGHT:
                ev.stopPropagation();
                this.triggerUp('navigationMove', {direction: 'right'});
                break;
            case $.ui.keyCode.DOWN:
                ev.stopPropagation();
                this.triggerUp('navigationMove', {direction: 'down'});
                break;
            case $.ui.keyCode.LEFT:
                ev.stopPropagation();
                this.triggerUp('navigationMove', {direction: 'left'});
                break;
        }
    },
    /**
