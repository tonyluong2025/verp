verp.define('web.FormRenderer', function (require) {
"use strict";
...
var FormRenderer = BasicRenderer.extend({
    ...
    events: _.extend({}, BasicRenderer.prototype.events, {
        'click .o-notification-box .oe-field-translate': '_onTranslate',
        'click .o-notification-box .close': '_onTranslateNotificationClose',
        'shown.bs.tab a[data-toggle="tab"]': '_onNotebookTabChanged',
        'click .o-form-label': '_onFieldLabelClicked',
    }),
    ...
    /**
     * Focuses the field having attribute 'defaultFocus' set, if any, or the
     * first focusable field otherwise.
     * In read mode, delegate which button to give the focus to, to the formRenderer
     *
    ...
    /**
     * Extend the method so that labels also receive the 'o-field-invalid' class
     * if necessary.
     *
     * @override
    ...
    canBeSaved: function () {
        ...
        $labels.removeClass('o-field-invalid');
    ...
    updateAlertFields: function (alertFields) {
        if (this.state.resId) {
            this.alertFields[this.state.resId] = _.extend(this.alertFields[this.state.resId] || {}, alertFields);
    ...
    displayTranslationAlert: function () {
        this.$('.o-notification-box').remove();
        if (this.alertFields[this.state.resId]) {
            var $notification = $(qweb.render('notification-box', {type: 'info'}))
                .append(qweb.render('translation-alert', {
                    fields: this.alertFields[this.state.resId],
                    lang: _t.database.parameters.name
                }));
            if (this.$('.o-form-statusbar').length) {
                this.$('.o-form-statusbar').after($notification);
            } else if (this.$('.o-form-sheet-bg').length) {
                this.$('.o-form-sheet-bg').prepend($notification);
            } else {
    ...
    confirmChange: function () {
        ...
        return this._super.apply(this, arguments).then(function (resetWidgets) {
            ...
            if (self.$('.o-field-invalid').length) {
    ...
    disableButtons: function () {
        const allButtons = this.$el[0].querySelectorAll('.o-statusbar-buttons button, .oe-button-box button');
    ...
    getLocalState: function () {
        const state = {};
        const sheetBg = this.el.querySelector('.o-form-sheet-bg');
        if (sheetBg) {
            state.scrollValue = sheetBg.scrollTop;
        }
        for (const notebook of this.el.querySelectorAll(':scope div.o-notebook')) {
            const name = notebook.dataset.name;
            const navs = notebook.querySelectorAll(':scope .o-notebook-headers .nav-item > .nav-link');
    ...
    resetLocalState() {
        for (const notebook of this.el.querySelectorAll(':scope div.o-notebook')) {
            [...notebook.querySelectorAll(':scope .o-notebook-headers .nav-item .nav-link')]
    ...
    setLocalState: function (state) {
        for (const notebook of this.el.querySelectorAll(':scope div.o-notebook')) {
            if (notebook.closest(".o-field-widget")) {
                continue;
            }
            const name = notebook.dataset.name;
            if (name in state) {
                const navs = notebook.querySelectorAll(':scope .o-notebook-headers .nav-item');
                ...
                if (navs[activeIndex].classList.contains('o-invisible-modifier')) {
                    activeIndex = [...navs].findIndex(
                        nav => !nav.classList.contains('o-invisible-modifier')
                    );
                }
                if (activeIndex <= 0) {
    ...
    _activateFirstVisibleTab(tabs) {
        const visibleTabs = tabs.filter(
            (tab) => !tab.$header.hasClass("o-invisible-modifier")
        );
    ...
    _activateNextFieldWidget: function (record, currentIndex) {
        ...
        if ( (currentIndex + 1) >= (this.allFieldWidgets[record.id] || []).length) {
            this.triggerUp('focusControlButton');
            this.lastActivatedFieldIndex = -1;
        } else {
    ...
    _addOnClickAction: function ($el, node) {
        if (node.attrs.special || node.attrs.confirm || node.attrs.type || $el.hasClass('oe-stat-button')) {
            var self = this;
            $el.on("click", function () {
                self.triggerUp('buttonclicked', {
                    attrs: node.attrs,
                    record: self.state,
                });
            });
    ...
    _applyFormSizeClass: function () {
        ...
        if (config.device.sizeClass <= config.device.SIZES.XS) {
            formEl.classList.add('o-xxs-form-view');
        } else {
            formEl.classList.remove('o-xxs-form-view');
        }
        if (config.device.sizeClass === config.device.SIZES.XXL) {
            formEl.classList.add('o-xxl-form-view');
        } else {
            formEl.classList.remove('o-xxl-form-view');
        }
    },
    /**
     * @private
    ...
    _getIDForLabel: function (uid) {
        if (!this.idsForLabels[uid]) {
            this.idsForLabels[uid] = _.uniqueId('o-field-input_');
        }
    ...
    _renderButtonBox: function (node) {
        var self = this;
        var $result = $('<' + node.tag + '>', {class: 'o-not-full'});
        ...
        Promise.all(defs).then(function () {
            ...
            var buttonsPartition = _.partition(buttons, function (button) {
                return $(button).is('.o-invisible-modifier');
            });
            var invisibleButtons = buttonsPartition[0];
            var visibleButtons = buttonsPartition[1];

            // Get the unfolded buttons according to window size
            var nbButtons = self._renderButtonBoxNbButtons();
            var unfoldedBbuttons = visibleButtons.slice(0, nbButtons).concat(invisibleButtons);

            // Get the folded buttons
            var foldedButtons = visibleButtons.slice(nbButtons);
            if (foldedButtons.length === 1) {
                unfoldedBbuttons = buttons;
                foldedButtons = [];
            }

            // Toggle class to tell if the button box is full (CSS requirement)
            var full = (visibleButtons.length > nbButtons);
            $result.toggleClass('o-full', full).toggleClass('o-not-full', !full);

            // Add the unfolded buttons
            _.each(unfoldedBbuttons, function (button) {
                $(button).appendTo($result);
            });

            // Add the dropdown with folded buttons if any
            if (foldedButtons.length) {
                $result.append(dom.renderButton({
                    attrs: {
                        'class': 'oe-stat-button o-button-more dropdown-toggle',
                        'data-toggle': 'dropdown',
                    },
                    text: _t("More"),
                }));

                var $dropdown = $("<div>", {class: "dropdown-menu o-dropdown-more", role: "menu"});
                _.each(foldedButtons, function (button) {
    ...
    */
    _renderButtonBoxNbButtons: function () {
        return [2, 2, 2, 4][config.device.sizeClass] || 7;
    },
    /**
    ...
    _renderHeaderButton: function (node) {
        ...
        // Current API of verp for rendering buttons is "if classes are given
    ...
    _renderInnerGroup: function (node) {
        var self = this;
        var $result = $('<table/>', {class: 'o-group o-inner-group'});
        ...
        if (node.attrs.string) {
            var $sep = $('<tr><td colspan="' + col + '" style="width: 100%;"><div class="o-horizontal-separator">' + node.attrs.string + '</div></td></tr>');
            $result.append($sep);
        }
        ...
        node.children.forEach(function (child) {
            ...
            if (child.tag === 'field') {
                ...
                if ($child.hasClass('o-td-label')) { // transfer classname to outer td for css reasons
                    $td.addClass('o-td-label');
                    $child.removeClass('o-td-label');
                }
                $tds = $td.append($child);
            }
            if (finalColspan > 1) {
        ...
        _.each(rows, function ($tr) {
            var nonLabelColSize = 100 / (col - $tr.children('.o-td-label').length);
            _.each($tr.children(':not(.o-td-label)'), function (el) {
                var $el = $(el);
    ...
    _renderInnerGroupField: function (node) {
        ...
        if (node.attrs.nolabel !== '1') {
            ...
            if (/\boe-edit-only\b/.test(node.attrs.class)) {
                $tds.addClass('oe-edit-only');
            }
            if (/\boe-read-only\b/.test(node.attrs.class)) {
                $tds.addClass('oe-read-only');
            }
        }
    ...
    _renderNode: function (node) {
        ...
        if (node.tag === 'div' && node.attrs.name === 'buttonBox') {
    ...
    _renderOuterGroup: function (node) {
        ...
        $result.append(_.map(node.children, function (child) {
            ...
            var $child = self._renderNode(child);
            $child.addClass('o-group-col-' + (colSize * (parseInt(child.attrs.colspan, 10) || 1)));
            return $child;
        }));
        this._handleAttributes($result, node);
        this._registerModifiers(node, this.state, $result);
    ...
    _renderStatusbarButtons: function (buttons) {
        var $statusbarButtons = $('<div>', {class: 'o-statusbar-buttons'});
     ...
     * @returns {jQueryElement}
     */
    _renderTabHeader: function (page, pageId) {
        var $a = $('<a>', {
            'data-toggle': 'tab',
            disableAnchor: 'true',
            href: '#' + pageId,
     ...
     * @returns {jQueryElement}
     */
    _renderTabPage: function (page, pageId) {
        var $result = $('<div class="tab-pane" id="' + pageId + '">');
    ...
    _renderTagHeader: function (node) {
        var self = this;
        var $statusbar = $('<div>', {class: 'o-form-statusbar'});
    ...
    _renderTagLabel: function (node) {
        ...
        if (fieldName) {
            modifiersOptions = {
                callback: function (element, modifiers, record) {
                    ...
                    element.$el.toggleClass('o-form-label-empty', !!( // FIXME condition is evaluated twice (label AND widget...)
                        record.data.id
        ...
        // this worked in 10.0 for "o-form-label-empty" reevaluation but not for
        // "o-invisible-modifier" reevaluation on labels...
    ...
    _renderTagNotebook: function (node) {
        ...
        var renderedTabs = _.map(node.children, function (child, index) {
            var pageID = _.uniqueId('notebookPage_');
            ...
            self._registerModifiers(tab.node, self.state, tab.$header, {
                callback: function (element, modifiers) {
                    ...
                    if (!modifiers.invisible) {
                        ...
                        var $visibleTabs = $headers.find('li:not(.o-invisible-modifier)');
                        if ($visibleTabs.length === 1) {
                            self.inactiveNotebooks.push(renderedTabs);
                        }
                    }
                    // if all pages are invisible then hide notebooks header's ul as well
                    $headers.toggleClass('o-invisible-modifier', !$headers.find('li:not(.o-invisible-modifier)').length);
                },
            });
        });
        this._activateFirstVisibleTab(renderedTabs);
        var $notebookHeaders = $('<div class="o-notebook-headers">').append($headers);
        var $notebook = $('<div class="o-notebook">').append($notebookHeaders, $pages);
    ...
    _renderTagSeparator: function (node) {
        var $separator = $('<div/>').addClass('o-horizontal-separator').text(node.attrs.string);
    ...
    _renderTagSheet: function (node) {
        this.hasSheet = true;
        var $sheet = $('<div>', {class: 'clearfix position-relative o-form-sheet'});
    ...
    _renderView: function () {
        ...
        return Promise.all(defs).then(() => this.__renderView()).then(function () {
            ...
            if (self.state.resId in self.alertFields) {
    ...
    _updateView: function ($newContent) {
        ...
        this.$el.toggleClass('o-form-nosheet', !this.hasSheet);
        if (this.hasSheet) {
            this.$el.children().not('.o-form-renderer-chatter-container')
                .wrapAll($('<div/>', {class: 'o-form-sheet-bg', 'data-command-category': "app"}));
        }
        this.$el.toggleClass('o-form-editable', this.mode === 'edit');
        this.$el.toggleClass('o-form-readonly', this.mode === 'readonly');
        ...
        _.each(this.allFieldWidgets[this.state.id], function (widget) {
            ...
            var $label = idForLabel ? self.$('.o-form-label[for=' + idForLabel + ']') : $();
            self._addFieldTooltip(widget, $label);
            if (widget.attrs.widget === 'upgradeBoolean') {
    ...
    _onAnchorLinkClicked(ev) {
        ...
        function _getNotebookParent() {
            const notebook = anchor.closest(".o-notebook");
    ...
    /**
     * Listen to notebook tab changes and trigger a domUpdated event such that
    ...
    _onTranslateNotificationClose: function(ev) {
        delete this.alertFields[this.state.resId];
    },
});
