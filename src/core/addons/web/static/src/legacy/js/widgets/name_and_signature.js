verp.define('web.nameAndSignature', function (require) {
'use strict';
...
var NameAndSignature = Widget.extend({
    template: 'web.signNameAndSignature',
    xmlDependencies: ['/web/static/src/legacy/xml/name_and_signature.xml'],
    events: {
        // name
        'input .o-web-sign-name-input': '_onInputSignName',
        // signature
        'click .o-web-sign-signature': '_onClickSignature',
        'change .o-web-sign-signature': '_onchangeSignature',
        // draw
        'click .o-web-sign-draw-button': '_onClickSignDrawButton',
        'click .o-web-sign-draw-clear a': '_onClickSignDrawClear',
        // auto
        'click .o-web-sign-auto-button': '_onClickSignAutoButton',
        'click .o-web-sign-auto-select-style a': '_onClickSignAutoSelectStyle',
        'click .o-web-sign-auto-font-selection a': '_onClickSignAutoFontSelection',
        'mouseover .o-web-sign-auto-font-selection a': '_onMouseOverSignAutoFontSelection',
        'touchmove .o-web-sign-auto-font-selection a': '_onTouchStartSignAutoFontSelection',
        // load
        'click .o-web-sign-load-button': '_onClickSignLoadButton',
        'change .o-web-sign-load-file input': '_onchangeSignLoadInput',
    },
    ...
    willStart: function () {
        ...
        return Promise.all([
            this._super.apply(this, arguments),
            this._rpc({route: '/web/sign/getFonts/' + self.defaultFont}).then(function (data) {
                self.fonts = data;
            })
        ]);
    },
    ...
    start: function () {
        ...
        this.$signatureGroup = this.$('.o-web-sign-signature-group');
        this.$signatureField = this.$('.o-web-sign-signature');
        this.$nameInput = this.$('.o-web-sign-name-input');
        this.$nameInputGroup = this.$('.o-web-sign-name-group');

        // mode selection buttons
        this.$drawButton = this.$('a.o-web-sign-draw-button');
        this.$autoButton = this.$('a.o-web-sign-auto-button');
        this.$loadButton = this.$('a.o-web-sign-load-button');

        // mode: draw
        this.$drawClear = this.$('.o-web-sign-draw-clear');

        // mode: auto
        this.$autoSelectStyle = this.$('.o-web-sign-auto-select-style');
        this.$autoFontSelection = this.$('.o-web-sign-auto-font-selection');
        this.$autoFontList = this.$('.o-web-sign-auto-font-list');
        ...
        // mode: load
        this.$loadFile = this.$('.o-web-sign-load-file');
        this.$loadInvalid = this.$('.o-web-sign-load-invalid');
        ...
        $(window).on('resize.o-web-sign-name-and-signature', _.debounce(function () {
            if (self.isDestroyed()) {
    ...
    destroy: function () {
        this._super.apply(this, arguments);
        $(window).off('resize.o-web-sign-name-and-signature');
    },
    ...
    validateSignature: function () {
        ...
        this.$nameInput.parent().toggleClass('o-has-error', !name)
    ...
    _getSVGText: function (font, text, width, height) {
        var $svg = $(core.qweb.render('web.signSvgText', {
            width: width,
            height: height,
            font: font,
            text: text,
    ...
    _printImage: function (imgSrc) {
        ...
        image.onload = function () {
            ...
            self.drawTimeout = setTimeout(function () {
                ...
                self.triggerUp('signatureChanged');
            }, 0);
        };
        image.src = imgSrc;
    },
    ...
    _waitForSignatureNotEmpty: function (def) {
        ...
        if (!this.isSignatureEmpty()) {
            this.triggerUp('signatureChanged');
            def.resolve();
        } else {
