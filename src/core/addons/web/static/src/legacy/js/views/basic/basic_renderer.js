verp.define('web.BasicRenderer', function (require) {
"use strict";
...
var BasicRenderer = AbstractRenderer.extend(WidgetAdapterMixin, {
    ...
    onAttachCallback: function () {
        this._isInDom = true;
        // call onAttachCallback on field widgets
        for (const handle in this.allFieldWidgets) {
            this.allFieldWidgets[handle].forEach(widget => {
                if (!utils.isComponent(widget.constructor) && widget.onAttachCallback) {
                    widget.onAttachCallback();
                }
            });
        }
        // call onAttachCallback on widgets
        this.widgets.forEach(widget => {
            if (widget.onAttachCallback) {
                widget.onAttachCallback();
            }
        });
        // call onAttachCallback on child components (including field components)
    ...
    onDetachCallback: function () {
        this._isInDom = false;
        // call onDetachCallback on field widgets
        for (const handle in this.allFieldWidgets) {
            this.allFieldWidgets[handle].forEach(widget => {
                if (!utils.isComponent(widget.constructor) && widget.onDetachCallback) {
                    widget.onDetachCallback();
                }
            });
        }
        // call onDetachCallback on widgets
        this.widgets.forEach(widget => {
            if (widget.onDetachCallback) {
                widget.onDetachCallback();
            }
        });
        // call onDetachCallback on child components (including field components)
     ...
     * toggling the o-form-invalid css class. It has to be done both on the
    ...
    canBeSaved: function (recordId) {
        ...
        _.each(this.allFieldWidgets[recordId], function (widget) {
            ...
            if (widget.el) { // widget may not be started yet
                widget.$el.toggleClass('o-field-invalid', !canBeSaved);
    ...
    _applyModifiers: function (modifiersData, record, element) {
        ...
        function _apply(element) {
            ...
            element.$el.toggleClass("o-invisible-modifier", !!modifiers.invisible);
            element.$el.toggleClass("o-readonly-modifier", !!modifiers.readonly);
            element.$el.toggleClass("o-required-modifier", !!modifiers.required);
    ...
    _getTooltipOptions: function (widget) {
        return {
            ...
            title: function () {
                ...
                if (session.displaySwitchCompanyMenu && widget.field.companyDependent) {
    ...
    _moveRecord: function (recordId, toIndex) {
        ...
        this.triggerUp('resequenceRecords', {
        ...
        def.then(function () {
            ...
            self._registerModifiers(node, record, widget, {
                callback: function (element, modifiers, record) {
                    element.$el.toggleClass('o-field-empty', !!(
                        record.data.id &&
    ...
    _rerenderFieldWidget: function (widget, record, options) {
        ...
        def.then(() => {
            ...
            if (this._isInDom && newWidget.onAttachCallback) {
                newWidget.onAttachCallback();
            }
        });
    },
    /**
