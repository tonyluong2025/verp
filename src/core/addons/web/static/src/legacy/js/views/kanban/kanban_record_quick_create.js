verp.define('web.kanbanRecordQuickCreate', function (require) {
"use strict";
...
var RecordQuickCreate = Widget.extend({
    className: 'o-kanban-quick-create',
    customEvents: {
        add: '_onAdd',
        cancel: '_onCancel',
    },
    events: {
        'click .o-kanban-add': '_onAddClicked',
        'click .o-kanban-edit': '_onEditClicked',
        'click .o-kanban-cancel': '_onCancelClicked',
    ...
    willStart: function () {
        ...
        if (this.formViewRef) {
            ...
            var context = _.extend({}, this.context, {
                formViewRef: this.formViewRef,
            });
            viewsLoaded = this.loadViews(this.model, context, views);
        } else {
            var fieldsView = {};
            fieldsView.arch = '<form>' +
                '<field name="displayName" placeholder="Title" modifiers=\'{"required": true}\'/>' +
            '</form>';
            var fields = {
    ...
    _add: function (options) {
        ...
        this.controller.commitChanges().then(function () {
            ...
            if (canBeSaved) {
                self.triggerUp('quickCreateAddRecord', {
     ...
     */
    _cancel: function () {
        this.triggerUp('cancelQuickCreate');
    },
    /**
    ...
    _disableQuickCreate: function () {
        ...
        this.$el.addClass('o-disabled');
        this.$('input:not(:disabled)')
            .addClass('o-temporarily-disabled')
    ...
    _enableQuickCreate: function () {
        ...
        this.$el.removeClass('o-disabled');
        this.$('input.o-temporarily-disabled')
            .removeClass('o-temporarily-disabled')
            .attr('disabled', false);
    },
    ...
    _onWindowClicked: function (ev) {
        ...
        if ($(ev.target).closest('.o-kanban-button-new, .o-kanban-quick-add').length) {
            return;
        }
