verp.define('web.Colorpicker', function (require) {
'use strict';
...
var ColorpickerWidget = Widget.extend({
    ...
    events: {
        ...
        'mousedown .o-color-pick-area': '_onMouseDownPicker',
        'mousedown .o-color-slider': '_onMouseDownSlider',
        'mousedown .o-opacity-slider': '_onMouseDownOpacitySlider',
     ...
     */
    start: function () {
        this.$colorpickerArea = this.$('.o-color-pick-area');
        this.$colorpickerPointer = this.$('.o-picker-pointer');
        this.$colorSlider = this.$('.o-color-slider');
        this.$colorSliderPointer = this.$('.o-slider-pointer');
        this.$opacitySlider = this.$('.o-opacity-slider');
        this.$opacitySliderPointer = this.$('.o-opacity-pointer');
        ...
        Object.entries(this.colorComponents).forEach(([component, value]) => {
            const input = this.el.querySelector(`.o-${component.replace(/_/g, '-').replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase()}-input`);
            if (input) {
                input.value = value;
            }
        });
    ...
    _updateUI: function () {
        ...
        _.each(this.colorComponents, function (value, color) {
            self.$(_.str.sprintf('.o-%s-input', color.replace(/_/g, '-'))).val(value);
        });

        // Update preview
        this.$('.o-color-preview').css('background-color', this.colorComponents.cssColor);
    ...
    _colorSelected: function () {
        this.triggerUp('colorpickerSelect', this.colorComponents);
    },
    /**
    ...
    _updateCssColor: function () {
        ...
        if (this.previewActive) {
            this.triggerUp('colorpickerPreview', this.colorComponents);
        }
    },
    ...
    _onKeypress: function (ev) {
        if (ev.charCode === $.ui.keyCode.ENTER) {
            ...
            ev.preventDefault();
            this.triggerUp('enterKeyColorColorpicker');
        }
    },
    /**
     * @param {Event} ev
    ...
    _onClick: function (ev) {
        ...
        $(ev.target).find('> .o-opacity-pointer, > .o-slider-pointer, > .o-picker-pointer').addBack('.o-opacity-pointer, .o-slider-pointer, .o-picker-pointer').focus();
    ...
    _onchangeInputs: function (ev) {
        switch ($(ev.target).data('colorMethod')) {
            ...
            case 'hsl':
                ...
                break;
            case 'opacity':
                this._updateOpacity(parseInt(this.$('.o-opacity-input').val()));
                break;
        }
        this._updateUI();
        this._colorSelected();
...
const ColorpickerDialog = Dialog.extend({
    ...
    _onFinalPick: function () {
        this.triggerUp('colorpicker:saved', this.colorPicker.colorComponents);
    },
});
