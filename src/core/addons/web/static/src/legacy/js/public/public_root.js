/** @verp-module alias=web.public.root */

import ajax from 'web.ajax';
import dom from 'web.dom';
import env from 'web.publicEnv';
...
import legacyEnv from "web.publicEnv";
...
var lang = utils.getCookie('frontend_lang') || getLang(); // FIXME the cookie value should maybe be in the ctx?
...
export const PublicRoot = publicWidget.RootWidget.extend({
    events: _.extend({}, publicWidget.RootWidget.prototype.events || {}, {
        'submit .js-website-submit-form': '_onWebsiteFormSubmit',
        'click .js-disable-on-click': '_onDisableOnClick',
    }),
    customEvents: _.extend({}, publicWidget.RootWidget.prototype.customEvents || {}, {
        callService: '_onCallService',
        contextGet: '_onContextGet',
        mainObjectRequest: '_onMainObjectRequest',
        widgetsStartRequest: '_onWidgetsStartRequest',
        widgetsStopRequest: '_onWidgetsStopRequest',
    }),
    ...
    _getRegistry: function () {
        return registry.category("publicRootWidgets");
    },
    /**
    ...
    _onCallService: function (ev) {
        ...
        if (payload.service === 'ajax' && payload.method === 'rpc') {
            ...
            var route = args[0];
            if (_.str.startsWith(route, '/web/dataset/callKw/')) {
                var params = args[1];
                var options = args[2];
                var noContextKeys;
                if (options) {
...
export async function createPublicRoot(RootWidget) {
    ...
    // to the wowl env
    serviceRegistry.add("legacyRpc", makeLegacyRpcService(legacyEnv));
    ...
    serviceRegistry.add("legacyDialogMapping", makeLegacyDialogMappingService(legacyEnv));
    serviceRegistry.add("legacyRainbowmanService", makeLegacyRainbowManService(legacyEnv));
    await Promise.all([owl.utils.whenReady(), session.isBound]);
    ...
    wowlEnv.qweb.addTemplates(await loadBundleTemplates("web.assetsFrontend"));
    await startServices(wowlEnv);
    mapLegacyEnvToWowlEnv(legacyEnv, wowlEnv);

    const adapter = new ComponentAdapter(null, { Component }); // Used for _triggerUp compat layer
