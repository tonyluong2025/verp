verp.define('web.fieldUtils', function (require) {
"use strict";
...
/**
 * Convert binary to binSize
 *
 * @param {string} [value] base64 representation of the binary (might be already a binSize!)
 * @param {Object} [field]
 *        a description of the field (note: this parameter is ignored)
 * @param {Object} [options] additional options (note: this parameter is ignored)
 *
 * @returns {string} binSize (which is human-readable)
 */
 ...
 * @param {Object} [field] a description of the field (returned by fieldsGet
 ...
 *   javascript description of the field.
 ...
 *   formatFloat acts like utils.humanNumber
 * @returns {string}
 */
function formatFloat(value, field, options) {
    ...
    if (options.humanReadable && options.humanReadable(value)) {
        return utils.humanNumber(value, options.decimals, options.minDigits, options.formatterCallback);
    }
...
function formatInteger(value, field, options) {
    ...
    return utils.insertThousandSeps(_.str.sprintf('%d', value));
}
...
function formatMany2one(value, field, options) {
    if (!value) {
        ...
        // value is a datapoint, so we read its displayName field, which
        // may in turn be a datapoint (if the name field is a many2one)
        while (value.data) {
            value = value.data.displayName || '';
        }
    }
    if (options && options.escape) {
        value = _.escape(value);
 ...
 * @param {float|false} value the value that should be formatted
 * @param {Object} [field]
 *        a description of the field (returned by fieldsGet for example). It
 *        may contain a description of the number of digits that should be used.
 * @param {Object} [options]
 *        additional options to override the values in the javascript description of
 *        the field.
 * @param {Object} [options.currency] the description of the currency to use
 * @param {integer} [options.currencyId]
 *        the id of the 'res.currency' to use (ignored if options.currency)
 * @param {string} [options.currencyField]
 *        the name of the field whose value is the currency id
 *        (ignore if options.currency or options.currencyId)
 *        Note: if not given it will default to the field currencyField value
 *        or to 'currencyId'.
...
function formatMonetary(value, field, options) {
    ...
    if (!currency) {
        ...
        if (!currencyId && options.data) {
            var currencyField = options.currencyField || field.currencyField || 'currencyId';
            currencyId = options.data[currencyField] && options.data[currencyField].resId;
        }
        currency = session.getCurrency(currencyId);
    }

    var digits = (currency && currency.digits) || options.digits;
    if (options.fieldDigits === true) {
        digits = field.digits || digits;
    }
    var formattedValue = formatFloat(value, field,
...
function formatPercentage(value, field, options) {
    ...
    if (!options.humanReadable || !options.humanReadable(value * 100)) {
        result = parseFloat(result).toString().replace('.', _t.database.parameters.decimalPoint);
    }
...
function parseDateTime(value, field, options) {
    ...
    if (smartDate) {
        ...
        if (options && options.isUTC) {
            ...
            datetime = moment.utc(value.replace(' ', 'T') + (value.endsWith('Z') ? '' : 'Z'));
        } else {
...
function parseNumber(value) {
    if (core._t.database.parameters.thousandsSep) {
        var escapedSep = _.str.escapeRegExp(core._t.database.parameters.thousandsSep);
        value = value.replace(new RegExp(escapedSep, 'g'), '');
    }
    if (core._t.database.parameters.decimalPoint) {
        value = value.replace(core._t.database.parameters.decimalPoint, '.');
    }
    return Number(value);
}
 ...
 * @param {Object} [field]
 *        a description of the field (returned by fieldsGet for example).
 * @param {Object} [options] additional options.
 * @param {Object} [options.currency] - the description of the currency to use
 * @param {integer} [options.currencyId]
 *        the id of the 'res.currency' to use (ignored if options.currency)
...
function parseFloatTime(value) {
    ...
    var floatTimePair = value.split(":");
    if (floatTimePair.length !== 2)
        return factor * parseFloat(value);
    var hours = parseInteger(floatTimePair[0]);
    var minutes = parseInteger(floatTimePair[1]);
...
/**
 * Creates an object with id and displayName.
 *
 * @param {Array|number|string|Object} value
 *        The given value can be :
 *        - an array with id as first element and displayName as second element
 *        - a number or a string representing the id (the displayName will be
 *          returned as undefined)
 *        - an object, simply returned untouched
 * @returns {Object} (contains the id and displayName)
...
return {
    format: {
        ...
        float: formatFloat,
        floatFactor: formatFloatFactor,
        floatTime: formatFloatTime,
    ...
    parse: {
        ...
        float: parseFloat,
        floatFactor: parseFloatFactor,
        floatTime: parseFloatTime,
        html: _.identity, // todo
