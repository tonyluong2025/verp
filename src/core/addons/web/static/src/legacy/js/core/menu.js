...
export async function initAutoMoreMenu(el, options) {
    ...
    const isUserNavbar = el.parentElement.classList.contains('o-main-navbar');
    ...
    function _addExtraItemsButton(target) {
        ...
        extraItemsToggle.className = 'nav-item dropdown o-extra-menu-items';
    ...
    function _afterImagesLoading(images) {
        const defs = images.map((image) => {
            ...
            return new Promise(function (resolve, reject) {
                if (!image.width) {
                    // The purpose of the 'o-menu-image-placeholder' class is to add a default
                    // size to non loaded images (on the first update) to prevent flickering.
                    image.classList.add('o-menu-image-placeholder');
                }
                image.addEventListener('load', () => {
                    image.classList.remove('o-menu-image-placeholder');
                    resolve();
                });
            });
        });
