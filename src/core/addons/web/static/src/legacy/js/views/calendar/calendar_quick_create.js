verp.define('web.CalendarQuickCreate', function (require) {
"use strict";
...
var QuickCreate = Dialog.extend({
        ...
        this._super(parent, {
            ...
            buttons: this._buttons ? [
                ...
                {text: _t("Edit"), click: function () {
                    ...
                    dataCalendar.onSave = self.destroy.bind(self);
                    self.triggerUp('openCreate', dataCalendar);
                }},
                {text: _t("Cancel"), close: true},
            ] : [],
            $content: QWeb.render('CalendarView.quickCreate', {widget: this})
        });
    },
    ...
    /**
     * Gathers data from the quick create dialog a launch quickCreate(data) method
     */
    _quickAdd: function (dataCalendar) {
        dataCalendar = $.extend({}, this.dataTemplate, dataCalendar);
        var val = this.$('input').val().trim();
        if (!val) {
            this.$('label, input').addClass('o-field-invalid');
            this.displayNotification({ title: _t("Invalid fields:"), message: _t("Meeting Subject"), type: 'danger' });
        }
        dataCalendar.title = val;
        return (val)? this.triggerUp('quickCreate', {data: dataCalendar, options: this.options}) : false;
    },
