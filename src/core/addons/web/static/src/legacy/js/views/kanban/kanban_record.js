verp.define('web.KanbanRecord', function (require) {
"use strict";
...
var KANBAN_RECORD_COLORS = require('web.basicFields').FieldColorPicker.prototype.RECORD_COLORS;
var NB_KANBAN_RECORD_COLORS = KANBAN_RECORD_COLORS.length;

var KanbanRecord = Widget.extend(WidgetAdapterMixin, {
    events: {
        'click .oe-kanban-action': '_onKanbanActionClicked',
        'click .o-kanban-manage-toggle-button': '_onManageTogglerClicked',
    },
    /**
     * @override
     */
    init: function (parent, state, options) {
        ...
        this.readOnlyMode = options.readOnlyMode;
    ...
    onAttachCallback: function () {
        this.isInDOM = true;
        _.invoke(this.subWidgets, 'onAttachCallback');
        // call onAttachCallback on child components (tag <widget>)
    ...
    onDetachCallback: function () {
        this.isInDOM = false;
        _.invoke(this.subWidgets, 'onDetachCallback');
        // call onDetachCallback on child components (tag <widget>)
    ...
    _attachTooltip: function () {
        ...
        this.$('[tooltip]').each(function () {
            ...
            if (tooltip) {
                $el.tooltip({
                    title: self.qweb.render(tooltip, self.qwebContext)
                });
            }
        });
    },
    ...
    _getImageURL: function (model, field, id, placeholder) {
        ...
        var url;
        if (isCurrentRecord && this.record[field] && this.record[field].rawValue && !utils.isBinSize(this.record[field].rawValue)) {
            // Use magic-word technique for detecting image type
            url = 'data:image/' + this.fileTypeMagicWord[this.record[field].rawValue[0]] + ';base64,' + this.record[field].rawValue;
        } else if (placeholder && (!model || !field || !id || (isCurrentRecord && this.record[field] && !this.record[field].rawValue))) {
            url = placeholder;
        } else {
            ...
            if (isCurrentRecord) {
                params.unique = this.record.__lastUpdate && this.record.__lastUpdate.value.replace(/[^0-9]/g, '');
            }
    ...
    _openRecord: function () {
        var editMode = this.$el.hasClass('oe-kanban-global-click-edit');
        this.triggerUp('openRecord', {
            id: this.dbId,
    ...
    _processFields: function () {
        ...
        this.$("field").each(function () {
            ...
            var fieldWidget = $field.attr("widget");
            ...
            if (fieldWidget || self.fields[fieldName].type === 'many2many') {
                var widget = self.subWidgets[fieldName];
                if (!widget) {
                    // the widget doesn't exist yet, so instanciate it
                    var Widget = self.fieldsInfo[fieldName].Widget;
                    if (Widget) {
                        widget = self._processWidget($field, fieldName, Widget);
                        self.subWidgets[fieldName] = widget;
                    } else if (config.isDebug()) {
                        // the widget is not implemented
                        $field.replaceWith($('<span>', {
                            text: _.str.sprintf(_t('[No widget %s]'), fieldWidget),
                        }));
                    }
                } else {
                    ...
                    if (self.isInDOM && widget.onAttachCallback) {
                        widget.onAttachCallback();
                    }
                }
            } else {
                self._processField($field, fieldName);
            }
        });
    },
    /**
     ...
     */
    _processField: function ($field, fieldName) {
        ...
        this._setFieldDisplay($result, fieldName);
        return $result;
    },
    /**
     ...
     */
    _processWidget: function ($field, fieldName, Widget) {
        ...
        _.each(this.fieldsInfo[fieldName], function (value, key) {
        ...
        if (utils.isComponent(Widget)) {
            ...
            widget = new Widget(this, fieldName, this.state, options);
            def = widget.replace($field);
        }
        this.defs.push(def);
        def.then(function () {
            self._setFieldDisplay(widget.$el, fieldName);
        });
        return widget;
    },
    ...
    _render: function () {
        this.defs = [];
        // call 'onDetachCallback' on each subwidget as they will be removed
        // from the DOM at the next line
        _.invoke(this.subWidgets, 'onDetachCallback');
        this._replaceElement(this.qweb.render('kanban-box', this.qwebContext));
        this.$el.addClass('o-kanban-record').attr("tabindex", 0);
        this.$el.attr('role', 'article');
        this.$el.data('record', this);
        // forcefully add class oe-kanban-global-click to have clickable record always to select it
        if (this.selectionMode) {
            this.$el.addClass('oe-kanban-global-click');
        }
        if (this.$el.hasClass('oe-kanban-global-click') ||
            this.$el.hasClass('oe-kanban-global-click-edit')) {
        ...
        this.$('span.o-tag').tooltip({ delay: { 'show': 50 } });
    ...
    _setCoverImage: function (fieldName, autoOpen) {
        ...
        this._rpc({
            ...
            self.imageUploadID = _.uniqueId('oCoverImageUpload');
            self.acceptedFileExtensions = 'image/*';  // prevent uploading of other file types
            self.attachmentCount = attachmentIds.length;
            var coverId = self.record[fieldName] && self.record[fieldName].rawValue;
            var $content = $(QWeb.render('KanbanView.SetCoverModal', {
                coverId: coverId,
                attachmentIds: attachmentIds,
                widget: self,
            }));
            var $imgs = $content.find('.o-kanban-cover-image');
            var dialog = new Dialog(self, {
                ...
                buttons: [{
                    ...
                    disabled: !coverId,
                    click: function () {
                        var $img = $imgs.filter('.o-selected').find('img');
                        var data = {};
                        data[fieldName] = {
                            id: $img.data('id'),
                            displayName: $img.data('name')
                        };
                        self.triggerUp('kanbanRecordUpdate', data);
                    },
                }, {
                    ...
                    close: false,
                    click: function () {
                        $content.find('input.o-input-file').click();
                    },
                }, {
            ...
            dialog.opened().then(function () {
                ...
                if (autoOpen && !self.attachmentCount) {
                    $selectBtn.click();
                }

                $content.on('click', '.o-kanban-cover-image', function (ev) {
                    $imgs.not(ev.currentTarget).removeClass('o-selected');
                    $selectBtn.prop('disabled', !$(ev.currentTarget).toggleClass('o-selected').hasClass('o-selected'));
                });

                $content.on('dblclick', '.o-kanban-cover-image', function (ev) {
                ...
                $content.on('change', 'input.o-input-file', function () {
                    $content.find('form.o-form-binary-form').submit();
                });
    ...
    _setFieldDisplay: function ($el, fieldName) {
        ...
        if (this.fieldsInfo[fieldName].display === 'right') {
            ...
            $el.addClass('o-text-block');
        }

        // attribute bold
        if (this.fieldsInfo[fieldName].bold) {
            $el.addClass('o-text-bold');
        }
    },
    /**
    ...
    _setState: function (recordState) {
        ...
        this.id = recordState.resId;
        this.dbId = recordState.id;
        this.recordData = recordState.data;
        this.record = this._transformRecord(recordState.data);
        this.qwebContext = {
            context: this.state.getContext(),
            kanbanImage: this._getImageURL.bind(this),
            kanbanColor: this._getColorClassname.bind(this),
            kanbanGetcolor: this._getColorID.bind(this),
            kanbanGetcolorname: this._getColorname.bind(this),
            kanbanComputeDomain: this._computeDomain.bind(this),
    ...
    _setupColor: function () {
        var colorField = this.$el.attr('color');
        if (colorField && colorField in this.fields) {
            var colorHelp = _.str.sprintf(_t("Card color: %s"), this._getColorname(this.recordData[colorField]));
            var colorClass = this._getColorClassname(this.recordData[colorField]);
            this.$el.addClass(colorClass);
            this.$el.prepend('<span title="' + colorHelp + '" aria-label="' + colorHelp + '" role="img" class="oe-kanban-color-help"/>');
        }
    },
    /**
    ...
    _setupColorPicker: function () {
        var $colorpicker = this.$('ul.oe-kanban-colorpicker');
    ...
    _transformRecord: function (recordData) {
        ...
        for (const name of this.state.getFieldNames()) {
            ...
            const r = newRecord[name] = _.clone(this.fields[name] || {});
            const formatter = r.type ? fieldUtils.format[r.type] : _.identity;

            r.rawValue = value;
            r.value = formatter(value, this.fields[name], recordData, this.state);
            switch (r.type) {
            case 'date': case 'datetime':
                r.rawValue = value && value.toDate();
                break;
            case 'one2many': case 'many2many':
                r.rawValue = value.count ? value.resIds : [];
                break;
            case 'many2one':
                r.rawValue = value && value.resId || false;
                break;
    ...
    _updateRecord: function (data) {
        this.triggerUp('kanbanRecordUpdate', data);
    },
    ...
    _onColorChanged: function (event) {
        ...
        var data = {};
        var colorField = $(event.delegateTarget).data('field') || 'color';
        data[colorField] = $(event.currentTarget).data('color');
        this.triggerUp('kanbanRecordUpdate', data);
    },
    /**
     * @private
    ...
    _onGlobalClick: function (event) {
        if ($(event.target).parents('.o-dropdown-kanban').length) {
            return;
        }
        if (this.$el.hasClass('o-currently-dragged')) {
                ...
                _.each(events.click, function (clickEvent) {
                    if (clickEvent.selector) {
                        ...
                        _.each(children, function (child) {
                            if ($(child).is(clickEvent.selector)) {
                                trigger = false;
                            }
                        });
                    }
    ...
    _onKanbanActionClicked: function (event) {
        ...
        switch (type) {
            case 'edit':
                this.triggerUp('openRecord', { id: this.dbId, mode: 'edit' });
                break;
            case 'open':
                this.triggerUp('openRecord', { id: this.dbId });
                break;
            case 'delete':
                this.triggerUp('kanbanRecordDelete', { id: this.dbId, record: this });
                break;
            case 'action':
            case 'object':
            ...
            case 'setCover':
                ...
                if (this.fields[fieldName].type === 'many2one' &&
                    ...
                    this.fieldsInfo[fieldName].widget === 'attachmentImage') {
    ...
    /**
     * This event is linked to the kanban card when there is a global-click
     * class on this card
     *
     * @private
    ...
    _onKeyDownCard: function (event) {
        switch (event.keyCode) {
            case $.ui.keyCode.ENTER:
                if ($(event.target).hasClass('oe-kanban-global-click')) {
                    event.preventDefault();
    ...
    /**
     * This event is linked ot the kanban card when there is no global-click
     * class on the card
     *
     * @private
     * @param {KeyDownEvent} event
    ...
    _onManageTogglerClicked: function (event) {
        event.preventDefault();
        this.$el.parent().find('.o-kanban-record').not(this.$el).removeClass('o-dropdown-open');
        this.$el.toggleClass('o-dropdown-open');
        var colorClass = this._getColorClassname(this.recordData.color || 0);
        this.$('.o-kanban-manage-button-section').toggleClass(colorClass);
    },
});
