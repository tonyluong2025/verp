/** @verp-module alias=web.AbstractView **/

/**
 * This is the base class inherited by all (JS) views. Verp JS views are the
 ...
 * the word 'view' has historical significance in Verp code, so we replaced the
...
var AbstractView = Factory.extend({
    ...
    init: function (viewInfo, params) {
        ...
        this.controllerParams = {
            ...
            bannerRoute: this.arch.attrs.bannerRoute,
        ...
        this.loadParams = {
            ...
            modelName: params.modelName,
            resId: currentId,
            resIds: controllerState.resIds || params.ids || (currentId ? [currentId] : undefined),
        };
        ...
        var defaultOrder = this.arch.attrs.defaultOrder;
        if (defaultOrder) {
        ...
        this.withSearchPanel = this.withSearchPanel &&
            this.multiRecord && params.withSearchPanel &&
            !('searchpanel' in params.context && !params.searchpanel);
        ...
        if (this.withControlPanel) {
            ...
            const controlPanelProps = {
                ...
                views: params.action.views && params.action.views.filter(
                    v => v.multiRecord === this.multiRecord
                ),
     ...
     * @param {boolean} [action.context.noBreadcrumbs=false]
     * @param {integer} [action.context.activeId]
     * @param {integer[]} [action.context.activeIds]
     * @param {Object} [action.controlPanelFieldsView]
     * @param {string} [action.displayName]
     * @param {Array[]} [action.domain=[]]
     * @param {string} [action.help]
     * @param {integer} [action.id]
     * @param {integer} [action.limit]
     * @param {string} [action.label]
     * @param {string} [action.resModel]
    ...
    _extractParamsFromAction: function (action) {
        ...
        const params = {
            ...
            activateDefaultFavorite: !context.activeId && !context.activeIds,
            context: action.context || {},
            controlPanelFieldsView: action.controlPanelFieldsView,
            currentId: action.resId ? action.resId : undefined,  // load returns 0
            ...
            withBreadcrumbs: 'noBreadcrumbs' in context ? !context.noBreadcrumbs : true,
