verp.define('web.domReady', function (require) {
'use strict';
...
verp.define('web.dom', function (require) {
"use strict";
 ...
 * functionalities with the verp framework.  A common theme in these functions
 ...
 * @params {Array} [callbacks] array of {widget: w, callbackArgs: args} such
 * that onAttachCallback() will be called on each w with arguments args
 */
function _notify(content, callbacks) {
    callbacks.forEach(function (c) {
        if (c.widget && c.widget.onAttachCallback) {
            c.widget.onAttachCallback(c.callbackArgs);
        }
    });
    core.bus.trigger('domUpdated', content);
}
...
var dom = {
     ...
     * @param {number} [options.minHeight] by default, 50.
    ...
    autoresize: function ($textarea, options) {
        ...
        options = options || {};
        minHeight = 'minHeight' in options ? options.minHeight : 50;
        ...
        $fixedTextarea.data("autoResize", true);
        ...
        if (options.parent) {
            core.bus.on('domUpdated', options.parent, function () {
                resize();
                removeVerticalResize();
            });
        }
    ...
    /**
     * Detaches widgets from the DOM and performs their onDetachCallback()
     *
     * @param {Array} [toDetach] array of {widget: w, callbackArgs: args} such
     *   that w.$el will be detached and w.onDetachCallback(args) will be
     *   called
     * @param {jQuery} [options.$toDetach] if given, detached instead of
     *   widgets' $el
     * @return {jQuery} the detached elements
     */
    detach: function (toDetach, options) {
        toDetach.forEach( function (d) {
            if (d.widget.onDetachCallback) {
                d.widget.onDetachCallback(d.callbackArgs);
            }
        });
        var $toDetach = options && options.$toDetach;
        if (!$toDetach) {
            $toDetach = $(toDetach.map(function (d) {
                return d.widget.el;
            }));
        }
        return $toDetach.detach();
    },
    /**
    ...
    makeButtonHandler: function (fct) {
        ...
        return function (ev) {
            ...
            $button.addClass('o-debounce-disabled');
            Promise.resolve(dom.DEBOUNCE && concurrency.delay(dom.DEBOUNCE)).then(function () {
                $button.removeClass('o-debounce-disabled');
    ...
    addButtonLoadingEffect: function (btn) {
        const $btn = $(btn);
        $btn.addClass('o-website-btn-loading disabled');
        ...
        $btn.prepend($loader);
        return () => {
             $btn.removeClass('o-website-btn-loading disabled');
    ...
    /**
     * Renders a button with standard verp template. This does not use any xml
    ...
    renderButton: function (options) {
        ...
        if (extraClasses) {
            // If we got extra classes, check if old oe-highlight/oe-link
            ...
            extraClasses = extraClasses.replace(/\boe-highlight\b/g, 'btn-primary')
                                       .replace(/\boe-link\b/g, 'btn-link');
        }
        ...
        if (options.icon) {
            if (options.icon.substr(0, 3) === 'fa-') {
                $button.append($('<i/>', {
                    class: 'fa fa-fw o-button-icon ' + options.icon,
                }));
            } else {
                $button.append($('<img/>', {
                    src: options.icon,
    ...
    /**
     * Renders a checkbox with standard verp/BS template. This does not use any
    ...
    scrollFixedOffset() {
        let size = 0;
        for (const el of $('.o-top-fixed-element')) {
            size += $(el).outerHeight();
        }
        return size;
    },
    ...
    scrollTo(el, options = {}) {
        ...
        return new Promise(resolve => {
            ...
            clonedOptions.progress = function (a, b, remainingMs) {
                ...
                if (Math.abs(newScrollTop - originalScrollTop) <= 1.0 && !(el.classList.contains('o-transitioning'))) {
                    return;
                }
                $scrollable.stop();
