...
const ProfilingQwebView = basicFfields.AceEditor.extend({
    ...
    init: function () {
        ...
        this.viewIDs = Array.from(new Set(this.profileLines.map((line) => line.viewId)));
        for (const line of this.profileLines) {
            line.xpath = line.xpath.replace(/([^\]])\//g, "$1[1]/").replace(/([^\]])$/g, "$1[1]");
        }
        this.viewID = this.profileLines.length ? this.profileLines[0].viewId : 0;
        this.mode = "readonly";
    },
    /**
     * Search xml view
    ...
    willStart: async function () {
        ...
        this.views = await this._rpc({
            model: "ir.ui.view",
            method: "searchRead",
            fields: ["id", "displayName", "key"],
            domain: [["id", "in", this.viewIDs]],
        });
        for (const view of this.views) {
            view.delay = 0;
            view.query = 0;
            const lines = this.profileLines.filter((l) => l.viewId === view.id);
    ...
    _renderProfilingInformation: function () {
        ...
        const $rows = this.$(".ace-gutter .ace-gutter-cell");
        $rows.find(".o-info").remove();

        this.$(".ace-tag-open, .ace-end-tag-close, .ace-end-tag-open, .ace-qweb").each(
            (i, node) => {
                const $node = $(node);
                const parent = arch[arch.length - 1];
                let xpath = parent.xpath;
                if ($node.hasClass("ace-end-tag-close")) {
                    // Close tag.
                    const tag = $node.prevAll(".ace-tag-name:first").text();
                    if (parent.tag === tag) {
                        // can be different when scroll because ace does not display the previous lines.
                        arch.pop();
                    }
                } else if ($node.hasClass("ace-end-tag-open")) {
                    // Auto close tag.
                ...
                } else if ($node.hasClass("ace-qweb")) {
                    // QWeb attribute.
                    ...
                    if ((delay || query) && !$node.children(".o-info").length) {
                        $(
                ...
                } else if ($node.hasClass("ace-tag-open")) {
                    // Open tag.
                    const $tag = $node.next(".ace-tag-name");
                    const tag = $tag.text();
                    const $row = $($rows[$tag.parent(".ace-line").index()]);
                    ...
                    // Compute delay and query number.
                    const closed = !!$row.find(".ace-closed").length;
                    const delays = [];
                    const querys = [];
                    const groups = {};
                    ...
                    if (delays.length && !$row.children(".o-info").length) {
                        $(
...
registry.add("profilingQwebView", ProfilingQwebView);
