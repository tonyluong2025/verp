/** @verp-module alias=web.SampleServer **/
    ...
    class SampleServer {
        ...
        _generateFieldValue(modelName, fieldName, id) {
            ...
            switch (field.type) {
                ...
                case "char":
                case "text":
                    if (["displayName", "label"].includes(fieldName)) {
                ...
                case "many2one":
                    if (field.relation === 'res.currency') {
                        return session.companyCurrencyId;
                    }
                ...
                for (const fieldName of fieldNames) {
                    ...
                    if (!field) {
                        ...
                        record[fieldName] = relRecord ?
                            [relRecord.id, relRecord.displayName] :
                            false;
                    } else {
        ...
        _mockReadProgressBar(params) {
            const groupByFieldName = viewUtils.getGroupByField(params.kwargs.groupby);
    ...
    // for all models:
    //   SampleServer.mockRegistry.add('someRoute', () => "abcd");
    // for a specific model (e.g. 'res.partner'):
    //   SampleServer.mockRegistry.add('res.partner/someMethod', () => 23);
