verp.define('web.FormController', function (require) {
"use strict";
...
var FormController = BasicController.extend({
    customEvents: _.extend({}, BasicController.prototype.customEvents, {
        ...
        editedList: '_onEditedList',
        openOne2manyRecord: '_onOpenOne2ManyRecord',
        openRecord: '_onOpenRecord',
        toggleColumnOrder: '_onToggleColumnOrder',
        focusControlButton: '_onFocusControlButton',
        formDialogDiscarded: '_onFormDialogDiscarded',
        ...
        if (!this.disableAutofocus) {
            ...
            if (!isControlActivted) {
                ...
                if (this.$buttons && this.mode === 'readonly') {
                    return this.$buttons.find('.o-form-button-edit').focus();
                }
            }
        }
    },
    ...
    /**
     * Returns the current resId, wrapped in a list. This is only used by the
     * action menus (and the debugmanager)
     *
     * @override
     *
     * @returns {number[]} either [current resId] or []
     */
    ...
    renderButtons: function ($node) {
        ...
        if ((this.defaultButtons && !this.$buttons) || mustRenderFooterButtons) {
            ...
            if (mustRenderFooterButtons) {
                ...
                this.$buttons.on('click', '.o-form-button-edit', this._onEdit.bind(this));
                this.$buttons.on('click', '.o-form-button-create', this._onCreate.bind(this));
                this.$buttons.on('click', '.o-form-button-save', this._onSave.bind(this));
                this.$buttons.on('click', '.o-form-button-cancel', this._onDiscard.bind(this));
                this._assignSaveCancelKeyboardBehavior(this.$buttons.find('.o-form-buttons-edit'));
                this.$buttons.find('.o-form-buttons-edit').tooltip({
                ...
                otherActionItems.push({
                    ...
                    callback: () => {
                        Dialog.confirm(this, _t("Are you sure that you want to archive this record?"), {
                            confirmCallback: () => this._toggleArchiveState(true),
                        });
                    },
                });
            } else {
    ...
    saveRecord: async function () {
        ...
        if (_t.database.multiLang && changedFields.length) {
    ...
    updateButtons: function () {
        ...
        this.$buttons.find('.o-form-buttons-edit')
            .toggleClass('o-hidden', !editMode);
        this.$buttons.find('.o-form-buttons-view')
            .toggleClass('o-hidden', editMode);
    },
    ...
    _shouldBounceOnClick(element) {
        return this.mode === 'readonly' &&
            !!element.closest('.oe-title, .o-inner-group') &&
            this.quickEditTimeout === undefined;
    },
    ...
    /**
     * Save the record when we are about to leave Verp.
     *
     * @override
     */
    ...
    _onButtonClicked: function (ev) {
        ...
        function saveAndExecuteAction () {
            return self.saveRecord(self.handle, {
                ...
                // by the basic model, such as the new resId, if the record is
                // new.
        ...
        if (attrs.confirm) {
            def = new Promise(function (resolve, reject) {
                Dialog.confirm(self, attrs.confirm, {
                    confirmCallback: saveAndExecuteAction,
    ...
    _onOpenOne2ManyRecord: async function (ev) {
        ...
        new dialogs.FormViewDialog(this, {
            ...
            resId: record && record.resId,
            resModel: data.field.relation,
    ...
    _onOpenRecord: function (ev) {
        ...
        new dialogs.FormViewDialog(self, {
            ...
            fieldsView: ev.data.fieldsView,
            onSaved: ev.data.onSaved,
            onRemove: ev.data.onRemove,
            readonly: ev.data.readonly,
            deletable: ev.data.deletable,
            editable: ev.data.editable,
            resId: record.resId,
            resModel: record.model,
