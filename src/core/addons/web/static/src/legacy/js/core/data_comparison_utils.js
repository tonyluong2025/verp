verp.define('web.dataComparisonUtils', function (require) {
"use strict";
...
function renderVariation(variation, field, options) {
    var className = 'o-variation';
    var value;
    if (!isNaN(variation)) {
        if (variation > 0) {
            className += ' o-positive text-success';
        } else if (variation < 0) {
            className += ' o-negative text-danger';
        } else {
            className += ' o-null';
        }
