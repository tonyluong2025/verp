verp.define('web.ActionMenus', function (require) {
    "use strict";
    ...
    class ActionMenus extends Component {
        ...
        async _setActionItems(props) {
            ...
            const formattedActions = [...actionActions, ...relateActions].map(
                action => ({ action, description: action.label, key: action.id })
            );
        ...
        async _executeAction(action) {
            ...
            if (this.props.isDomainSelected) {
                activeIds = await this.rpc({
                    model: this.env.action.resModel,
                    method: 'search',
                    args: [this.props.domain],
                    kwargs: {
                        limit: this.env.session.activeIdsLimit,
            ...
            const activeIdsContext = {
                ...
                activeIds: activeIds,
                activeModel: this.env.action.resModel,
            };
            if (this.props.domain) {
                // keep activeDomain in context for backward compatibility
                // reasons, and to allow actions to bypass the activeIdsLimit
                activeIdsContext.activeDomain = this.props.domain;
            }

            const context = new Context(this.props.context, activeIdsContext).eval();
            const result = await this.rpc({
                route: '/web/action/load',
                params: { actionId: action.id, context },
            });
            result.context = new Context(result.context || {}, activeIdsContext)
                .setEvalContext(context);
            result.flags = result.flags || {};
            result.flags.newWindow = true;
            this.trigger('do-action', {
                action: result,
                options: {
                    onClose: () => this.trigger('reload'),
                },
            });
        }
