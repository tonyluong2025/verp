/* global FullCalendar */
verp.define('web.CalendarRenderer', function (require) {
"use strict";
...
var SidebarFilterM2O = relationalFields.FieldMany2One.extend({
    _getSearchBlacklist: function () {
        return this._super.apply(this, arguments).concat(this.filterIds || []);
    },
});

var SidebarFilter = Widget.extend(FieldManagerMixin, {
    template: 'CalendarView.sidebar.filter',
    customEvents: _.extend({}, FieldManagerMixin.customEvents, {
     ...
     *   following keys: id, value, label, active, avatarModel, color,
     *   canBeRemoved
    ...
    init: function (parent, options) {
        ...
        this.writeField = options.writeField;
        ...
        this.filterCheckAll = options.checkAll;
    },
    /**
     * @override
     */
    willStart: function () {
        ...
        if (this.writeModel || this.writeField) {
            var def = this.model.makeRecord(this.writeModel, [{
    ...
    start: function () {
        ...
        if (this.many2one) {
            ...
            this.many2one.filterIds = _.without(_.pluck(this.filters, 'value'), 'all');
        }
        this.$el.on('click', '.o-remove', this._onFilterRemove.bind(this));
        this.$el.on('click', '.o-calendar-filter-items input', this._onFilterActive.bind(this));
        this.$el.on('click', '.o-calendar-filter-items-checkall input', this._onFilterCheckAll.bind(this));
    },
    ...
    _onFieldChanged: function (event) {
        ...
        var createValues = {'userId': session.uid};
        var value = event.data.changes[this.writeField].id;
        createValues[this.writeField] = value;
        this._rpc({
    ...
    _onFilterActive: function (e) {
        ...
        this.filterCheckAll[this.fieldName] = false;
        this.triggerUp('changeFilter', {
            'fieldName': this.fieldName,
            'value': $input.closest('.o-calendar-filter-item').data('value'),
    ...
    _onFilterRemove: function (e) {
        var self = this;
        var $filter = $(e.currentTarget).closest('.o-calendar-filter-item');
        this.filterCheckAll[this.fieldName] = false;
        self._rpc({
                model: self.writeModel,
                method: 'unlink',
                args: [[$filter.data('id')]],
            })
            .then(function () {
    ...
    _onFilterCheckAll: function (e) {
        ...
        let $filter = $(e.currentTarget).closest('.o-calendar-filter-item');
        this.filterCheckAll[this.fieldName] = $input.prop('checked');
        this.triggerUp('changeFilter', {
            'fieldName': this.fieldName,
            'value': 'checkAll',
            'active': $input.prop('checked'),
...
return AbstractRenderer.extend({
    ...
    customEvents: _.extend({}, AbstractRenderer.prototype.customEvents || {}, {
        editEvent: '_onEditEvent',
        deleteEvent: '_ondeleteEvent',
        renderEvent: '_renderEvents',
    }),
    ...
    destroy: function () {
        ...
        if (this.$smallCalendar) {
            this.$smallCalendar.datepicker('destroy');
    ...
    getAvatars: function (record, fieldName, imageField) {
        ...
        if (field.type === 'one2many' || field.type === 'many2many') {
            ...
            if (isNaN(color)) {
                return ['<span class="o-avatar-square" style="background-color:'+color+';"/>'];
            }
            else {
                return ['<span class="o-avatar-square o-calendar-color-'+color+'"/>'];
            }
        }
    },
    /**
    ...
    getColor: function (key) {
        ...
        if (typeof key === 'number' && !(key in this.colorMap)) {
            return this.colorMap[key] = key;
        }
        var index = (((_.keys(this.colorMap).length + 1) * 5) % 24) + 1;
        ...
        if (!moment.isMoment(fc4Event.start)) {
            ...
            if (fc4Event.extendedProps) {
                event = Object.assign({}, event, {
                    rStart: fc4Event.extendedProps.rStart && moment(fc4Event.extendedProps.rStart).utcOffset(0, true),
                    rEnd: fc4Event.extendedProps.rEnd && moment(fc4Event.extendedProps.rEnd).utcOffset(0, true),
    ...
    _eventRender: function (event) {
        var qwebContext = {
            ...
            color: this.getColor(event.extendedProps.colorIndex),
            showTime: !this.hideTime && event.extendedProps.showTime,
            showLocation: this.state.scale !== 'month'
        };
        this.qwebContext = qwebContext;
        if (_.isEmpty(qwebContext.record)) {
            return '';
        } else {
            return qweb.render(this.config.eventTemplate, qwebContext);
        }
    },
    /**
     * @private
    ...
    _getFullCalendarOptions: function (fcOptions) {
        var self = this;
        const options = Object.assign({}, this.state.fcOptions, {
            plugins: [
                'moment',
                'interaction',
                'dayGrid',
            ...
            eventClick: function (eventClickInfo) {
                ...
                self._unselectEvent();
                $(self.calendarElement).find(self._computeEventSelector(eventClickInfo)).addClass('o-cw-custom-highlight');
            ...
            yearDateClick: function (info) {
                ...
                if (!info.events.length) {
                    if (info.selectable) {
                        ...
                        if (self.state.context.default_label) {
                            data.title = self.state.context.default_label;
                        }
                        self.triggerUp('openCreate', self._convertEventToFC3Event(data));
                    }
                } else {
            ...
            eventRender: function (info) {
                ...
                if (view.type === 'dayGridYear') {
                    const color = this.getColor(event.extendedProps.colorIndex);
                    if (typeof color === 'string') {
                        element.css({
                            backgroundColor: color,
                        });
                    } else if (typeof color === 'number') {
                        element.addClass(`o-calendar-color-${color}`);
                    } else {
                        element.addClass('o-calendar-color-1');
                    }
                } else {
                ...
                if (view.type === 'dayGridMonth' && event.extendedProps.record) {
                    var start = event.extendedProps.rStart || event.start;
                    var end = event.extendedProps.rEnd || event.end;
                ...
                element.on('dblclick', function () {
                    self.triggerUp('editEvent', {id: event.id});
                });
                }
            },
            ...
            eventMouseEnter: function (mouseEnterInfo) {
                $(self.calendarElement).find(self._computeEventSelector(mouseEnterInfo)).addClass('o-cw-custom-hover');
            },
            eventMouseLeave: function (mouseLeaveInfo) {
                if (!mouseLeaveInfo.event.id) {
                    return;
                }
                $(self.calendarElement).find(self._computeEventSelector(mouseLeaveInfo)).removeClass('o-cw-custom-hover');
            },
            eventDragStart: function (mouseDragInfo) {
                mouseDragInfo.el.classList.add(mouseDragInfo.view.type);
                $(self.calendarElement).find(`[data-event-id=${mouseDragInfo.event.id}]`).addClass('o-cw-custom-hover');
                self._unselectEvent();
            },
            eventResizeStart: function (mouseResizeInfo) {
                $(self.calendarElement).find(`[data-event-id=${mouseResizeInfo.event.id}]`).addClass('o-cw-custom-hover');
                self._unselectEvent();
            },
    ...
    _initCalendar: function () {
        this.calendarElement = this.$(".o-calendar-widget")[0];
    ...
    _initCalendarMini: function () {
        var self = this;
        this.$smallCalendar = this.$(".o-calendar-mini");
        this.$smallCalendar.datepicker({
            'onSelect': function (datum, obj) {
                self.triggerUp('changeDate', {
                    date: moment(new Date(+obj.currentYear , +obj.currentMonth, +obj.currentDay))
                });
            },
            'showOtherMonths': true,
            'dayNamesMin': this.state.fcOptions.dayNamesMin.map(x => x[0]),
            'monthNames': this.state.fcOptions.monthNamesShort,
            'firstDay': this.state.fcOptions.firstDay,
        });
    },
    /**
    ...
    _initSidebar: function () {
        this.$sidebar = this.$('.o-calendar-sidebar');
        this.$calendarSyncContainer = this.$('#calendarSync');
        this.$sidebarContainer = this.$(".o-calendar-sidebar-container");
    ...
    _onPopoverShown: function ($popoverElement, calendarPopover) {
        ...
        $popover.find('.o-cw-popover-close').on('click', this._unselectEvent.bind(this));
        $popover.find('.o-cw-body').replaceWith(calendarPopover.$el);
    },
    /**
     ...
     */
    async _renderView() {
        this.$('.o-calendar-view')[0].prepend(this.calendarElement);
        if (this._isInDOM) {
            this._renderCalendar();
        }
        this.$smallCalendar.datepicker("setDate", this.state.highlightDate.toDate())
                            .find('.o-selected-range')
                            .removeClass('o-color o-selected-range');
        var $a;
        switch (this.state.scale) {
            case 'year': $a = this.$smallCalendar.find('td'); break;
            case 'month': $a = this.$smallCalendar.find('td'); break;
            case 'week': $a = this.$smallCalendar.find('tr:has(.ui-state-active)'); break;
            case 'day': $a = this.$smallCalendar.find('a.ui-state-active'); break;
        }
    ...
    _renderCalendar() {
        ...
        if (this.targetDate !== this.state.targetDate.toString()) {
            this.calendar.gotoDate(moment(this.state.targetDate).toDate());
            this.targetDate = this.state.targetDate.toString();
        } else {
    ...
    _renderFilters: function () {
        ...
        this.$('.o-calendar-filter-item').popover('dispose');
                ...
                if (options.avatarField) {
                    _.each(options.filters, function (filter) {
                        if (!['all', false].includes(filter.value)) {
                            var selector = `.o-calendar-filter-item[data-value=${filter.value}]`;
                            sidebarFilter.$el.find(selector).popover({
                                ...
                                content: function () {
                                    return $('<img>', {
                                        src: `/web/image/${options.avatarModel}/${filter.value}/${options.avatarField}`,
                                        class: 'mx-auto',
                                    });
                                },
                            });
    ...
    _getDbTimeFormat: function () {
        return _t.database.parameters.timeFormat.search('%H') !== -1 ? 'HH:mm' : 'hh:mm a';
    },
    /**
    ...
    _getPopoverContext: function (eventData) {
        ...
        var start = moment((eventData.extendedProps && eventData.extendedProps.rStart) || eventData.start);
        var end = moment((eventData.extendedProps && eventData.extendedProps.rEnd) || eventData.end);
    ...
    _getPopoverParams: function (eventData) {
        ...
        const displayLock = eventData.extendedProps.record.privacy === 'private' && eventData.extendedProps.record.partnerIds.includes(session.partnerId);
        let title = eventData.extendedProps.record.displayName;
        if (title.length > 30) {
        ...
        return {
            ...
            {color: this.getColor(eventData.extendedProps.colorIndex), displayLock: displayLock}),
    ...
    _renderYearEventPopover: function (date, events, $el) {
        ...
        for (const event of events) {
            const start = moment(event.extendedProps.rStart);
            const end = moment(event.extendedProps.rEnd);
            const duration = moment.duration(end.diff(start)).asDays();
            event.extendedProps.record.startHour = !event.extendedProps.record.allday && duration < 1 ? start.format("HH:mm") : "";
        ...
        $el.popover({
            ...
            $('.o-cw-popover .o-cw-popover-close').on('click', () => this._unselectEvent());
            $('.o-cw-popover .o-cw-popover-create').on('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                this._unselectEvent();
                const data = {
                ...
                this.triggerUp('openCreate', this._convertEventToFC3Event(data));
            });
            $('.o-cw-popover .o-cw-popover-link').on('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                this._unselectEvent();
                this.triggerUp('openEvent', {
    ...
    _unselectEvent: function () {
        this.$('.fc-event').removeClass('o-cw-custom-highlight');
        this.$('.o-cw-popover').popover('dispose');
    },
    /**
     * @private
     * @param {MouseEvent} e
    ...
    _onWindowClick: function (e) {
        const popover = this.el.querySelector('.o-cw-popover');
    ...
    _ondeleteEvent: function (event) {
        this._unselectEvent();
        this.triggerUp('deleteRecord', {id: parseInt(event.data.id, 10), event: event.target.event.extendedProps});
    },
});
