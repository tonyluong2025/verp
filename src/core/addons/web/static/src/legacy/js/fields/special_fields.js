verp.define('web.specialFields', function (require) {
"use strict";
 ...
 * This widget depends on a field given with the param 'tzOffsetField', which contains
...
var FieldTimezoneMismatch = FieldSelection.extend({
    ...
    _renderDateTimeTimezone: function () {
        ...
        var offset = this.recordData.tzOffset.match(/([+-])([0-9]{2})([0-9]{2})/);
        offset = (offset[1] === '-' ? -1 : 1) * (parseInt(offset[2])*60 + parseInt(offset[3]));
        var datetime = fieldUtils.format.datetime(moment.utc().add(offset, 'minutes'), this.field, {timezone: false});
    ...
    _renderTimezoneMismatch: function () {
        ...
        this.$el.last().filter('.o-tz-warning').remove();
        this.$el = this.$el.first();
        var value = this.$el.val();
        var $span = $('<span class="fa fa-exclamation-triangle o-tz-warning"/>');

        if (this.$option && this.$option.html()) {
            this.$option.html(this.$option.html().split(' ')[0]);
        }

        var userOffset = this.recordData.tzOffset;
        this.mismatch = false;
        ...
        if (this.mismatch){
            ...
            if (this.nodeOptions.mismatchTitle) {
                $span.attr('title', this.nodeOptions.mismatchTitle);    
            }
            else {
...
var FieldReportLayout = relationalFields.FieldMany2One.extend({
    ...
    _render: function () {
        ...
        _.each(this.previews, function (val) {
            ...
            var $img = $('<img>')
                ...
                .toggleClass('btn-info', val.viewId[0] === value)
                .attr('src', val.image)
                .data('key', val.viewId[0]);
...
const IframeWrapper = AbstractField.extend({
    ...
    _render() {
        ...
        const spinner = this.el.querySelector('.o-iframe-wrapper-spinner');
        const iframe = this.el.querySelector('.o-preview-iframe');
