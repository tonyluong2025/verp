/** @verp-module alias=web.FieldManagerMixin **/
...
var FieldManagerMixin = {
    ...
    _onFieldChanged: function (event) {
        ...
        // of a one2many subrecord), the fieldChanged event must be stopped as
     ...
     * actually reload the data, then call the onSuccess callback.
     *
     * @param {VerpEvent} event
     * @param {number} [event.data.limit]
     * @param {number} [event.data.offset]
     * @param {function} [event.data.onSuccess] callback
     */
    _onLoad: function (event) {
        ...
        this.mutex.exec(function () {
            return self.model.reload(data.id, params).then(function (dbId) {
                data.onSuccess(self.model.get(dbId));
            });
        });
    },
    /**
