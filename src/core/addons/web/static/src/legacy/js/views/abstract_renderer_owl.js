/** @verp-module alias=web.AbstractRendererOwl **/
...
class AbstractRenderer extends owl.Component {
    ...
    constructor() {
        ...
        // 1. receive the 'o-sample-data-disabled' class (greyd out & no user events)
     ...
     * the 'o-sample-data-disabled' class to these root elements.
     *
     * @private
    ...
    _suppressFocusableElements() {
        if (!this.props.isSample || this.props.isEmbedded) {
            const disabledEls = this.el.querySelectorAll(`.o-sample-data-disabled`);
            disabledEls.forEach(el => el.classList.remove('o-sample-data-disabled'));
            return;
        }
        const rootEls = [];
        ...
        for (const rootEl of rootEls) {
            rootEl.classList.add('o-sample-data-disabled');
