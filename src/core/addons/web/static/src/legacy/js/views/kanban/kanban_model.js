verp.define('web.KanbanModel', function (require) {
"use strict";
...
var KanbanModel = BasicModel.extend({
    ...
    addRecordToGroup: function (groupID, resId) {
        ...
        var newRecord = this._makeDataPoint({
            resId: resId,
        ...
        var def = this._fetchRecord(newRecord).then(function (result) {
            group.data.unshift(newRecord.id);
            group.resIds.unshift(resId);
            group.count++;

            // update the resIds and count of the parent
            self.localData[group.parentId].count++;
            self._updateParentResIDs(group);

            return result.id;
        });
        return this._reloadProgressBarGroupFromRecord(newRecord.id, def);
    },
    /**
     * Creates a new group from a name (performs a nameCreate).
     *
     * @param {string} name
    ...
    createRecordInGroup: function (groupID, values) {
        ...
        var def;
        if (Object.keys(values).length === 1 && 'displayName' in values) {
            // only 'displayName is given, perform a 'nameCreate'
            def = this._rpc({
                    model: parent.model,
                    method: 'nameCreate',
                    args: [values.displayName],
                    context: context,
    ...
    moveRecord: function (recordId, groupID, parentId) {
        var self = this;
        var parent = this.localData[parentId];
        var newGroup = this.localData[groupID];
        var changes = {};
        ...
        if (['date', 'datetime'].includes(groupedField.type)) {
            ...
            var value = _.findWhere(groupedField.selection, {1: newGroup.value});
        ...
        for (var i = 0; i < parent.data.length; i++) {
            ...
            var index = _.indexOf(oldGroup.data, recordId);
            if (index >= 0) {
    ...
    _readProgressBar: function (list) {
        return this._rpc({
            model: list.model,
            method: 'readProgressBar',
            kwargs: {
                domain: list.domain,
                groupby: list.groupedBy[0],
     ...
     * Data for the tooltips (groupByTooltip) are fetched in batch for all
     * groups, to avoid doing multiple calls.
     * Data are stored in a special key `tooltipData` on the datapoint.
     * Note that the option `groupByTooltip` is only for m2o fields.
     *
     * @private
    ...
    _readTooltipFields: function (list) {
        ...
        var groupIds = _.reduce(list.data, function (groupIds, id) {
            var resId = self.get(id, {raw: true}).resId;
        ...
        if (groupedByFieldInfo && groupedByFieldInfo.options) {
            tooltipFields = Object.keys(groupedByFieldInfo.options.groupByTooltip || {});
        }
        if (groupIds.length && tooltipFields.length) {
            var fieldNames = _.union(['displayName'], tooltipFields);
            return this._rpc({
                ...
                _.each(list.data, function (id) {
                    ...
                    dp.tooltipData = _.findWhere(result, {id: dp.resId});
                });
            });
        }
        return Promise.resolve();
   ...
   async  _reloadProgressBarGroupFromRecord(recordId, def) {
       ...
       if (element.type === 'list') {
           ...
           if (element.activeFilter) {
               // We must not readGroup when an active filter (and thus
