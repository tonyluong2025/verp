verp.define('web.GraphController', function (require) {
"use strict";
...
var GraphController = AbstractController.extend({
    ...
    getOwnedQueryParams: function () {
        ...
        return {
            context: {
                graphMeasure: state.measure,
                graphMode: state.mode,
                graphGroupbys: state.groupby,
            }
        };
    },
    /**
    ...
    updateButtons: function () {
        ...
        var state = this.model.get();
        this.$buttons.find('.o-graph-button').removeClass('active');
        this.$buttons
            .find('.o-graph-button[data-mode="' + state.mode + '"]')
            .addClass('active');
        this.$buttons
            .find('.o-graph-button[data-mode="stack"]')
            .data('stacked', state.stacked)
            .toggleClass('active', state.stacked)
            .toggleClass('o-hidden', state.mode !== 'bar');
        this.$buttons
            .find('.o-graph-button[data-order]')
            .toggleClass('o-hidden', state.mode === 'pie' || !!Object.keys(state.timeRanges).length)
            .filter('.o-graph-button[data-order="' + state.orderby + '"]')
    ...
    async _attachDropdownComponents() {
        ...
        this.measureMenu.el.classList.add('o-graph-measures-list');
        if (this.isEmbedded) {
            // Attach "groupby" button
            actionsContainer.appendChild(this.groupByMenu.el);
            this.groupByMenu.el.classList.add('o-groupby-menu');
        }
    ...
    _onButtonClick: function (ev) {
        ...
        if ($target.hasClass('o-graph-button')) {
