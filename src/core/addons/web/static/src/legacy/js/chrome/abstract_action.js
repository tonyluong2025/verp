...
const AbstractAction = Widget.extend(ActionMixin, {
    ...
    init: function (parent, action, options) {
        this._super(parent);
        this._title = action.displayName || action.label;
        ...
        if (this.hasControlPanel) {
            ...
            this.viewId = action.searchViewId && action.searchViewId[0];
    ...
    willStart: async function () {
        ...
        if (this.hasControlPanel) {
            if (this.loadControlPanel) {
                ...
                const options = { loadFilters: this.searchMenuTypes.includes('favorite') };
