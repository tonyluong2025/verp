verp.define("web.DomainSelector", function (require) {
"use strict";
...
// "childOf", "parentOf", "like", "not like", "=like", "=ilike"
...
var operatorMapping = {
    ...
    "childOf": _lt("child of"),
    "parentOf": _lt("parent of"),
    "like": "like",
    "not like": "not like",
    "=like": "=like",
    "=ilike": "=ilike",
...
var DomainNode = Widget.extend({
    events: {
        ...
        "click .o-domain-add-node-button": "_onAddButtonClick",
        "click .o-domain-delete-node-button": "_ondeleteButtonClick",
    ...
    /**
     * Called when the add button is clicked -> triggerUp an event to ask
    ...
    _onAddButtonClick: function (e) {
        e.preventDefault();
        e.stopPropagation();
        this.triggerUp("addNodeClicked", {newBranch: !!$(e.currentTarget).data("branch"), child: this});
    },
    /**
     * Called when the delete button is clicked -> triggerUp an event to ask
    ...
    _ondeleteButtonClick: function (e) {
        e.preventDefault();
        e.stopPropagation();
        this.triggerUp("deleteNodeClicked", {child: this});
    },
    /**
    ...
    _onButtonEntered: function (e) {
        ...
        this.$el.toggleClass("o-hover-btns", $target.hasClass("o-domain-delete-node-button"));
        this.$el.toggleClass("o-hover-add-node", $target.hasClass("o-domain-add-node-button"));
        this.$el.toggleClass("o-hover-add-inset-node", !!$target.data("branch"));
    },
    /**
    ...
    _onButtonLeft: function (e) {
        e.preventDefault();
        e.stopPropagation();
        this.$el.removeClass("o-hover-btns o-hover-add-node o-hover-add-inset-node");
    },
});
...
var DomainTree = DomainNode.extend({
    ...
    events: _.extend({}, DomainNode.prototype.events, {
        "click .o-domain-tree-operator-selector .dropdown-item": "_onOperatorChange",
    }),
    customEvents: {
        ...
        "addNodeClicked": "_onNodeAdditionAsk",
        "deleteNodeClicked": "_onNodeDeletionAsk",
    },
    /**
     * @constructor
     * @see DomainNode.init
     ...
     * -> triggerUp "domainChanged" if the child is added
     *
     * @private
    ...
    _addChild: function (domain, afterNode) {
        ...
        this.triggerUp("domainChanged", {child: this});
        return true;
    },
    /**
    ...
    _changeOperator: function (operator, silent) {
        ...
        if (!silent) this.triggerUp("domainChanged", {child: this});
    },
    /**
     * @see DomainTree.init
     * @private
    ...
    _postRender: function () {
        this.$childrenContainer = this.$("> .o-domain-node-children-container");
    },
    /**
     * Removes a given child from the widget.
     * -> triggerUp domainChanged if the child is removed
     *
     * @private
 ...
 * Known limitations:
 *
 * - Some operators like "childOf", "parentOf", "like", "not like",
 *   "=like", "=ilike" will come only if you use them from demo data or
 *   debug input.
 * - Some kind of domain can not be build right now
 *   e.g ("countryId", "in", [1,2,3]) but you can insert from debug input.
 */
var DomainSelector = DomainTree.extend({
    template: "DomainSelector",
    events: _.extend({}, DomainTree.prototype.events, {
        "click .o-domain-add-first-node-button": "_onAddFirstButtonClick",
        "change .o-domain-debug-input": "_onDebugInputChange",
    }),
    customEvents: _.extend({}, DomainTree.prototype.customEvents, {
        domainChanged: "_onDomainChange",
    }),
    ...
    _postRender: function () {
        ...
        this.$debugInput = this.$(".o-domain-debug-input");
    ...
    _onDebugInputChange: function (e) {
        ...
        // is syntax-valid a "domainChanged" event is triggered to notify the
...
var DomainLeaf = DomainNode.extend({
    template: "DomainLeaf",
    events: _.extend({}, DomainNode.prototype.events, {
        "change .o-domain-leaf-operator-select": "_onOperatorSelectChange",
        "change .o-domain-leaf-value-input": "_onValueInputChange",

        // Handle the tags widget part (TODO should be an independant widget)
        "click .o-domain-leaf-value-add-tag-button": "onAddTag",
        "keyup .o-domain-leaf-value-tags input": "onAddTag",
        "click .o-domain-leaf-value-remove-tag-button": "onRemoveTag",
    }),
    customEvents: {
        "fieldChainChanged": "_onFieldChainChange",
    },
    /**
     * @see DomainNode.init
     */
    ...
    willStart: function () {
        ...
        defs.push(this.fieldSelector.appendTo($("<div/>")).then((function () {
            ...
            if (!this.readonly) {
                ...
                if (_.contains(["childOf", "parentOf", "like", "not like", "=like", "=ilike"], this.operator)) {
                    // In case user entered manually or from demo data
                    this.operators[this.operator] = operatorMapping[this.operator];
                ...
                try {
                    if (selectedField && !selectedField.relation && !_.isArray(this.value)) {
                        this.displayValue = fieldUtils.format[selectedField.type](this.value, selectedField);
                    }
                } catch (err) {/**/}
                ...
                if (_.contains(["date", "datetime"], selectedField.type)) {
                    ...
                    wDefs.push(this.valueWidget.appendTo("<div/>").then((function () {
                        this.valueWidget.$el.addClass("o-domain-leaf-value-input");
                        this.valueWidget.setValue(moment(this.value));
                        this.valueWidget.on("datetimeChanged", this, function () {
     ...
     */
    start: function () {
        this.fieldSelector.$el.prependTo(this.$("> .o-domain-leaf-info, > .o-domain-leaf-edition")); // place the field selector
        if (!this.readonly && this.valueWidget) { // In edit mode, place the value widget if any
            this.$(".o-domain-leaf-value-input").replaceWith(this.valueWidget.$el);
        }
     ...
     *
     * -> triggerUp domainChanged event to ask for a re-rendering (if not
     * silent)
     *
     * @param {string[]} chain - the new field chain
     * @param {boolean} silent - true if the method call should not triggerUp a
     *                         domainChanged event
     */
     ...
     *
     * -> triggerUp domainChanged event to ask for a re-rendering
     * (if not silent)
     *
     * @param {string} operator - the new operator
     * @param {boolean} silent - true if the method call should not triggerUp a
    ...
    _changeValue: function (value, silent) {
        ...
        var selectedField = this.fieldSelector.getSelectedField() || {};
        try {
            this.value = fieldUtils.parse[selectedField.type](value, selectedField);
        } catch (err) {
            this.value = value;
            couldNotParse = true;
        }
        ...
        if (selectedField.type === "boolean") {
            ...
            if (couldNotParse || _.isBoolean(this.value)) {
                this.value = fieldUtils.parse[selectedField.type](fieldUtils.format[selectedField.type](moment())).toJSON(); // toJSON to get date with server format
            } else {
    ...
    _getOperatorsFromType: function (type) {
        ...
        switch (type) {
            ...
            case "text":
            case "html":
                operators = _.pick(operatorMapping, "=", "!=", "ilike", "not ilike", "set", "not set", "in", "not in");
                break;

            case "many2many":
            case "one2many":
            case "many2one":
                operators = _.pick(operatorMapping, "=", "!=", "ilike", "not ilike", "set", "not set");
                break;
            ...
            case "selection":
                operators = _.pick(operatorMapping, "=", "!=", "set", "not set");
                break;
            ...
            default:
                operators = _.extend({}, operatorMapping);
                break;
        }
    ...
    onAddTag: function (e) {
        ...
        var $input = this.$(".o-domain-leaf-value-tags input");
