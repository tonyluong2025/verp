verp.define('web.config', function (require) {
"use strict";
...
var config = {
    device: {
        ...
        *   - 'sizeChanged' : triggered when window size is
        *     corresponding to a new bootstrap breakpoint. The new sizeClass
         *    is provided.
        */
        bus: bus,
        /**
        ...
        touch: hasTouch(),
        /**
         * sizeClass is an integer: 0, 1, 2, 3 or 4, depending on the (current)
    ...
    isDebug: function (debugMode) {
        if (debugMode) {
            return verp.debug && verp.debug.indexOf(debugMode) !== -1;
        }
        return verp.debug;
    },
};
...
function _updateSizeProps() {
    ...
    if (sc !== config.device.sizeClass) {
        config.device.sizeClass = sc;
        config.device.isMobile = config.device.sizeClass <= config.device.SIZES.SM;
        config.device.bus.trigger('sizeChanged', config.device.sizeClass);
    }
}
