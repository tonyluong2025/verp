verp.define('web.Dialog', function (require) {
"use strict";
...
var Dialog = Widget.extend({
    ...
    customEvents: _.extend({}, Widget.prototype.customEvents, {
        focusControlButton: '_onFocusControlButton',
        closeDialog: '_onCloseDialog',
    }),
     ...
     * @param {string} [options.title=Verp]
    ...
    init: function (parent, options) {
        ...
        if (this.onAttachCallback) {
            this._opened = this.opened(this.onAttachCallback);
        }
        options = _.defaults(options || {}, {
            title: _t('Verp'), subtitle: '',
            size: 'large',
            fullscreen: false,
            dialogClass: '',
            $content: false,
            ...
            technical: true,
            $parentNode: false || $(document.body.querySelector(".o-dialog-container")),
            backdrop: 'static',
            renderHeader: true,
            renderFooter: true,
            onForceClose: false,
        ...
        core.bus.on('closeDialogs', this, this.destroy.bind(this));
    },
    /**
    ...
    setTitle: function (title, subtitle) {
        ...
        var $subtitle = $title.find('.o-subtitle').detach();
    ...
    open: function (options) {
        ...
        this.appendTo($('<div/>')).then(function () {
            ...
            core.bus.trigger("legacyDialogOpened", self);
        });
     ...
     *   `onClose` handler will pass this information related to closing this
     *   information.
    ...
    destroy: function (options) {
        ...
        if (this.el) {
            ...
            // Notifies new webclient to adjust UI active element
            core.bus.trigger("legacyDialogDestroyed", this);
        }
...
Dialog.alert = function (owner, message, options) {
    var buttons = [{
        text: _t("Ok"),
        close: true,
        click: options && options.confirmCallback,
    }];
    return new Dialog(owner, _.extend({
        ...
        title: _t("Alert"),
        onForceClose: options && (options.onForceClose || options.confirmCallback),
    ...
    var buttons = [
        {
            ...
            classes: 'btn-primary',
            close: true,
            click: makeCallback('confirmCallback'),
        },
        {
            text: _t("Cancel"),
            close: true,
            click: makeCallback('cancelCallback'),
        }
    ];
    return new Dialog(owner, _.extend({
        size: 'medium',
        ...
        onForceClose: options && (options.onForceClose || options.cancelCallback),
    }, options)).open({shouldFocusButtons:true});
};
...
Dialog.safeConfirm = function (owner, message, options) {
    ...
    var buttons = [
        {
            text: _t("Ok"),
            classes: 'btn-primary o-safe-confirm-button',
            close: true,
            click: options && options.confirmCallback,
            disabled: true,
        },
        {
            text: _t("Cancel"),
            close: true,
            click: options && options.cancelCallback
        }
    ];
    ...
    dialog.opened(function () {
        var $button = dialog.$footer.find('.o-safe-confirm-button');
