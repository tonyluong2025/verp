...
(function () {
    ...
    var verp = window.verp;
    var debug = verp.debug;
    ...
    verp.testing = typeof QUnit === "object";
    verp.remainingJobs = jobs;
    verp.__DEBUG__ = {
    ...
    verp.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === "string" ? args.shift() : "__verp_job" + jobUID++;
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: verp.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name,
                };
                if (jobs[k].error) {
                ...
                var deps = verp.__DEBUG__.getDependencies(job.name);
            ...
            missing = verp.__DEBUG__.getMissingJobs();
            failed = verp.__DEBUG__.getFailedJobs();
    ...
    verp.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec)
                        .then(function (data) {
                            ...
                            resolve();
                            verp.processJobs(jobs, services);
                        })
