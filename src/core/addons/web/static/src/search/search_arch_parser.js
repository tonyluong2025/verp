...
/**
 * Returns the split 'groupby' key from the given context attribute.
 * This helper accepts any invalid context or one that does not have
 * a valid 'groupby' key, and falls back to an empty list.
 * @param {string} context
 * @returns {string[]}
 */
const getContextGroubBy = (context) => {
    try {
        return makeContext([context]).groupby.split(":");
    } catch (err) {
        return [];
    }
};
...
export class SearchArchParser extends XMLParser {
    ...
    visitField(node) {
        ...
        if (node.hasAttribute("filterDomain")) {
            preField.filterDomain = node.getAttribute("filterDomain");
            ...
            if (name in this.searchDefaults) {
                ...
                if (fieldType === "selection") {
                    ...
                    this.labels.push((orm) => {
                        orm.call(relation, "nameGet", [value], { context }).then((results) => {
    ...
    visitSearchPanel(searchpanelNode) {
        ...
        if (searchpanelNode.hasAttribute("viewTypes")) {
            this.searchpanelInfo.viewTypes = searchpanelNode.getAttribute("viewTypes").split(",");
        }
        ...
        for (const node of searchpanelNode.children) {
            ...
            const section = {
                ...
                enableCounters: Boolean(evaluateExpr(attrs.enableCounters || "0")),
