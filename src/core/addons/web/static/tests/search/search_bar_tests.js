...
QUnit.module("Search", (hooks) => {
            ...
            views: {
                ...
                "partner,false,search": `
                    <search>
                        ...
                        <filter string="Birthday" name="date_groupby" context="{'groupby': 'birthday:day'}"/>
                    </search>
                `,
            },
            actions: {
    ...
    QUnit.test("basic rendering", async function (assert) {
        ...
        assert.strictEqual(
            ...
            controlPanel.el.querySelector(".o-searchview input"),
    ...
    QUnit.test("navigation with facets", async function (assert) {
        ...
        const controlPanel = await makeWithSearch({
            ...
            searchViewId: false,
            context: { searchDefault_date_groupby: 1 },
        });
        ...
        assert.strictEqual(
            document.activeElement,
            controlPanel.el.querySelector(".o-searchview .o-searchview-facet")
        );
    ...
    QUnit.test("navigation with facets (2)", async function (assert) {
        const controlPanel = await makeWithSearch({
            ...
            searchViewId: false,
            context: {
                searchDefault_dateGroupby: 1,
                searchDefault_foo: 1,
            },
        });

        assert.containsN(controlPanel, ".o-searchview .o-searchview-facet", 2);
        assert.strictEqual(
            document.activeElement,
            controlPanel.el.querySelector(".o-searchview input")
        );
        ...
        assert.strictEqual(
            document.activeElement,
            controlPanel.el.querySelector(".o-searchview .o-searchview-facet:nth-child(2)")
        );
        ...
        assert.strictEqual(
            document.activeElement,
            controlPanel.el.querySelector(".o-searchview .o-searchview-facet:nth-child(1)")
        );
    ...
    QUnit.test("search date and datetime fields. Support of timezones", async function (assert) {
        ...
        let searchInput = controlPanel.el.querySelector(".o-searchview input");
        ...
        // Close Facet
        await click(controlPanel.el.querySelector(".o-searchview-facet .o-facet-remove"));

        // DateTime case
        await editSearch(controlPanel, "07/15/1983 00:00:00");
        searchInput = controlPanel.el.querySelector(".o-searchview input");
        await triggerEvent(searchInput, null, "keydown", { key: "ArrowDown" });
        await triggerEvent(searchInput, null, "keydown", { key: "ArrowDown" });
        await triggerEvent(searchInput, null, "keydown", { key: "Enter" }); // select
    ...
    QUnit.test("autocomplete menu clickout interactions", async function (assert) {
        ...
        const input = controlPanel.el.querySelector(".o-searchview input");

        assert.containsNone(controlPanel, ".o-searchview-autocomplete");

        await editSearch(controlPanel, "Hello there");

        assert.strictEqual(input.value, "Hello there", "input value should be updated");
        assert.containsOnce(controlPanel, ".o-searchview-autocomplete");
    ...
    QUnit.test("select an autocomplete field", async function (assert) {
        ...
        assert.containsN(
            controlPanel,
            ".o-searchview-autocomplete li",
            2,
            "there should be 2 result for 'a' in search bar autocomplete"
        );

        const searchInput = controlPanel.el.querySelector(".o-searchview input");
        await triggerEvent(searchInput, null, "keydown", { key: "Enter" });
        assert.strictEqual(
            controlPanel.el
                .querySelector(".o-searchview-input-container .o-facet-values")
                .innerText.trim(),
            "a",
    ...
    QUnit.test("no search text triggers a reload", async function (assert) {
        ...
        assert.containsNone(controlPanel, ".o-searchview-facet-label");
    ...
    QUnit.test("selecting (no result) triggers a search bar rendering", async function (assert) {
        ...
        assert.strictEqual(
            controlPanel.el
                .querySelector(".o-searchview-autocomplete .o-selection-focus")
                .innerText.trim(),
            "(no result)",
        ...
        assert.containsNone(controlPanel, ".o-searchview-facet-label");
        assert.strictEqual(
            controlPanel.el.querySelector(".o-searchview input").value,
            "",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.strictEqual(
                controlPanel.el.querySelector(".o-searchview-autocomplete li").innerText.trim(),
                "Search Foo for: TEST",
    ...
    QUnit.test("select autocompleted many2one", async function (assert) {
        ...
        await click(controlPanel.el.querySelector(".o-searchview-autocomplete li:last-child"));

        assert.deepEqual(getDomain(controlPanel), [["bar", "childOf", "rec"]]);
        ...
        await editSearch(controlPanel, "rec");
        await click(controlPanel.el.querySelector(".o-expand"));
        await click(
            controlPanel.el.querySelector(".o-searchview-autocomplete li.o-menu-item.o-indent")
        );

        assert.deepEqual(getDomain(controlPanel), [["bar", "childOf", 1]]);
    });
    ...
    QUnit.test('"null" as autocomplete value', async function (assert) {
        ...
        assert.strictEqual(
            controlPanel.el.querySelector(".o-searchview-autocomplete .o-selection-focus")
                .innerText,
            "Search Foo for: null"
        );

        await click(
            controlPanel.el.querySelector(".o-searchview-autocomplete li.o-selection-focus a")
        );
    ...
    QUnit.test("autocompletion with a boolean field", async function (assert) {
        ...
        assert.containsN(controlPanel, ".o-searchview-autocomplete li", 1);
        assert.strictEqual(
            controlPanel.el.querySelector(".o-searchview-autocomplete li:last-child").innerText,
            "Search Bool for: Yes"
        );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const mockRPC = async (route) => {
                if (route.includes("/partner/nameSearch")) {
                    await def;
                }
            };
            const controlPanel = await makeWithSearch({
            ...
            await triggerEvent(
                controlPanel.el,
                ".o-searchview-autocomplete li.o-menu-item:first-child",
                "mousemove"
            );
            assert.containsNone(controlPanel, ".o-searchview-autocomplete li.o-menu-item.o-indent");

            def.resolve();
            await nextTick();
            assert.containsN(controlPanel, ".o-searchview-autocomplete li.o-menu-item.o-indent", 5);
        }
    );
    ...
    QUnit.test("checks that an arrowDown always selects an item", async function (assert) {
        ...
        await editSearch(controlPanel, "rec");
        await click(controlPanel.el.querySelector(".o-expand"));
        click(controlPanel.el.querySelector(".o-expand"));
        triggerEvent(
            controlPanel.el,
            ".o-searchview-autocomplete li.o-menu-item.o-indent:last-child",
            "mousemove"
        );
        const searchInput = controlPanel.el.querySelector(".o-searchview input");
        await triggerEvent(searchInput, null, "keydown", { key: "ArrowDown" });
        assert.containsOnce(controlPanel, ".o-selection-focus");
    });
