...
QUnit.module("GroupBy Class", {}, () => {
    ...
    QUnit.test("simple valid group by", async function (assert) {
        assert.expect(6);
        let groupBy = getGroupBy("displayName");
        assert.strictEqual(groupBy.fieldName, "displayName");
        assert.strictEqual(groupBy.interval, null);
        assert.strictEqual(groupBy.spec, "displayName");
        groupBy = getGroupBy("displayName:quarter");
        assert.strictEqual(groupBy.fieldName, "displayName");
        assert.strictEqual(groupBy.interval, "quarter");
        assert.strictEqual(groupBy.spec, "displayName:quarter");
    });
    ...
    QUnit.test("simple valid group by", async function (assert) {
        assert.expect(3);
        let groupBy = getGroupBy("displayName", fields);
        assert.strictEqual(groupBy.fieldName, "displayName");
        assert.strictEqual(groupBy.interval, null);
        assert.strictEqual(groupBy.spec, "displayName");
    });
    QUnit.test("simple invalid group by", async function (assert) {
        ...
        try {
            getGroupBy("displayName:day", fields);
        } catch (e) {
            assert.step("Error 2");
        }
        try {
