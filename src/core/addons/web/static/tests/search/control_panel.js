...
QUnit.module("Search", (hooks) => {
    ...
    QUnit.test("simple rendering", async (assert) => {
        ...
        assert.containsOnce(controlPanel, ".o-cp-top");
        assert.containsOnce(controlPanel, ".o-cp-top-left");
        assert.containsNone(controlPanel, ".o-cp-top-right");
        assert.containsOnce(controlPanel, ".o-cp-bottom");
        assert.containsOnce(controlPanel, ".o-cp-bottom-left");
        assert.containsOnce(controlPanel, ".o-cp-bottom-right");

        assert.containsNone(controlPanel, ".o-cp-switch-buttons");
    ...
    QUnit.test("viewSwitcherEntries prop", async (assert) => {
        ...
        assert.containsOnce(controlPanel, ".o-cp-switch-buttons");
        assert.containsN(controlPanel, ".o-switch-view", 2);
        const views = controlPanel.el.querySelectorAll(".o-switch-view");
