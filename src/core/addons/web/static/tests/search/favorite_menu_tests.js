...
QUnit.module("Search", (hooks) => {
    ...
    QUnit.test("simple rendering with no favorite", async function (assert) {
        ...
        assert.containsOnce(controlPanel, ".dropdown-menu .o-add-favorite");
    });
    ...
    QUnit.test("delete an active favorite", async function (assert) {
        ...
        assert.hasClass(webClient.el.querySelector(".o_favorite_menu .o-menu-item"), "selected");

        await deleteFavorite(webClient, 0);

        assert.verifySteps([]);

        await click(document.querySelector("div.o-dialog footer button"));

        assert.deepEqual(getFacetTexts(webClient), []);
        assert.containsNone(webClient, ".o_favorite_menu .o-menu-item");
        assert.containsOnce(webClient, ".o_favorite_menu .o-add-favorite");
