...
QUnit.module("Search", (hooks) => {
    ...
    QUnit.test("simple rendering", async function (assert) {
        ...
        const customGroupByItem = controlPanel.el.querySelector(".o-add-custom_group_menu");
        ...
        assert.deepEqual(
            [...controlPanel.el.querySelectorAll(".o-add-custom_group_menu select option")].map(
                (el) => el.innerText
            ),
            ["Birthday", "Date", "Foo"]
        );
            ...
            assert.deepEqual(
                [
                    ...controlPanel.el.querySelectorAll(
                        ".o-add-custom_group_menu .dropdown-menu select option"
                    ),
                ].map((el) => el.innerText),
                ["Foo"]
            );
    ...
    QUnit.test("click on add custom group toggle group selector", async function (assert) {
        ...
        const addCustomGroupMenu = controlPanel.el.querySelector(".o-add-custom_group_menu");
        ...
        assert.containsOnce(controlPanel, ".o-add-custom_group_menu .dropdown-menu select");
        assert.strictEqual(
            controlPanel.el
                .querySelector(".o-add-custom_group_menu .dropdown-menu select option")
                .innerText.trim(),
            "Super Date"
        );

        // Button apply
        assert.containsOnce(controlPanel, ".o-add-custom_group_menu .dropdown-menu .btn");
    });
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.containsOnce(controlPanel, ".o-group-by-menu .o-menu-item");
            assert.containsOnce(controlPanel, ".o-add-custom-group-menu .dropdown-toggle");
            assert.containsOnce(controlPanel, ".o-add-custom-group-menu .dropdown-menu");
