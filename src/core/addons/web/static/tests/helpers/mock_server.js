...
export function _fieldsViewGet(params) {
    ...
    const isBaseModel = !context.baseModelName || modelName === context.baseModelName;
    ...
    traverseElementTree(doc, (node) => {
        ...
        modifiersNames.forEach((attr) => {
            ...
            if (mod) {
                // TODO
                // const pyevalContext = window.vj.dict.fromJSON(context || {});
    ...
    Object.entries(fieldNodes).forEach(([name, node]) => {
        ...
        if (field.type === "many2one" || field.type === "many2many") {
            ...
            const canWrite = node.getAttribute("canWrite");
            ...
            Array.from(node.children).forEach((children) => {
                if (children.tagName) {
                    ...
                    field.views[children.tagName] = _fieldsViewGet({
                        ...
                        modelName: relModel,
                        fields: relFields,
                        context: Object.assign({}, context, { baseModelName: modelName }),
                        processedNodes,
                    });
                }
            });
...
export class MockServer {
    ...
    mockLoadAction(kwargs) {
        const action = this.actions[kwargs.actionId];
        if (!action) {
            // when the action doesn't exist, the real server doesn't crash, it
            // simply returns false
            console.warn(
                `No action found for ID ${kwargs.actionId} during test ${QUnit.config.current.testName}`
            );
        }
        return action || false;
    }
    ...
    mockNameGet(model, args) {
        ...
        var names = ids.map((id) =>
            id ? [id, records.find((r) => r.id === id).displayName] : [null, "false"]
        );
        return names;
    }
    ...
    mockNameSearch(model, args, kwargs) {
        ...
        for (const r of records) {
            ...
            if (isInDomain && (!str.length || r.displayName.includes(str))) {
                result.push([r.id, r.displayName]);
            }
        }
        ...
        return records.map((record) => {
            ...
            for (const fieldName of fields) {
                ...
                if (["float", "integer", "monetary"].includes(field.type)) {
                    ...
                    if (relRecord) {
                        result[fieldName] = [record[fieldName], relRecord.displayName];
                    } else {
    ...
    mockSearchPanelSelectRange(model, [fieldName], kwargs) {
        ...
        if (hierarchize && this.models[field.relation].fields.parentId) {
    ...
    mockSearchPanelSelectMultiRange(model, [fieldName], kwargs) {
        ...
        if (field.type === "many2one") {
            let domainImage;
            if (enableCounters || !expand) {
                extraDomain = new Domain([...extraDomain, ...(kwargs.groupDomain || [])]).toList();
                modelDomain = new Domain([...modelDomain, ...(kwargs.groupDomain || [])]).toList();
                const newKwargs = Object.assign({}, kwargs, {
                    model_domain: modelDomain,
                    extra_domain: extraDomain,
                    only_counters: expand,
    ...
    mockSearchController(params) {
        ...
        const activeTest = context && "activeTest" in context ? context.activeTest : true;
        let records = this.getRecords(
            params.model,
            params.domain || [],
            Object.assign({}, params.context, { activeTest })
        );
        if (params.sort) {
     ...
     * the domain.
     */
    getRecords(modelName, domain, { activeTest = true } = {}) {
        ...
        if (activeTest && "active" in model.fields) {
