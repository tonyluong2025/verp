verp.define('web.action_menus_tests', function (require) {
    "use strict";
    ...
    QUnit.module('Components', {
        ...
        QUnit.test('basic interactions', async function (assert) {
            ...
            assert.strictEqual(dropdowns[0].querySelector('.o-dropdown-title').innerText.trim(), "Print");
            assert.strictEqual(dropdowns[1].querySelector('.o-dropdown-title').innerText.trim(), "Action");
            ...
            assert.containsN(actionMenus, '.dropdown-menu .o-menu-item', 4);
            const actionsTexts = [...dropdowns[1].querySelectorAll('.o-menu-item')].map(el => el.innerText.trim());
            ...
            assert.containsOnce(actionMenus, '.dropdown-menu');
            assert.containsN(actionMenus, '.dropdown-menu .o-menu-item', 1);
        ...
        QUnit.test("empty action menus", async function (assert) {
            ...
            assert.containsNone(actionMenus, ".o-cp-action-menus > *");
        ...
        QUnit.test('execute action with context', async function (assert) {
            ...
            const actionMenus = await createComponent(ActionMenus, {
                ...
                props: {
                    ...
                    context: {
                        allowedCompanyIds: [112],
                    },
                },
                async mockRPC(route, args) {
                    if (route === "/web/dataset/callKw/hobbit/search"){
                        assert.deepEqual(args.kwargs.context, { allowedCompanyIds: [112] }, "The kwargs should contains the right context");
                    }
                    return this._super(...arguments);
                },
            });
