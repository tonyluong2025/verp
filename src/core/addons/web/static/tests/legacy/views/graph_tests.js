verp.define('web.graph_view_tests', function (require) {
"use strict";
...
QUnit.module('Views', {
    beforeEach: function () {
        this.data = {
            foo: {
                ...
                records: [
                    { id: 1, foo: 3, bar: true, productId: 37, date: "2016-01-01", revenue: 1 },
                    { id: 2, foo: 53, bar: true, productId: 37, color_id: 7, date: "2016-01-03", revenue: 2 },
                    { id: 3, foo: 2, bar: true, productId: 37, date: "2016-03-04", revenue: 3 },
                    { id: 4, foo: 24, bar: false, productId: 37, date: "2016-03-07", revenue: 4 },
                    { id: 5, foo: 4, bar: false, productId: 41, date: "2016-05-01", revenue: 5 },
                    { id: 6, foo: 63, bar: false, productId: 41 },
                    { id: 7, foo: 42, bar: false, productId: 41 },
                    { id: 8, foo: 48, bar: false, productId: 41, date: "2016-04-01", revenue: 8 },
                ]
            },
            product: {
                fields: {
    ...
    QUnit.test('title attribute', async function (assert) {
        ...
        assert.strictEqual(graph.$('.o-legacy-graph-renderer label').text(), "Partners",
    ...
    QUnit.test('displaying chart data with multiple groupbys', async function (assert) {
        ...
        const graph = await createView({
            ...
            groupBy: ['productId', 'bar', 'color_id'],
        });
    ...
    QUnit.test('no content helper after update', async function (assert) {
        ...
        await testUtils.graph.reload(graph, { domain: [['productId', '=', 4]] });
        assert.containsOnce(graph, 'div.o-graph-canvas-container canvas');
        assert.containsNone(graph, 'div.o-view-nocontent');
        assert.containsNone(graph, '.abc');
        graph.destroy();
    ...
    QUnit.test('getOwnedQueryParams correctly returns mode, measure, and groupbys', async function (assert) {
        ...
        await testUtils.graph.reload(graph, { groupBy: ['productId', 'color_id'] }); // change groupbys
        assert.deepEqual(graph.getOwnedQueryParams(), {
            context: {
                graphMode: 'line',
                graphMeasure: 'foo',
                graphGroupbys: ['productId', 'color_id'],
            },
        }, "context should be correct");
    ...
    QUnit.test('reload graph with correct fields', async function (assert) {
        ...
        const graph = await createView({
            ...
            mockRPC: function (route, args) {
                if (args.method === 'readGroup') {
                    assert.deepEqual(args.kwargs.fields, ['productId', 'foo'],
    ...
    QUnit.test('not use a many2one as a measure by default', async function (assert) {
        ...
        assert.notOk(graph.measures.productId,
    ...
    QUnit.test('use a many2one as a measure if set as additional fields', async function (assert) {
        ...
        assert.ok(graph.measures.find(m => m.fieldName === 'productId'),
    ...
    QUnit.test('measure dropdown consistency', async function (assert) {
        ...
        assert.containsOnce(webClient, '.o-control-panel .o_graph_measures_list',
    ...
    QUnit.test('graph view with string attribute on view architecture fields', async function (assert) {
        ...
        const graph = await createView({
            ...
            arch: `
            <graph string="Partners">
                <field name="productId" type="measure" string="Jethalal"/>
                <field name="foo" type="measure"/>
            </graph>`,
        });
    ...
    QUnit.test('graph view "graphMeasure" field in context not in measures', async function (assert) {
        assert.expect(2);
    ...
    QUnit.test('clicking on bar and pie charts triggers a doAction', async function (assert) {
        assert.expect(5);
    ...
    QUnit.test('clicking charts trigger a doAction with correct views', async function (assert) {
        assert.expect(3);
    ...
    QUnit.test('graph view without invisible attribute on field', async function (assert) {
        ...
        assert.containsN(graph, 'li.o-menu-item', 3,
            "there should be three menu item in the measures dropdown (count, revenue and foo)");
        assert.containsOnce(graph, 'li.o-menu-item a:contains("Revenue")');
        assert.containsOnce(graph, 'li.o-menu-item a:contains("Foo")');
        assert.containsOnce(graph, 'li.o-menu-item a:contains("Count")');
    ...
    QUnit.test('graph view with invisible attribute on field', async function (assert) {
        ...
        await testUtils.dom.click(graph.$('.btn-group:first button'));
        assert.containsN(graph, 'li.o-menu-item', 2,
            "there should be only two menu item in the measures dropdown (count and foo)");
        assert.containsNone(graph, 'li.o-menu-item a:contains("Revenue")');
    ...
    QUnit.test('graph view sort by measure', async function (assert) {
        ...
        this.data.product.records.push({ id: 38, displayName: "zphone" });
        this.data.foo.records[7].productId = 38;
    ...
    QUnit.test('empty graph view with sample data', async function (assert) {
        ...
        assert.hasClass(graph.$('.o-graph-canvas-container'), 'o-sample-data-disabled');
        ...
        assert.containsOnce(graph, '.o-graph-canvas-container canvas');
        assert.doesNotHaveClass(graph.$('.o-graph-canvas-container'), 'o-sample-data-disabled');
    ...
    QUnit.module('GraphView: comparison mode (legacy)', {
        beforeEach: async function () {
            ...
            this.data.foo.records.push({ id: 9, foo: 48, bar: false, productId: 41, color_id: 7, date: "2016-12-01" });
            this.data.foo.records.push({ id: 10, foo: 17, bar: true, productId: 41, color_id: 7, date: "2016-12-01" });
            this.data.foo.records.push({ id: 11, foo: 45, bar: true, productId: 37, color_id: 14, date: "2016-12-01" });
            this.data.foo.records.push({ id: 12, foo: 48, bar: false, productId: 37, color_id: 7, date: "2016-12-10" });
            this.data.foo.records.push({ id: 13, foo: 48, bar: false, productId: undefined, color_id: 14, date: "2016-11-30" });
            this.data.foo.records.push({ id: 14, foo: -50, bar: true, productId: 41, color_id: 14, date: "2016-12-15" });
            this.data.foo.records.push({ id: 15, foo: 53, bar: false, productId: 41, color_id: 14, date: "2016-11-01" });
            this.data.foo.records.push({ id: 16, foo: 53, bar: true, productId: undefined, color_id: 7, date: "2016-09-01" });
            this.data.foo.records.push({ id: 17, foo: 48, bar: false, productId: 41, color_id: undefined, date: "2016-08-01" });
            this.data.foo.records.push({ id: 18, foo: -156, bar: false, productId: 37, color_id: undefined, date: "2016-07-15" });
            this.data.foo.records.push({ id: 19, foo: 31, bar: false, productId: 41, color_id: 14, date: "2016-12-15" });
            this.data.foo.records.push({ id: 20, foo: 109, bar: true, productId: 41, color_id: 7, date: "2015-06-01" });
            ...
            const graph = await createView({
                ...
                archs: {
                    'foo,false,search': `
                        <search>
                            <filter name="date" string="Date" context="{'groupby': 'date'}"/>
                            ...
                            <filter name="color_id" string="Color" context="{'groupby': 'color_id'}"/>
                        </search>`,
                },
                viewOptions: {
                    additionalMeasures: ['productId'],
                ...
                for await (const combination of graphGenerator(combinations)) {
                    ...
                    if (combination.toString() in this.combinationsToCheck) {
                        if (this.combinationsToCheck[combination].errorMessage) {
                            assert.strictEqual(
                                graph.$('.o-nocontent-help p').eq(1).text().trim(),
            ...
            this.selectTimeRanges = async (comparisonOptionId, basicDomainId) => {
                const facetEls = graph.el.querySelectorAll('.o-searchview-facet');
            ...
            this.setMode = async (mode) => {
                await testUtils.dom.click($(`.o-control-panel .o_graph_button[data-mode="${mode}"]`));
            };
