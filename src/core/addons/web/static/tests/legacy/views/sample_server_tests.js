verp.define('web.sample_server_tests', function (require) {
    "use strict";

    const SampleServer = require('web.SampleServer');
    const session = require('web.session');
    const { mock } = require('web.testUtils');
    ...
    QUnit.module("Sample Server", {
        beforeEach() {
            this.fields = {
                'res.users': {
                    ...
                    tagIds: { string: "Tags", type: 'many2many', relation: 'tag' },
        ...
        QUnit.test("Sample data: people type + all field names", async function (assert) {
            ...
            // Basic fields
            assert.ok(SAMPLE_PEOPLE.includes(rec.displayName));
            assert.ok(SAMPLE_PEOPLE.includes(rec.name));
            assert.strictEqual(rec.email,
                `${rec.displayName.replace(/ /, ".").toLowerCase()}@sample.demo`
            );
            ...
            assert.strictEqual(rec.tagIds.length, 2);
            assert.ok(rec.tagIds.every(
                (id) => typeof id === 'number')
            );
        ...
        QUnit.test("Sample data: country type", async function (assert) {
            ...
            assert.ok(SAMPLE_COUNTRIES.includes(records[0].displayName));
        });
        ...
        QUnit.test("Sample data: any type", async function (assert) {
            ...
            assert.ok(SAMPLE_TEXTS.includes(records[0].displayName));
        });

        QUnit.module("RPC calls");

        QUnit.test("Send 'searchRead' RPC: valid field names", async function (assert) {
            assert.expect(3);
            ...
            assert.ok(/\w+/.test(result.records[0].displayName),
                "Display name has been mocked"
            );
        });

        QUnit.test("Send 'searchRead' RPC: invalid field names", async function (assert) {
            assert.expect(3);
        ...
        QUnit.test("Send 'webReadGroup' RPC: no group", async function (assert) {
            assert.expect(1);
        ...
        QUnit.test("Send 'webReadGroup' RPC: 2 groups", async function (assert) {
            assert.expect(5);
        ...
        QUnit.test("Send 'webReadGroup' RPC: all groups", async function (assert) {
            assert.expect(5);
        ...
        QUnit.test("Send 'readGroup' RPC: no group", async function (assert) {
            assert.expect(1);
        ...
        QUnit.test("Send 'readGroup' RPC: groupBy", async function (assert) {
            assert.expect(3);
        ...
        QUnit.test("Send 'readGroup' RPC: groupBy and field", async function (assert) {
            assert.expect(4);
        ...
        QUnit.test("Send 'readGroup' RPC: multiple groupBys and lazy", async function (assert) {
            assert.expect(2);
        ...
        QUnit.test("Send 'readGroup' RPC: multiple groupBys and not lazy", async function (assert) {
            assert.expect(2);
        ...
        // QUnit.test("Send 'readProgressBar' RPC", async function (assert) { ... });
    });
});
