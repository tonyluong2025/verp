verp.define('web.data_manager_tests', function (require) {
    "use strict";
        ...
        const serverMethods = {
            async load_views({ kwargs, model }) {
                ...
                if (options.loadFilters) {
                    result.filters = data['ir.filters'].records.filter(r => r.modelId === model);
                }
                return result;
            },
            async get_filters({ args, model }) {
                return data[model].records.filter(r => r.modelId === args[0]);
            },
    ...
    QUnit.module("Services", {
            ...
            this.data = {
                ...
                'ir.filters': {
                    fields: {
                        ...
                        modelId: { type: "Selection", string: "Model" },
        ...
        QUnit.test("Load views with filters (non-debug mode)", async function (assert) {
            ...
            const filters = await dataManager.loadFilters({ modelName: 'oui' });
        ...
        QUnit.test("Cache invalidation and filters addition/deletion", async function (assert) {
            ...
            // A few unnecessary 'loadFilters' are done in this test to assert
            ...
            filters = await dataManager.loadFilters({ modelName: 'oui' });
            assert.deepEqual(firstLoad.search.favoriteFilters, filters,
                "loadFilters and load_views.search should return the same filters");
            ...
            const expectedFilters = await dataManager.loadFilters({ modelName: 'oui' });

            assert.deepEqual(filters, expectedFilters,
                "Filters loaded by the load_views should be equal to the result of a loadFilters");
