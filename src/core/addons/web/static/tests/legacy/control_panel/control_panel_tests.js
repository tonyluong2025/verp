verp.define('web.control_panel_tests', function (require) {
    "use strict";
    ...
    QUnit.module('ControlPanel', {
        ...
        QUnit.test('default field operator', async function (assert) {
            ...
            const arch = `
                <search>
                    <field name="bar"/>
                    <field name="bar_op" operator="childOf"/>
                    <field name="foo"/>
        ...
        QUnit.test('remove a facet with backspace', async function (assert) {
            ...
            // delete a facet
            const searchInput = controlPanel.el.querySelector('input.o-searchview-input');
            await testUtils.dom.triggerEvent(searchInput, 'keydown', { key: 'Backspace' });

            assert.containsNone(controlPanel, 'div.o-searchview div.o-searchview-facet');
        ...
        QUnit.test('fields and filters with groups/invisible attribute', async function (assert) {
            ...
            const arch = `
                <search>
                    <field name="displayName" string="Foo B" invisible="1"/>
                    ...
                    <filter name="groupByA" string="GA" context="{ 'groupby': 'date_field:day' }"/>
                    <filter name="groupByB" string="GB" context="{ 'groupby': 'date_field:day' }" invisible="1"/>
                </search>`;
            ...
            const params = {
                cpModelConfig: {
                    ...
                    context: {
                        show_filterC: true,
                        search_default_displayName: 'value',
            ...
            assert.containsN(controlPanel, 'div.o-searchview-facet', 3);
            selectorContainsValue('.o-searchview-facet', "FooBvalue", true);
            selectorContainsValue('.o-searchview-facet .o-facet-values', "FB", true);
            selectorContainsValue('.o-searchview-facet .o-facet-values', "GB", true);

            await cpHelpers.toggleFilterMenu(controlPanel);

            selectorContainsValue('.o-menu-item', "FA", true);
            selectorContainsValue('.o-menu-item', "FB", false);
            selectorContainsValue('.o-menu-item', "FC", true);

            await cpHelpers.toggleGroupByMenu(controlPanel);

            selectorContainsValue('.o-menu-item', "GA", true);
            selectorContainsValue('.o-menu-item', "GB", false);

            // 'a' to filter nothing on bar
            await cpHelpers.editSearch(controlPanel, 'a');

            // the only item in autocomplete menu should be FooA: a
            selectorContainsValue('.o-searchview-autocomplete', "SearchFooAfor:a", true);
            await cpHelpers.validateSearch(controlPanel);
            selectorContainsValue('.o-searchview-facet', "FooAa", true);
        ...
        QUnit.test('invisible fields and filters with unknown related fields should not be rendered', async function (assert) {
            ...
            // the "base.groupSystem" group and both "bar" and "date_field" fields
            ...
            // - set filters with groups="base.groupSystem" as `invisible=1` in
            ...
            assert.containsNone(controlPanel.el, 'div.o-search-options div.o-filter-menu',
                "there should not be filter dropdown");
            assert.containsNone(controlPanel.el, 'div.o-search-options div.o-group-by-menu',
        ...
        QUnit.test('groupby menu is not rendered if searchMenuTypes does not have groupby', async function (assert) {
            ...
            assert.containsOnce(controlPanel.el, 'div.o-search-options div.o-filter-menu');
            assert.containsNone(controlPanel.el, 'div.o-search-options div.o-group-by-menu');
        ...
        QUnit.test('search field should be autofocused', async function (assert) {
            ...
            assert.containsOnce(controlPanel, '.o-searchview-input', "has a search field");
            assert.containsOnce(controlPanel, '.o-searchview-input:focus-within',
        ...
        QUnit.test("search field's autofocus should be disabled on mobile device", async function (assert) {
            ...
            assert.containsOnce(controlPanel, '.o-searchview-input', "has a search field");
            assert.containsNone(controlPanel, '.o-searchview-input:focus-within',
