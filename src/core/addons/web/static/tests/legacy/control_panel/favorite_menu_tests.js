verp.define('web.favorite_menu_tests', function (require) {
    "use strict";
    ...
    QUnit.module('Components', {
        ...
        QUnit.test('simple rendering with no favorite', async function (assert) {
            ...
            assert.containsOnce(controlPanel, '.o-add-favorite');
            assert.strictEqual(controlPanel.el.querySelector('.o-add-favorite > button').innerText.trim(),
                "Save current search");
            ...
            assert.containsN(controlPanel, '.o-add-favorite .custom-checkbox input[type="checkbox"]', 2);
        ...
        QUnit.test('dynamic filters are saved dynamic', async function (assert) {
            ...
            const arch = `
            <search>
                <filter string="Float" name="positive" domain="[('date_field', '>=', (contextToday() + relativedelta()).strftime('%Y-%m-%d'))]"/>
            </search>
        `;
            const params = {
                cpModelConfig: {
        ...
        QUnit.test('delete an active favorite remove it both in list of favorite and in search bar', async function (assert) {
            ...
            assert.deepEqual(cpHelpers.getFacetTexts(controlPanel), ["My favorite"]);
            assert.hasClass(controlPanel.el.querySelector('.o_favorite_menu .o-menu-item'), 'selected');

            await cpHelpers.deleteFavorite(controlPanel, 0);

            // confirm deletion
            await testUtils.dom.click(document.querySelector('div.o-dialog footer button'));
            assert.deepEqual(cpHelpers.getFacetTexts(controlPanel), []);
        ...
        QUnit.test('toggle favorite correctly clears filter, groupbys, comparison and field "options"', async function (assert) {
            ...
            <search>
                ...
                <filter string="Date Field Groupby" name="coolName" context="{'groupby': 'date_field'}"/>
            </search>
        `;
        ...
        QUnit.test('save search filter in modal', async function (assert) {
            ...
            const data = {
                partner: {
                    ...
                    records: [
                        { id: 1, displayName: "First record", foo: "yop", bar: 2, date_field: "2017-01-25", birthday: "1983-07-15", float_field: 1 },
                        { id: 2, displayName: "Second record", foo: "blip", bar: 1, date_field: "2017-01-24", birthday: "1982-06-04", float_field: 2 },
                        { id: 3, displayName: "Third record", foo: "gnap", bar: 1, date_field: "2017-01-13", birthday: "1985-09-13", float_field: 1.618 },
                        { id: 4, displayName: "Fourth record", foo: "plop", bar: 2, date_field: "2017-02-25", birthday: "1983-05-05", float_field: -1 },
                        { id: 5, displayName: "Fifth record", foo: "zoup", bar: 2, date_field: "2016-01-25", birthday: "1800-01-01", float_field: 13 },
            ...
            assert.strictEqual(document.querySelector('.o-filter-condition select.o-generator-menu-field').value,
                'date_field',
            ...
            await testUtils.dom.click(document.querySelector('.o-add-favorite button.btn-primary'));
            ...
            const data = {
                partner: {
                    ...
                    records: Array.apply(null, Array(10)).map(function(_, i) {
                        return { id: i, displayName: "Record " + i, bar: 1 };
                    })
                },
            };
            const form = await createView({
                ...
                interceptsPropagate: {
                    load_views: function (ev) {
                        assert.ok(ev.data.options.loadFilters, "opening dialog should load the filters");
                    },
                },
            });
