verp.define('web.groupby_menu_tests', function (require) {
    "use strict";
    ...
    QUnit.module('Components', {
        ...
        QUnit.test('simple rendering with neither groupbys nor groupable fields', async function (assert) {
            ...
            assert.containsNone(controlPanel, '.o-menu-item, .dropdown-divider, .o-add-custom_group_menu');
        ...
        QUnit.test('simple rendering with no groupby', async function (assert) {
            ...
            assert.containsNone(controlPanel, '.o-menu-item, .dropdown-divider');
            assert.containsOnce(controlPanel, '.o-add-custom_group_menu');

            await cpHelpers.toggleAddCustomGroup(controlPanel);

            const optionEls = controlPanel.el.querySelectorAll('.o-add-custom_group_menu select option');
            assert.deepEqual(
        ...
        QUnit.test('simple rendering with a single groupby', async function (assert) {
            ...
            const arch = `
                <search>
                    <filter string="Groupby Foo" name="gb_foo" context="{'groupby': 'foo'}"/>
                </search>`;
            const params = {
            ...
            assert.strictEqual(controlPanel.el.querySelector('.o-menu-item').innerText.trim(), "Groupby Foo");
        ...
        QUnit.test('toggle a "simple" groupby in groupby menu works', async function (assert) {
            ...
            assert.containsOnce(controlPanel.el.querySelector('.o-searchview .o-searchview-facet'),
                'span.fa.fa-bars.o-searchview-facet-label');
        ...
        QUnit.test('remove a "Group By" facet properly unchecks groupbys in groupby menu', async function (assert) {
            ...
            await cpHelpers.toggleGroupByMenu(controlPanel);
            const facetEl = controlPanel.el.querySelector('.o-searchview .o-searchview-facet');
            assert.strictEqual(facetEl.innerText.trim(), "Groupby Foo");
            assert.ok(cpHelpers.isItemSelected(controlPanel, 0));

            await testUtils.dom.click(facetEl.querySelector('i.o-facet-remove'));
            assert.containsNone(controlPanel, '.o-searchview .o-searchview-facet');
            await cpHelpers.toggleGroupByMenu(controlPanel);
            assert.notOk(cpHelpers.isItemSelected(controlPanel, 0));
        ...
        QUnit.test('group by a date field using interval works', async function (assert) {
            ...
            const arch = `
                <search>
                    <filter string="Date" name="date" context="{'groupby': 'date_field:week'}"/>
                </search>`;
            const params = {
                cpModelConfig: {
                    arch,
        ...
        QUnit.test('interval options are correctly grouped and ordered', async function (assert) {
            ...
            const arch = `
                <search>
                    <filter string="Bar" name="bar" context="{'groupby': 'bar'}"/>
                    <filter string="Date" name="date" context="{'groupby': 'date_field'}"/>
                    <filter string="Foo" name="foo" context="{'groupby': 'foo'}"/>
                </search>`;
            const params = {
                cpModelConfig: {
                    arch,
        ...
        QUnit.test('default groupbys can be ordered', async function (assert) {
            ...
            const arch = `
                <search>
                    <filter string="Birthday" name="birthday" context="{'groupby': 'birthday'}"/>
                    <filter string="Date" name="date" context="{'groupby': 'date_field:week'}"/>
                </search>`;
            const params = {
                cpModelConfig: {
        ...
        QUnit.test('a separator in groupbys does not cause problems', async function (assert) {
            ...
            const arch = `
                <search>
                    <filter string="Date" name="coolName" context="{'groupby': 'date_field'}"/>
                    <separator/>
                    <filter string="Bar" name="superName" context="{'groupby': 'bar'}"/>
                </search>`;
            const params = {
                cpModelConfig: {
                    arch,
        ...
        QUnit.test('falsy search default groupbys are not activated', async function (assert) {
            ...
            const arch = `
                <search>
                    <filter string="Birthday" name="birthday" context="{'groupby': 'birthday'}"/>
                    <filter string="Date" name="date" context="{'groupby': 'foo'}"/>
                </search>`;
            const params = {
                cpModelConfig: {
                    arch,
