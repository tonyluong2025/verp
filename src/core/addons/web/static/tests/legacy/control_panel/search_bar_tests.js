/** @verp-module alias="web.search_bar_tests" **/
    ...
    hooks.beforeEach(() => {
        serverData = {
            models: {
                partner: {
                    ...
                    records: [
                        { id: 1, displayName: "First record", foo: "yop", bar: 2, bool: true, birthday: '1983-07-15', birth_datetime: '1983-07-15 01:00:00' },
                        { id: 2, displayName: "Second record", foo: "blip", bar: 1, bool: false, birthday: '1982-06-04', birth_datetime: '1982-06-04 02:00:00' },
                        { id: 3, displayName: "Third record", foo: "gnap", bar: 1, bool: false, birthday: '1985-09-13', birth_datetime: '1985-09-13 03:00:00' },
                        { id: 4, displayName: "Fourth record", foo: "plop", bar: 2, bool: true, birthday: '1983-05-05', birth_datetime: '1983-05-05 04:00:00' },
                        { id: 5, displayName: "Fifth record", foo: "zoup", bar: 2, bool: true, birthday: '1800-01-01', birth_datetime: '1800-01-01 05:00:00' },
                    ],
                }
            },
            views: {
                ...
                "partner,false,search": `
                    <search>
                        ...
                        <filter string="Date Field Groupby" name="coolName" context="{'groupby': 'birthday:day'}"/>
                    </search>
                `,
            },
            actions: {
        ...
        QUnit.test("basic rendering", async function (assert) {
            ...
            assert.strictEqual(
                ...
                webClient.el.querySelector(".o-searchview input.o-searchview-input"),
        ...
        QUnit.test("navigation with facets", async function (assert) {
            ...
            assert.containsOnce(webClient, '.o-searchview .o-searchview-facet',
            ...
            assert.strictEqual(document.activeElement, webClient.el.querySelector('.o-searchview .o-searchview-facet'));

            // press right to focus the input
            await testUtils.dom.triggerEvent(document.activeElement, 'keydown', { key: 'ArrowRight' });
            assert.strictEqual(document.activeElement, webClient.el.querySelector('.o-searchview input.o-searchview-input'));
        });
        ...
        QUnit.test('search date and datetime fields. Support of timezones', async function (assert) {
            ...
            // Date case
            let searchInput = webClient.el.querySelector('.o-searchview-input');
            ...
            assert.strictEqual(webClient.el.querySelector('.o-searchview-facet .o-facet-values').innerText.trim(),
                '07/15/1983',
            ...
            // DateTime case
            searchInput = webClient.el.querySelector('.o-searchview-input');
        ...
        QUnit.test("autocomplete menu clickout interactions", async function (assert) {
            ...
            const input = searchBar.el.querySelector('.o-searchview-input');

            assert.containsNone(searchBar, '.o-searchview-autocomplete');
            ...
            assert.containsOnce(searchBar, '.o-searchview-autocomplete');
        ...
        QUnit.test('select an autocomplete field', async function (assert) {
            ...
            const searchInput = webClient.el.querySelector('.o-searchview-input');
            await testUtils.fields.editInput(searchInput, 'a');
            assert.containsN(webClient, '.o-searchview-autocomplete li', 2,
                "there should be 2 result for 'a' in search bar autocomplete");

            await testUtils.dom.triggerEvent(searchInput, 'keydown', { key: 'Enter' });
            assert.strictEqual(webClient.el.querySelector('.o-searchview-input-container .o-facet-values').innerText.trim(),
        ...
        QUnit.test('select an autocomplete field with `context` key', async function (assert) {
            ...
            await doAction(webClient, 1);
            await firstLoading;
            assert.strictEqual(searchReadCount, 1, "there should be 1 searchRead");
            const searchInput = webClient.el.querySelector('.o-searchview-input');
            ...
            assert.strictEqual(searchReadCount, 2, "there should be 2 searchRead");
            ...
            assert.strictEqual(webClient.el.querySelector('.o-searchview-input-container .o-facet-values').innerText.trim(),
                ...
                "the autocompletion facet should be correct");
            assert.strictEqual(searchReadCount, 3, "there should be 3 searchRead");
        });
        ...
        QUnit.test('no search text triggers a reload', async function (assert) {
            ...
            assert.containsNone(webClient, '.o-searchview-facet-label');
        ...
        QUnit.test('selecting (no result) triggers a re-render', async function (assert) {
            ...
            assert.strictEqual(webClient.el.querySelector('.o-searchview-autocomplete .o-selection-focus').innerText.trim(), "(no result)",
                "there should be no result for 'a' in bar");

            await testUtils.dom.triggerEvent(searchInput, 'keydown', { key: 'Enter' });

            assert.containsNone(webClient, '.o-searchview-facet-label');
            assert.strictEqual(webClient.el.querySelector('.o-searchview-input').value, "",
        ...
        QUnit.test('update suggested filters in autocomplete menu with Japanese IME', async function (assert) {
            ...
            assert.containsOnce(webClient.el, '.o-searchview-autocomplete',
                "should display autocomplete dropdown menu on typing something in search view"
            );
            assert.strictEqual(
                webClient.el.querySelector('.o-searchview-autocomplete li').innerText.trim(),
                "Search Foo for: TEST",
        ...
        QUnit.test('open search view autocomplete on paste value using mouse', async function (assert) {
            ...
            assert.containsOnce(webClient, '.o-searchview-autocomplete',
        ...
        QUnit.test('select autocompleted many2one', async function (assert) {
            ...
            serverData.views['partner,false,search'] = `
                <search>
                    ...
                    <field name="birthday"/>
                    <field name="birth_datetime"/>
                    <field name="bar" operator="childOf"/>
                </search>
            `;
            ...
            await testUtils.dom.click(webClient.el.querySelector('.o-searchview-autocomplete li:last-child'));

            await cpHelpers.removeFacet(webClient, 0);

            await testUtils.controlPanel.editSearch(webClient, "rec");
            await testUtils.dom.click(webClient.el.querySelector('.o-expand'));
            await testUtils.dom.click(webClient.el.querySelector('.o-searchview-autocomplete li.o-menu-item.o-indent'));

            assert.verifySteps([
                '[]',
                '[["bar","childOf","rec"]]', // Incomplete string -> Name search
                '[]',
                '[["bar","childOf",1]]', // Suggestion select -> Specific ID
            ]);
        ...
        QUnit.test('"null" as autocomplete value', async function (assert) {
            ...
            assert.strictEqual(
                webClient.el.querySelector('.o-searchview-autocomplete .o-selection-focus').innerText,
                "Search Foo for: null"
            );

            await testUtils.dom.click(webClient.el.querySelector('.o-searchview-autocomplete li.o-selection-focus a'));
        ...
        QUnit.test('autocompletion with a boolean field', async function (assert) {
            ...
            assert.containsOnce(webClient, '.o-searchview-autocomplete li');
            assert.strictEqual(webClient.el.querySelector('.o-searchview-autocomplete li').innerText, "Search Bool: Yes");
            assert.doesNotHaveClass(webClient.el.querySelector('.o-searchview-autocomplete li'), 'o-indent');

            // select "Yes"
            await testUtils.dom.click(webClient.el.querySelector('.o-searchview-autocomplete li'));
            ...
            assert.containsOnce(webClient, '.o-searchview-autocomplete li');
            assert.strictEqual(webClient.el.querySelector('.o-searchview-autocomplete li').innerText, "Search Bool: No");
            assert.doesNotHaveClass(webClient.el.querySelector('.o-searchview-autocomplete li'), 'o-indent');
        ...
        QUnit.test('autocompletion with a selection field', async function (assert) {
            ...
            assert.containsN(webClient, '.o-searchview-autocomplete li', 2);
            assert.strictEqual(webClient.el.querySelector('.o-searchview-autocomplete li:first-child').innerText, "Search Status: New");
            assert.strictEqual(webClient.el.querySelector('.o-searchview-autocomplete li:last-child').innerText, "Search Status: Cancelled");
            assert.doesNotHaveClass(webClient.el.querySelector('.o-searchview-autocomplete li:first-child'), 'o-indent');
            assert.doesNotHaveClass(webClient.el.querySelector('.o-searchview-autocomplete li:last-child'), 'o-indent');
        });
        ...
        QUnit.test("reference fields are supported in search view", async function (assert) {
            ...
            assert.containsOnce(webClient, ".o-data-row");
