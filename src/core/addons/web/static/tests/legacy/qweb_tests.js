verp.define('web.qweb_tests', function (require) {
"use strict";
...
/**
 * Promise-based wrapper for QWeb2.Engine#addTemplate
 *
 ...
 */
function addTemplate(qweb, template) {
    return new Promise(function (resolve, reject) {
        qweb.addTemplate(template, (error, doc) => error ? reject(error) : resolve(doc));
    });
}
...
async function loadTest(assert, templateFile, context) {
    ...
    const doc = await addTemplate(qweb, qwebPath + templateFile);
...
QUnit.module('QWeb', {
    ...
    QUnit.module('t-out', {}, () => {
        QUnit.test("basics", async assert => {
            ...
            await addTemplate(qweb, `<templates>
