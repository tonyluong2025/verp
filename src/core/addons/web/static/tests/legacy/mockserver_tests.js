verp.define('web.mockserver_tests', function (require) {
"use strict";
...
QUnit.module("MockServer", {
    ...
    QUnit.test("performRpc: searchRead with an empty array of fields", async function (assert) {
        assert.expect(1);
        ...
        const expectedFields = ["id", "email", "name", "displayName"];
        assert.strictEqual(_.difference(expectedFields, Object.keys(result[0])).length, 0,
            "should contains all the fields");
    });

    QUnit.test("performRpc: searchRead without fields", async function (assert) {
        assert.expect(1);
        const server = new MockServer(this.data, {});
        const result = await server.performRpc("", {
            model: "res.partner",
    ...
    QUnit.test("performRpc: search with activeTest=false", async function (assert) {
        assert.expect(1);
        const server = new MockServer(this.data, {});
        const result = await server.performRpc("", {
            model: "res.partner",
    ...
    QUnit.test("performRpc: search with activeTest=true", async function (assert) {
        assert.expect(1);
        const server = new MockServer(this.data, {});
        const result = await server.performRpc("", {
            model: "res.partner",
            ...
            args: [[]],
            kwargs: {
                context: { activeTest: true },
            },
        });
        assert.deepEqual(result, [1]);
    });

    QUnit.test("performRpc: searchRead with activeTest=false", async function (assert) {
        assert.expect(1);
        const server = new MockServer(this.data, {});
        const result = await server.performRpc("", {
            model: "res.partner",
    ...
    QUnit.test("performRpc: searchRead with activeTest=true", async function (assert) {
        assert.expect(1);
        const server = new MockServer(this.data, {});
        const result = await server.performRpc("", {
            model: "res.partner",
    ...
    QUnit.test("performRpc: readGroup, group by date", async function (assert) {
        assert.expect(10);
        const server = new MockServer(this.data, {});
    ...
    QUnit.test("performRpc: readGroup, group by datetime", async function (assert) {
        const server = new MockServer(this.data, {});
        let result = await server.performRpc("", {
            model: "bar",
            method: "readGroup",
    ...
    QUnit.test("performRpc: readGroup, group by m2m", async function (assert) {
        const server = new MockServer(this.data, {});
        const result = await server.performRpc("", {
            model: "bar",
            method: "readGroup",
    ...
    QUnit.test("performRpc: readGroup, group by integer", async function (assert) {
        const server = new MockServer(this.data, {});
        const result = await server.performRpc("", {
            model: "bar",
            method: "readGroup",
