verp.define('web.model_field_selector_tests', function (require) {
"use strict";
...
QUnit.module('ModelFieldSelector', {
    ...
    QUnit.test("creating a field chain from scratch", async function (assert) {
        ...
        var $value = fieldSelector.$("> .o-field-selector-value");

        // Focusing the field selector input should open a field selector popover
        fieldSelector.$el.trigger('focusin');
        var $fieldSelectorPopover = fieldSelector.$(".o-field-selector-popover:visible");
        ...
        fieldSelector.$el.trigger('focusin');
        await testUtils.nextTick();
        await testUtils.dom.click(fieldSelector.$('.o-field-selector-prev-page'));
        await testUtils.dom.click(fieldSelector.$('.o-field-selector-close'));

        fieldSelector.$el.trigger('focusin');
        await testUtils.nextTick();
        $fieldSelectorPopover = fieldSelector.$(".o-field-selector-popover:visible");
        $lis = $fieldSelectorPopover.find("li");
        $productLi = $();
        $lis.each(function () {
            var $li = $(this);
        ...
        function getValueFromDOM($dom) {
            return _.map($dom.find(".o-field-selector-chain-part"), function (part) {
    ...
    QUnit.test("default field chain should set the page data correctly", async function (assert) {
        ...
        // Create the field selector and its mock environment
        // passing 'productId' as a prefilled field-chain
        var fieldSelector = new ModelFieldSelector(null, "partner", ['productId'], {
            readonly: false,
            debugMode: true,
        });
        await testUtils.mock.addMockEnvironment(fieldSelector, {data: this.data});
    ...
    QUnit.test("default `showSearchInput` option", async function (assert) {
        ...
        await testUtils.nextTick();
        var $fieldSelectorPopover = fieldSelector.$(".o-field-selector-popover:visible");
        var $searchInput = $fieldSelectorPopover.find(".o-field-selector-search input");
    ...
    QUnit.test("create a field chain with value 1 i.e. TRUE_LEAF", async function (assert) {
        ...
        var $fieldName = fieldSelector.$('.o-field-selector-chain-part');
