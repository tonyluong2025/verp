verp.define('web.special_fields_tests', function (require) {
"use strict";
...
QUnit.module('special_fields', {
    ...
    QUnit.test('widget timezone_mismatch in a list view', async function (assert) {
        ...
        this.data.partner.fields.tzOffset = {
            string: "tzOffset",
            type: "char"
        };
        ...
        this.data.partner.onchanges = {
            color: function (r) {
                r.tzOffset = '+4800'; // make sur we have a mismatch
            }
        };
        ...
        var list = await createView({
            ...
            arch: '<tree string="Colors" editable="top">' +
                        '<field name="tzOffset" invisible="true"/>' +
        ...
        assert.strictEqual($td.find('.o-tz-warning').length, 1, "Should display icon alert");
    ...
    QUnit.test('widget timezone_mismatch in a form view', async function (assert) {
        ...
        this.data.partner.records[0].tzOffset = '+4800';
        ...
        var $timezoneMismatch = form.$('.o-tz-warning');
    ...
    QUnit.test('widget timezone_mismatch in a form view edit mode with mismatch', async function (assert) {
        ...
        var form = await createView({
            ...
            arch: '<form>' +
                    '<field name="tzOffset" invisible="true"/>' +
                    '<field name="tz" widget="timezone_mismatch" options="{\'tz_offset_field\': \'tzOffset\'}"/>' +
                '</form>',
            viewOptions: {
                mode: 'edit',
            },
    ...
    QUnit.test('report_layout widget in form view', async function (assert) {
        ...
        this.data['report.layout'] = {
            fields: {
                viewId: {string: "Document Template", type: "many2one", relation: "product"},
        ...
        var form = await createView({
            ...
            data: this.data,
            arch: '<form>' +
                    '<field name="productId" widget="report_layout"/> '+
                  '</form>',
            resId: 2,
            viewOptions: {
                mode: 'edit',
