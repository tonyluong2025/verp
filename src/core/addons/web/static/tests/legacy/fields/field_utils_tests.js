verp.define('web.field_utils_tests', function (require) {
"use strict";
...
QUnit.test("format_datetime (with different timezone offset)", function (assert) {
    ...
    var dateFormat = core._t.database.parameters.dateFormat;
    core._t.database.parameters.dateFormat = '%m/%d/%Y';
    ...
    core._t.database.parameters.dateFormat = dateFormat;
});
...
QUnit.test('format float time', function (assert) {
    ...
    assert.strictEqual(fieldUtils.format.floatTime(2), '02:00');
    assert.strictEqual(fieldUtils.format.floatTime(3.5), '03:30');
    assert.strictEqual(fieldUtils.format.floatTime(0.25), '00:15');

    assert.strictEqual(fieldUtils.format.floatTime(-0.5), '-00:30');

    const options = {
        noLeadingZeroHour: true,
    };
    assert.strictEqual(fieldUtils.format.floatTime(2, null, options), '2:00');
    assert.strictEqual(fieldUtils.format.floatTime(3.5, null, options), '3:30');
    assert.strictEqual(fieldUtils.format.floatTime(-0.5, null, options), '-0:30');
});
...
QUnit.test('parse monetary', function(assert) {
    ...
    assert.strictEqual(fieldUtils.parse.monetary("$&nbsp;125.00", {}, {currencyId: 3}), 125);
    assert.strictEqual(fieldUtils.parse.monetary("1,000.00&nbsp;€", {}, {currencyId: 1}), 1000);
    assert.throws(function() {fieldUtils.parse.monetary("$ 12.00", {}, {currencyId: 3})}, /is not a correct/);
    assert.throws(function() {fieldUtils.parse.monetary("$&nbsp;12.00", {}, {currencyId: 1})}, /is not a correct/);
    assert.throws(function() {fieldUtils.parse.monetary("$&nbsp;12.00&nbsp;34", {}, {currencyId: 3})}, /is not a correct/);
    ...
    assert.strictEqual(fieldUtils.parse.monetary(`1${nbsp}000.00${nbsp}€`, {}, {currencyId: 1}), 1000);
    assert.strictEqual(fieldUtils.parse.monetary(`$${nbsp}1${nbsp}000.00`, {}, {currencyId: 3}), 1000);
...
QUnit.test('parse datetime', function (assert) {
    ...
    _.extend(core._t.database.parameters, {dateFormat: '%m/%d/%Y', timeFormat: '%H:%M:%S'});
    assert.throws(function () {
...
QUnit.test('parse date without separator', function (assert) {
    ...
    _.extend(core._t.database.parameters, {dateFormat: '%d.%m/%Y'});
...
QUnit.test('parse datetime without separator', function (assert) {
    ...
    _.extend(core._t.database.parameters, {dateFormat: '%d.%m/%Y', timeFormat: '%H:%M/%S'});
