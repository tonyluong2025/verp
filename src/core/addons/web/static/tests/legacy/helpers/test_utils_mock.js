verp.define('web.test_utils_mock', function (require) {
"use strict";
 ...
 * requested services (+ ajax, localStorage and session_storage) deployed.
 *
 * @private
 * @param {Object} params
...
async function _getMockedOwlEnv(params, mockServer) {
    ...
    const env = Object.assign({}, params.env, {
        ...
        dataManager: Object.assign({
            ...
            load_views: (params, options) => {
                return mockServer.performRpc('/web/dataset/callKw/' + params.model, {
                    args: [],
                    kwargs: {
            ...
            loadFilters: params => {
                if (debug) {
                    console.log('[mock] loadFilters', params);
                }
    ...
    if (!servicesToDeploy.localStorage) {
        services.localStorage = new RamStorageService(env);
    }
...
function _mockGlobalObjects(params) {
    ...
    const sessionPatch = Object.assign({
        ...
        async userHasGroup() { return false; },
    }, params.session);
    // patch session
    ...
    if ('config' in params) {
        ...
        if ('debug' in params.config) {
            verp.debug = params.config.debug;
        }
    }
...
function _observe(widget) {
    var _triggerUp = widget._triggerUp.bind(widget);
    widget._triggerUp = function (event) {
 ...
 * dataManager to return an equivalent structure.
 *
 ...
 * must be a "custom event", i.e. an event generated by the method 'triggerUp'.
 *
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any Verp widget)
...
async function addMockEnvironmentOwl(Component, params, mockServer) {
    ...
    function cleanUp() {
        ...
        // clear the caches (e.g. dataManager, ModelFieldSelector) at the end
        // of each test to avoid collisions
        core.bus.trigger('clearCache');

        $('body').off('DOMNodeInserted.removeSRC');
        $('.blockUI').remove(); // fixme: move to qunit_config in VerpAfterTestHook?
 ...
 * @param {Widget} widget
 * @param {Object} params
 * @param {Object} [params.archs] a map of string [model,viewId,viewType] to
 ...
 *   used to generate mock answers for every kind of routes supported by verp
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught Verp
 *   events.
 ...
 *   after this method returns. Some events ('callService', "load_views",
 *   "get_session", "loadFilters") have a special treatment beforehand.
...
async function addMockEnvironment(widget, params) {
    ...
    intercept(widget, 'callService', function (ev) {
    ...
    intercept(widget, "load_views", async ev => {
        ...
        ev.data.onSuccess(views);
    });
    intercept(widget, "get_session", ev => {
        ev.data.callback(session);
    });
    intercept(widget, "loadFilters", async ev => {
        const filters = await env.dataManager.loadFilters(ev.data);
        ev.data.onSuccess(filters);
    });

    // make sure all other Verp events bubbling up are intercepted
