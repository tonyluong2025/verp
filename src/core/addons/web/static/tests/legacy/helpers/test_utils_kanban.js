verp.define('web.test_utils_kanban', function (require) {
"use strict";
...
function toggleGroupSettings($column) {
    var $dropdownToggler = $column.find('.o-kanban-config > a.dropdown-toggle');
...
function quickCreate(kanban, value, fieldName) {
    ...
    return testUtilsFields.editAndTrigger(
        kanban.$('.o-kanban-quick-create input' + additionalSelector),
        value,
        ['input', enterEvent]
    );
}
...
function toggleRecordDropdown($record) {
    var $dropdownToggler = $record.find('.o-dropdown-kanban > a.dropdown-toggle');
    if (!$dropdownToggler.is(':visible')) {
        $dropdownToggler.css('display', 'block');
    }
    return testUtilsDom.click($dropdownToggler);
