verp.define('web.MockServer', function (require) {
"use strict";
...
var MockServer = Class.extend({
    ...
    init: function (data, options) {
        ...
        for (var modelName in this.data) {
            ...
            if (!('displayName' in model.fields)) {
                model.fields.displayName = {string: "Display Name", type: "char"};
            }
    ...
    /**
     * Mocks a fieldsGet RPC for a given model.
     *
     * @param {string} model
     * @returns {Object}
     */
     ...
     * 'field_view_get' call to the server. Calls processViews() of dataManager
     * to mimick the real behavior of a call to loadViews().
     *
     * @param {Object} params
     * @param {string|Object} params.arch a string OR a parsed xml document
     * @param {Number} [params.viewId] the id of the arch's view
     ...
     * javascript `onchange` method.
     * Specifically, it applies `nameget` on many2one's and transforms raw id
    ...
    _convertToOnChange(model, values) {
        Object.entries(values).forEach(([fname, val]) => {
            ...
            if (field.type === 'many2one' && typeof val === 'number') {
                ...
                values[fname] = [val, m2oRecord.displayName];
    ...
    _fieldsViewGet: function (arch, model, fields, context) {
        ...
        const isBaseModel = !context.baseModelName || (model === context.baseModelName);
        ...
        this._traverse(doc, function (node) {
            ...
            _.each(modifiersNames, function (a) {
                ...
                if (mod) {
                    var pyevalContext = window.vj.dict.fromJSON(context || {});
                    var v = vjUtils.py_eval(mod, {context: pyevalContext}) ? true: false;
                    if (inTreeView && !inListHeader && a === 'invisible') {
                        modifiers.columninvisible = v;
        ...
        _.each(fieldNodes, function (node, name) {
            ...
            if (field.type === "many2one" || field.type === "many2many") {
                ...
                var canWrite = node.getAttribute('canWrite');
                node.setAttribute('canWrite', canWrite || "true");
            }
                ...
                _.each(node.childNodes, function (children) {
                    if (children.tagName) { // skip text nodes
                        ...
                        field.views[children.tagName] = self._fieldsViewGet(children, relModel,
                            relFields, _.extend({}, context, {baseModelName: model}));
                    }
                });
            }
     ...
     * @param {any[]} domain
     * @param {Object} [params={}]
     * @param {boolean} [params.activeTest=true]
     * @returns {Object[]} a list of records
     */
    _getRecords: function (model, domain, { activeTest = true } = {}) {
        if (!_.isArray(domain)) {
        ...
        if (activeTest && 'active' in this.data[model].fields) {
    ...
    _mockCopy: function (modelName, id) {
        ...
        duplicateRecord.displayName = originalRecord.displayName + ' (copy)';
    ...
    /**
     * Simulate a 'defaultGet' operation
     *
     * @private
     * @param {string} modelName
    ...
    /**
     * Simulate a 'fieldsGet' operation
     *
     * @private
     * @param {string} modelName
     * @param {any} args
     ...
     * @param {(string|boolean)} parentName 'parentId' or false
     */
    _mockSearchPanelGlobalCounters: function (valuesRange, parentName) {
        ...
        for (let [id, values] of valuesRange.entries()) {
            ...
            if (count) {
                ...
                while (parentId) {
                    values = valuesRange.get(parentId);
    ...
    /**
     * Simulates a call to the server 'searchpanelSelectRange' method.
     *
     * @private
     * @param {string} model
     * @param {string[]} args
     ...
     * @param {Object} [kwargs={}]
     * @param {Array[]} [kwargs.categoryDomain] domain generated by categories
     *      (this parameter is used in _search_panel_range)
     * @param {Array[]} [kwargs.comodelDomain] domain of field values (if relational)
     *      (this parameter is used in _search_panel_range)
     * @param {boolean} [kwargs.enableCounters] whether to count records by value
     * @param {Array[]} [kwargs.filterDomain] domain generated by filters
     * @param {integer} [kwargs.limit] maximal number of values to fetch
     * @param {Array[]} [kwargs.searchDomain] base domain of search (this parameter
    ...
    _mockSearchPanelSelectRange: function (model, [fieldName], kwargs) {
        ...
        if (hierarchize && this.data[field.relation].fields.parentId) {
            parentName = 'parentId'; // in tests, parent field is always 'parentId'
            fieldNames.push(parentName);
            getParentId = record => record.parentId && record.parentId[0];
        } else {
            hierarchize = false;
        }
        ...
        if (!expand && !hierarchize && !comodelDomain.length) {
            if (limit && domainImage.size === limit) {
                return { errorMsg: "Too many items to display." };
            }
            return {
    ...
    /**
     * Simulates a call to the server 'searchPanelSelectMultiRange' method.
     *
     * @private
     * @param {string} model
     * @param {string[]} args
     ...
     * @param {boolean} [kwargs.enableCounters] whether to count records by value
     * @param {Array[]} [kwargs.filterDomain] domain generated by filters
     * @param {string} [kwargs.groupby] extra field to read on comodel, to group
     *      comodel records
     * @param {Array[]} [kwargs.groupDomain] dict, one domain for each activated
     *      group for the groupby (if any). Those domains are used to fech accurate
     *      counters for values in each group
     * @param {integer} [kwargs.limit] maximal number of values to fetch
     * @param {Array[]} [kwargs.searchDomain] base domain of search
     * @returns {Object}
     */
    _mockSearchPanelSelectMultiRange: function (model, [fieldName], kwargs) {
        ...
        let modelDomain = kwargs.searchDomain || [];
        ...
        if (field.type === 'many2many') {
            ...
            for (const record of comodelRecords) {
                ...
                if (groupBy) {
                    ...
                    values.groupId = groupId = gId;
                    values.groupName = gName;
                }
                let count;
                let inImage;
                if (enableCounters || !expand) {
     ...
     * @private
     * @param {Object} kwargs
     * @param {integer} kwargs.actionId
     * @returns {Object}
     */
    _mockLoadAction: function (kwargs) {
        var action = _.findWhere(this.actions, {id: parseInt(kwargs.actionId)});
        if (!action) {
            // when the action doesn't exist, the real server doesn't crash, it
            // simply returns false
            console.warn(`No action found for ID ${kwargs.actionId} during test ${QUnit.config.current.testName} (legacy)`);
        }
        return action || false;
    },
    /**
    ...
    /**
     * Simulate a 'nameget' operation
     *
     * @private
     * @param {string} model
     * @param {Array} args
     * @returns {Array[]} a list of [id, displayName]
     */
    _mockNameGet: function (model, args) {
        var ids = args[0];
        if (!args.length) {
            throw new Error("nameget: expected one argument");
        }
        else if (!ids) {
            return []
        }
        ...
        var names = _.map(ids, function (id) {
            return id ? [id, _.findWhere(records, {id: id}).displayName] : [null, "false"];
        });
        return names;
    },
    /**
     * Simulate a 'nameCreate' operation
     *
     * @private
     * @param {string} model
     * @param {Array} args
    ...
    /**
     * Simulate a 'nameSearch' operation.
     *
    ...
    _mockNameSearch: function (model, args, _kwargs) {
        ...
        if (str.length) {
            records = _.filter(records, function (record) {
                return record.displayName.indexOf(str) !== -1;
            });
        }
    ...
    _mockOnchange: function (model, args, kwargs) {
        ...
        if (firstOnChange) {
            ...
            // a defaultGet + onchange situation
        ...
        var results = _.map(records, function (record) {
            ...
            for (var i = 0; i < fields.length; i++) {
                ...
                if (field.type === 'float' ||
                    ...
                    if (relatedRecord) {
                        result[fields[i]] =
                            [record[fields[i]], relatedRecord.displayName];
                    } else {
        ...
        for (const [groupId, groupRecords] of Object.entries(groups)) {
            ...
            for (const gbField of groupBy) {
                ...
                if (["many2one", "many2many"].includes(type) && !Array.isArray(value)) {
                    ...
                    if (relatedRecord) {
                        group[gbField] = [value, relatedRecord.displayName];
                    } else {
                        group[gbField] = false;
                    }
                }
            ...
            const groupByNoLeaf = kwargs.context ? "groupby_noLeaf" in kwargs.context : false;
            let countKey;
    ...
    /**
     * Simulates a 'readProgressBar' operation
     *
     * @private
     * @param {string} model
     * @param {Object} kwargs
    ...
    _mockReadProgressBar: function (model, kwargs) {
        ...
        var progressBar = kwargs.progressBar;
        ...
        _.each(records, function (record) {
            ...
            if (!(groupByValue in data)) {
                data[groupByValue] = {};
                _.each(progressBar.colors, function (val, key) {
                    data[groupByValue][key] = 0;
                });
            }

            var fieldValue = record[progressBar.field];
    ...
    _mockSearch: function (model, args, kwargs) {
        ...
        const activeTest =
          context && "activeTest" in context ? context.activeTest : true;
        return this._getRecords(model, args[0], { activeTest }).map(r => r.id).slice(0, limit);
    },
    /**
    ...
    /**
     * Simulate a 'searchRead' operation on a model
     *
     * @private
     * @param {Object} args
    ...
    /**
     * Simulate a 'searchRead' operation, from the controller point of view
     *
     * @private
     * @private
     * @param {Object} args
        ...
        var processedRecords = _.map(records, function (r) {
            ...
            _.each(_.uniq(fields.concat(['id'])), function (fieldName) {
                ...
                if (field.type === 'many2one') {
                    ...
                    result[fieldName] =
                        related_record ? [r[fieldName], related_record.displayName] : false;
                } else {
    ...
    /**
     * Simulate a 'webReadGroup' call to the server.
     *
     ...
     *   group
     * @param {integer} [kwargs.expandLimit]
     * @param {integer} [kwargs.expandOrderby]
     * @returns {Object[]}
     */
        ...
        if (kwargs.expand && kwargs.groupby.length === 1) {
            groups.forEach(function (group) {
                group.__data = self._mockSearchReadController({
                    ...
                    limit: kwargs.expandLimit,
                    order: kwargs.expandOrderby,
                });
            });
        }
    ...
    _writeRecord: function (model, values, id, { ensureIntegrity = true } = {}) {
        ...
        for (var fieldChanged in values) {
            var field = this.data[model].fields[fieldChanged];
            var value = values[fieldChanged];
            if (!field) {
                throw Error(`Mock: Can't write value "${JSON.stringify(value)}" on field "${fieldChanged}" on record "${model},${id}" (field is undefined)`);
            }
            if (_.contains(['one2many', 'many2many'], field.type)) {
                var ids = _.clone(record[fieldChanged]) || [];
                ...
                for (const command of value || []) {
                    if (command[0] === 0) { // CREATE
                        ...
                        throw Error(`Command "${JSON.stringify(value)}" not supported by the MockServer on field "${fieldChanged}" on record "${model},${id}"`);
                    }
                }
                record[fieldChanged] = ids;
            } else if (field.type === 'many2one') {
                if (value) {
                    ...
                    if (!relatedRecord && ensureIntegrity) {
                        throw Error(`Wrong id "${JSON.stringify(value)}" for a many2one on field "${fieldChanged}" on record "${model},${id}"`);
                    }
                    record[fieldChanged] = value;
                } else {
                    record[fieldChanged] = false;
                }
            } else {
                record[fieldChanged] = value;
            }
