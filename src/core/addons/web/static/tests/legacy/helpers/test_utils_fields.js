verp.define('web.test_utils_fields', function (require) {
    "use strict";
    ...
    async function clickM2OCreateAndEdit(fieldName, text = "ABC") {
        ...
        const match = document.querySelector(`.o-field-many2one[name=${fieldName}] input`);
    ...
    async function clickM2OHighlightedItem(fieldName, selector) {
        const m2oSelector = `${selector || ''} .o-field-many2one[name=${fieldName}] input`;
     ...
     *
     * Example:
     *    testUtils.fields.many2one.clickM2OItem('partnerId', 'George');
     *
     * @param {string} fieldName
    ...
    async function clickM2OItem(fieldName, searchText) {
        const m2oSelector = `.o-field-many2one[name=${fieldName}] input`;
        const $dropdown = $(m2oSelector).autocomplete('widget');
     ...
     *
     * Example:
     *    testUtils.fields.many2one.searchAndClickM2OItem('partnerId', {search: 'George'});
     *
     * @param {string} fieldName
