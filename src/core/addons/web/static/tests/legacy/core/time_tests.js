verp.define('web.time_tests', function (require) {
"use strict";
...
QUnit.module('core', {}, function () {
    ...
    QUnit.test('Parse server date', function (assert) {
        ...
        var date = time.strToDate("2009-05-04");
        assert.deepEqual(
            [date.getFullYear(), date.getMonth(), date.getDate()],
            [2009, 5 - 1, 4]);
    });
    ...
    QUnit.test('Parse server date on 31', function (assert) {
        ...
        try {
            ...
            var date = time.strToDate('2013-11-21');
    ...
    QUnit.test('Parse server time', function (assert) {
        ...
        var date = time.strToTime("12:34:23");
        assert.deepEqual(
            [date.getHours(), date.getMinutes(), date.getSeconds()],
            [12, 34, 23]);

        date = time.strToTime("12:34:23.5467");
        assert.deepEqual(
    ...
    QUnit.test('Format server datetime', function (assert) {
        ...
        assert.strictEqual(time.datetimeToStr(date), "2009-05-04 12:34:23");
    });
    ...
    QUnit.test('Format server date', function (assert) {
        ...
        assert.strictEqual(time.dateToStr(date), "2009-05-04");
    });
    ...
    QUnit.test('Format server time', function (assert) {
        ...
        date.setMinutes(34);
        date.setSeconds(23);
        assert.strictEqual(time.timeToStr(date), "12:34:23");
    });
