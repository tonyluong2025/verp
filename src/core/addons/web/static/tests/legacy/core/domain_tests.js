verp.define('web.domain_tests', function (require) {
"use strict";
...
QUnit.module('core', {}, function () {
    ...
    QUnit.test("or", function (assert) {
        ...
        assert.ok(new Domain(currentDomain).compute(_.extend({}, web, {userId: 3})));
    ...
    QUnit.test("invalid domains should not succeed", function (assert) {
        ...
        assert.throws(
            () => new Domain(['|', '|', ['hrPresenceState', '=', 'absent'], ['attendance_state', '=', 'checked_in']]),
            /invalid domain .* \(missing 1 segment/
        );
        assert.throws(
    ...
    QUnit.test("domain <=> condition", function (assert) {
        ...
        assert.deepEqual(Domain.prototype.conditionToDomain(
            `field in ("foo", "bar") and displayName in ['boo','far']`),
            ['&', ['field', 'in', ['foo', 'bar']], ['displayName', 'in', ['boo', 'far']]]);
    });
    ...
    QUnit.test("arrayToString", function (assert) {
        ...
        assert.strictEqual(arrayToString([['label', '=', null]]), '[["name","=",None]]');
        assert.strictEqual(arrayToString([['label', '=', false]]), '[["name","=",false]]');
        assert.strictEqual(arrayToString([['label', '=', true]]), '[["name","=",true]]');
        assert.strictEqual(arrayToString([['label', '=', 'null']]), '[["name","=","null"]]');
        assert.strictEqual(arrayToString([['label', '=', 'false']]), '[["name","=","false"]]');
        assert.strictEqual(arrayToString([['label', '=', 'true']]), '[["name","=","true"]]');
        assert.strictEqual(arrayToString([['label', 'in', [true, false]]]), '[["name","in",[true,false]]]');
        assert.strictEqual(arrayToString([['label', 'in', [null]]]), '[["name","in",[None]]]');

        assert.strictEqual(arrayToString([['label', 'in', ["foo", "bar"]]]), '[["name","in",["foo","bar"]]]');
        assert.strictEqual(arrayToString([['label', 'in', [1, 2]]]), '[["name","in",[1,2]]]');
        assert.strictEqual(arrayToString(), '[]');

        assert.strictEqual(arrayToString(['&', ['label', '=', 'foo'], ['type', '=', 'bar']]), '["&",["name","=","foo"],["type","=","bar"]]');
        assert.strictEqual(arrayToString(['|', ['label', '=', 'foo'], ['type', '=', 'bar']]), '["|",["name","=","foo"],["type","=","bar"]]');
