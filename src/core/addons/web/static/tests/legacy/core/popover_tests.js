verp.define('web.popover_tests', function (require) {
    'use strict';
    ...
    QUnit.module('core', {}, function () {
        ...
        QUnit.test('Basic rendering & props', async function (assert) {
            ...
            let popover, title;
            // Show/hide
            assert.containsNone(body, '.o-popover');
            await testUtils.dom.click('#target');
            assert.containsOnce(body, '.o-popover');
            assert.containsOnce(body, '.o_subcomponent');
            assert.containsOnce(body, '.o-popover--right');
            await testUtils.dom.click('#passiveTarget');
            assert.containsNone(body, '.o-popover');
            // Reactivity of title
            await testUtils.dom.click('#target');
            popover = document.querySelector('.o-popover');
            title = popover.querySelector('.o-popover-header').innerText.trim();
            ...
            assert.ok(
                pointsTo(
                    document.querySelector('.o-popover'),
                    element,
            ...
            await testUtils.dom.click('#target');
            assert.ok(
                pointsTo(document.querySelector('.o-popover'), element, 'right'),
