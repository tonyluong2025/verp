...
verp.define('web.owl_dialog_tests', function (require) {
    "use strict";
    ...
    QUnit.module('core', {}, function () {
        ...
        QUnit.test("Rendering of all props", async function (assert) {
            ...
            const dialog = document.querySelector('.o-dialog');
            ...
            assert.containsOnce(dialog, '.modal.o-technical-modal');
            ...
            assert.containsNone(document.body, '.modal-backdrop'); // No backdrop *element* for Verp modal...
            ...
            assert.doesNotHaveClass(dialog.querySelector('.modal'), 'o-modal-full');
            await changeProps('fullscreen', true);
            assert.hasClass(dialog.querySelector('.modal'), 'o-modal-full');
            ...
            assert.strictEqual(dialog.querySelector('span.o-subtitle').innerText.trim(), "The Subtitle",
                "Subtitle should match with its given text");

            // Technical (default: true)
            assert.hasClass(dialog.querySelector('.modal'), 'o-technical-modal');
            await changeProps('technical', false);
            assert.doesNotHaveClass(dialog.querySelector('.modal'), 'o-technical-modal');

            // Title (default: 'Verp')
            assert.strictEqual(dialog.querySelector('h4.modal-title').innerText.trim(), "Verp" + "The Subtitle",
        ...
        QUnit.test("Interactions between multiple dialogs", async function (assert) {
            ...
            assert.notOk(modals[modals.length - 1].classList.contains('o-inactive-modal'),
            ...
            assert.containsN(document.body, '.o-dialog', 3);
            ...
            assert.notOk(modals[modals.length - 1].classList.contains('o_legacy_dialog'),
                "active dialog should not have the legacy class");
            assert.containsN(document.body, '.o-dialog', 2);
            assert.containsN(document.body, '.o_legacy_dialog', 2);

            await testUtils.dom.click(modals[modals.length - 1].querySelector('.btn.btn-primary')); // Click on 'Ok' button

            modals = document.querySelectorAll('.modal');
            assert.containsOnce(document.body, '.modal.o_legacy_dialog:not(.o-inactive-modal)',
                "active dialog should have the legacy class");
            assert.containsOnce(document.body, '.o-dialog');
            assert.containsN(document.body, '.o_legacy_dialog', 2);
        ...
        QUnit.test("Z-index toggling and interactions", async function (assert) {
            ...
            const backEndModal = createCustomModal('modal o-technical-modal');

            // querySelector will target the first modal (the static one).
            const owlIndexBefore = getComputedStyle(document.querySelector('.o-dialog .modal')).zIndex;
            ...
            assert.ok(owlIndexBefore < getComputedStyle(document.querySelector('.o-dialog .modal')).zIndex,
        ...
        QUnit.test("remove tabindex on inactive dialog", async (assert) => {
            const serverData = {
                ...
                models: {
                    event: {
                        ...
                        records: [{ id: 1, displayName: "Event 1", start: "2020-07-13" }],
                        methods: {
                ...
                views: {
                    "event,false,calendar": `<calendar dateStart="start" event_open_popup="true" />`,
                    ...
                    `,
                    "event,2,form": `<form><sheet><field name="displayName" /></sheet></form>`,
                },
            };
            ...
            await testUtils.dom.click(webClient.el.querySelector(`.o-cw-popover-edit`));

            assert.containsNone(webClient.el, ".o-dialog");
            assert.containsOnce(webClient.el, ".modal");
            assert.containsOnce(webClient.el, ".modal[tabindex='-1']");

            assert.strictEqual(
                webClient.el.querySelector(`.o-field-widget[name="displayName"]`).value,
                "Event 1"
            );
            ...
            assert.containsOnce(webClient.el, ".o-dialog");
            assert.containsN(webClient.el, ".modal", 2);
            assert.containsOnce(webClient.el, ".modal:not([tabindex='-1'])");
            assert.containsOnce(webClient.el, ".o-dialog .modal[tabindex='-1']");

            assert.strictEqual(
                webClient.el.querySelector(`.o-dialog .o-field-widget[name="displayName"]`).value,
                ""
            );
            await testUtils.fields.editInput(
                webClient.el.querySelector(`.o-dialog .o-field-widget[name="displayName"]`),
                "wowl"
            );
            assert.strictEqual(
                webClient.el.querySelector(`.o-dialog .o-field-widget[name="displayName"]`).value,
            ...
            await testUtils.dom.click(webClient.el.querySelector(`.o-dialog .modal-header .close`));
            assert.containsNone(webClient.el, ".o-dialog");
            assert.containsOnce(webClient.el, ".modal");
            assert.containsOnce(webClient.el, ".modal[tabindex='-1']");
