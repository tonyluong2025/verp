verp.define('web.OwlCompatibilityTests', function (require) {
    "use strict";
    ...
    QUnit.module("Owl Compatibility", function () {
        ...
        QUnit.test("sub widget methods are correctly called", async function (assert) {
            ...
            assert.verifySteps(['onDetachCallback', 'onAttachCallback']);

            parent.destroy();

            assert.verifySteps(['onDetachCallback', 'destroy']);
        });
        ...
        QUnit.test("sub widget that triggers events", async function (assert) {
            ...
            widget.triggerUp('some-event', { value: 'a' });
            widget.triggerUp('some_event', { value: 'b' }); // _ are converted to -
        ...
        QUnit.test("sub widget that calls load_views", async function (assert) {
            ...
            const cleanUp = await addMockEnvironmentOwl(Parent, {
                mockRPC: function (route, args) {
                    assert.strictEqual(route, '/web/dataset/callKw/some_model');
        ...
        QUnit.test("sub widget in a t-if, and events", async function (assert) {
            ...
            assert.strictEqual(parent.el.innerHTML, '<div>Hi</div>');
            myWidget.triggerUp('some-event', { value: 'a' });

            parent.state.flag = false;
            await nextTick();

            assert.strictEqual(parent.el.innerHTML, '');
            myWidget.triggerUp('some-event', { value: 'b' });

            parent.state.flag = true;
            await nextTick();

            assert.strictEqual(parent.el.innerHTML, '<div>Hi</div>');
            myWidget.triggerUp('some-event', { value: 'c' });
        ...
        QUnit.test("sub component that triggers events", async function (assert) {
            ...
            const MyWidget = WidgetAdapter.extend({
                customEvents: _.extend({}, Widget.customEvents, {
        ...
        QUnit.test("Legacy over Owl over legacy", async function (assert) {
            ...
            leafWidget.triggerUp('root-event', { value: 1 });
            leafWidget.triggerUp('component-event', { value: 2 });
            leafWidget.triggerUp('both-event', { value: 3 });
            leafWidget.triggerUp('both-event', { value: 4 }); // will be stopped by component
        ...
        QUnit.test("correctly update widget component during mounting", async function (assert) {
            ...
            await testUtils.dom.click(form.$(".o-form-label")[0]);
