...
QUnit.module("Views", (hooks) => {
    hooks.beforeEach(async () => {
        serverData = {
            ...
            views: {
                ...
                "animal,2,toy": `<toy jsClass="toy_imp">Arch content (id=2)</toy>`,
                "animal,false,other": `<other/>`,
                "animal,false,search": `<search/>`,
                "animal,1,search": `
                    <search>
                        <filter name="filter" domain="[[1,'=',1]]"/>
                        <filter name="groupby" context="{ 'groupby': 'name' }"/>
                    </search>
                `,
            },
        };
    ...
    QUnit.test("simple rendering", async function (assert) {
        ...
        assert.hasClass(view.el, "o-action o-view-controller o_toy_view");
    ...
    QUnit.test("can click on action-bound links -- 1", async (assert) => {
        ...
        assert.verifySteps(["/web/dataset/callKw/animal/setTheControl"]);
    });
    ...
    QUnit.test("renders bannerRoute", async (assert) => {
        assert.expect(3);
        serverData.views["animal,1,toy"] = `
            <toy bannerRoute="/mybody/isacage">
    ...
    QUnit.test("renders bannerRoute with js and css assets", async (assert) => {
        assert.expect(7);
        serverData.views["animal,1,toy"] = `
            <toy bannerRoute="/mybody/isacage">
                <Banner t-if="env.config.bannerRoute" />
    ...
    QUnit.test("click on action-bound links in banner (concurrency)", async (assert) => {
        ...
        serverData.views["animal,1,toy"] = `
            <toy bannerRoute="/bannerRoute">
                <Banner t-if="env.config.bannerRoute" />
                <a type="action" data-method="setTheControl" data-model="animal" />
            </toy>`;

        const mockRPC = async (route) => {
            if (route.includes("bannerRoute")) {
                return {
    ...
    QUnit.test("real life banner", async (assert) => {
        ...
        const bannerArch = `
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <a type="action" class="btn btn-primary" data-dismiss="modal" data-toggle="collapse" href=".o-onboarding-container" data-model="mah.model" data-method="mah_method">Remove</a>
            ...
            <div class="o-onboarding-container collapse show">
                <div class="o-onboarding" />
                    <div class="o-onboarding-wrap" />
                        <a href="#" data-toggle="modal" data-target=".o-onboarding-modal" class="float-right o-onboarding-btn-close">
        ...
        const prom = new Promise((resolve) => {
            const complete = (ev) => {
                if (ev.target.classList.contains("o-onboarding-container")) {
                    resolve();
                }
            };
        ...
        assert.hasClass(toy.el.querySelector(".o-onboarding-container"), "collapse show");
        ...
        await prom;
        assert.doesNotHaveClass(toy.el.querySelector(".o-onboarding-container"), "show");
        assert.hasClass(toy.el.querySelector(".o-onboarding-container"), "collapse");
        assert.isNotVisible(toy.el.querySelector(".modal"));
    });
    ...
    QUnit.test("rendering with loaded arch attribute 'jsClass'", async function (assert) {
        assert.expect(4);
        const view = await makeView({
            serverData,
            mockRPC: (_, args) => {
    ...
    QUnit.test("rendering with given arch attribute 'jsClass'", async function (assert) {
        assert.expect(2);
        const view = await makeView({
            serverData,
            mockRPC: () => {
            ...
            resModel: "animal",
            type: "toy",
            arch: `<toy jsClass="toy_imp">Specific arch content for specific class</toy>`,
            fields: {},
        });
        assert.hasClass(view.el, "o_toy_view_imp");
    ...
    QUnit.test(
        "rendering with loaded arch attribute 'jsClass' and given jsClass",
        async function (assert) {
            assert.expect(3);
    ...
    QUnit.test(
        "rendering with given arch attribute 'jsClass' and given jsClass",
        async function (assert) {
            assert.expect(1);
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await makeView({
                ...
                domain: [[0, "=", 1]],
                groupBy: ["birthday"],
                context: { searchDefault_filter: 1, searchDefault_groupby: 1 },
                orderby: ["bar"],
            });
        }
    );
