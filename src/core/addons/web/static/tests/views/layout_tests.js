...
QUnit.module("Views", (hooks) => {
    ...
    QUnit.test("Simple rendering", async (assert) => {
        ...
        assert.hasClass(comp.el, "o_toy_view o-view-sample-data");
        assert.containsNone(comp, ".o-control-panel");
        assert.containsNone(comp, ".o-component-with-searchpanel");
        assert.containsNone(comp, ".o-searchpanel");
        assert.containsOnce(comp, ".o-content > .toy_content");
    });
    ...
    QUnit.test("Simple rendering: with search", async (assert) => {
        ...
        assert.doesNotHaveClass(root.el, "o-view-sample-data");
        assert.containsOnce(root, ".o-control-panel .o-cp-top-right .toy_search_bar");
        assert.containsOnce(root, ".o-component-with-searchpanel .o-searchpanel");
        assert.containsNone(root, ".o-cp-searchview");
        assert.containsOnce(root, ".o-content > .toy_content");
    });
    ...
    QUnit.test("Nested layouts", async (assert) => {
        ...
        assert.doesNotHaveClass(root.el, "o-view-sample-data");
        assert.containsOnce(root, ".o-content .o_toy_b_view .o-content .o_toy_c_view .o-content"); // Full chain of contents
        assert.containsN(root, ".o-control-panel", 2); // Component C has hidden its control panel
        assert.containsN(root, ".o-content.o-component-with-searchpanel", 3);
        assert.containsOnce(root, ".o-searchpanel"); // Standard search panel
    ...
    QUnit.test("Custom control panel", async (assert) => {
        ...
        assert.containsNone(root, ".o-control-panel");
    });
    ...
    QUnit.test("Custom search panel", async (assert) => {
        ...
        assert.containsOnce(root, ".o_toy_content");
        assert.containsOnce(root, ".o_toy_search_panel");
        assert.containsNone(root, ".o-searchpanel");
    });
