...
function checkTooltip(assert, graph, expectedTooltipContent, index, datasetIndex) {
    ...
    assert.containsOnce(graph, "div.o-graph-custom-tooltip");
    const tooltipTitle = graph.el.querySelector("table thead tr th.o-measure");
    assert.strictEqual(tooltipTitle.innerText, title || "Count", `Tooltip title`);
    assert.deepEqual(
        [...graph.el.querySelectorAll("table tbody tr td span.o-label")].map((td) => td.innerText),
        lineLabels,
        `Tooltip line labels`
    );
    assert.deepEqual(
        [...graph.el.querySelectorAll("table tbody tr td.o-value")].map((td) => td.innerText),
        lineValues,
        `Tooltip line values`
    );
}
...
function getModeButton(comp, mode) {
    return comp.el.querySelector(`.o-graph-button[data-mode="${mode}"`);
}
...
QUnit.module("Views", (hooks) => {
    hooks.beforeEach(async () => {
        serverData = {
            models: {
                foo: {
                    fields: {
                        ...
                        foo: { string: "Foo", type: "integer", store: true, groupOperator: "sum", sortable: true },
                    ...
                    records: [
                        ...
                        { id: 6, foo: 63, bar: false, productId: 41 },
                        { id: 7, foo: 42, bar: false, productId: 41 },
                        {
                            id: 8,
                            foo: 48,
                            bar: false,
            ...
            views: {
                ...
                "foo,false,search": `
                    <search>
                        <filter name="falseDomain" string="false Domain" domain="[[0, '=', 1]]"/>
                        <filter name="filterWithContext"
                            string="Filter With Context"
                            domain="[]"
                            context="{ 'graphMeasure': 'foo', 'graphMode': 'line', 'graphGroupbys': ['colorId'] }"
                            />
                        <filter name="groupbyColor" string="Color" context="{ 'groupby': 'colorId' }"/>
                        <filter name="groupbyProduct" string="Product" context="{ 'groupby': 'productId' }"/>
                    </search>
                `,
            },
        };
    ...
    QUnit.test("simple bar chart rendering", async function (assert) {
        ...
        assert.containsOnce(graph, "div.o-graph-canvas-container canvas");
    ...
    QUnit.test("simple bar chart rendering with no data", async function (assert) {
        ...
        const graph = await makeView({ serverData, type: "graph", resModel: "foo" });
        assert.containsOnce(graph, "div.o-graph-canvas-container canvas");
        assert.containsNone(graph, ".o-nocontent-help");
    ...
    QUnit.test("simple bar chart rendering (one groupBy)", async function (assert) {
        ...
        assert.containsOnce(graph.el, "div.o-graph-canvas-container canvas");
    ...
    QUnit.test("pie chart rendering (no data, several domains)", async function (assert) {
        assert.expect(11);
        serverData.models.foo.records = [{ productId: 37, bar: true }];
        const graph = await makeView({
            serverData,
            type: "graph",
            resModel: "foo",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.containsOnce(graph, ".o-view-nocontent");
            assert.strictEqual(
                graph.el.querySelector(".o-view-nocontent").innerText.replace(/[\s\n]/g, " "),
            ...
            assert.containsNone(graph, ".o-graph-canvas-container");
        }
    );
    ...
    QUnit.test("switching measure", async function (assert) {
        ...
        function checkMeasure(measure) {
            ...
            const item = [...graph.el.querySelectorAll(".o-menu-item")].find(
    ...
    QUnit.test("process simple arch (no field tag)", async function (assert) {
        ...
        const arch1 = `<graph order="ASC" disableLinking="1" type="line"/>`;
        ...
        let arch2 = `<graph disablelinking="0" string="Title" stacked="false"/>`;
    ...
    QUnit.test("no content helper", async function (assert) {
        ...
        assert.containsOnce(graph, "div.o-graph-canvas-container canvas");
        assert.containsNone(graph, "div.o-view-nocontent");
    ...
    QUnit.test("save params succeeds", async function (assert) {
        ...
        const expectedContexts = [
            {
                graphMode: "bar",
                graphMeasure: "__count",
                graphGroupbys: ["productId"],
                groupby: [],
            },
            {
                graphMode: "bar",
            ...
            },
            {
                graphMode: "line",
                graphMeasure: "foo",
                graphGroupbys: ["productId", "colorId"],
                groupby: ["productId", "colorId"],
            },
        ];
        ...
        const graph = await makeView({
            mockRPC: function (_, args) {
                if (args.method === "createOrReplace") {
    ...
    QUnit.test("correctly uses graph_ keys from the context", async function (assert) {
        ...
        const graph = await makeView({
            ...
            type: "graph",
            resModel: "foo",
            arch: '<graph><field name="productId"/></graph>',
            context: {
                graphMeasure: "foo",
                graphMode: "line",
                graphGroupbys: ["colorId"],
            },
        });
    ...
    QUnit.test("correctly use groupby key from the context", async function (assert) {
        ...
        const graph = await makeView({
            ...
            `,
            context: {
                searchDefault_filterWithContext: 1,
            },
        });
        checkLabels(assert, graph, ["red", "black"]);
        checkLegend(assert, graph, "Foo");
    ...
    QUnit.test("correctly uses graph_ keys from the context (at reload)", async function (assert) {
        ...
        const graph = await makeView({
            ...
            searchViewArch: `
                <search>
                    <filter name="context" domain="[]" string="Context" context="{ 'graphMeasure': 'foo', 'graphMode': 'line' }"/>
                </search>
            `,
        });
        checkLegend(assert, graph, "Count");
    ...
    QUnit.test("differentiate many2one values with same label", async function (assert) {
        assert.expect(1);
        serverData.models.product.records.push({ id: 39, displayName: "xphone" });
        serverData.models.foo.records.push({ id: 18, productId: 39 });
        const graph = await makeView({
            serverData,
            type: "graph",
            resModel: "foo",
    ...
    QUnit.test("not use a many2one as a measure by default", async function (assert) {
        ...
        await toggleMenu(graph, "Measures");
        assert.deepEqual(
            [...graph.el.querySelectorAll(".o-cp-bottom-left .o-menu-item")].map(
                (el) => el.innerText
            ),
            ["Foo", "Revenue", "Count"]
        );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await triggerEvent(graph.el, ".o-searchview input", "keydown", { key: "ArrowDown" });
    ...
    QUnit.test('graph view "graphMeasure" field in context', async function (assert) {
        assert.expect(6);
        const graph = await makeView({
            serverData,
            type: "graph",
    ...
    QUnit.test(
        '"graphMeasure" in context is prefered to measure in arch',
        async function (assert) {
            assert.expect(6);
            const graph = await makeView({
                serverData,
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await toggleMenu(graph, "Measures");
            assert.notOk(
                [...graph.el.querySelectorAll(".o-menu-item")].find(
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const graph = await makeView({
                ...
                type: "graph",
                resModel: "foo",
                groupBy: ["date:year", "productId", "date", "date:quarter"],
                arch: `<graph type="line"/>`,
                config: {
                    views: [[false, "search"]],
                },
    ...
    QUnit.test("action name is displayed in breadcrumbs", async function (assert) {
        ...
        assert.strictEqual(
            graph.el.querySelector(".o-control-panel .breadcrumb-item.active").innerText,
            "Glou glou"
        );
    });

    QUnit.test("clicking on bar charts triggers a doAction", async function (assert) {
        assert.expect(6);
    ...
    QUnit.test(
        "clicking on a pie chart trigger a doAction with correct views",
        async function (assert) {
            assert.expect(6);
    ...
    QUnit.test('graph view with attribute disableLinking="1"', async function (assert) {
        assert.expect(4);
    ...
    QUnit.test("graph view without invisible attribute on field", async function (assert) {
        ...
        assert.containsOnce(graph, '.o-menu-item:contains("Revenue")');
        assert.containsOnce(graph, '.o-menu-item:contains("Foo")');
        assert.containsOnce(graph, '.o-menu-item:contains("Count")');
    });
    ...
    QUnit.test("graph view with invisible attribute on field", async function (assert) {
        ...
        assert.containsNone(graph, '.o-menu-item:contains("Revenue")');
    });
    ...
    QUnit.test("graph view sort by measure", async function (assert) {
        ...
        serverData.models.product.records.push({ id: 38, displayName: "zphone" });
        serverData.models.foo.records[7].productId = 38;
    ...
    QUnit.test("graph view sort by measure for multiple grouped data", async function (assert) {
        ...
        serverData.models.foo.records.splice(
            0,
            4,
            { id: 9, foo: 48, bar: false, productId: 41, date: "2016-04-01" },
            { id: 10, foo: 49, bar: false, productId: 41, date: "2016-04-01" },
            { id: 11, foo: 50, bar: true, productId: 37, date: "2016-01-03" },
            { id: 12, foo: 50, bar: true, productId: 41, date: "2016-01-03" }
        );
    ...
    QUnit.test("empty graph view with sample data", async function (assert) {
        ...
        const graph = await makeView({
            ...
            `,
            context: { searchDefault_falseDomain: 1 },
            searchViewArch: `
                <search>
                    <filter name="falseDomain" string="false Domain" domain="[[0, '=', 1]]"/>
                </search>
        ...
        assert.hasClass(graph.el, "o-view-sample-data");
        assert.containsOnce(graph, ".o-view-nocontent");
        assert.containsOnce(graph, ".o-graph-canvas-container canvas");
        assert.hasClass(graph.el.querySelector(".o-graph-renderer"), "o-sample-data-disabled");

        await toggleFilterMenu(graph);
        await toggleMenuItem(graph, "false Domain");
        assert.doesNotHaveClass(graph.el, "o-view-sample-data");
        assert.containsNone(graph, ".o-view-nocontent");
        assert.containsOnce(graph, ".o-graph-canvas-container canvas");
        assert.doesNotHaveClass(
            graph.el.querySelector(".o-graph-renderer"),
    ...
    QUnit.test("non empty graph view with sample data", async function (assert) {
        ...
        assert.containsOnce(graph, ".o-graph-canvas-container canvas");
        assert.doesNotHaveClass(
            graph.el.querySelector(".o-graph-canvas-container"),
            "o-sample-data-disabled"
        );
        await toggleFilterMenu(graph);
        await toggleMenuItem(graph, "false Domain");
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const graph = await makeView({
                ...
                searchViewArch: `
                    <search>
                        <filter name="groupbyFoo" string="Foo" domain="[]" context="{ 'groupby': 'foo'}"/>
                    </search>
                `,
                context: {
                    searchDefault_groupbyFoo: 1,
                },
            });
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const graph = await makeView({
                ...
                searchViewArch: `
                    <search>
                        <filter name="myFilter" string="My Filter" domain="[('id', '&lt;', 6)]"/>
                    </search>`,
    ...
    QUnit.test("change graph mode while loading a filter", async function (assert) {
        ...
        const graph = await makeView({
            ...
            searchViewArch: `
                <search>
                    <filter name="myFilter" string="My Filter" domain="[('id', '&lt;', 2)]"/>
                </search>`,
            mockRPC: function (route, args) {
                if (args.method === "webReadGroup") {
                    return Promise.resolve(def);
    ...
    QUnit.test("only process most recent data for concurrent groupby", async function (assert) {
        ...
        const graph = await makeView({
            ...
            searchViewArch: `
                <search>
                    <filter name="groupbyColor" string="Color" context="{ 'groupby': 'colorId' }"/>
                    <filter name="groupbyDate" string="Date" context="{ 'groupby': 'date' }"/>
                </search>
            `,
            mockRPC() {
                return Promise.resolve(def);
    ...
    QUnit.test("fillTemporal is true by default", async function (assert) {
        assert.expect(1);
        ...
        const graph = await makeView({
            ...
            mockRPC: function (route, args) {
                if (args.method === "webReadGroup") {
                    assert.strictEqual(
                        args.kwargs.context.fillTemporal,
                        true,
                        "The observal state of fillTemporal should be true"
                    );
                }
            },
        });
    ...
    QUnit.test("fillTemporal can be changed throught the context", async function (assert) {
        assert.expect(1);
        ...
        const graph = await makeView({
            ...
            type: "graph",
            resModel: "foo",
            context: { fillTemporal: false },
            mockRPC: function (route, args) {
                if (args.method === "webReadGroup") {
                    assert.strictEqual(
                        args.kwargs.context.fillTemporal,
                        false,
                        "The observal state of fillTemporal should be false"
                    );
                }
            },
        });
    ...
    QUnit.test('fake data in line chart', async function (assert) {
        ...
        const graph = await makeView({
            ...
            resModel: "foo",
            serverData,
            context: { searchDefault_dateFilter: 1, },
            arch: `
                <graph type="line">
                    <field name="date"/>
                </graph>
            ...
            searchViewArch:`
                <search>
                    <filter name="dateFilter" domain="[]" date="date" default_period="thirdQuarter"/>
                </search>
            `,
        });
    ...
    QUnit.test('no filling color for period of comparison', async function (assert) {
        ...
        const graph = await makeView({
            ...
            searchViewArch: `
                <search>
                    <filter name="dateFilter" domain="[]" date="date" default_period="thisYear"/>
                </search>
            `,
        });
    ...
    QUnit.test("graphGroupbys should be also used after first load", async function (assert) {
        const graph = await makeView({
            serverData,
            type: "graph",
            resModel: "foo",
