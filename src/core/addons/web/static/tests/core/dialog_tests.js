...
QUnit.module("Components", (hooks) => {
    ...
    QUnit.test("simple rendering", async function (assert) {
        ...
        assert.containsOnce(target, ".o-dialog main", "a dialog has always a main node");
        assert.strictEqual(target.querySelector("main").textContent, " Hello! ");
        assert.containsOnce(target, ".o-dialog footer", "the footer is rendered by default");
        assert.containsOnce(
            target,
            ".o-dialog footer button",
    ...
    QUnit.test("simple rendering with two dialogs", async function (assert) {
        ...
        assert.containsN(target, ".o-dialog", 2);
        assert.deepEqual(
            [...target.querySelectorAll("header .modal-title")].map((el) => el.textContent),
            ["First Title", "Second Title"]
        );
        assert.deepEqual(
            [...target.querySelectorAll(".o-dialog .modal-body")].map((el) => el.textContent),
            [" Hello! ", " Hello again! "]
        );
    });
    ...
    QUnit.test("click on the button x triggers the service close", async function (assert) {
        ...
        parent = await mount(Parent, { env, target });
        assert.containsOnce(target, ".o-dialog");
        await click(target, ".o-dialog header button.close");
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            parent = await mount(Parent, { env, target });
            assert.containsOnce(target, ".o-dialog");
            await click(target, ".o-dialog footer button");
            assert.verifySteps(["close"]);
        }
    );
    ...
    QUnit.test("render custom footer buttons is possible", async function (assert) {
        ...
        parent = await mount(Parent, { env, target });
        assert.containsOnce(target, ".o-dialog");
        assert.containsN(target, ".o-dialog footer button", 2);
    });
    ...
    QUnit.test("embed an arbitrary component in a dialog is possible", async function (assert) {
        ...
        parent = await mount(Parent, { env, target });
        assert.containsOnce(target, ".o-dialog");
        assert.containsOnce(target, ".o-dialog main .o_subcomponent");
    ...
    QUnit.test("dialog without header/footer", async function (assert) {
        ...
        parent = await mount(Parent, { env, target });
        assert.containsOnce(target, ".o-dialog");
        assert.containsNone(target, ".o-dialog header");
        assert.containsOnce(target, "main", "a dialog has always a main node");
        assert.containsNone(target, ".o-dialog footer");
    });
    ...
    QUnit.test("dialog size can be chosen", async function (assert) {
      ...
      <div>
        ...
        const env = await makeTestEnv();
        parent = await mount(Parent, { env, target });
        assert.containsN(target, ".o-dialog", 4);
        assert.containsOnce(
            target,
            target.querySelectorAll(".o-dialog .modal-dialog.modal-xl .xl")
        );
        assert.containsOnce(
            target,
            target.querySelectorAll(".o-dialog .modal-dialog.modal-lg .lg")
        );
        assert.containsOnce(
            target,
            target.querySelectorAll(".o-dialog .modal-dialog.modal-md .md")
        );
        assert.containsOnce(
            target,
            target.querySelectorAll(".o-dialog .modal-dialog.modal-sm .sm")
        );
    });
    ...
    QUnit.test("dialog can be rendered on fullscreen", async function (assert) {
        ...
        parent = await mount(Parent, { env, target });
        assert.containsOnce(target, ".o-dialog");
        assert.hasClass(target.querySelector(".o-dialog .modal"), "o-modal-full");
    });
