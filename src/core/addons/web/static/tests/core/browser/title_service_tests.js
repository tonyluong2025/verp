...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Verp" });
    assert.strictEqual(env.services.title.current, "Verp");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Verp", chat: null });
    assert.strictEqual(env.services.title.current, "Verp");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Verp - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Verp", action: "Import" });
    assert.strictEqual(env.services.title.current, "Verp - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Verp", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Verp - Import"); // parts is a copy!
});
