...
import { evaluateExpr, formatAST, parseExpr } from "@web/core/ast/index";
import { toPyValue } from "@web/core/ast/vj_utils";
import { PyDate, PyDateTime } from '@web/core/ast/py_date';
...
QUnit.module("vj", {}, () => {
    ...
    QUnit.test("contextToday", function (assert) {
        assert.ok(checkAST(`contextToday().strftime("%Y-%m-%d")`, "context today call"));
    });
    ...
    QUnit.test("various expressions", function (assert) {
        ...
        const expr = `[("type", "=", "in"), ("day", "<=", time.strftime("%Y-%m-%d")), ("day", ">", (contextToday() - datetime.timedelta(days = 15)).strftime("%Y-%m-%d"))]`;
