...
QUnit.test("add user context to a simple read request", async (assert) => {
    ...
    assert.strictEqual(query.route, "/web/dataset/callKw/my.model/read");
...
QUnit.test("create method", async (assert) => {
    ...
    assert.strictEqual(query.route, "/web/dataset/callKw/partner/create");
    assert.deepEqual(query.params, {
        args: [
            {
                color: "red",
...
QUnit.test("unlink method", async (assert) => {
    ...
    assert.strictEqual(query.route, "/web/dataset/callKw/partner/unlink");
    assert.deepEqual(query.params, {
        args: [[43]],
        kwargs: {
            context: {
...
QUnit.test("write method", async (assert) => {
    ...
    assert.strictEqual(query.route, "/web/dataset/callKw/partner/write");
    assert.deepEqual(query.params, {
...
QUnit.test("webReadGroup method", async (assert) => {
    ...
    assert.strictEqual(query.route, "/web/dataset/callKw/sale.order/webReadGroup");
    assert.deepEqual(query.params, {
        args: [],
        kwargs: {
            domain: [["userId", "=", 2]],
            fields: ["amountTotal:sum"],
            groupby: ["dateOrder"],
            context: {
                lang: "en",
                uid: 7,
...
QUnit.test("readGroup method", async (assert) => {
    ...
    assert.strictEqual(query.route, "/web/dataset/callKw/sale.order/readGroup");
    assert.deepEqual(query.params, {
        args: [],
        kwargs: {
            domain: [["userId", "=", 2]],
...
QUnit.test("searchRead method", async (assert) => {
    ...
    serviceRegistry.add("rpc", rpc);
    const env = await makeTestEnv();
    await env.services.orm.searchRead("sale.order", [["userId", "=", 2]], ["amountTotal"]);
    assert.strictEqual(query.route, "/web/dataset/callKw/sale.order/searchRead");
    assert.deepEqual(query.params, {
        args: [],
        kwargs: {
            context: {
...
QUnit.test("webSearchRead method", async (assert) => {
    ...
    serviceRegistry.add("rpc", rpc);
    const env = await makeTestEnv();
    await env.services.orm.webSearchRead("sale.order", [["userId", "=", 2]], ["amountTotal"]);
    assert.strictEqual(query.route, "/web/dataset/callKw/sale.order/web_search_read");
    assert.deepEqual(query.params, {
        args: [],
        kwargs: {
            context: {
...
QUnit.test("silent mode", async (assert) => {
    ...
    assert.verifySteps([
        "/web/dataset/callKw/my_model/my_method",
        "/web/dataset/callKw/my_model/my_method (silent)",
        "/web/dataset/callKw/my_model/my_method",
        "/web/dataset/callKw/my_model/read",
        "/web/dataset/callKw/my_model/read (silent)",
        "/web/dataset/callKw/my_model/read",
    ]);
});
...
QUnit.test("optimize read and unlink if no ids", async (assert) => {
    ...
    assert.verifySteps(["/web/dataset/callKw/my_model/read"]);

    await orm.read("my_model", [], []);
    assert.verifySteps([]);

    await orm.unlink("my_model", [1], {});
    assert.verifySteps(["/web/dataset/callKw/my_model/unlink"]);
