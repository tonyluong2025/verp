...
QUnit.test("can parse a hash with 2 key/value pairs", (assert) => {
    const hash = "#action=114&activeId=mail.boxInbox";
    assert.deepEqual(parseHash(hash), { action: 114, activeId: "mail.boxInbox" });
});
...
QUnit.test("can parse a realistic hash", (assert) => {
    const hash = "#action=114&activeId=mail.boxInbox&cids=1&menuId=91";
    const expected = {
        action: 114,
        activeId: "mail.boxInbox",
        cids: 1,
        menuId: 91,
    };
