    ...
    () => {
        ...
        QUnit.module("dates utils compatibility with legacy", {
            beforeEach() {
                ...
                const unpatchDate = legacy.testUtils.mock.patchDate(2020, 6, 15, 12, 30, 0);
                registerCleanup(() => {
                    if (unpatchDate) {
                        unpatchDate();
                    }
        ...
        QUnit.test("parseDate", async (assert) => {
            ...
            runTestSet(assert, testSet, {
                ...
                legacyFn: (input) => legacy.fieldUtils.parse.date(input).toISOString(),
            });
        });
        ...
        QUnit.test("parseDate (with legacy options.isUTC = true)", async (assert) => {
            ...
            runTestSet(assert, testSet, {
                ...
                legacyFn: (input) =>
                    legacy.fieldUtils.parse.date(input, null, { isUTC: true }).toISOString(),
            });
        });
        ...
        QUnit.test("parseDateTime", async (assert) => {
            ...
            runTestSet(assert, testSet, {
                ...
                legacyFn: (input) => legacy.fieldUtils.parse.datetime(input).toISOString(),
            });
        });
        ...
        QUnit.test("parseDateTime (with legacy options.isUTC = true)", async (assert) => {
            ...
            runTestSet(assert, testSet, {
                ...
                legacyFn: (input) =>
                    legacy.fieldUtils.parse.datetime(input, null, { isUTC: true }).toISOString(),
            });
        });
