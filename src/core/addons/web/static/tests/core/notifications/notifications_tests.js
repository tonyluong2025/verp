...
QUnit.test("can display a basic notification", async (assert) => {
    ...
    await nextTick();
    assert.containsOnce(target, ".o-notification");
    const notif = target.querySelector(".o-notification");
    assert.strictEqual(
...
QUnit.test("title and message are escaped by default", async (assert) => {
    ...
    const notif = target.querySelector(".o-notification");
    assert.strictEqual(
        notif.querySelector(".o-notification-title").textContent,
        "<b>Some title</b>"
    );
    assert.strictEqual(
        notif.querySelector(".o-notification-content").textContent,
...
QUnit.test("notification with messageIsHtml option", async (assert) => {
    ...
    assert.containsOnce(target, ".o-notification");
    const notif = target.querySelector(".o-notification");
    assert.strictEqual(notif.querySelector(".o-notification-content").textContent, "Some message");
    assert.strictEqual(
        notif.querySelector(".o-notification-content").innerHTML,
        "<i>Some message</i>"
    );
});
...
QUnit.test("can display a notification with a button", async (assert) => {
    ...
    assert.containsOnce(target, ".o-notification");
    const notif = target.querySelector(".o-notification");
    assert.strictEqual(notif.querySelector(".o-notification-buttons").textContent, "I'm a button");
...
QUnit.test("can display a notification with a callback when closed", async (assert) => {
    ...
    await click(target, ".o-notification .o-notification-close");
    assert.verifySteps(["Notification closed"]);
    assert.containsNone(target, ".o-notification");
});
...
QUnit.test("close a non-sticky notification while another one remains", async (assert) => {
    ...
    await nextTick();
    assert.containsN(target, ".o-notification", 2);
