...
QUnit.test("Simple rendering with a single dialog", async (assert) => {
    ...
    assert.containsNone(target, ".o-dialog-container .o-dialog");
    env.services.dialog.add(CustomDialog);
    await nextTick();
    assert.containsOnce(target, ".o-dialog-container .o-dialog");
    assert.strictEqual(target.querySelector("header .modal-title").textContent, "Welcome");
    await click(target.querySelector(".o-dialog-container .o-dialog footer button"));
    assert.containsNone(target, ".o-dialog-container .o-dialog");
});
...
QUnit.test("rendering with two dialogs", async (assert) => {
    ...
    await nextTick();
    assert.containsN(target, ".o-dialog-container .o-dialog", 2);
    assert.deepEqual(
...
QUnit.test("multiple dialogs can become the UI active element", async (assert) => {
    ...
    let dialogModal = target.querySelector(
        ".o-dialog-container .o-dialog .modal:not(.o-inactive-modal)"
    );
...
QUnit.test("Interactions between multiple dialogs", async (assert) => {
    ...
    function activity(modals) {
        ...
        for (let i = 0; i < modals.length; i++) {
            active[i] = !modals[i].classList.contains("o-inactive-modal");
    ...
    modals = document.querySelectorAll(".modal");
    assert.containsN(target, ".o-dialog", 1);
    res = activity(modals);
    ...
    lastDialog = modals[modals.length - 1];
    await click(lastDialog, "footer button");
    assert.containsNone(target, ".o-dialog-container .modal");
    assert.containsOnce(target, ".o-dialog-container");
});
...
QUnit.test("dialog component crashes", async (assert) => {
    ...
    assert.containsOnce(pseudoWebClient, ".modal .o-dialog-error");
});
