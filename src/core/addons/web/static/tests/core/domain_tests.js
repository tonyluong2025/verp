...
import { PyDate } from "../../src/core/ast/py_date";
...
QUnit.module("domain", {}, () => {
    ...
    QUnit.test("or", function (assert) {
        ...
        assert.ok(new Domain(currentDomain).contains({ ...record, userId: 3 }));
    ...
    QUnit.test("toString", function (assert) {
        ...
        assert.strictEqual(new Domain(["!", ["a", "=", 3]]).toString(), `["!", ("a", "=", 3)]`);
        assert.strictEqual(new Domain([["label", "=", null]]).toString(), '[("label", "=", None)]');
        assert.strictEqual(new Domain([["label", "=", false]]).toString(), '[("label", "=", false)]');
        assert.strictEqual(new Domain([["label", "=", true]]).toString(), '[("label", "=", true)]');
        assert.strictEqual(
            new Domain([["label", "=", "null"]]).toString(),
            '[("label", "=", "null")]'
        );
        assert.strictEqual(
            new Domain([["label", "=", "false"]]).toString(),
            '[("label", "=", "false")]'
        );
        assert.strictEqual(
            new Domain([["label", "=", "true"]]).toString(),
            '[("label", "=", "true")]'
        );
        assert.strictEqual(new Domain().toString(), "[]");
        assert.strictEqual(new Domain([['label', 'in', [true, false]]]).toString(), '[("label", "in", [true, false])]');
        assert.strictEqual(new Domain([['label', 'in', [null]]]).toString(), '[("label", "in", [None])]');
        assert.strictEqual(new Domain([['label', 'in', ["foo", "bar"]]]).toString(), '[("label", "in", ["foo", "bar"])]');
        assert.strictEqual(new Domain([['label', 'in', [1, 2]]]).toString(), '[("label", "in", [1, 2])]');
        assert.strictEqual(new Domain(['&', ['label', '=', 'foo'], ['type', '=', 'bar']]).toString(), '["&", ("label", "=", "foo"), ("type", "=", "bar")]');
        assert.strictEqual(new Domain(['|', ['label', '=', 'foo'], ['type', '=', 'bar']]).toString(), '["|", ("label", "=", "foo"), ("type", "=", "bar")]');
        assert.strictEqual(new Domain().toString(), "[]");

        // string domains are only reformatted
        assert.strictEqual(new Domain('[("label","ilike","foo")]').toString(), '[("label", "ilike", "foo")]');
    });
    ...
    QUnit.test("can evaluate a javascript expression", function (assert) {
    ...
    QUnit.test("some expression with date stuff", function (assert) {
        ...
        let domainStr =
            "[('date','>=', (contextToday() - datetime.timedelta(days=30)).strftime('%Y-%m-%d'))]";
        assert.deepEqual(new Domain(domainStr).toList(), [["date", ">=", "2013-03-25"]]);
        domainStr = "[('date', '>=', contextToday() - relativedelta(days=30))]";
    ...
    QUnit.test("invalid domains should not succeed", function (assert) {
        assert.throws(
            () => new Domain(["|", ["hrPresenceState", "=", "absent"]]),
    ...
    QUnit.test("combining one domain", function (assert) {
        ...
        assert.strictEqual(
            Domain.combine([`[("userId", "=", uid)]`], "AND").toString(),
            `[("userId", "=", uid)]`
        );
