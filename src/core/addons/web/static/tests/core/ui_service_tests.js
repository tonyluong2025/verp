...
QUnit.test("block and unblock once ui with ui service", async (assert) => {
    ...
    ({ Component: BlockUI, props } = registry.category("mainComponents").get("BlockUI"));
    const ui = env.services.ui;
    await mount(BlockUI, { env, target, props });
    let blockUI = target.querySelector(".o-block-uI");
    assert.strictEqual(blockUI, null, "ui should not be blocked");
    ui.block();
    await nextTick();
    blockUI = target.querySelector(".o-block-uI");
