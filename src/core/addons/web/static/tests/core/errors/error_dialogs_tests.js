...
QUnit.test("ErrorDialog with traceback", async (assert) => {
    ...
    Parent.template = tags.xml`<ErrorDialog traceback="traceback" name="label" message="message" data="data"/>`;
    assert.containsNone(target, ".o-dialog");
    env = await makeTestEnv();
    parent = await mount(Parent, { env, target });
    assert.containsOnce(target, ".o-dialog");
    assert.strictEqual(target.querySelector("header .modal-title").textContent, "Verp Error");
    ...
    assert.containsNone(target, "div.o-error-detail");
    assert.strictEqual(target.querySelector(".o-dialog footer button").textContent, "Ok");
    click(mainButtons[1]);
    await nextTick();
    assert.deepEqual(
        [...target.querySelectorAll("main .clearfix p")].map((el) => el.textContent),
    ...
    assert.containsOnce(target, "div.o-error-detail");
    assert.strictEqual(
        target.querySelector("div.o-error-detail").textContent,
...
QUnit.test("Client ErrorDialog with traceback", async (assert) => {
    ...
    Parent.template = tags.xml`<ClientErrorDialog traceback="traceback" name="label" message="message" data="data"/>`;
    assert.containsNone(target, ".o-dialog");
    env = await makeTestEnv();
    parent = await mount(Parent, { env, target });
    assert.containsOnce(target, ".o-dialog");
...
QUnit.test("WarningDialog", async (assert) => {
    ...
    class Parent extends Component {
        constructor() {
            super(...arguments);
            this.name = "verp.exceptions.UserError";
            this.message = "...";
    ...
    assert.containsOnce(target, "main .o-dialog-warning");
...
QUnit.test("RedirectWarningDialog", async (assert) => {
    ...
    parent = await mount(Parent, { env, target });
    assert.containsOnce(target, ".o-dialog");
    assert.strictEqual(target.querySelector("header .modal-title").textContent, "Verp Warning");
    assert.strictEqual(target.querySelector("main").textContent, "Some strange unreadable message");
...
QUnit.test("SessionExpiredDialog", async (assert) => {
    ...
    assert.strictEqual(
        target.querySelector("header .modal-title").textContent,
        "Verp Session Expired"
    );
    assert.strictEqual(
        target.querySelector("main p").textContent,
        " Your Verp session expired. The current page is about to be refreshed. "
    );
    const footerButton = target.querySelector(".o-dialog footer button");
