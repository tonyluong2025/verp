...
QUnit.module("DebugMenu", (hooks) => {
    hooks.beforeEach(async () => {
        ...
        const mockRPC = async (route, args) => {
            if (args.method === "checkAccessRights") {
    ...
    QUnit.test("Don't display the DebugMenu if debug mode is disabled", async (assert) => {
        ...
        assert.containsOnce(target, ".o-dialog");
        assert.containsNone(target, ".o-dialog .o-debug-manager .fa-bug");
    });
    ...
    QUnit.test(
        ...
        async (assert) => {
            ...
            assert.containsOnce(target, ".o-dialog");
            assert.containsOnce(target, ".o-dialog .o-debug-manager .fa-bug");
            await click(target, ".o-dialog .o-debug-manager button");
            const debugManagerEl = target.querySelector(".o-debug-manager");
    ...
    QUnit.test("can regenerate assets bundles", async (assert) => {
        const mockRPC = async (route, args) => {
            ...
            if (route === "/web/dataset/callKw/ir.attachment/search") {
                assert.step("ir.attachment/search");
                return [1, 2, 3];
            }
            if (route === "/web/dataset/callKw/ir.attachment/unlink") {
    ...
    QUnit.test("can open a view", async (assert) => {
        ...
        Object.assign(serverData.views, {
            "ir.ui.view,false,list": `<list><field name="label"/><field name="type"/></list>`,
            "ir.ui.view,false,search": `<search/>`,
            "partner,1,form": `<form><div class="some_view"/></form>`,
        });

        const webClient = await createWebClient({ serverData, mockRPC });
        await click(webClient.el.querySelector(".o-debug-manager button"));
        await click(webClient.el.querySelector(".o-debug-manager .dropdown-item"));
        await legacyExtraNextTick();
        assert.containsOnce(webClient, ".modal .o-list-view");

        await click(webClient.el.querySelector(".modal .o-list-view .o-data-row"));
        await legacyExtraNextTick();
    ...
    QUnit.test("can edit a pivot view", async (assert) => {
        ...
        await click(webClient.el.querySelector(".o-debug-manager .dropdown-item"));
        await legacyExtraNextTick();
        assert.containsOnce(webClient, ".modal .o-form-view");
        assert.strictEqual(
            webClient.el.querySelector(".modal .o-form-view .o-field-widget[name=id]").value,
            "18"
        );
    });
    ...
    QUnit.test("can edit a search view", async (assert) => {
        ...
        serverData.actions[1].searchViewId = [293, "some_search_view"];
        serverData.models["ir.ui.view"] = {
            fields: {},
            records: [{ id: 293 }],
        };
    ...
    QUnit.test("edit search view on action without searchViewId", async (assert) => {
    ...
    QUnit.test(
        ...
        async (assert) => {
            ...
            await click(webClient.el.querySelector(".o-dialog .o-debug-manager button"));
            assert.containsNone(webClient, ".o-debug-manager .dropdown-item");
        }
    );
});
