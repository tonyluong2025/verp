...
const commandProviderRegistry = registry.category("commandProvider");

export async function editSearchBar(searchValue) {
    const searchBar = document.querySelector(".o-command-palette-search input");
...
QUnit.test("useCommand hook", async (assert) => {
    ...
    await nextTick();
    assert.containsOnce(target, ".o-command");
    assert.deepEqual(target.querySelector(".o-command").textContent, "Take the throne");

    await click(target, ".o-command");
...
QUnit.test("useCommand hook when the activeElement change", async (assert) => {
    ...
    assert.deepEqual(
        [...target.querySelectorAll(".o-command")].map((e) => e.textContent),
...
QUnit.test("data-hotkey added to command palette", async (assert) => {
  ...
  `;
    ...
    assert.containsN(target, ".o-command", 2);
    assert.deepEqual(
        [...target.querySelectorAll(".o-command span:first-child")].map((el) => el.textContent),
        ["Aria stark", "Bran stark"]
    );

    // Click on first command
    await click(target, "#o-command-0");
    assert.containsNone(target, ".o-command-palette", "palette is closed due to command action");
    ...
    await click(target, "#o-command-1");
    assert.containsNone(target, ".o-command-palette", "palette is closed due to command action");
    assert.strictEqual(
        document.activeElement,
        target.querySelector("input[title='Bran Stark']"),
...
QUnit.test("can be searched", async (assert) => {
    ...
    assert.deepEqual(
        target.querySelector(".o-command-palette-search input").value,
        "",
...
QUnit.test("configure the empty message based on the namespace", async (assert) => {
    ...
    const commandEmptyMessageRegistry = registry.category("commandEmptyList");
...
QUnit.test("multi level commands with hotkey", async (assert) => {
    ...
    assert.deepEqual(
        [...target.querySelectorAll(".o-command")].map((el) => el.textContent.toLowerCase()),
...
QUnit.test("command categories", async (assert) => {
    ...
    assert.deepEqual(
        [...target.querySelectorAll(".o-command-category")].map((el) => el.textContent),
        ["a", "b", "cd"]
    );
});
