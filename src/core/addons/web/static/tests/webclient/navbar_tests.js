...
QUnit.test("can be rendered", async (assert) => {
    ...
    assert.containsOnce(
        navbar.el,
        ".o-navbar-apps-menu button.dropdown-toggle",
...
QUnit.test("dropdown menu can be toggled", async (assert) => {
    ...
    const target = getFixture();
    const navbar = await mount(NavBar, { env, target });
    const dropdown = navbar.el.querySelector(".o-navbar-apps-menu");
...
QUnit.test("href attribute on apps menu items", async (assert) => {
    ...
    const target = getFixture();
    const navbar = await mount(NavBar, { env, target });
    const appsMenu = navbar.el.querySelector(".o-navbar-apps-menu");
    await click(appsMenu, "button.dropdown-toggle");
    const dropdownItem = navbar.el.querySelector(".o-navbar-apps-menu .dropdown-item");
    assert.strictEqual(dropdownItem.getAttribute("href"), "#menuId=1&action=339");
...
QUnit.test("data-menu-xmlid attribute on AppsMenu items", async (assert) => {
    ...
    await nextTick();
    assert.containsOnce(navbar, ".o-menu-sections .dropdown-item[data-menu-xmlid=menu_3]");

    // check sub menus toggler
    assert.containsOnce(navbar, ".o-menu-sections button.dropdown-toggle[data-menu-xmlid=menu_4]");

    // check sub menus
    await click(navbar.el.querySelector(".o-menu-sections .dropdown-toggle"));
    assert.containsOnce(navbar, ".o-menu-sections .dropdown-item[data-menu-xmlid=menu_5]");
...
QUnit.test("navbar can display systray items ordered based on their sequence", async (assert) => {
    ...
    const target = getFixture();
    const navbar = await mount(NavBar, { env, target });
    const menuSystray = navbar.el.getElementsByClassName("o-menu-systray")[0];
...
QUnit.test("can adapt with 'more' menu sections behavior", async (assert) => {
    ...
    assert.containsN(
        navbar.el,
        ".o-menu-sections > *:not(.o-menu-sections_more):not(.d-none)",
        3,
        "should have 3 menu sections displayed (that are not the 'more' menu)"
    );
    assert.containsNone(navbar.el, ".o-menu-sections_more", "the 'more' menu should not exist");
    ...
    assert.containsOnce(
        navbar.el,
        ".o-menu-sections > *:not(.d-none)",
        "only one menu section should be displayed"
    );
    assert.containsOnce(
        navbar.el,
        ".o-menu-sections_more:not(.d-none)",
        "the displayed menu section should be the 'more' menu"
    );
    // Open the more menu
    await click(navbar.el, ".o-menu-sections_more .dropdown-toggle");
    assert.deepEqual(
