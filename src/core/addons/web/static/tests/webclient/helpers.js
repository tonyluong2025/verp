...
import { legacyServiceProvider } from "@web/legacy/legacyServiceProvider";
import {
...
// web.testUtils easyload xml templates at line : 124:130.
...
export function addLegacyMockEnvironment(env, legacyParams = {}) {
    ...
    const dataManager = Object.assign(
        {
            ...
            load_views: async (params, options) => {
                const result = await env.services.rpc(`/web/dataset/callKw/${params.model}`, {
                    args: [],
                    kwargs: {
                        context: params.context,
                        options: options,
                        views: params.viewsDescr,
                    },
                    method: "load_views",
                    model: params.model,
                });
                const views = result.fields_views;
                for (const [, viewType] of params.viewsDescr) {
    ...
    function patchLegacySession() {
        const userContext = Object.getOwnPropertyDescriptor(session, "userContext");
        registerCleanup(() => {
            Object.defineProperty(session, "userContext", userContext);
        });
    }
    patchLegacySession();
    serviceRegistry.add("legacySession", makeLegacySessionService(legacyEnv, session));
    ...
    if (legacyParams.withLegacyMockServer) {
        ...
        const W = Widget.extend({ doPushState() {} });
...
export function getActionManagerServerData() {
    ...
    const actionsArray = [
        ...
        },
        {
            ...
            xmlid: "action_11",
            name: "Another Report",
            reportName: "another_report",
            reportType: "qweb-pdf",
            type: "ir.actions.report",
            closeOnReportDownload: true,
        },
        {
            id: 12,
            xmlid: "action_12",
    ...
    const archs = {
        ...
        "pony,false,list": '<tree><field name="label"/></tree>',
        // form views
        "partner,false,form":
            "<form>" +
            "<header>" +
      ...
      <header></header>
      <sheet>
      <div class="oe-button-box" name="buttonBox" modifiers="{}">
      <button class="oe-stat-button" type="action" name="1" icon="fa-star" context="{'default_partner': activeId}">
      ...
      <field name="displayName"/>
        ...
        "pony,false,form": "<form>" + '<field name="label"/>' + "</form>",
        // search views
    ...
    const models = {
        partner: {
            ...
            records: [
                { id: 1, displayName: "First record", foo: "yop", bar: 2, o2m: [2, 3], m2o: 3 },
                {
                    id: 2,
                ...
                { id: 3, displayName: "Third record", foo: "gnap", bar: 1, o2m: [], m2o: 1 },
                { id: 4, displayName: "Fourth record", foo: "plop", bar: 2, o2m: [], m2o: 1 },
                { id: 5, displayName: "Fifth record", foo: "zoup", bar: 2, o2m: [], m2o: 1 },
            ],
        },
        pony: {
            fields: {
