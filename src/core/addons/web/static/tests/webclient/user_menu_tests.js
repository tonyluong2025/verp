...
QUnit.test("can be rendered", async (assert) => {
    ...
    assert.containsOnce(userMenuEl, "img.o-user-avatar");
    assert.strictEqual(
        userMenuEl.querySelector("img.o-user-avatar").dataset['src'],
        "http://lordofthering/web/image?model=res.users&field=avatar128&id=7"
    );
    assert.containsOnce(userMenuEl, "span.oe-topbar-name");
    assert.strictEqual(userMenuEl.querySelector(".oe-topbar-name").textContent, "Sauron");
...
QUnit.test("display the correct name in debug mode", async (assert) => {
    ...
    assert.containsOnce(userMenuEl, "img.o-user-avatar");
    assert.containsOnce(userMenuEl, "span.oe-topbar-name");
    assert.strictEqual(userMenuEl.querySelector(".oe-topbar-name").textContent, "Sauron (test)");
});

QUnit.test("can execute the callback of settings", async (assert) => {
    const mockRPC = (route) => {
        if (route === "/web/dataset/callKw/res.users/action_get") {
