...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("actions can be cached", async function (assert) {
        ...
        // With same activeId
        await loadAction(3, { activeId: 1 });
        await loadAction(3, { activeId: 1 });

        // With activeId change
        await loadAction(3, { activeId: 2 });

        // With same activeIds
        await loadAction(3, { activeIds: [1, 2] });
        await loadAction(3, { activeIds: [1, 2] });

        // With activeIds change
        await loadAction(3, { activeIds: [1, 2, 3] });

        // With same activeModel
        await loadAction(3, { activeModel: "a" });
        await loadAction(3, { activeModel: "a" });

        // With activeModel change
        await loadAction(3, { activeModel: "b" });

        assert.verifySteps(
            [
                '{"actionId":3,"additionalContext":{}}',
                '{"actionId":3,"additionalContext":{"activeId":1}}',
                '{"actionId":3,"additionalContext":{"activeId":2}}',
                '{"actionId":3,"additionalContext":{"activeIds":[1,2]}}',
                '{"actionId":3,"additionalContext":{"activeIds":[1,2,3]}}',
                '{"actionId":3,"additionalContext":{"activeModel":"a"}}',
                '{"actionId":3,"additionalContext":{"activeModel":"b"}}',
            ],
            "should load from server once per activeId/activeIds/activeModel change, nothing else"
        );
    });
    ...
    QUnit.test('action with "no_breadcrumbs" set to true', async function (assert) {
        ...
        assert.containsOnce(webClient, ".o-control-panel .breadcrumb-item");
        // push another action flagged with 'no_breadcrumbs=true'
        await doAction(webClient, 4);
        assert.containsNone(webClient, ".o-control-panel .breadcrumb-item");
    });

    QUnit.test("document's title is updated when an action is executed", async function (assert) {
        assert.expect(8);
        const defaultTitle = { zopenerp: "Verp" };
        const webClient = await createWebClient({ serverData });
        ...
        assert.deepEqual(currentHash, { action: 4, model: "partner", viewType: "kanban" });
        await doAction(webClient, 8);
        currentTitle = webClient.env.services.title.getParts();
        assert.deepEqual(currentTitle, {
            ...defaultTitle,
        ...
        currentHash = webClient.env.services.router.current.hash;
        assert.deepEqual(currentHash, { action: 8, model: "pony", viewType: "list" });
        ...
        currentHash = webClient.env.services.router.current.hash;
        assert.deepEqual(currentHash, { action: 8, id: 4, model: "pony", viewType: "form" });
    });
    ...
    QUnit.test('handles "historyBack" event', async function (assert) {
        assert.expect(3);
        const webClient = await createWebClient({ serverData });
        await doAction(webClient, 4);
        await doAction(webClient, 3);
    ...
    QUnit.test("stores and restores scroll position", async function (assert) {
        ...
        await doAction(webClient, 3);
        assert.strictEqual(webClient.el.querySelector(".o-content").scrollTop, 0);
        // simulate a scroll
        webClient.el.querySelector(".o-content").scrollTop = 100;
        ...
        await legacyExtraNextTick();
        assert.strictEqual(webClient.el.querySelector(".o-content").scrollTop, 100);
    });
      ...
      <form>
        <field name="o2m">
          ...
          <form><field name="foo"/></form>
            ...
            await testUtils.dom.click($(webClient.el).find(".o-form-view .o-data-row:first"));
            await legacyExtraNextTick();
      ...
      <form>
        <field name="o2m">
          ...
          <form><field name="foo"/></form>
            ...
            assert.containsN(document.body, ".modal .o-form-view", 2);
        }
    );
    ...
    QUnit.test("bootstrap tooltip destroyed on click", async (assert) => {
        ...
        triggerEvents(webClient.el, ".o-form-statusbar button", ["mouseover", "focusin"]);
        await tooltipProm;
        // check on webClient DOM
        assert.containsOnce(webClient.el, ".tooltip");
        await click(webClient.el, ".o-content");
        // check on the whole DOM
        assert.containsNone(document.body, ".tooltip");
    });
});
