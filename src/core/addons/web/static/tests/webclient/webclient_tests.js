...
QUnit.module("WebClient", {
    async beforeEach() {
        serviceRegistry
            ...
            .add("legacyServiceProvider", legacyServiceProvider)
...
QUnit.test("can be rendered", async (assert) => {
    ...
    assert.containsOnce(webClient.el, "header > nav.o-main-navbar");
    webClient.destroy();
});
...
QUnit.test("webclient for the superuser", async (assert) => {
    ...
    const target = getFixture();
    const webClient = await mount(WebClient, { env, target });
    assert.hasClass(webClient.el, "o-is-superuser");
    webClient.destroy();
});
...
QUnit.test("webclient for a non superuser", async (assert) => {
    ...
    const target = getFixture();
    const webClient = await mount(WebClient, { env, target });
    assert.doesNotHaveClass(webClient.el, "o-is-superuser");
    webClient.destroy();
});
