verp.define('base.settingsTests', function (require) {
"use strict";
...
var BaseSettingsView = viewRegistry.get('baseSettings');

const { legacyExtraNextTick } = require("@web/../tests/helpers/utils");
const { createWebClient, doAction } = require("@web/../tests/webclient/helpers");

let serverData;
QUnit.module('baseSettingsTests', {
    beforeEach: function () {
        this.data = {
            'res.config.settings': {
                fields: {
    ...
    QUnit.test('change setting on nav bar click in base settings', async function (assert) {
        ...
        var form = await createView({
            ...
            data: this.data,
            arch: '<form string="Settings" class="oe-form-configuration o-base-settings">' +
                    '<div class="o-panel">' +
                    ...
                    '<div class="o-setting-container">' +
                        ...
                        '<div class="settings">' +
                            '<div class="notFound o-hidden">No Record Found</div>' +
                            '<div class="app-settings-block" string="CRM" data-key="crm">' +
                                '<div class="row mt16 o-settings-container">'+
                                    ...
                                    '<div class="col-12 col-lg-6 o-setting-box">'+
                                        ...
                                        '</div>'+
                                        '<div class="o-setting-right-pane">'+
                                            '<span class="o-form-label">Foo</span>'+
                                            '<div class="text-muted">'+
                                                'this is foo'+
                                            '</div>'+
                                        '</div>' +
        ...
        assert.isVisible(form.$(".settings .app-settings-block"), "res.config.settings settings show");
        ...
        assert.strictEqual(form.$('span.o-form-label .highlighter').html(), "F", "F word highlighted");
        form.destroy();
    });
    ...
    QUnit.test('unhighlight section not matching anymore', async function(assert) {
        ...
        const form = await createView({
            ...
            data: this.data,
            arch: `
                <form string="Settings" class="oe-form-configuration o-base-settings">
                    <div class="o-panel">
                        <div class="setting_search">
                            <input type="text" class="searchInput" placeholder="Search..." />
                        </div>
                                    ...
                                    <div class="col-12 col-lg-6 o-setting-box">
                                        <div class="o-setting-right-pane">
                                            ...
                                            <div class="content-group">
                                                <div class="mt16">
                                                    <field name="baz" class="o-light-label" widget="radio"/>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
        ...
        assert.hasAttrValue(form.$('.selected'), 'data-key',"crm","crm setting selected");
        assert.isVisible(form.$(".settings .app-settings-block"), "project settings show");
        ...
        async function (assert) {
            ...
            serverData.views = {
                "res.config.settings,1,form":
                    `<form string="Settings" jsClass="baseSettings">
                        <div class="app-settings-block" string="CRM" data-key="crm">
                            <button name="4" string="Execute action" type="action"/>
                        </div>
                    </form>`,
                "task,2,list": '<tree><field name="displayName"/></tree>',
            ...
            await testUtils.dom.click($(".o-control-panel .breadcrumb-item a"));
            await legacyExtraNextTick();
            assert.hasClass($(webClient.el).find(".o-form-view"), "o-form-editable");
            assert.verifySteps([
                "loadViews", // initial setting action
                ...
                "loadViews", // for other action in breadcrumb,
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await legacyExtraNextTick();
            assert.containsOnce(webClient, ".o-list-view", "should be open list view");
            ...
            await legacyExtraNextTick();
            assert.containsOnce(webClient, ".o-form-view", "should be remain on form view");
    ...
    QUnit.test('settings view does not display other settings after reload', async function (assert) {
                    ...
                    '<div class="o-setting-container">' +
                        ...
                        '<div class="settings">' +
                            ...
                            '<div class="app-settings-block" string="CRM" data-key="crm">' +
                                'crm tab' +
                            '</div>' +
                            '<div class="app-settings-block o-not-app" string="Other App" data-key="otherapp">' +
                                'other app tab' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
        ...
        assert.strictEqual(form.$('.app-settings-block').text().replace(/\s/g,''), 'CRMcrmtab');
        await form.reload();
        assert.strictEqual(form.$('.app-settings-block').text().replace(/\s/g,''), 'CRMcrmtab');
        form.destroy();
    });
    ...
    QUnit.test('settings view shows a message if there are changes', async function (assert) {
        ...
        var form = await createView({
            ...
            arch: '<form string="Settings" class="oe-form-configuration o-base-settings">' +
                    ...
                    '<div class="o-setting-container">' +
                        ...
                        '<div class="settings">' +
                            '<div class="notFound o-hidden">No Record Found</div>' +
                            '<div class="app-settings-block" string="Base Setting" data-key="base-setting">' +
        ...
        assert.containsNone(form, '.o-field-boolean input:checked', "checkbox should not be checked");
        assert.containsNone(form, ".o-dirty-warning", "warning message should not be shown");
        await testUtils.dom.click(form.$("input[type='checkbox']"));
        assert.containsOnce(form, '.o-field-boolean input:checked' ,"checkbox should be checked");
        assert.containsOnce(form, ".o-dirty-warning", "warning message should be shown");
        form.destroy();
    });
    ...
    QUnit.test('settings view shows a message if there are changes even if the save failed', async function (assert) {
        ...
        assert.containsOnce(form, ".o-dirty-warning", "warning message should be shown");
        await testUtils.form.clickSave(form);
        assert.containsNone(form, ".o-dirty-warning", "warning message should be shown");
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            serverData.views = {
                "task,1,list": '<tree><field name="displayName"/></tree>',
                ...
                "task,3,list": '<tree><field name="displayName"/></tree>',
                "res.config.settings,false,search": "<search></search>",
                "task,false,search": "<search></search>",
            };
    ...
    QUnit.test('settings can contain one2many fields', async function (assert) {
        ...
        await testUtils.fields.editInput($('.modal-body input[name=displayName]'), 'Added Task');
        await testUtils.dom.click($('.modal-dialog footer button:first-child'));

        assert.strictEqual(form.$('table.o-list-table:eq(0) tr.o-data-row td.o-data-cell:eq(0)').text(),
            'Added Task',
