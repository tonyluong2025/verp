    ...
    function () {
        ...
        QUnit.test("Loading correctly", async function (assert) {
            ...
            const list = await createView({
                ...
                data: this.data,
                arch:
                    '<tree editable="top" jsClass="lazy_column_list">' +
                    '<field name="label"/>' +
                    '<field name="slowField" options=\'{"lazy": true}\'/>' +
                    "</tree>",
                mockRPC: async function (route, args) {
                    if (route === "/web/dataset/callKw/person/read") {
            ...
            assert.containsN(list, ".o-data-row", 2, "should have 2 records");
        ...
        QUnit.test("With the priority widget", async function (assert) {
            ...
            assert.strictEqual(list.$('.o-data-row').first().find('.o-priority a.fa-star').length, 0,
                "widget shouldn't be considered set");
            await testUtils.dom.click(list.$('.o-data-row').first().find('.o-priority a.fa-star-o').first());
            
            assert.strictEqual(list.$('.o-data-row').first().find('.o-priority a.fa-star').length, 1,
