...
function factory(dependencies) {
    ...
    class ComposerView extends dependencies['mail.model'] {
        ...
        handleCurrentPartnerIsTyping() {
            ...
            if (
                this.suggestionModelName === 'mail.channelCommand' ||
        ...
        async openFullComposer() {
            ...
            const context = {
                default_attachmentIds: attachmentIds,
                default_body: escapeAndCompactTextContent(this.composer.textInputContent),
                default_isLog: this.composer.isLog,
                default_model: this.composer.activeThread.model,
                default_partnerIds: this.composer.recipients.map(partner => partner.id),
                default_resId: this.composer.activeThread.id,
        ...
        async postMessage() {
            ...
            const params = {
                'postData': postData,
                'threadId': composer.thread.id,
                'threadModel': composer.thread.model,
            };
            try {
                ...
                if (composer.thread.model === 'mail.channel') {
                    ...
                    Object.assign(postData, {
                        subtypeXmlid: composer.isLog ? 'mail.mtNote' : 'mail.mtComment',
                    });
                    if (!composer.isLog) {
                        params.context = { mailPostAutofollow: true };
                    }
                }
                if (this.threadView && this.threadView.replyingToMessageView && this.threadView.thread !== this.messaging.inbox) {
                    postData.parentId = this.threadView.replyingToMessageView.message.id;
                }
        ...
        async updateMessage() {
            ...
            let data = {
                body: body,
                attachmentIds: composer.attachments.concat(this.messageViewInEditing.message.attachments).map(attachment => attachment.id),
            };
            try {
                composer.update({ isPostingMessage: true });
        ...
        _computeSuggestionModelName() {
            switch (this.suggestionDelimiter) {
                case '@':
                    return 'mail.partner';
                case ':':
                    return 'mail.cannedResponse';
                case '/':
                    return 'mail.channelCommand';
                case '#':
                    return 'mail.thread';
                default:
                    return clear();
        ...
        _generateMentionsLinks(body) {
            ...
            for (const partner of this.composer.mentionedPartners) {
                ...
                const text = `@${owl.utils.escape(partner.label)}`;
                mentions.push({
                    class: 'o-mail-redirect',
                    id: partner.id,
                    model: 'res.partner',
                    placeholder,
                    text,
            ...
            for (const channel of this.composer.mentionedChannels) {
                ...
                const text = `#${owl.utils.escape(channel.label)}`;
                mentions.push({
                    class: 'o-channel-redirect',
                    id: channel.id,
                    model: 'mail.channel',
                    placeholder,
                    text,
         ...
         * @returns {mail.channelCommand|undefined} command, if any in the content
         */
            ...
            if (content.startsWith('/')) {
                ...
                return this.messaging.commands.find(command => {
                    ...
                    if (command.channelTypes) {
                        return command.channelTypes.includes(this.composer.thread.channelType);
                    }
                    return true;
                });
            }
        ...
        _getMessageData() {
            ...
            body = this._generateEmojisOnHtml(body);
            return {
                attachmentIds: this.composer.attachments.map(attachment => attachment.id),
                body,
                messageType: 'comment',
                partnerIds: this.composer.recipients.map(partner => partner.id),
            };
        }
    ...
    ComposerView.fields = {
         ...
         */
        messageViewInEditing: one2one('mail.message.view', {
    ...
    ComposerView.modelName = 'mail.composer.view';

    return ComposerView;
}

registerNewModel('mail.composer.view', factory);
