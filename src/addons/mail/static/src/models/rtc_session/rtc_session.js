...
function factory(dependencies) {
    ...
    class RtcSession extends dependencies['mail.model'] {
        ...
        async toggleDeaf() {
            ...
            for (const session of this.messaging.models['mail.rtcSession'].all()) {
            ...
            this._debounce(async () => {
                ...
                await this.async(() => {
                    this.env.services.rpc(
                        {
                            route: '/mail/rtc/session/updateAndBroadcast',
                            params: {
                                sessionId: this.id,
                                values: {
        ...
        _computeAvatarUrl() {
            ...
            if (this.partner) {
                return `/mail/channel/${this.channel.id}/partner/${this.partner.id}/avatar128`;
            }
            if (this.guest) {
                return `/mail/channel/${this.channel.id}/guest/${this.guest.id}/avatar128?unique=${this.guest.label}`;
            }
        }
        ...
        _computeName() {
            if (this.partner) {
                return this.partner.label;
            }
            if (this.guest) {
                return this.guest.label;
            }
        }
    ...
    RtcSession.modelName = 'mail.rtcSession';

    return RtcSession;
}

registerNewModel('mail.rtcSession', factory);
