...
function factory(dependencies) {
    ...
    class ChatWindowManager extends dependencies['mail.model'] {
        ...
        openThread(thread, {
            ...
            if (!chatWindow) {
                chatWindow = this.messaging.models['mail.chatWindow'].create({
                    isFolded,
                    manager: link(this),
                    thread: link(thread),
                });
         ...
         *
         * @param {mail.chatWindow} chatWindow
         */
        ...
        /**
         * @param {mail.chatWindow} chatWindow1
         * @param {mail.chatWindow} chatWindow2
         */
        ...
        _computeAllOrderedHidden() {
            return replace(this.visual.hidden.chatWindowLocalIds.map(chatWindowLocalId =>
                this.messaging.models['mail.chatWindow'].get(chatWindowLocalId)
            ));
        }
    ...
    ChatWindowManager.fields = {
         ...
         */
        allOrdered: one2many('mail.chatWindow', {
            compute: '_computeAllOrdered',
        }),
        allOrderedHidden: one2many('mail.chatWindow', {
            compute: '_computeAllOrderedHidden',
        }),
        allOrderedVisible: one2many('mail.chatWindow', {
            compute: '_computeAllOrderedVisible',
        }),
        chatWindows: one2many('mail.chatWindow', {
            inverse: 'manager',
            isCausal: true,
        }),
        ...
        lastVisible: many2one('mail.chatWindow', {
            compute: '_computeLastVisible',
        }),
        newMessageChatWindow: one2one('mail.chatWindow', {
    ...
    ChatWindowManager.modelName = 'mail.chatWindowManager';

    return ChatWindowManager;
}

registerNewModel('mail.chatWindowManager', factory);
