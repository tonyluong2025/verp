...
function factory(dependencies) {
    ...
    class Messaging extends dependencies['mail.model'] {
        ...
        _willDelete() {
            if (this.env.services['busService']) {
                this.env.services['busService'].off('windowFocus', null, this._handleGlobalWindowFocus);
            }
         ...
         */
        async start() {
            this.env.services['busService'].on('windowFocus', null, this._handleGlobalWindowFocus);
        ...
        toggleFocusedRtcSession(sessionId) {
            const rtcSession = this.messaging.models['mail.rtcSession'].findFromIdentifyingData({
                id: sessionId,
            });
    ...
    Messaging.fields = {
         ...
         * periodically fetch the imStatus of all users.
         */
        autofetchPartnerImStatus: attr({
            default: true,
        }),
        cannedResponses: one2many('mail.cannedResponse'),
        chatWindowManager: one2one('mail.chatWindowManager', {
            default: insertAndReplace(),
            isCausal: true,
            readonly: true,
        }),
        commands: one2many('mail.channelCommand'),
        companyName: attr(),
        ...
        dialogManager: one2one('mail.dialogManager', {
            default: insertAndReplace(),
            isCausal: true,
            readonly: true,
        }),
        ...
        focusedRtcSession: one2one('mail.rtcSession'),
        /**
         * Mailbox History.
         */
        ...
        initializer: one2one('mail.messagingInitializer', {
            default: insertAndReplace(),
            isCausal: true,
            readonly: true,
        }),
        ...
        messagingMenu: one2one('mail.messagingMenu', {
            default: insertAndReplace(),
            isCausal: true,
            readonly: true,
        }),
        notificationGroupManager: one2one('mail.notificationGroupManager', {
            default: insertAndReplace(),
            isCausal: true,
            readonly: true,
        }),
        notificationHandler: one2one('mail.messagingNotificationHandler', {
            default: insertAndReplace(),
            isCausal: true,
            readonly: true,
        }),
        ...
        soundEffects: one2one('mail.soundEffects', {
            default: insertAndReplace(),
            isCausal: true,
            readonly: true,
        }),
        ...
        userSetting: one2one('mail.userSetting', {
            isCausal: true,
        }),
    };
