...
QUnit.test('openChat: open new chat for user', async function (assert) {
    ...
    this.data['res.partner'].records.push({ id: 14 });
    this.data['res.users'].records.push({ id: 11, partnerId: 14 });
    await this.start();
    ...
    assert.strictEqual(chat.threadViews.length, 1, 'the chat should be displayed in a `mail.threadView`');
});
...
QUnit.test('openChat: open existing chat for user', async function (assert) {
    ...
    assert.strictEqual(existingChat.threadViews.length, 0, 'the chat should not be displayed in a `mail.threadView`');
    ...
    assert.strictEqual(existingChat.threadViews.length, 1, 'the chat should now be displayed in a `mail.threadView`');
});
