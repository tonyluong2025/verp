...
function factory(dependencies) {
    ...
    class Composer extends dependencies['mail.model'] {
        ...
        _computeMentionedPartners() {
            ...
            for (const partner of this.mentionedPartners) {
                const fromIndex = namesIndex[partner.label] !== undefined
                    ? namesIndex[partner.label] + 1 :
                    0;
                const index = this.textInputContent.indexOf(`@${partner.label}`, fromIndex);
                if (index !== -1) {
                    namesIndex[partner.label] = index;
                } else {
        ...
        _computeMentionedChannels() {
            ...
            for (const channel of this.mentionedChannels) {
                const fromIndex = namesIndex[channel.label] !== undefined
                    ? namesIndex[channel.label] + 1 :
                    0;
                const index = this.textInputContent.indexOf(`#${channel.label}`, fromIndex);
                if (index !== -1) {
                    namesIndex[channel.label] = index;
                } else {
    ...
    Composer.fields = {
        ...
        composerViews: one2many('mail.composer.view', {
            inverse: 'composer',
            isCausal: true,
        }),
        /**
        ...
        /**
         * Determines whether a postMessage request is currently pending.
         */
