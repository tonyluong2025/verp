...
function factory(dependencies) {
    ...
    class Rtc extends dependencies['mail.model'] {
         ...
         *
         * @param {mail.rtcSession[]} currentSessions list of sessions of this call.
         */
        ...
        async handleNotification(sender, content) {
            ...
            const rtcSession = this.messaging.models['mail.rtcSession'].findFromIdentifyingData({ id: sender });
        ...
        /**
         * @param {Object} param0
         * @param {string} param0.currentSessionId the Id of the 'mail.rtcSession'
        ...
        /**
         * @private
         * @param {mail.rtcSession} rtcSession
         * @param {Object} param1
        ...
        async _pingServer() {
            ...
            const { rtcSessions } = await this.env.services.rpc({
                ...
                params: {
                    ...
                    'checkRtcSessionIds': channel.rtcSessions.map(rtcSession => rtcSession.id),
                    'rtcSessionId': this.currentRtcSession.id,
                },
            }, { shadow: true });
            if (channel.exists()) {
         ...
         */
        _removePeer(token) {
            const rtcSession = this.messaging.models['mail.rtcSession'].findFromIdentifyingData({ id: token });
            if (rtcSession) {
                rtcSession.reset();
            }
            const dataChannel = this._dataChannels[token];
        ...
        async _sendPeerNotifications() {
            ...
            try {
                await this.env.services.rpc({
                    route: '/mail/rtc/session/notifyCallMembers',
                    params: {
                        'peerNotifications': peerNotifications.map(peerNotification =>
                            [
        ...
        /**
         * Updates the mail.rtcSession associated to the token with a new track.
         *
         * @private
         * @param {Track} [track]
    ...
    Rtc.fields = {
         ...
         */
        currentRtcSession: one2one('mail.rtcSession', {
            inverse: 'rtc',
        }),
        /**
        ...
        peerNotificationsToSend: one2many('mail.rtcPeerNotification', {
            isCausal: true,
        }),
        /**
