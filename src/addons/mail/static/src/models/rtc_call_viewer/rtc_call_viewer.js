...
function factory(dependencies) {
    ...
    class RtcCallViewer extends dependencies['mail.model'] {
        ...
        _computeMainParticipantCard() {
            ...
            if (this.messaging.focusedRtcSession && this.messaging.focusedRtcSession.channel === this.threadView.thread) {
                return insert({
                    relationalId: `rtcSession_${this.messaging.focusedRtcSession.localId}_${this.threadView.thread.localId}`,
        ...
        _computeTileParticipantCards() {
            ...
            for (const rtcSession of this.threadView.thread.rtcSessions) {
                ...
                tileCards.push({
                    relationalId: `rtcSession_${rtcSession.localId}_${this.threadView.thread.localId}`,
            ...
            for (const partner of this.threadView.thread.invitedPartners) {
                ...
                tileCards.push({
                    relationalId: `invitedPartner_${partner.localId}_${this.threadView.thread.localId}`,
            ...
            for (const guest of this.threadView.thread.invitedGuests) {
                ...
                tileCards.push({
                    relationalId: `invitedGuest_${guest.localId}_${this.threadView.thread.localId}`,
    ...
    RtcCallViewer.fields = {
         ...
         */
        mainParticipantCard: one2one('mail.rtcCallParticipantCard', {
         ...
         */
        rtcController: one2one('mail.rtcController', {
         ...
         */
        rtcLayoutMenu: one2one('mail.rtcLayoutMenu', {
            inverse: 'callViewer',
            isCausal: true,
        }),
        /**
         ...
         */
        tileParticipantCards: one2many('mail.rtcCallParticipantCard', {
    ...
    RtcCallViewer.modelName = 'mail.rtcCallViewer';

    return RtcCallViewer;
}

registerNewModel('mail.rtcCallViewer', factory);
