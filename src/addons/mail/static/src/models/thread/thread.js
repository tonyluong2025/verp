...
function factory(dependencies) {
    ...
    class Thread extends dependencies['mail.model'] {
        ...
        _willCreate() {
            ...
            /**
             * Last 'isTyping' status of current partner that has been notified
        ...
        async changeDescription(description) {
            ...
            return this.env.services.rpc({
                model: 'mail.channel',
                method: 'channelChangeDescription',
                args: [[this.id]],
                kwargs: { description },
            });
        }
        ...
        static convertData(data) {
            ...
            if ('channelType' in data) {
                data2.channelType = data.channelType;
                data2.model = 'mail.channel';
            }
            if ('createdUid' in data) {
                data2.creator = insert({ id: data.createdUid });
            }
            if ('customChannelName' in data) {
                data2.customChannelName = data.customChannelName;
            }
            if ('groupBasedSubscription' in data) {
                data2.groupBasedSubscription = data.groupBasedSubscription;
            }
            ...
            if ('isMinimized' in data && 'state' in data) {
                data2.serverFoldState = data.isMinimized ? data.state : 'closed';
            }
            if ('isPinned' in data) {
                data2.isServerPinned = data.isPinned;
            }
            if ('lastInterestDt' in data && data.lastInterestDt) {
                data2.lastInterestDateTime = strToDatetime(data.lastInterestDt);
            }
            if ('lastMessage' in data && data.lastMessage) {
            ...
            if ('lastMessageId' in data && data.lastMessageId) {
                const messageData = this.messaging.models['mail.message'].convertData({
            ...
            if ('messageNeedactionCounter' in data) {
                data2.messageNeedactionCounter = data.messageNeedactionCounter;
            }
            if ('messageUnreadCounter' in data) {
                data2.serverMessageUnreadCounter = data.messageUnreadCounter;
            }
            if ('label' in data) {
                data2.label = data.label;
            }
            if ('isPublic' in data) {
                data2.isPublic = data.isPublic;
            }
            if ('seenMessageId' in data) {
                data2.lastSeenByCurrentPartnerMessageId = data.seenMessageId || 0;
            }
            if ('uuid' in data) {
            ...
            if ('rtcInvitingSession' in data) {
                data2.rtcInvitingSession = insert(data.rtcInvitingSession);
            }
            if ('rtcSessions' in data) {
                data2.rtcSessions = data.rtcSessions;
            }
            if ('seenPartnersInfo' in data) {
                if (!data.seenPartnersInfo) {
                    data2.partnerSeenInfos = unlinkAll();
                } else {
                    data2.partnerSeenInfos = insertAndReplace(
                        data.seenPartnersInfo.map(
                            ({ fetchedMessageId, partnerId, seenMessageId }) => {
                                return {
                                    lastFetchedMessage: fetchedMessageId ? insert({ id: fetchedMessageId }) : unlinkAll(),
                                    lastSeenMessage: seenMessageId ? insert({ id: seenMessageId }) : unlinkAll(),
                                    partner: insertAndReplace({ id: partnerId }),
                            };
                        })
                    );
                    if (data.id || this.id) {
                        const messageIds = data.seenPartnersInfo.reduce((currentSet, { fetchedMessageId, seenMessageId }) => {
                            if (fetchedMessageId) {
                                currentSet.add(fetchedMessageId);
                            }
                            if (seenMessageId) {
                                currentSet.add(seenMessageId);
                            }
                            return currentSet;
                        }, new Set());
         ...
         *
         * @param {Object} param0
         * @param {number[]} param0.partnersTo Ids of the partners to add as channel
         * members.
         * @param {boolean|string} param0.defaultDisplayMode
         * @returns {mail.thread} The newly created group chat.
         */
        static async createGroupChat({ defaultDisplayMode, partnersTo }) {
        ...
        static getSuggestionSortFunction(searchTerm, { thread } = {}) {
            ...
            return (a, b) => {
                const isAPublic = a.model === 'mail.channel' && a.isPublic === 'public';
                const isBPublic = b.model === 'mail.channel' && b.isPublic === 'public';
        ...
        static async loadPreviews(threads) {
            ...
            const channelPreviews = await this.env.services.rpc({
                model: 'mail.channel',
                method: 'channelFetchPreview',
                args: [channelIds],
            }, { shadow: true });
            this.messaging.models['mail.message'].insert(channelPreviews.filter(p => p.lastMessage).map(
                channelPreview => this.messaging.models['mail.message'].convertData(channelPreview.lastMessage)
            ));
        }

        /**
         * Performs the `channelFold` RPC on `mail.channel`.
         *
         * @static
         * @param {string} uuid
         * @param {string} state
        ...
        /**
         * Performs the `channelInfo` RPC on `mail.channel`.
         *
         * @static
         * @param {Object} param0
        ...
        /**
         * Performs the `/mail/channel/setLastSeenMessage` RPC.
         *
         * @static
         * @param {Object} param0
        ...
        static async performRpcChannelSeen({ id, lastMessageId }) {
            return this.env.services.rpc({
                route: `/mail/channel/setLastSeenMessage`,
                params: {
                    channelId: id,
        ...
        /**
         * Performs the `channelPin` RPC on `mail.channel`.
         *
         * @static
         * @param {Object} param0
        ...
        /**
         * Performs the `channelCreate` RPC on `mail.channel`.
         *
         * @static
         * @param {Object} param0
        ...
        static async performRpcCreateChannel({ name, privacy }) {
            ...
            const data = await this.env.services.rpc({
                ...
                args: [name, privacy],
                kwargs: {
                    context: Object.assign({}, this.env.session.userContent, {
        ...
        /**
         * Performs the `channelGet` RPC on `mail.channel`.
         *
        ...
        async performRpcLeaveCall() {
            await this.async(() => this.env.services.rpc({
                route: '/mail/rtc/channel/leaveCall',
                params: { channelId: this.id },
            }, { shadow: true }));
        }

        /**
         * Performs RPC on the route `/mail/getSuggestedRecipients`.
         *
         * @static
         * @param {Object} param0
         ...
         */
        static async performRpcMailGetSuggestedRecipients({ model, resIds }) {
            const data = await this.env.services.rpc({
                route: '/mail/getSuggestedRecipients',
                params: {
                    model,
                    resIds,
                },
        ...
        static async searchChannelsToOpen({ limit, searchTerm }) {
            ...
            const fields = ['channelType', 'label'];
            const channelsData = await this.env.services.rpc({
                model: "mail.channel",
                method: "searchRead",
                kwargs: {
        ...
        static searchSuggestions(searchTerm, { thread } = {}) {
            let threads;
            if (thread && thread.model === 'mail.channel' && thread.isPublic !== 'public') {
        ...
        async follow() {
            await this.async(() => this.env.services.rpc({
                ...
                args: [[this.id]],
                kwargs: {
                    partnerIds: [this.messaging.currentPartner.id],
                },
            }));
        ...
        async _joinCall({ startWithVideo = false, videoType } = {}) {
            ...
            const { rtcSessions, iceServers, sessionId, invitedPartners, invitedGuests } = await this.async(() => this.env.services.rpc({
                route: '/mail/rtc/channel/joinCall',
                params: {
                    channelId: this.id,
                    checkRtcSessionIds: this.rtcSessions.map(rtcSession => rtcSession.id),
                },
            }, { shadow: true }));
            if (!this.exists()) {
                return;
        ...
        async join() {
            await this.env.services.rpc({
                ...
                method: 'addMembers',
                args: [[this.id]],
                kwargs: { partnerIds: [this.messaging.currentPartner.id] }
            });
        }
        ...
        async refreshActivities() {
            ...
            const [{ activityIds: newActivityIds }] = await this.async(() => this.env.services.rpc({
                model: this.model,
                method: 'read',
                args: [this.id, ['activityIds']]
            }, { shadow: true }));
        ...
        async setCustomName(newName) {
            return this.env.services.rpc({
                model: 'mail.channel',
                method: 'channelSetCustomName',
                args: [this.id],
                kwargs: { label: newName },
            });
        }
         ...
         */
        _computeDisplayName() {
            if (this.channelType === 'chat' && this.correspondent) {
                return this.customChannelName || this.correspondent.nameOrDisplayName;
            }
            if (this.channelType === 'group' && !this.label) {
                const partnerNames = this.members.map(partner => partner.nameOrDisplayName);
                const guestNames = this.guestMembers.map(guest => guest.label);
        ...
        _computeHasCallFeature() {
            return ['channel', 'chat', 'group'].includes(this.channelType);
        }
        ...
        _computeHasSeenIndicators() {
            ...
            return ['chat', 'livechat'].includes(this.channelType);
        }
        ...
        */
        _computeIsChannelDescriptionChangeable() {
            return this.model === 'mail.channel' && ['channel', 'group'].includes(this.channelType);
        }
        ...
        _computeIsChannelRenamable() {
            return (
                ...
                ['chat', 'channel', 'group'].includes(this.channelType)
            );
        }
        ...
        _computeInvitationLink() {
            if (!this.uuid || !this.channelType || this.channelType === 'chat') {
                return clear();
            }
        ...
        _computeIsChatChannel() {
            return this.channelType === 'chat' || this.channelType === 'group';
        }
        ...
        _computeUrl() {
            ...
            if (this.model === 'mail.channel') {
                return `${baseHref}#action=mail.actionDiscuss&activeId=${this.model}_${this.id}`;
            }
        ...
        _promptAddFollower() {
            const action = {
                ...
                viewMode: 'form',
                views: [[false, 'form']],
                label: this.env._t("Invite Follower"),
                target: 'new',
                context: {
        ...
        async onClickLoadMoreMembers() {
            const members = await this.env.services.rpc({
                model: 'mail.channel',
                method: 'loadMoreMembers',
                args: [[this.id]],
                kwargs: {
                    'knownMemberIds': this.members.map(partner => partner.id),
                },
            });
            this.update({ members });
        }
    ...
    Thread.fields = {
         ...
         */
        avatarCacheKey: attr(),
        cache: one2one('mail.threadCache', {
         ...
         * either no value (to display the chat), or 'videoFullScreen' to
         ...
         */
        discussSidebarCategoryItem: one2one('mail.discussSidebarCategoryItem', {
        ...
        messagingMenuAsPinnedAndUnreadChannel: many2one('mail.messagingMenu', {
         ...
         */
        rtcInvitingSession: many2one('mail.rtcSession', {
        ...
        /**
         * Determines the `mail.suggestedRecipientInfo` concerning `this`.
         */
        suggestedRecipientInfoList: one2many('mail.suggestedRecipientInfo', {
            inverse: 'thread',
            isCausal: true,
        }),
        /**
        ...
        threadViews: one2many('mail.threadView', {
            inverse: 'thread',
        }),
        /**
