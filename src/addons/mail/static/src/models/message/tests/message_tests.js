...
QUnit.test('create', async function (assert) {
    ...
    const message = this.messaging.models['mail.message'].create({
        ...
        author: insert({ id: 5, displayName: "Demo" }),
        body: "<p>Test</p>",
        date: moment(strToDatetime("2019-05-05 10:00:00")),
        id: 4000,
        isNeedaction: true,
        isStarred: true,
    ...
    // from partnerId being in needactionPartnerIds
    assert.ok(message.threads.includes(this.messaging.inbox));
    // from partnerId being in starredPartnerIds
    ...
    assert.strictEqual(attachment.label, "test.txt");
    ...
    assert.strictEqual(channel.label, "General");
