...
import { dateToStr } from 'web.time';
...
QUnit.test('activity info layout when planned after tomorrow', async function (assert) {
    ...
    const activity = this.messaging.models['mail.activity'].create({
        dateDeadline: dateToStr(fiveDaysFromNow),
        id: 12,
        state: 'planned',
        thread: insert({ id: 42, model: 'res.partner' }),
    });
...
QUnit.test('activity info layout when planned tomorrow', async function (assert) {
    ...
    const activity = this.messaging.models['mail.activity'].create({
        dateDeadline: dateToStr(tomorrow),
        id: 12,
        state: 'planned',
        thread: insert({ id: 42, model: 'res.partner' }),
    });
...
QUnit.test('activity info layout when planned yesterday', async function (assert) {
    ...
    const activity = this.messaging.models['mail.activity'].create({
        dateDeadline: dateToStr(yesterday),
        id: 12,
        state: 'overdue',
        thread: insert({ id: 42, model: 'res.partner' }),
    });
...
QUnit.test('activity info layout when planned before yesterday', async function (assert) {
    ...
    const activity = this.messaging.models['mail.activity'].create({
        dateDeadline: dateToStr(fiveDaysBeforeNow),
        id: 12,
        state: 'overdue',
        thread: insert({ id: 42, model: 'res.partner' }),
    });
...
QUnit.test('activity details toggle', async function (assert) {
    ...
    tomorrow.setDate(today.getDate() + 1);
    const activity = this.messaging.models['mail.activity'].create({
        creator: insert({ id: 1, displayName: "Admin" }),
        dateCreate: dateToStr(today),
        dateDeadline: dateToStr(tomorrow),
        id: 12,
        state: 'planned',
        thread: insert({ id: 42, model: 'res.partner' }),
...
QUnit.test('activity details layout', async function (assert) {
    ...
    tomorrow.setDate(today.getDate() + 1);
    const activity = this.messaging.models['mail.activity'].create({
        assignee: insert({ id: 10, displayName: "Pauvre pomme" }),
        creator: insert({ id: 1, displayName: "Admin" }),
        dateCreate: dateToStr(today),
        dateDeadline: dateToStr(tomorrow),
        id: 12,
...
QUnit.test('activity with mail template: preview mail', async function (assert) {
    ...
    bus.on('do-action', null, payload => {
        ...
        assert.strictEqual(
            payload.action.context.default_resId,
            42,
        ...
        assert.ok(
            payload.action.context.default_useTemplate,
            'Action should have true as default useTemplate in context'
        );
        assert.strictEqual(
            payload.action.context.default_templateId,
            1,
        ...
        assert.strictEqual(
            ...
            'Action should be of type "ir.actions.actwindow"'
        );
        assert.strictEqual(
            payload.action.resModel,
            "mail.compose.message",
            'Action should have "mail.compose.message" as resModel'
        );
    });
...
QUnit.test('activity with mail template: send mail', async function (assert) {
    ...
    assert.verifySteps(
        ['activitySendMail'],
        "should have called activitySendMail rpc"
    );
});
...
QUnit.test('activity click on edit', async function (assert) {
    ...
    bus.on('do-action', null, payload => {
        ...
        assert.strictEqual(
            payload.action.context.default_resModel,
            'res.partner',
        ...
        assert.strictEqual(
            payload.action.resModel,
            "mail.activity",
            'Action should have "mail.activity" as resModel'
        );
        assert.strictEqual(
            payload.action.resId,
            12,
            'Action should have activity id as resId'
        );
    });
...
QUnit.test('activity click on cancel', async function (assert) {
    ...
    await this.start({
        async mockRPC(route, args) {
            if (route === '/web/dataset/callKw/mail.activity/unlink') {
                assert.step('unlink');
                assert.strictEqual(args.args[0].length, 1);
