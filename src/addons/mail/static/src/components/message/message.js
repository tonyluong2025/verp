...
import { format } from 'web.fieldUtils';
...
export class Message extends Component {
    ...
    setup() {
        super.setup();
        useComponentToModel({ fieldName: 'component', modelName: 'mail.message.view', propNameAsRecordLocalId: 'messageViewLocalId' });
    }
    ...
    get avatar() {
        if (this.messageView.message.author && (!this.messageView.message.originThread || this.messageView.message.originThread.model !== 'mail.channel')) {
            ...
            return `/mail/channel/${this.messageView.message.originThread.id}/partner/${this.messageView.message.author.id}/avatar128`;
            ...
            return `/mail/channel/${this.messageView.message.originThread.id}/guest/${this.messageView.message.guestAuthor.id}/avatar128?unique=${this.messageView.message.guestAuthor.label}`;
        } else if (this.messageView.message.messageType === 'email') {
    ...
    /**
     * @returns {mail.message.view}
     */
    get messageView() {
        return this.messaging && this.messaging.models['mail.message.view'].get(this.props.messageViewLocalId);
    }
    /**
     * @returns {string}
     */
     ...
     */
    get trackingValues() {
        return this.messageView.message.trackingValueIds.map(trackingValue => {
            const value = Object.assign({}, trackingValue);
            value.changedField = _.str.sprintf(this.env._t("%s:"), value.changedField);
            /**
             ...
             * tracking in Javascript. Also see `create_tracking_values` in Javascript.
             */
            switch (value.fieldType) {
                case 'boolean':
                    ...
                    break;
                /**
                 * many2one formatter exists but is expecting id/nameGet or data
                ...
                case 'monetary':
                    value.old_value = format.monetary(value.old_value, undefined, {
                        currency: value.currencyId
                            ? this.env.session.currencies[value.currencyId]
                            : undefined,
                        forceString: true,
                    });
