...
import { file, dom } from 'web.testUtils';
...
QUnit.test('open chat from "new message" chat window should open chat in place of this "new message" chat window', async function (assert) {
    ...
    this.data['res.partner'].records.push({ id: 131, label: "Partner 131" });
    this.data['res.users'].records.push({ partnerId: 131 });
...
QUnit.test('new message chat window should close on selecting the user if chat with the user is already open', async function (assert) {
    ...
    this.data['res.partner'].records.push({ id: 131, label: "Partner 131" });
    this.data['res.users'].records.push({ id: 12, partnerId: 131 });
    this.data['mail.channel'].records.push({
        channelType: "chat",
        id: 20,
        isMinimized: true,
...
QUnit.test('chat window: basic rendering', async function (assert) {
    ...
    this.data['mail.channel'].records.push({ id: 20, label: "General" });
    const { createMessagingMenuComponent } = await this.start();
    await createMessagingMenuComponent();
...
QUnit.test("Mobile: chat window shouldn't open automatically after receiving a new message", async function (assert) {
    ...
    this.data['res.partner'].records.push({ id: 10, label: "Demo" });
...
QUnit.test('chat window: close on ESCAPE', async function (assert) {
    ...
    this.data['res.partner'].records.push({ label: "TestPartner" });
    // a chat window with thread is expected to be initially open for this test
    this.data['mail.channel'].records.push({ isMinimized: true });
    await this.start({
        mockRPC(route, args) {
            if (args.method === 'channelFold') {
                assert.step(`rpc:channelFold/${args.kwargs.state}`);
            }
            return this._super(...arguments);
        },
    });
    ...
    assert.verifySteps(['rpc:channelFold/closed']);
});
...
QUnit.test('chat window: composer state conservation on toggle discuss', async function (assert) {
    ...
    const files = [
        await createFile({
            label: 'text state conservation on toggle home menu.txt',
            content: 'hello, world',
            contentType: 'text/plain',
        }),
        await createFile({
            label: 'text2 state conservation on toggle home menu.txt',
...
QUnit.test('open 2 folded chat windows: check shift operations are available', async function (assert) {
    ...
    this.data['res.partner'].records.push({ id: 7, label: "Demo" });
    const channel = {
        channelType: "channel",
        isMinimized: true,
        isPinned: true,
    ...
    await this.start({
        ...
        mockRPC(route, args) {
            if (route === '/mail/channel/messages') {
                ...
                assert.step(`rpc:/mail/channel/messages:${channelId}`);
            }
            return this._super(...arguments);
        },
    });
