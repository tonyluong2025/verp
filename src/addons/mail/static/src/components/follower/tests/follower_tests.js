...
QUnit.test('click on partner follower details', async function (assert) {
    ...
    bus.on('do-action', null, payload => {
        ...
        assert.strictEqual(
            ...
            "The redirect action should be of type 'ir.actions.actwindow'"
        );
        openFormDef.resolve();
    });
...
QUnit.test('click on edit follower', async function (assert) {
    ...
    this.data['res.partner'].records.push({ id: 100, messageFollowerIds: [2] });
    this.data['mail.followers'].records.push({
        id: 2,
        is_active: true,
        is_editable: true,
        partnerId: this.data.currentPartnerId,
        resId: 100,
