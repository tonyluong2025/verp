...
QUnit.test('click on "add followers" button', async function (assert) {
    ...
    bus.on('do-action', null, payload => {
        ...
        assert.strictEqual(
            ...
            "The 'add followers' action should be of type 'ir.actions.actwindow'"
        );
        const partner = this.data['res.partner'].records.find(
            partner => partner.id === payload.action.context.default_resId
        );
        partner.messageFollowerIds.push(1);
