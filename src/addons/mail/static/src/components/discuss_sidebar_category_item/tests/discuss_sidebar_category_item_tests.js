...
QUnit.test('channel - avatar: should have correct avatar', async function (assert) {
    ...
    assert.strictEqual(
        ...
        '/web/image/mail.channel/20/avatar128?unique=100111',
...
QUnit.test('channel - avatar: should update avatar url from bus', async function (assert) {
    ...
    assert.strictEqual(
        ...
        '/web/image/mail.channel/20/avatar128?unique=101010',
    );
    ...
    await afterNextRender(() => {
        this.env.services.rpc({
            model: 'mail.channel',
            method: 'write',
            args: [[20], { image128: 'This field does not matter' }],
        });
    });
    ...
    assert.strictEqual(
        ...
        `/web/image/mail.channel/20/avatar128?unique=${newCacheKey}`,
    );
});
...
QUnit.test('chat - avatar: should have correct avatar', async function (assert) {
    ...
    this.data['res.partner'].records.push({ id: 15, name: "Demo", imStatus: 'offline' });
    this.data['mail.channel'].records.push({
        channelType: 'chat',
        id: 10,
    ...
    assert.strictEqual(
        ...
        '/web/image/res.partner/15/avatar128',
...
QUnit.test('chat - sorting: should be sorted by last activity time', async function (assert) {
    ...
    this.data['mail.channel'].records.push({
        ...
        id: 10,
        isPublic: 'private',
        last_interest_dt: datetimeToStr(new Date(2021, 0, 1)), // less recent one
    }, {
        channelType: 'chat',
        id: 20,
        isPublic: 'private',
        last_interest_dt: datetimeToStr(new Date(2021, 0, 2)), // more recent one
    });
    await this.start();
