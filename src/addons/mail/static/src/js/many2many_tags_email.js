...
BasicModel.include({
    ...
    _setInvalidMany2ManyTagsEmail: function (record, fieldName) {
        ...
        return Promise.resolve(def).then(function () {
            return {
                invalidPartnerIds: _.pluck(invalidPartnerIds, 'resId'),
            };
        });
    },
});
...
var FieldMany2ManyTagsEmail = M2MTags.extend({
    tagTemplate: "FieldMany2ManyTagsEmail",
    ...
    _checkEmailPopup: function () {
        ...
        _.each(this.record.specialData[this.name].invalidPartnerIds, function (resID) {
            var def = new Promise(function (resolve, reject) {
                var pop = new formCommon.FormViewDialog(self, {
                    ...
                    onSaved: function (record) {
                        if (record.data.email) {
                            validPartners.push(record.resId);
                        }
                    },
                }).open();
...
fieldRegistry.add('many2manyTagsEmail', FieldMany2ManyTagsEmail);
