...
var FieldEmojiCommon = {
    ...
    _onKeydown: function () {
        this._super.apply(this, arguments);
        if (this.nodeOptions.onchangeOnKeydown) {
     ...
     *
     * Should be used inside 'onAttachCallback' because we need the element to be attached to the form first.
    ...
    _attachEmojisDropdown: function () {
        if (!this.$emojisIcon) {
            ...
            this.$emojisIcon.find('.o-mail-emoji').on('click', this._onEmojiClick.bind(this));

            if (this.$el.filter('span.o-field-translate').length) {
                // multi-languages activated, place the button on the left of the translation button
                this.$emojisIcon.addClass('o-mail-emojis-dropdown-translation');
            }
            if (this.$el.filter('textarea').length) {
                this.$emojisIcon.addClass('o-mail-emojis-dropdown-textarea');
            }
