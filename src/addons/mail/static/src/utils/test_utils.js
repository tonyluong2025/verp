    ...
    return Object.assign({}, callbacks, {
        ...
        mount: prevMount.concat(async params => {
            ...
            if (state.autoOpenDiscuss) {
                await discussWidget.onAttachCallback();
            }
        }),
...
function beforeEach(self) {
    ...
    data['res.partner'].records.push({
        displayName: "Your Company, Mitchell Admin",
        id: data.currentPartnerId,
        name: "Mitchell Admin",
        userIds: [data.currentUserId],
    });
    data['res.users'].records.push({
        displayName: "Your Company, Mitchell Admin",
        id: data.currentUserId,
        name: "Mitchell Admin",
        partnerId: data.currentPartnerId,
    });
    ...
    data['res.users'].records.push({
        ...
        name: "Public user",
        partnerId: data.publicPartnerId,
    });
...
function getCreateComposerComponent({ components, env, modelManager, widget }) {
    return async function createComposerComponent(composer, props) {
        const composerView = modelManager.messaging.models['mail.composer.view'].create({
...
function getCreateMessageComponent({ components, env, modelManager, widget }) {
    return async function createMessageComponent(message) {
        const messageView = modelManager.messaging.models['mail.message.view'].create({
 ...
 * @param {integer} [param0.resId] makes only sense when `param0.hasView` is set:
 *   the resId to use in createView.
 ...
 *   not initialized, test should mock RPC `mail/initMessaging` and block its
 *   resolution.
...
async function start(param0 = {}) {
    ...
    const services = Object.assign({}, {
        ...
        localStorage: AbstractStorageService.extend({ storage: new RamStorage() }),
    ...
    if (hasView) {
        ...
        const Parent = Widget.extend({ doPushState() {} });
