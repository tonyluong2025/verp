...
QUnit.test('basic chatter rendering', async function (assert) {
    ...
    this.data['res.partner'].records.push({ displayName: "second partner", id: 12, });
    await this.createView({
        data: this.data,
        hasView: true,
        // View params
...
QUnit.test('basic chatter rendering without followers', async function (assert) {
    ...
    assert.containsNone(
        document.body,
        '.o_FollowerListMenu',
        "there should be no followers menu because the 'messageFollowerIds' field is not present in 'oe-chatter'"
    );
    assert.containsOnce(
        document.body,
        '.o_Chatter_thread',
...
QUnit.test('basic chatter rendering without activities', async function (assert) {
    ...
    assert.containsNone(
        document.body,
        '.o_ChatterTopbar_buttonScheduleActivity',
        "there should be no schedule activity button because the 'activityIds' field is not present in 'oe-chatter'"
    );
    assert.containsOnce(
        document.body,
        '.o_FollowerListMenu',
...
QUnit.test('basic chatter rendering without messages', async function (assert) {
    ...
    assert.containsNone(
        document.body,
        '.o_Chatter_thread',
        "there should be no thread because the 'messageIds' field is not present in 'oe-chatter'"
    );
});
...
QUnit.test('chatter updating', async function (assert) {
    ...
    this.data['mail.message'].records.push({ body: "not empty", model: 'res.partner', resId: 12 });
    this.data['res.partner'].records.push(
        { displayName: "first partner", id: 11 },
        { displayName: "second partner", id: 12 }
    );
    await this.createView({
        data: this.data,
        hasView: true,
    ...
    await afterNextRender(() => this.afterEvent({
        eventName: 'o-thread-view-hint-processed',
        func: () => document.querySelector('.o-pager-next').click(),
...
QUnit.test('chatter should become enabled when creation done', async function (assert) {
    ...
    document.querySelectorAll('.o-field-char')[0].focus();
    document.execCommand('insertText', false, "hello");
    await afterNextRender(() => {
        document.querySelector('.o-form-button-save').click();
    });
    assert.notOk(
        document.querySelector(`.o_ChatterTopbar_buttonSendMessage`).disabled,
...
QUnit.test('read more/less links are not duplicated when switching from read to edit mode', async function (assert) {
    ...
    await afterNextRender(() => this.afterEvent({
        eventName: 'o-component-message-read-more-less-inserted',
        func: () => document.querySelector('.o-form-button_edit').click(),
    ...
    await afterNextRender(() => this.afterEvent({
        eventName: 'o-component-message-read-more-less-inserted',
        func: () => document.querySelector('.o-form-button_cancel').click(),
...
QUnit.test('Form view not scrolled when switching record', async function (assert) {
    ...
    const controllerContentEl = document.querySelector('.o-content');
    ...
    await afterNextRender(() =>
        document.querySelector('.o-pager-next').click()
    );
    assert.strictEqual(
        document.querySelector('.breadcrumb-item.active').textContent,
        'Partner 2',
    ...
    await afterNextRender(() =>
        document.querySelector('.o-pager-previous').click()
    );
    assert.strictEqual(controllerContentEl.scrollTop, 0,
...
QUnit.test('Attachments that have been unlinked from server should be visually unlinked from record', async function (assert) {
    ...
    this.data['res.partner'].records.push(
        { displayName: "Partner1", id: 11 },
        { displayName: "Partner2", id: 12 }
    );
    ...
    this.data['ir.attachment'].records.find(a => a.id === 11).resId = 0;
    await afterNextRender(() =>
        document.querySelector('.o-pager-previous').click()
    );
    assert.strictEqual(
...
QUnit.test('chatter just contains "creating a new record" message during the creation of a new record after having displayed a chatter for an existing record', async function (assert) {
    ...
    await afterNextRender(() => {
        document.querySelector('.o-form-button_create').click();
    });
    assert.containsOnce(
        document.body,
        '.o_Message',
