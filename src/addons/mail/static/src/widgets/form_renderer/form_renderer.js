 ...
 * subset of) the mail widgets (mailThread, mailFollowers and mailActivity).
 */
FormRenderer.include({
    ...
    init(parent, state, params) {
         ...
         * This is set when arch contains `div.oe-chatter`.
         */
    ...
    destroy() {
        this._super(...arguments);
        this._chatterContainerComponent = undefined;
        this.off('o-attachments-changed', this);
        this.off('o-chatter-rendered', this);
        this.off('o-message-posted', this);
     ...
     * This is based on arch, which should have `div.oe-chatter`.
     *
     * @private
     * @returns {boolean}
     */
    ...
    _makeChatterContainerComponent() {
        ...
        // Not in customEvents because other modules may remove this listener
        // while attempting to extend them.
        this.on('o-chatter-rendered', this, ev => this._onChatterRendered(ev));
        if (this.chatterFields.hasRecordReloadOnMessagePosted) {
            this.on('o-message-posted', this, ev => {
                this.triggerUp('reload', { keepChanges: true });
            });
        }
        if (this.chatterFields.hasRecordReloadOnAttachmentsChanged) {
            this.on('o-attachments-changed', this, ev => this.triggerUp('reload', { keepChanges: true }));
        }
        if (this.chatterFields.hasRecordReloadOnFollowersUpdate) {
            owl.Component.env.bus.on('mail.thread:promptAddFollower-closed', this, ev => this.triggerUp('reload', { keepChanges: true }));
        }
    },
    /**
     * @private
    ...
    _makeChatterContainerTarget() {
        const $el = $('<div class="o-form-renderer-chatter-container"/>');
     ...
     */
    _renderNode(node) {
        if (node.tag === 'div' && node.attrs.class === 'oe-chatter') {
