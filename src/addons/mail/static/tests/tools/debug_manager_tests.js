...
QUnit.test("Manage Messages", async function (assert) {
    ...
    async function mockRPC(route, args) {
        ...
        if (route === "/web/dataset/searchRead" && args.model === "mail.message") {
            assert.strictEqual(args.context.default_resId, 5);
            assert.strictEqual(args.context.default_resModel, "partner");
            assert.deepEqual(args.domain, ["&", ["resId", "=", 5], ["model", "=", "partner"]]);
        }
    }
    ...
    await click(wc.el, ".o-debug-manager .dropdown-toggle");

    const dropdownItems = wc.el.querySelectorAll(
        ".o-debug-manager .dropdown-menu .dropdown-item"
    );
