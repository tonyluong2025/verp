...
QUnit.test('activity menu widget: menu with no records', async function (assert) {
    ...
    const { widget } = await start({
        ...
        mockRPC: function (route, args) {
            if (args.method === 'systrayGetActivities') {
...
QUnit.test('activity menu widget: activity menu with 3 records', async function (assert) {
    ...
    assert.ok(activityMenu.$('.o-mail-preview').length);
    assert.containsOnce(activityMenu.$el, '.o-notification-counter', "widget should have notification counter");
    assert.strictEqual(parseInt(activityMenu.el.innerText), 8, "widget should have 8 notification counter");

    var context = {};
    testUtils.mock.intercept(activityMenu, 'doAction', function (event) {
    ...
    await testUtils.dom.click(activityMenu.$(".o-activity-filter-button[data-modelName='Issue'][data-filter='overdue']"));
    ...
    await testUtils.dom.click(activityMenu.$('.dropdown-toggle'));
    await testUtils.dom.click(activityMenu.$(".o-activity-filter-button[data-modelName='Issue'][data-filter='today']"));
    ...
    await testUtils.dom.click(activityMenu.$('.dropdown-toggle'));
    await testUtils.dom.click(activityMenu.$(".o-activity-filter-button[data-modelName='Issue'][data-filter='upcoming_all']"));
    ...
    await testUtils.dom.click(activityMenu.$('.dropdown-toggle'));
    await testUtils.dom.click(activityMenu.$(".o_mail_systray_dropdown_items > div[data-modelName='Issue']"));
...
QUnit.test('activity menu widget: activity view icon', async function (assert) {
    ...
    await testUtils.dom.click(activityMenu.$(".o_mail_activity_action[data-modelName='Issue']"));
    assert.doesNotHaveClass(activityMenu.$('.dropdown-menu'), 'show',
        "dropdown should be collapsed");

    // click on the "Note" activity icon
    await testUtils.dom.click(activityMenu.$('.dropdown-toggle'));
    await testUtils.dom.click(activityMenu.$(".o_mail_activity_action[data-modelName='Note']"));

    assert.verifySteps([
        'doAction:Issue',
        'doAction:mail.mail_activity_type_view_tree'
    ]);
