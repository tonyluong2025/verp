...
QUnit.module('document_viewer_tests.js', {
    ...
    QUnit.test('basic rendering', async function (assert) {
        ...
        assert.containsOnce(viewer, '.o-viewer-content',
            "there should be a preview");
        assert.containsOnce(viewer, '.o-close-btn',
            "there should be a close button");
        assert.containsOnce(viewer, '.o-viewer-header',
            "there should be a header");
        assert.containsOnce(viewer, '.o-image-caption',
            "there should be an image caption");
        assert.containsOnce(viewer, '.o-viewer-zoomer',
    ...
    QUnit.test('Document Viewer Youtube', async function (assert) {
        ...
        assert.strictEqual(viewer.$(".o-image-caption:contains('urlYoutube')").length, 1,
            "the viewer should be on the right attachment");
        assert.containsOnce(viewer, '.o-viewer-text[data-src="' + youtubeURL + '"]',
    ...
    QUnit.test('Document Viewer html/(txt)', async function (assert) {
        ...
        assert.strictEqual(viewer.$(".o-image-caption:contains('text.html')").length, 1,
    ...
    QUnit.test('Document Viewer mp4', async function (assert) {
        ...
        assert.strictEqual(viewer.$(".o-image-caption:contains('video.mp4')").length, 1,
            "the viewer be on the right attachment");
        assert.containsOnce(viewer, '.o-viewer-video',
            "there should be a video player");
    ...
    QUnit.test('Document Viewer jpg', async function (assert) {
        ...
        assert.strictEqual(viewer.$(".o-image-caption:contains('image.jpg')").length, 1,
            "the viewer be on the right attachment");
    ...
    QUnit.test('is closable by button', async function (assert) {
        ...
        await testUtils.dom.click(viewer.$('.o-close-btn'));
    ...
    QUnit.test('is closable by clicking on the wrapper', async function (assert) {
        ...
        assert.containsOnce(viewer, '.o-viewer-content',
            "should have a document viewer");
        assert.containsOnce(viewer, '.o-viewer-img-wrapper',
            "should have a wrapper");

        await testUtils.dom.click(viewer.$('.o-viewer-img-wrapper'));
