verp.define('board.AddToBoardMenu', function (require) {
    "use strict";
    ...
    const { useAutofocus } = require('web.customHooks');
    ...
    class AddToBoardMenu extends Component {
         ...
         * is supposed to call the route /board/addToDashboard with proper
         * information.
         * @private
         */
        async addToBoard() {
            ...
            for (const key in evalutatedContext) {
                if (evalutatedContext.hasOwnProperty(key) && /^searchDefault_/.test(key)) {
                    delete evalutatedContext[key];
                }
            }
            evalutatedContext.dashboardMergeDomainsContexts = false;
            ...
            const result = await this.rpc({
                route: '/board/addToDashboard',
                params: {
                    actionId: this.env.action.id || false,
                    contextToSave: evalutatedContext,
                    domain: domain,
                    viewMode: this.env.view.type,
