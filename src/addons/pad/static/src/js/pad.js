verp.define('pad.pad', function (require) {
"use strict";
...
var FieldPad = AbstractField.extend({
    ...
    events: _.extend({}, AbstractField.prototype.events, {
        'click .oe-pad-switch': '_onToggleFullScreen',
    }),
    ...
    start: function () {
        if (!this.isPadConfigured) {
            this.$(".oe-unconfigured").removeClass('d-none');
            this.$(".oe-configured").addClass('d-none');
    ...
    _renderReadonly: function () {
        if (_.str.startsWith(this.value, 'http')) {
            var self = this;
            this.$('.oe-pad-content')
                .addClass('oe-pad-loading')
                .text(_t("Loading"));
            this._rpc({
                method: 'padGetContent',
                model: this.model,
                args: [this.value]
            }, {
                shadow: true
                ...
                self.$('.oe-pad-content')
                    .removeClass('oe-pad-loading')
                    .html('<div class="oe-pad-readonly"><div>');
                self.$('.oe-pad-readonly').html(data);
                _.each(self.$('a'), (link) => {
                ...
                self.$('.oe-pad-content').text(_t('Unable to load pad'));
            });
        } else {
            this.$('.oe-pad-content')
                .addClass('oe-pad-loading')
    ...
    _onToggleFullScreen: function () {
        this.$el.toggleClass('oe-pad-fullscreen mb0');
        this.$('.oe-pad-switch').toggleClass('fa-expand fa-compress');
        this.$el.parents('.o-touch-device').toggleClass('o-scroll-hidden');
    },
});
