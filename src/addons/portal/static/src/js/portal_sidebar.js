verp.define('portal.PortalSidebar', function (require) {
'use strict';
...
var PortalSidebar = publicWidget.Widget.extend({
     ...
     * like : <span class="o-portal-sidebar-timeago" t-att-datetime="invoice.dateDue"/>
     *
     * @private
     */
    _setDelayLabel: function () {
        var $sidebarTimeago = this.$el.find('.o-portal-sidebar-timeago');
        _.each($sidebarTimeago, function (el) {
            var dateTime = moment(time.autoStrToDate($(el).attr('datetime'))),
                today = moment().startOf('day'),
                diff = dateTime.diff(today, 'days', true),
                displayStr;

            session.isBound.then(function (){
                if (diff === 0) {
    ...
    _printIframeContent: function (href) {
        ...
        if (!this.printContent) {
            this.printContent = $('<iframe id="printIframeContent" src="' + href + '" style="display:none"></iframe>');
