import { VerpEditor } from '../src/VerpEditor.js';

const localStorageKey = 'verp-editor-snippets';

function startEditor(html) {
    const editableContainer = document.getElementById('dom');
    editableContainer.innerHTML = html;
    new VerpEditor(editableContainer, {
    ...
    document.querySelector('#savedHtmlList').remove();
}
...
function renderSnippets() {
    const snippetContainer = document.querySelector('#savedHtmlList ul');
    const localHtmlStored = localStorage.getItem(localStorageKey);
    if (localHtmlStored) {
...
document.querySelector('#saveCHtmlButton').addEventListener('click', () => {
