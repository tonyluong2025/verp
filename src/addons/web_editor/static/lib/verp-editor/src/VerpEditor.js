...
export class VerpEditor extends EventTarget {
    constructor(editable, options = {}) {
        ...
        this.editable.classList.add("verp-editor-editable");
            ...
            for (const link of links) {
                if (
                    ...
                    if (link === linkInSelection) {
                        ...
                        link.classList.add('o-link-in-selection');
    ...
    _resetLinkZws(element = this.editable) {
        ...
        element.querySelectorAll('.o-link-in-selection').forEach(link => link.classList.remove('o-link-in-selection'));
    }
    _activateContenteditable() {
        ...
        for (const node of editableAreas) {
            if (!node.isContentEditable) {
                if (isVoidElement(node) || node.nodeName === 'IMG') {
                    node.classList.add('o-editable-media');
                } else {
        ...
        for (const element of this.options.getUnremovableElements()) {
            element.classList.add("oe-unremovable");
        }
    ...
    isSelectionInBlockRoot() {
        ...
        while (
            !currentNode.classList.contains('o-editable') &&
            !currentNode.classList.contains('verp-editor-editable') &&
