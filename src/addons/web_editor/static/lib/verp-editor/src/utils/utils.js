...
export function closestElement(node, selector, restrictToEditable=false) {
    ...
    return restrictToEditable && element && element.querySelector('.verp-editor-editable') ? null : element;
}
...
export function isUnbreakable(node) {
    ...
    return (
        ...
        node.classList.contains('oe-unbreakable')
    );
}
...
export function isUnremovable(node) {
    return (
        ...
        node.oid === 'root' ||
        (node.nodeType === Node.ELEMENT_NODE &&
            (node.classList.contains('o-editable') || node.getAttribute('t-set') || node.getAttribute('t-call'))) ||
        (node.classList && node.classList.contains('oe-unremovable')) ||
...
export function isMediaElement(node) {
    return (
        isFontAwesome(node) ||
        (node.classList &&
            (node.classList.contains('o-image') || node.classList.contains('media-iframe-video')))
    );
}
...
export function getListMode(pnode) {
    ...
    return pnode.classList.contains('o-checklist') ? 'CL' : 'UL';
}

export function createList(mode) {
    const node = document.createElement(mode == 'OL' ? 'OL' : 'UL');
    if (mode == 'CL') {
        node.classList.add('o-checklist');
    }
    return node;
}
...
export function prepareUpdate(...args) {
    ...
    while (positions.length) {
        ...
        if (options.debug) {
            const editable = el && closestElement(el, '.verp-editor-editable');
...
export function restoreState(prevStateData, debug=false) {
    ...
    if (debug) {
        const editable = closestElement(node, '.verp-editor-editable');
        console.log(
