verp.define('web_editor.field_html_tests', function (require) {
"use strict";
...
var weTestUtils = require('web_editor.testUtils');
            ...
            this.data = weTestUtils.wysiwygData({
                'note.note': {
                    ...
                    records: [{
                        ...
                        id: 6,
                        displayName: "sixth record",
                        header: "<p>Hello World</p>",
                        body: `
<div class="o-form-sheet-bg">
  <div class="clearfix position-relative o-form-sheet" style="width: 1140px;">
    <div class="o-notebook">
      <div class="tab-content">
        <div class="tab-pane active" id="notebook_page_820">
          <div class="oe-form-field oe-form-field-html o-field-widget" name="description" style="margin-bottom: 5px;">
            hacky code to test
          </div>
        </div>
      </div>
...
<p class="b o_not_editable">
    b
            ...
            testUtils.mock.patch(ajax, {
                loadAsset: function (xmlid) {
                    ...
                    if (xmlid === 'template.assetsAllStyle') {
                        return Promise.resolve({
        ...
        QUnit.test('simple rendering', async function (assert) {
            ...
            var $field = form.$('.oe-form-field[name="body"]');
            assert.strictEqual($field.children('.o-readonly').html(),
            ...
            await promise;
            $field = form.$('.oe-form-field[name="body"]');
        ...
        QUnit.test('check if required field is set', async function (assert) {
            ...
            await testUtils.form.clickEdit(form);
            await promise;
            await testUtils.dom.click(form.$('.o-form-button-save'));
        ...
        QUnit.test('colorpicker', async function (assert) {
            ...
            await testUtils.dom.click($('#toolbar .note-back-color-preview .o-we-color-btn[style="background-color:#00FFFF;"]'));
            ...
            await openColorpicker('#toolbar .note-back-color-preview');
            await testUtils.dom.click($('#toolbar .note-back-color-preview .o-we-color-btn.bg-o-color-3'));
    ...
    QUnit.test('media dialog: upload', async function (assert) {
            ...
            const form = await testUtils.createView({
                ...
                mockRPC: function (route, args) {
                    if (args.model === 'ir.attachment') {
                        if (args.method === "generateAccessToken") {
                            return Promise.resolve();
                        }
                    }
                    ...
                    if (route.indexOf('/web_editor/attachment/add_data') === 0) {
                        return Promise.resolve({"id": 5, "label": "test.jpg", "description": false, "mimetype": "image/jpeg", "checksum": "7951a43bbfb08fd742224ada280913d1897b89ab",
                                                "url": false, "type": "binary", "resId": 1, "resModel": "note.note", "isPublic": false, "accessToken": false,
                                                "imageSrc": "/web/image/1-a0e63e61/test.jpg", "imageWidth": 1, "imageHeight": 1, "originalId": false
                                                });
                        }
            ...
            await testUtils.form.clickEdit(form);
            const $field = form.$('.oe-form-field[name="body"]');
            ...
            await testUtils.dom.click($('.modal #editorMediaImage .o-existing-attachment-cell:first').removeClass('d-none'));
            ...
            assert.ok(form.$('.o-attachment[title="test.jpg"]')[0])
        ...
        QUnit.test('media dialog: image', async function (assert) {
            ...
            var $editable = form.$('.oe-form-field[name="body"] .note-editable');
        ...
        QUnit.test('link dialog - external link - no edit', async function (assert) {
            ...
            let $field = form.$('.oe-form-field[name="body"]');
            assert.strictEqual($field.children('.o-readonly').html(),
        ...
        QUnit.test('link dialog - internal link - no edit', async function (assert) {
            ...
            await defLinkDialog;
            $('.modal .tab-content .tab-pane').removeClass('fade'); // to be sync in test
            await testUtils.dom.click($('.modal input#o-link-dialog-url_strip_domain'));
            await testUtils.dom.click($('.modal .modal-footer button:contains(Save)'));
        ...
        QUnit.test('link dialog - external link - new', async function (assert) {
            ...
            let $field = form.$('.oe-form-field[name="body"]');
            assert.strictEqual($field.children('.o-readonly').html(), '<p>New external link</p>',
                "should have rendered a div with correct content in readonly");
            ...
            await defLinkDialog;
            $('.modal .tab-content .tab-pane').removeClass('fade'); // to be sync in test
            $('input#o-link-dialog-url_input').val('www.test.com');
            await testUtils.dom.click($('.modal .modal-footer button:contains(Save)'));
        ...
        QUnit.test('link dialog - internal link - new', async function (assert) {
            ...
            let $field = form.$('.oe-form-field[name="body"]');
            assert.strictEqual($field.children('.o-readonly').html(), '<p>New internal link</p>',
                "should have rendered a div with correct content in readonly");
            ...
            await defLinkDialog;
            $('.modal .tab-content .tab-pane').removeClass('fade'); // to be sync in test
            const $input = $('input#o-link-dialog-url_input');
            await testUtils.fields.editAndTrigger($input, window.location.href + '/test', ["change"]);
            $('.modal input#o-link-dialog-url_strip_domain').click();
            await testUtils.dom.click($('.modal .modal-footer button:contains(Save)'));
        ...
        QUnit.test('Quick Edition: click on link inside html field', async function (assert) {
            ...
            await testUtils.dom.click(form.$('.oe-form-field[name="body"] a'));
            await testUtils.nextTick();
            assert.strictEqual(quickEditCB, undefined, "no quickEdit callback should have been set");
            assert.containsOnce(form, '.o-form-view.o-form-readonly');

            await testUtils.dom.click(form.$('.oe-form-field[name="body"] p'));
            await testUtils.nextTick();
            assert.containsOnce(form, '.o-form-view.o-form-readonly');
        ...
        QUnit.test('rendering with iframe for readonly mode', async function (assert) {
            ...
            var $field = form.$('.oe-form-field[name="body"]');
            var $iframe = $field.find('iframe.o-readonly');
            await $iframe.data('loadDef');
            var doc = $iframe.contents()[0];
            assert.strictEqual($(doc).find('#iframeTarget').html(),
                '<p>toto toto toto</p><p>tata</p>',
                "should have rendered a div with correct content in readonly");
            ...
            $field = form.$('.oe-form-field[name="body"]');
            assert.strictEqual($field.find('#iframeTarget').length, 0);
        ...
        QUnit.test('field html translatable', async function (assert) {
            ...
            var form = await testUtils.createView({
                ...
                resId: 1,
                mockRPC: function (route, args) {
                    if (route === '/web/dataset/callButton' && args.method === 'translateFields') {
                        assert.deepEqual(args.args, ['note.note', 1, 'body'], "should call 'callButton' route");
                        return Promise.resolve({
                            domain: [],
                            context: {searchDefault_label: 'partnes,foo'},
                        });
                    }
                    if (route === "/web/dataset/callKw/res.lang/getInstalled") {
                        return Promise.resolve([["en_US"], ["fr_BE"]]);
                    }
                    return this._super.apply(this, arguments);
                },
            });
            assert.strictEqual(form.$('.oe-form-field-html .o-field-translate').length, 0,
                "should not have a translate button in readonly mode");

            await testUtils.form.clickEdit(form);
            var $button = form.$('.oe-form-field-html .o-field-translate');
            assert.strictEqual($button.length, 1, "should have a translate button");
            await testUtils.dom.click($button);
            assert.containsOnce($(document), '.o-translation-dialog', 'should have a modal to translate');

            form.destroy();
            _t.database.multiLang = multiLang;
        });
    });
});
});
