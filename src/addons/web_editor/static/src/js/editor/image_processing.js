verp.define('web_editor.imageProcessing', function (require) {
'use strict';
...
async function loadImageInfo(img, rpc, attachmentSrc = '') {
    ...
    const {original} = await rpc({
        route: '/web_editor/getImageInfo',
        params: {src: src.split(/[?#]/)[0]},
    });
    // Check that url is local.
    const isLocal = original && new URL(original.imageSrc, window.location.origin).origin === window.location.origin;
    if (isLocal && original.imageSrc) {
        img.dataset.originalId = original.id;
        img.dataset.originalSrc = original.imageSrc;
