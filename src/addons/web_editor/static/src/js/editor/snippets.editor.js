verp.define('web_editor.snippet.editor', function (require) {
'use strict';
...
// jQuery extensions
$.extend($.expr[':'], {
    'o-editable': function (node, i, m) {
        while (node) {
            if (node.className && _.isString(node.className)) {
                if (node.className.indexOf('o-not-editable') !== -1) {
                    return false;
                }
                if (node.className.indexOf('o-editable') !== -1) {
                    return true;
                }
            }
...
var SnippetEditor = Widget.extend({
    template: 'web_editor.snippetOverlay',
    xmlDependencies: ['/web_editor/static/src/xml/snippets.xml'],
    events: {
        'click .oe-snippet-remove': '_onRemoveClick',
        'wheel': '_onMouseWheel',
    },
    customEvents: {
        'optionUpdate': '_onOptionUpdate',
        'userValueWidgetRequest': '_onUserValueWidgetRequest',
        'snippetOptionVisibilityUpdate': '_onSnippetOptionVisibilityUpdate',
    },
    ...
    init: function (parent, target, templateOptions, $editable, options) {
        ...
        if (!this.$scrollingElement[0]) {
            this.$scrollingElement = $(this.ownerDocument).find('.o-editable');
        }
    ...
    start: function () {
        ...
        this.isTargetParentEditable = this.$target.parent().is(':o-editable');
        this.isTargetMovable = this.isTargetParentEditable && this.isTargetMovable && !this.$target.hasClass('oe-unmovable');
        this.isTargetRemovable = this.isTargetParentEditable && !this.$target.parent().is('[data-oe-type="image"]') && !this.$target.hasClass('oe-unremovable');
        ...
        if (this.isTargetMovable) {
            ...
            this.$('.o-overlay-move-options').addClass('d-none');
            $customize.find('.oe-snippet-clone').addClass('d-none');
        }

        if (!this.isTargetRemovable) {
            this.$el.add($customize).find('.oe-snippet-remove').addClass('d-none');
        }
        ...
        this.$target.on('transitionstart.snippet-editor, animationstart.snippet-editor', () => {
        ...
        this.$target.on('transitionend.snippet-editor, animationend.snippet-editor', postAnimationCover);
        ...
        // - beforeReplaceTarget: before the target gets replaced, makes sure
        //   jquery data() do not reference the old editor anymore
        // - replaceTarget: after the replacement happened in the DOM,
        //   activates the replacement element.
        this.$target.on('beforeReplaceTarget.snippet-editor', (ev, options) => {
            ev.stopPropagation();
            options.proms.push(new Promise(resolve => {
                this.triggerUp('destroyEditor', {
                    editor: this,
                    onSuccess: resolve,
                });
            }));
        });
        this.$target.on('replaceTarget.snippet-editor', (ev, replacementEl) => {
            this.triggerUp('activateSnippet', {$snippet: $(replacementEl)});
        });
    ...
    cleanForSave: async function () {
        ...
        await this.toggleTargetVisibility(!this.$target.hasClass('o-snippet-invisible'));
    ...
    cover: function () {
        ...
        targetEl.classList.add('o-transform-removal');
        ...
        this.$('.o-handles').css('height', $target.outerHeight());

        targetEl.classList.remove('o-transform-removal');

        const editableOffsetTop = this.$editable.offset().top - manipulatorOffset.top;
        this.$el.toggleClass('o-top-cover', offset.top - editableOffsetTop < 25);
    },
    /**
    ...
    getName: function () {
        ...
        if (this.$target.is('.media-iframe-video')) {
            return _t("Video");
        }
     ...
     */
    isShown: function () {
        return this.$el && this.$el.parent().length && this.$el.hasClass('oe-active');
    },
    /**
     * @returns {boolean}
     */
    isSticky: function () {
        return this.$el && this.$el.hasClass('o-we-overlay-sticky');
    },
    /**
     * @returns {boolean}
     */
    ...
    removeSnippet: async function (shouldRecordUndo = true) {
        ...
        // 'activateSnippet' call would allow to remove the 'toggleOverlay' and
        ...
        while (nextSibling && nextSibling.matches('.o-snippet-invisible')) {
            nextSibling = nextSibling.nextElementSibling;
        }
        let previousSibling = this.$target[0].previousElementSibling;
        while (previousSibling && previousSibling.matches('.o-snippet-invisible')) {
            previousSibling = previousSibling.previousElementSibling;
        }
        if ($(parent).is('.o-editable:not(body)')) {
        ...
        if ($parent.closest(':data("snippet-editor")').length) {
            const isEmptyAndRemovable = ($el, editor) => {
                ...
                return isEmpty && !$el.hasClass('oe-structure') && !$el.hasClass('oe-unremovable')
        ...
        this.$body.find('.o-table-handler').remove();

        this.triggerUp('snippetRemoved');
    ...
    toggleOverlay: function (show, previewMode) {
        ...
        if (previewMode) {
            ...
            this.$el.toggleClass('o-we-overlay-preview', show);
            this.$el.find('.o-handle').removeClass('d-none');
        } else {
            // In non preview mode, the preview class is always removed, and the
            // sticky class is added/removed when toggling/untoggling
            this.$el.removeClass('o-we-overlay-preview');
            this.$el.toggleClass('o-we-overlay-sticky', show);
            if (!this.displayOverlayOptions) {
                this.$el.find('.o-overlay-options-wrap').addClass('o-we-hidden-overlay-options');
            }
        }

        // Show/hide overlay in preview mode or not
        this.$el.toggleClass('oe-active', show);
        this.cover();
    ...
    toggleOverlayVisibility: function (show) {
        if (this.$el && !this.scrollingTimeout) {
            this.$el.toggleClass('o-overlay-hidden', !show && this.isShown());
        }
    },
    /**
    ...
    clone: async function (recordUndo) {
        this.triggerUp('snippetWillBeCloned', {$target: this.$target});
        ...
        if (recordUndo) {
            this.options.wysiwyg.verpEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.triggerUp('callForEachChildSnippet', {
                $snippet: $clone,
        ...
        this.triggerUp('snippetCloned', {$target: $clone, $origin: this.$target});

        $clone.trigger('contentChanged');
    },
    ...
    _initializeOptions: function () {
        ...
        var $optionsSection = $(core.qweb.render('web_editor.customizeBlockOptionsSection', {
            name: this.getName(),
        })).data('editor', this);
        ...
        $optionsSection.on('click', '.oe-snippet-clone', this._onCloneClick.bind(this));
        $optionsSection.on('click', '.oe-snippet-remove', this._onRemoveClick.bind(this));
        ...
        var defs = _.map(this.templateOptions, val => {
            ...
            var option = new (options.registry[optionName] || options.Class)(
                this,
                val.$el.children(),
                val.baseTarget ? this.$target.find(val.baseTarget).eq(0) : this.$target,
                this.$el,
                _.extend({
            ...
            if (option.forceNoDeleteButton) {
                this.$el.add($optionsSection).find('.oe-snippet-remove').addClass('d-none');
            }
    ...
    _canBeSanitizedUnless(el) {
        ...
        for (const snippetEl of [el, ...el.querySelectorAll('[data-snippet]')]) {
            this.triggerUp('findSnippetTemplate', {
                snippet: snippetEl,
    ...
    _onDragAndDropStart: function () {
        this.options.wysiwyg.verpEditor.observerUnactive('dragAndDropMoveSnippet');
        this.triggerUp('dragAndDropStart');
        this.options.wysiwyg.verpEditor.automaticStepUnactive();
        var self = this;
        this.dropped = false;
        this._dropSiblings = {
        ...
        self.$target.after('<div class="oe-drop-clone" style="display: none;"/>');
        self.$target.detach();
        ...
        this.triggerUp('activateSnippet', {$snippet: this.$target.parent()});
        this.triggerUp('activateInsertionZones', {
        ...
        this.$dropZones = this.$editable.find('.oe-drop-zone');
        if (!canBeSanitizedUnless) {
            this.$dropZones = this.$dropZones.not('[data-oe-sanitize] .oe-drop-zone');
        } else if (canBeSanitizedUnless === 'form') {
            this.$dropZones = this.$dropZones.not('[data-oe-sanitize][data-oe-sanitize!="allowForm"] .oe-drop-zone');
        }
        this.$dropZones.droppable({
            over: function () {
                if (self.dropped) {
                    self.$target.detach();
                    $('.oe-drop-zone').removeClass('invisible');
                }
                self.dropped = true;
        ...
        self.draggableComponent.$scrollTarget.on('scroll.scrollingElement', function () {
    ...
    _onDragAndDropStop: function (ev, ui) {
        this.options.wysiwyg.verpEditor.automaticStepActive();
        this.options.wysiwyg.verpEditor.automaticStepSkipStack();
        this.options.wysiwyg.verpEditor.unbreakableStepUnactive();

        // TODO lot of this is duplicated code of the d&d feature of snippets
        if (!this.dropped) {
            var $el = $.nearest({x: ui.position.left, y: ui.position.top}, '.oe-drop-zone', {container: document.body}).first();
        ...
        this.options.wysiwyg.verpEditor.observerActive('dragAndDropMoveSnippet');
        if (this.dropped) {
            if (prev) {
            ...
            this.$target.trigger('contentChanged');
            $from.trigger('contentChanged');
        }

        this.triggerUp('dragAndDropStop', {
            $snippet: this.$target,
        });
        this.draggableComponent.$scrollTarget.off('scroll.scrollingElement');
...
var SnippetsMenu = Widget.extend({
    ...
    events: {
        'click .oe-snippet': '_onSnippetClick',
        'click .o-install-btn': '_onInstallBtnClick',
        'click .o-we-add-snippet-btn': '_onBlocksTabClick',
        'click .o-we-customize-snippet-btn': '_onOptionsTabClick',
        'click .o-we-invisible-entry': '_onInvisibleEntryClick',
        'click #snippetCustom .o-rename-btn': '_onRenameBtnClick',
        'click #snippetCustom .o-delete-btn': '_ondeleteBtnClick',
        'mousedown': '_onMouseDown',
        'input .o-snippet-search-filter-input': '_onSnippetSearchInput',
        'click .o-snippet-search-filter-reset': '_onSnippetSearchResetClick',
        'click .o-we-website-top-actions button[data-action=save]': '_onSaveRequest',
        'click .o-we-website-top-actions button[data-action=cancel]': '_onDiscardClick',
        'click .o-we-website-top-actions button[data-action=mobile]': '_onMobilePreviewClick',
        'click .o-we-website-top-actions button[data-action=undo]': '_onUndo',
        'click .o-we-website-top-actions button[data-action=redo]': '_onRedo',
    },
    customEvents: {
        'activateInsertionZones': '_onActivateInsertionZones',
        'activateSnippet': '_onActivateSnippet',
        'callForEachChildSnippet': '_onCallForEachChildSnippet',
        'cloneSnippet': '_onCloneSnippet',
        'coverUpdate': '_onOverlaysCoverUpdate',
        'deactivateSnippet': '_onDeactivateSnippet',
        'destroyEditor': '_onDestroyEditor',
        'dragAndDropStop': '_onSnippetDragAndDropStop',
        'dragAndDropStart': '_onSnippetDragAndDropStart',
        'getSnippetVersions': '_onGetSnippetVersions',
        'findSnippetTemplate': '_onFindSnippetTemplate',
        'removeSnippet': '_onRemoveSnippet',
        'snippetEditionRequest': '_onSnippetEditionRequest',
        'snippetEditorDestroyed': '_onSnippetEditorDestroyed',
        'snippetRemoved': '_onSnippetRemoved',
        'snippetCloned': '_onSnippetCloned',
        'snippetOptionUpdate': '_onSnippetOptionUpdate',
        'snippetOptionVisibilityUpdate': '_onSnippetOptionVisibilityUpdate',
        'snippetThumbnailUrlRequest': '_onSnippetThumbnailURLRequest',
        'reloadSnippetDropzones': '_disableUndroppableSnippets',
        'requestSave': '_onSaveRequest',
        'hideOverlay': '_onHideOverlay',
        'blockPreviewOverlays': '_onBlockPreviewOverlays',
        'unblockPreviewOverlays': '_onUnblockPreviewOverlays',
        'userValueWidgetOpening': '_onUserValueWidgetOpening',
        'userValueWidgetClosing': '_onUserValueWidgetClosing',
        'reloadSnippetTemplate': '_onReloadSnippetTemplate',
        'requestEditable': '_onRequestEditable',
        'disableLoadingEffect': '_onDisableLoadingEffect',
        'enableLoadingEffect': '_onEnableLoadingEffect',
    },
    ...
    async start() {
        ...
        this.customizePanel.classList.add('o-we-customize-panel', 'd-none');
        this._addToolbar();
        this._checkEditorToolbarVisibilityCallback = this._checkEditorToolbarVisibility.bind(this);
        $(this.options.wysiwyg.verpEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);

        if (this.options.enableTranslation) {
            // Load the sidebar with the style tab only.
            await this._loadSnippetsTemplates();
            this.$el.find('.o-we-website-top-actions').removeClass('d-none');
            this.$('.o-snippet-search-filter').addClass('d-none');
            this.$('#oScroll').addClass('d-none');
            this.$('button[data-action="mobilePreview"]').addClass('d-none');
            this.$('#snippetsMenu button').removeClass('active').prop('disabled', true);
            this.$('.o-we-customize-snippet-btn').addClass('active').prop('disabled', false);
            this.$('o-we-ui-loading').addClass('d-none');
            $(this.customizePanel).removeClass('d-none');
            this.$('#oWeEditorToolbarContainer').hide();
            this.$('#oWeEditorTableContainer').addClass('d-none');
            return Promise.all(defs);
        }
        this.invisibleDOMPanelEl = document.createElement('div');
        this.invisibleDOMPanelEl.classList.add('o-we-invisible-el-panel');
        this.invisibleDOMPanelEl.appendChild(
            $('<div/>', {
                text: _t('Invisible Elements'),
                class: 'o-panel-header',
            })[0]
        );
        ...
        const onClick = ev => {
            ...
            if ($target.parents('.o-edit-menu-popover').length && !$target.parent('a').addBack('a').length) {
                return;
            }
            ...
            if (!$target.closest('we-button, we-toggler, we-select, .o-we-color-preview').length) {
                this._closeWidgets();
            }
            if (!$target.closest('body > *').length || $target.is('#iframeTarget')) {
                return;
            }
            if ($target.closest(this._notActivableElementsSelector).length) {
                return;
            }
            const $oeStructure = $target.closest('.oe-structure');
            if ($oeStructure.length && !$oeStructure.children().length && this.$snippets) {
                // If empty oe-structure, encourage using snippets in there by
                // making them "wizz" in the panel.
                this._activateSnippet(false).then(() => {
                    this.$snippets.verpBounce();
                });
                return;
            }
        ...
        this.$document.on('click.snippetsMenu', '*', onClick);
        // Needed as bootstrap stop the propagation of click events for dropdowns
        this.$document.on('mouseup.snippetsMenu', '.dropdown-toggle', onClick);

        core.bus.on('deactivateSnippet', this, this._onDeactivateSnippet);
        ...
        this.$window.on('resize.snippetsMenu', debouncedCoverUpdate);
        this.$window.on('contentChanged.snippetsMenu', debouncedCoverUpdate);

        // On keydown add a class on the active overlay to hide it and show it
        // again when the mouse moves
        this.$document.on('keydown.snippetsMenu', () => {
            this.__overlayKeyWasDown = true;
            this.snippetEditors.forEach(editor => {
                editor.toggleOverlayVisibility(false);
            });
        });
        this.$document.on('mousemove.snippetsMenu, mousedown.snippetsMenu', _.throttle(() => {
        ...
        // on text changes
        this.$document.on('click.snippetsMenu', '.o-default-snippet-text', function (ev) {
            $(ev.target).closest('.o-default-snippet-text').removeClass('o-default-snippet-text');
            $(ev.target).selectContent();
            $(ev.target).removeClass('o-default-snippet-text');
        });
        this.$document.on('keyup.snippetsMenu', function () {
            const selection = document.getSelection();
            if (!Selection.rangeCount) {
                return;
            }
            const range = selection.getRangeAt(0);
            $(range.startContainer).closest('.o-default-snippet-text').removeClass('o-default-snippet-text');
        });
        ...
        this.options.wysiwyg.verpEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.verpEditor.addEventListener('historyRedo', refreshSnippetEditors);

        const $autoFocusEls = $('.o-we-snippet-autofocus');
        ...
        return Promise.all(defs).then(() => {
            const $undoButton = this.$('.o-we-external-history-buttons button[data-action="undo"]');
            const $redoButton = this.$('.o-we-external-history-buttons button[data-action="redo"]');
            if ($undoButton.length) {
                const updateHistoryButtons = () => {
                    $undoButton.attr('disabled', !this.options.wysiwyg.verpEditor.historyCanUndo());
                    $redoButton.attr('disabled', !this.options.wysiwyg.verpEditor.historyCanRedo());
                };
                this.options.wysiwyg.verpEditor.addEventListener('historyStep', updateHistoryButtons);
                this.options.wysiwyg.verpEditor.addEventListener('observerApply', () => {
                    $(this.options.wysiwyg.verpEditor.editable).trigger('contentChanged');
                });
                this.options.wysiwyg.verpEditor.addEventListener('historyRevert', _.debounce(() => {
    ...
    destroy: function () {
        ...
        if (this.$window) {
            ...
            this.$window.off('.snippetsMenu');
            this.$document.off('.snippetsMenu');

            if (this.$scrollingTarget) {
                this.$scrollingTarget[0].removeEventListener('scroll', this._onScrollingElementScroll, {capture: true});
            }
        }
        core.bus.off('deactivateSnippet', this, this._onDeactivateSnippet);
    ...
    cleanForSave: async function () {
        ...
        this.triggerUp('readyToCleanForSave');
        ...
        this.getEditableArea().find('.o-we-selected-image')
            .removeClass('o-we-selected-image');
    },
    /**
     * Load snippets.
    ...
    _activateInsertionZones: function ($selectorSiblings, $selectorChildren) {
        ...
        if ($selectorChildren) {
            $selectorChildren.each(function () {
                var data;
                var $zone = $(this);
                var $children = $zone.find('> :not(.oe-drop-zone, .oe-drop-clone)');

                if (!$zone.children().last().is('.oe-drop-zone')) {
                ...
                if (!$zone.children().first().is('.oe-drop-clone')) {
        ...
        var noDropZonesSelector = '[data-invisible="1"], .o-we-no-overlay, :not(:visible)';
        if ($selectorSiblings) {
            $selectorSiblings.not(`.oe-drop-zone, .oe-drop-clone, ${noDropZonesSelector}`).each(function () {
                var data;
                var $zone = $(this);
                ...
                if (!$zoneToCheck.prev('.oe-drop-zone:visible, .oe-drop-clone').length) {
                ...
                if (!$zoneToCheck.next('.oe-drop-zone:visible, .oe-drop-clone').length) {
                    data = setDropZoneDirection($zone, $zone.parent());
        ...
        do {
            count = 0;
            $zones = this.getEditableArea().find('.oe-drop-zone > .oe-drop-zone').remove(); // no recursive zones
            count += $zones.length;
            $zones.remove();
        } while (count > 0);
        ...
        $zones = this.getEditableArea().find('.oe-drop-zone:not(.oe-vertical)');
        $zones.each(function () {
            var zone = $(this);
            var prev = zone.prev();
            var next = zone.next();
            // remove consecutive zone
            if (prev.is('.oe-drop-zone') || next.is('.oe-drop-zone')) {
                zone.remove();
                return;
            }
    ...
    _updateInvisibleDOM: function () {
        return this._execWithLoadingEffect(() => {
            ...
            $invisibleDOMPanelEl.find('.o-we-invisible-entry').remove();
            const $invisibleSnippets = globalSelector.all().find('.o-snippet-invisible').addBack('.o-snippet-invisible');
            ...
            const proms = _.map($invisibleSnippets, async el => {
                ...
                const $invisEntry = $('<div/>', {
                    class: 'o-we-invisible-entry d-flex align-items-center justify-content-between',
                    text: editor.getName(),
    ...
    _activateSnippet: async function ($snippet, previewMode, ifInactiveOptions) {
        ...
        if ($snippet && $snippet.length) {
            ...
            if (!$globalSnippet.length) {
                $snippet = $snippet.closest('[data-oe-model="ir.ui.view"]:not([data-oe-type]):not(.oe-structure), [data-oe-type="html"]:not(.oe-structure)');
            } else {
    ...
    _computeSelectorFunctions: function (selector, exclude, target, noCheck, isChildren, excludeParent) {
        ...
        exclude += `${exclude && ', '}.o-snippet-not-selectable`;
    ...
    _computeSnippetTemplates: function (html) {
        var self = this;
        var $html = $(html);
        var $scroll = $html.siblings('#oScroll');
        ...
        const $logoTypeSelector = $html.find('[data-js="HeaderNavbar"] [data-name="optionHeaderBrandNone"]').parent();
        if ($logoTypeSelector.length) {
            ...
            $logoHeightOptions.closest('[data-selector]')
                .find('[data-name="optionHeaderBrandNone"]').parent().remove();
        ...
        const snippetAdditionDropIn = $styles.filter('#soSnippetAddition').data('drop-in');
        $styles.each(function () {
            var $style = $(this);
            var selector = $style.data('selector');
            var exclude = $style.data('exclude') || '';
            const excludeParent = $style.attr('id') === "soContentAddition" ? snippetAdditionDropIn : '';
        ...
        this.$snippets = $scroll.find('.o-panel-body').children()
            .addClass('oe-snippet')
            .each((i, el) => {
                const $snippet = $(el);
                const name = _.escape(el.getAttribute('name'));
                const thumbnailSrc = _.escape(el.dataset.oeThumbnail);
                const $sbody = $snippet.children().addClass('oe-snippet-body');
                const isCustomSnippet = !!el.closest('#snippetCustom');
                ...
                let snippetClasses = $sbody.attr('class').match(/s-[^ ]+/g);
                ...
                if (!$snippet.data('oe-forbid-sanitize')) {
                    ...
                    if ($snippet[0].querySelector('.s-embed-code-embedded')) {
                ...
                const $thumbnail = $(`
                    <div class="oe-snippet-thumbnail">
                        <div class="oe-snippet-thumbnail-img" style="background-image: url(${thumbnailSrc});"/>
                        <span class="oe-snippet-thumbnail-title">${name}</span>
                    </div>
                `);
                ...
                if (moduleID) {
                    el.classList.add('o-snippet-install');
                    $thumbnail.append($('<button/>', {
                        class: 'btn btn-primary o-install-btn w-100',
                        type: 'button',
                        text: _t("Install"),
                    }));
                }
                ...
                if (isCustomSnippet) {
                    ...
                    btnRenameEl.classList.add('o-rename-btn', 'fa', 'fa-pencil', 'btn', 'o-we-hover-success');
                    ...
                    btnEl.classList.add('o-delete-btn', 'fa', 'fa-trash', 'btn', 'o-we-hover-danger');
        ...
        $html.find('.o-not-editable').attr('contentEditable', false);
        ...
        this.$el.addClass('o-loaded');
        this.triggerUp('snippetsLoaded', self.$el);
        $(this.el.ownerDocument.body).addClass('editor-has-snippets');
    },
    /**
    ...
    _createSnippetEditor: function ($snippet) {
        ...
        var def;
        if (!$snippet[0].classList.contains('o-no-parent-editor')) {
        ...
        return Promise.resolve(def).then(function (parentEditor) {
            ...
            snippetEditor = new SnippetEditor(parentEditor || self, $snippet, self.templateOptions, $snippet.closest('[data-oe-type="html"], .oe-structure').add(editableArea), self.options);
            self.snippetEditors.push(snippetEditor);
            // Keep parent below its child inside the DOM as its `o-handle`
    ...
    _disableUndroppableSnippets: function () {
        ...
        this.$snippets.each(function () {
            var $snippet = $(this);
            var $snippetBody = $snippet.find('.oe-snippet-body');
            const isSanitizeForbidden = $snippet.data('oe-forbid-sanitize');
            const filterSanitize = isSanitizeForbidden === 'form'
                ? $els => $els.filter((i, el) => !el.closest('[data-oe-sanitize]:not([data-oe-sanitize="allowForm"])'))
                : isSanitizeForbidden
            ...
            _.each(self.templateOptions, function (option, k) {
                if (check || !($snippetBody.is(option.baseSelector) && !$snippetBody.is(option.baseExclude))) {
                    return;
                }
            ...
            $snippet.toggleClass('o-disabled', !check);
            $snippet.attr('title', check ? '' : _t("No location to drop in"));
            const $icon = $snippet.find('.o-snippet-undroppable').remove();
            if (check) {
                $icon.remove();
            } else if (!$icon.length) {
                const imgEl = document.createElement('img');
                imgEl.classList.add('o-snippet-undroppable');
    ...
    _filterSnippets(search) {
        const searchInputEl = this.el.querySelector('.o-snippet-search-filter-input');
        const searchInputReset = this.el.querySelector('.o-snippet-search-filter-reset');
        if (search !== undefined) {
        ...
        for (const panelEl of this.el.querySelectorAll('.o-panel')) {
            let hasVisibleSnippet = false;
            const panelTitle = panelEl.querySelector('.o-panel-header').textContent;
            const isPanelTitleMatch = strMatches(panelTitle);
            for (const snippetEl of panelEl.querySelectorAll('.oe-snippet')) {
    ...
    _insertDropzone: function ($hook, vertical, style) {
        ...
        if (this._insertDropzoneCanBeSanitizedUnless === 'form') {
            forbidSanitize = $hook.closest('[data-oe-sanitize]:not([data-oe-sanitize="allowForm"])').length;
        } else {
            forbidSanitize = !this._insertDropzoneCanBeSanitizedUnless && $hook.closest('[data-oe-sanitize]').length;
        }
        var $dropzone = $('<div/>', {
            'class': 'oe-drop-zone oe-insert' + (vertical ? ' oe-vertical' : '') +
                (forbidSanitize ? ' text-center oe-drop-zone-danger' : ''),
        });
        if (style) {
            $dropzone.css(style);
        }
    ...
    _makeSnippetDraggable: function ($snippets) {
        ...
        if (!$scrollingElement[0] || $scrollingElement.find('body.o-in-iframe').length) {
            $scrollingElement = $(this.ownerDocument).find('.o-editable');
        }

        const smoothScrollOptions = this._getScrollOptions({
            jQueryDraggableOptions: {
                handle: '.oe-snippet-thumbnail:not(.o-we-already-dragging)',
                helper: function () {
                    const dragSnip = this.cloneNode(true);
                    dragSnip.querySelectorAll('.o-delete-btn, .o-rename-btn').forEach(
                        el => el.remove()
                    );
                    return dragSnip;
                },
                start: function () {
                    ...
                    const doc = self.options.wysiwyg.verpEditor.document;
                    $(doc.body).addClass('oe-dropzone-active');

                    self.options.wysiwyg.verpEditor.automaticStepUnactive();

                    self.$el.find('.oe-snippet-thumbnail').addClass('o-we-already-dragging');
                    self.options.wysiwyg.verpEditor.observerUnactive('dragAndDropCreateSnippet');

                    dropped = false;
                    $snippet = $(this);
                    var $baseBody = $snippet.find('.oe-snippet-body');
                    var $selectorSiblings = $();
                    var $selectorChildren = $();
                    var temp = self.templateOptions;
                    for (var k in temp) {
                        if ($baseBody.is(temp[k].baseSelector) && !$baseBody.is(temp[k].baseExclude)) {
                            if (temp[k]['drop-near']) {
                    ...
                    if ($baseBody[0].matches('.s-popup, .o-newsletter-popup')) {
                    ...
                    if (!$selectorSiblings.length && !$selectorChildren.length) {
                        console.warn($snippet.find('.oe-snippet-thumbnail-title').text() + " have not insert action: data-drop-near or data-drop-in");
                        return;
                    }

                    // TODO Done this way in stable but should be a parameter in
                    // master.
                    const forbidSanitize = $snippet.data('oe-forbid-sanitize');
                    ...
                    $dropZones = self.getEditableArea().find('.oe-drop-zone');
                    if (forbidSanitize === 'form') {
                        $dropZones = $dropZones.filter((i, el) => !el.closest('[data-oe-sanitize]:not([data-oe-sanitize="allowForm"]) .oe-drop-zone'));
                    } else if (forbidSanitize) {
                        $dropZones = $dropZones.filter((i, el) => !el.closest('[data-oe-sanitize] .oe-drop-zone'));
                    }
                    $dropZones.droppable({
                        over: function () {
                            if (dropped) {
                                $toInsert.detach();
                                $toInsert.addClass('oe-snippet-body');
                                $('.oe-drop-zone').removeClass('invisible');
                            }
                            dropped = true;
                            $(this).first().after($toInsert).addClass('invisible');
                            $toInsert.removeClass('oe-snippet-body');
                            self.triggerUp('dropZoneOver');
                        },
                        out: function () {
                    ...
                    self.triggerUp('dropZoneStart');
                },
                stop: async function (ev, ui) {
                    const doc = self.options.wysiwyg.verpEditor.document;
                    $(doc.body).removeClass('oe-dropzone-active');
                    self.options.wysiwyg.verpEditor.automaticStepActive();
                    self.options.wysiwyg.verpEditor.automaticStepSkipStack();
                    $toInsert.removeClass('oe-snippet-body');
                    self.draggableComponent.$scrollTarget.off('scroll.scrollingElement');
                    if (!dropped && ui.position.top > 3 && ui.position.left + ui.helper.outerHeight() < self.el.getBoundingClientRect().left) {
                        ...
                        let droppedOnNotNearest = doc.defaultView.$.touching(
                            point, '.oe-structure-not-nearest', container
                        ).first();
                        // If dropped outside of a dropzone with class oe-structure-not-nearest,
                        // move the snippet to the nearest dropzone without it
                        const selector = droppedOnNotNearest.length
                            ? '.oe-drop-zone'
                            : ':not(.oe-structure-not-nearest) > .oe-drop-zone';
                    ...
                    self.getEditableArea().find('.oe-drop-zone').remove();
                    ...
                    self.options.wysiwyg.verpEditor.observerActive('dragAndDropCreateSnippet');
                    ...
                    if (dropped) {
                        ...
                        _.defer(async function () {
                            ...
                            self.options.wysiwyg.verpEditor.unbreakableStepUnactive();
                            self.options.wysiwyg.verpEditor.historyStep();
                            self.$el.find('.oe-snippet-thumbnail').removeClass('o-we-already-dragging');
                        });
                    } else {
                        $toInsert.remove();
    ...
    /**
     * Adds the 'o-default-snippet-text' class on nodes which contain only
    ...
    _registerDefaultTexts: function ($in) {
        if ($in === undefined) {
            $in = this.$snippets.find('.oe-snippet-body');
        }
        ...
        $in.find('*').addBack()
            ...
            }).parent().addClass('o-default-snippet-text');
    },
    /**
    ...
    _updateRightPanelContent: function ({content, tab, ...options}) {
        ...
        this.$('.o-snippet-search-filter').toggleClass('d-none', this._currentTab !== this.tabs.BLOCKS);
        this.$('#oScroll').toggleClass('d-none', this._currentTab !== this.tabs.BLOCKS);
        ...
        this.$('#snippetsMenu button').removeClass('active');
        this.$('.o-we-add-snippet-btn').toggleClass('active', this._currentTab === this.tabs.BLOCKS);
        this.$('.o-we-customize-snippet-btn').toggleClass('active', this._currentTab === this.tabs.OPTIONS);
    },
    /**
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _onFindSnippetTemplate(ev) {
        this.$snippets.each(function () {
            const snippetBody = this.querySelector(`.oe-snippet-body[data-snippet=${ev.data.snippet.dataset.snippet}]`);
            if (snippetBody) {
    ...
    _onInstallBtnClick: function (ev) {
        ...
        new Dialog(this, {
            ...
            $content: $('<div/>', {text: _.str.sprintf(_t("Do you want to install the %s App?"), name)}).append(
                $('<a/>', {
                    target: '_blank',
                    href: '/web#id=' + moduleID + '&viewType=form&model=ir.module.module&action=base.openModuleTree',
            ...
            buttons: [{
                ...
                classes: 'btn-primary',
                click: function () {
                    this.$footer.find('.btn').toggleClass('o-hidden');
                    this._rpc({
                        model: 'ir.module.module',
                        method: 'buttonImmediateInstall',
                        args: [[moduleID]],
                    }).then(() => {
                        self.triggerUp('requestSave', {
                            reloadEditor: true,
                            _toMutex: true,
                        });
                    }).guardedCatch(reason => {
                ...
                classes: 'btn-primary disabled o-hidden',
            }, {
                text: _t("Cancel"),
                close: true,
            }],
    ...
    _ondeleteBtnClick: function (ev) {
        const $snippet = $(ev.target).closest('.oe-snippet');
    ...
    _onRenameBtnClick: function (ev) {
        ...
        const $input = $(`
            <we-input class="o-we-user-value-widget w-100 mx-1">
                <div>
                    <input type="text" autocomplete="chrome-off" value="${snippetName}" class="text-left"/>
                    <we-button class="o-we-confirm-btn o-we-text-success fa fa-check" title="${confirmText}"/>
                    <we-button class="o-we-cancel-btn o-we-text-danger fa fa-times" title="${cancelText}"/>
                </div>
            </we-input>
        `);
        $snippet.find('we-button').remove();
        $snippet.find('span.oe-snippet-thumbnail-title').replaceWith($input);
        const $textInput = $input.find('input');
        $textInput.focus();
        $textInput.select();
        $snippet.find('.oe-snippet-thumbnail').addClass('o-we-already-dragging'); // prevent drag
        $input.find('.o-we-confirm-btn').click(async () => {
            const name = $textInput.val();
            if (name !== snippetName) {
                this._execWithLoadingEffect(async () => {
                    await this._rpc({
                        model: 'ir.ui.view',
                        method: 'renameSnippet',
                        kwargs: {
                            'name': name,
                            'viewId': parseInt(ev.target.dataset.snippetId),
                            'templateKey': this.options.snippets,
                        },
                    });
                }, true);
        ...
        $input.find('.o-we-cancel-btn').click(async () => {
    ...
    _onMouseDown: function () {
        ...
        this.options.wysiwyg.verpEditor.automaticStepSkipStack();
        $blockedArea.addClass('o-we-no-pointer-events');
        const reenable = () => {
            this.options.wysiwyg.verpEditor.automaticStepSkipStack();
            $blockedArea.removeClass('o-we-no-pointer-events');
        };
    ...
    _onGetSnippetVersions: function (ev) {
        const snippet = this.el.querySelector(`.oe-snippet > [data-snippet="${ev.data.snippetName}"]`);
        ...
        this._execWithLoadingEffect(() => {
            if (data.reloadEditor) {
                ...
                data.onSuccess = async function () {
                    ...
                    window.location.href = window.location.origin + window.location.pathname + '?enableEditor=1';
                };
            }
            this.triggerUp('requestSave', data);
        }, true);
    },
    /**
     * @private
     */
    _onSnippetClick() {
        const $els = this.getEditableArea().find('.oe-structure.oe-empty').addBack('.oe-structure.oe-empty');
        for (const el of $els) {
            if (!el.children.length) {
                $(el).verpBounce('o-we-snippet-area-animation');
            }
        }
    },
    /**
    ...
    _onUserValueWidgetOpening: function () {
        this._closeWidgets();
        this.el.classList.add('o-we-backdrop');
    },
    /**
    ...
    _onUserValueWidgetClosing: function () {
        this.el.classList.remove('o-we-backdrop');
    },
    /**
    ...
    _addToolbar(toolbarMode = "text") {
        ...
        this._$toolbarContainer = $('<WE-CUSTOMIZEBLOCK-OPTIONS id="oWeEditorToolbarContainer"/>');
        ...
        this.options.wysiwyg.verpEditor.bindExecCommand($customizeTableBlock[0]);
    ...
    _checkEditorToolbarVisibility: function (e) {
        const $toolbarContainer = this.$('#oWeEditorToolbarContainer');
        const $toolbarTableContainer = this.$('#oWeEditorTableContainer');
        const selection = this.options.wysiwyg.verpEditor.document.getSelection();
        ...
        // during link edition).
        if ($currentSelectionTarget.closest('#oWeEditorToolbarContainer').length ||
            (e && $(e.target).closest('#oWeEditorToolbarContainer').length)
        ) {
            return;
        }
        if (!range ||
            !$currentSelectionTarget.parents('#wrapwrap, .iframe-editor-wrapper .o-editable').length ||
        ...
        const isInsideTD = !!(
            range &&
            $(range.startContainer).closest('.o-editable td').length &&
            $(range.endContainer).closest('.o-editable td').length
        );
    ...
    _onRequestEditable: function (ev) {
        ev.data.callback($(this.options.wysiwyg.verpEditor.editable));
    },
    /**
