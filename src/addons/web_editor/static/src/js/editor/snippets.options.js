verp.define('web_editor.snippets.options', function (require) {
'use strict';
...
} = require('web_editor.imageProcessing');
...
function _buildCollapseElement(title, options) {
    ...
    togglerEl.classList.add('o-we-collapse-toggler');
...
const UserValueWidget = Widget.extend({
    className: 'o-we-user-value-widget',
    customEvents: {
        'userValueUpdate': '_onUserValueNotification',
    },
    ...
    async start() {
        ...
        if (this.el.classList.contains('o-we-img-animate')) {
            ...
            this.$el.on('mouseenter.img-animate', buildImgExtensionSwitcher('png', 'gif'));
            this.$el.on('mouseleave.img-animate', buildImgExtensionSwitcher('gif', 'png'));
        }
    },
    /**
     * @override
    ...
    isPreviewed: function () {
        ...
        return this.el.classList.contains('o-we-preview');
    },
    /**
    ...
    notifyValueChange: function (previewMode, isSimulatedEvent) {
        ...
        if (previewMode === true || previewMode === false) {
            ...
            data.prepare = () => this.el.classList.add('o-we-preview');
        } else if (previewMode === 'reset') {
            data.prepare = () => this.el.classList.remove('o-we-preview');
        }

        this.triggerUp('userValueUpdate', data);
    },
    /**
     * Opens the widget (only meaningful for widgets that can be opened).
     */
    open() {
        this.triggerUp('userValueWidgetOpening');
        this.el.classList.add('o-we-widget-opened');
    },
    /**
    ...
    async setValue(value, methodName) {
        this._value = value;
        this.el.classList.remove('o-we-preview');
    },
    /**
     * @param {boolean} show
     */
...
const ButtonUserValueWidget = UserValueWidget.extend({
    ...
    _makeDescriptive() {
        const $el = this._super(...arguments);
        if (this.imgEl) {
            $el[0].classList.add('o-we-icon-button');
        }
        if (this.activeImgEl) {
...
const CheckboxUserValueWidget = ButtonUserValueWidget.extend({
    className: (ButtonUserValueWidget.prototype.className || '') + ' o-we-checkbox-wrapper',
...
const SelectUserValueWidget = BaseSelectionUserValueWidget.extend({
    ...
    async start() {
        ...
        if (this.iconEl) {
            this.el.classList.add('o-we-icon-select');
        ...
        dropdownCaretEl.classList.add('o-we-dropdown-caret');
    ...
    async setValue() {
        ...
        if (activeWidget) {
            ...
            if (value) {
                ...
                const fakeImgEl = activeWidget.el.querySelector('.o-we-fake-img-item');
                if (fakeImgEl) {
...
const InputUserValueWidget = UnitUserValueWidget.extend({
    ...
    start: async function () {
        ...
        if (unit.length > 3) {
            this.el.classList.add('o-we-large');
        }
    },
...
const ColorpickerUserValueWidget = SelectUserValueWidget.extend({
    className: (SelectUserValueWidget.prototype.className || '') + ' o-we-so-color-palette',
    customEvents: _.extend({}, SelectUserValueWidget.prototype.customEvents, {
        'customColorPicked': '_onCustomColorPicked',
        'colorPicked': '_onColorPicked',
        'colorHover': '_onColorHovered',
        'colorLeave': '_onColorLeft',
        'enterKeyColorColorpicker': '_onEnterKey'
    }),
    ...
    start: async function () {
        ...
        this.colorPreviewEl.classList.add('o-we-color-preview');
    ...
    async setValue(color, methodName, ...rest) {
        ...
        if (this._ccValue) {
            this.colorPreviewEl.classList.add('o-cc', `o-cc${this._ccValue}`);
        }
        if (this._value) {
    ...
    _renderColorPalette: function () {
        ...
        if (this.options.dataAttributes.noTransparency) {
            ...
            options.excluded = [...(options.excluded || []), 'transparentGrayscale'];
        }
...
const MediapickerUserValueWidget = UserValueWidget.extend({
    ...
    async start() {
        ...
        if (this.options.dataAttributes.buttonStyle) {
            ...
            this.el.classList.add('o-we-no-toggle', 'o-we-bg-success');
    ...
    _openDialog(el, {images = false, videos = false}) {
        el.src = this._value;
        const $editable = this.$target.closest('.o-editable');
...
const DatetimePickerUserValueWidget = InputUserValueWidget.extend({
    ...
    start: async function () {
        ...
        libObject._getTemplate = function () {
            ...
            $template.addClass('o-we-no-overlay o-we-datetimepicker');
            return $template;
        };
    },
...
const ListUserValueWidget = UserValueWidget.extend({
    tagName: 'we-list',
    events: {
        'click we-button.o-we-select-remove-option': '_onRemoveItemClick',
        'click we-button.o-we-list-add-optional': '_onAddCustomItemClick',
        'click we-button.o-we-list-add-existing': '_onAddExistingItemClick',
        'click we-select.o-we-user-value-widget.o-we-add-list-item': '_onAddItemSelectClick',
        'click we-button.o-we-checkbox-wrapper': '_onAddItemCheckboxClick',
    ...
    setValue() {
        ...
        if (this.createWidget) {
            ...
            if (this.isCustom) {
                ...
                this.addItemButton.classList.add('o-we-list-add-optional');
            } else {
                // TODO use a real select widget ?
                this.addItemButton = document.createElement('we-select');
                this.addItemButton.classList.add('o-we-user-value-widget', 'o-we-add-list-item');
        ...
        currentValues.forEach(value => {
            if (typeof value === 'object') {
                ...
                const { id, displayName } = recordData;
                delete recordData.id;
                delete recordData.displayName;
                this._addItemToTable(id, displayName, recordData);
            } else {
    ...
    _addItemToTable(id, value = this.el.dataset.defaultValue || _t("Item"), recordData) {
        ...
        if (!this.el.dataset.unsortable) {
            ...
            draggableEl.classList.add('o-we-drag-handle', 'o-we-link', 'fa', 'fa-fw', 'fa-arrows');
        ...
        buttonEl.classList.add('o-we-select-remove-option', 'o-we-link', 'o-we-text-danger', 'fa', 'fa-fw', 'fa-minus');
        buttonEl.dataset.removeOption = id;
        const inputTdEl = document.createElement('td');
        inputTdEl.classList.add('o-we-list-record-name');
        inputTdEl.appendChild(inputEl);
        trEl.appendChild(inputTdEl);
        if (this.hasDefault) {
            const checkboxEl = document.createElement('we-button');
            checkboxEl.classList.add('o-we-user-value-widget', 'o-we-checkbox-wrapper');
     ...
     */
    _notifyCurrentState() {
        const values = [...this.listTable.querySelectorAll('.o-we-list-record-name input')].map(el => {
        ...
        if (this.hasDefault) {
            const checkboxes = [...this.listTable.querySelectorAll('we-button.o-we-checkbox-wrapper.active')];
    ...
    _reloadSelectDropdown(currentValues) {
        ...
        this.records.forEach(el => {
            if (!currentValues.find(v => v.id === el.id)) {
                ...
                option.classList.add('o-we-list-add-existing');
                option.dataset.addOption = el.id;
                option.dataset.noPreview = 'true';
                const divEl = document.createElement('div');
                divEl.textContent = el.displayName;
    ...
    _onAddItemCheckboxClick: function (ev) {
        ...
        if (this.hasDefault === 'unique') {
            this.listTable.querySelectorAll('we-button.o-we-checkbox-wrapper.active').forEach(el => el.classList.remove('active'));
        }
...
const RangeUserValueWidget = UnitUserValueWidget.extend({
    ...
    async start() {
        ...
        if (min > max) {
            [min, max] = [max, min];
            this.input.classList.add('o-we-inverted-range');
        }
...
const SelectPagerUserValueWidget = SelectUserValueWidget.extend({
    className: (SelectUserValueWidget.prototype.className || '') + ' o-we-select-pager',
    events: Object.assign({}, SelectUserValueWidget.prototype.events, {
        'click .o-we-pager-next, .o-we-pager-prev': '_onPageChange',
    }),
    ...
    async start() {
        ...
        prev.classList.add('o-we-pager-prev', 'fa', 'fa-chevron-left');

        this.pageNum = document.createElement('span');
        this.currentPage = 0;

        const next = document.createElement('i');
        next.classList.add('o-we-pager-next', 'fa', 'fa-chevron-right');

        const pagerControls = document.createElement('div');
        pagerControls.classList.add('o-we-pager-controls');
        ...
        pagerHeader.classList.add('o-we-pager-header');
        pagerHeader.appendChild(this.pageName);
        pagerHeader.appendChild(pagerControls);

        await _super(...arguments);
        this.menuEl.classList.add('o-we-has-pager');
     ...
     */
    _shouldIgnoreClick(ev) {
        return !!ev.target.closest('.o-we-pager-header') || this._super(...arguments);
    },
    /**
    ...
    _onPageChange(ev) {
        ev.preventDefault();
        ev.stopPropagation();
        const delta = ev.target.matches('.o-we-pager-next') ? 1 : -1;
...
const Many2oneUserValueWidget = SelectUserValueWidget.extend({
    className: (SelectUserValueWidget.prototype.className || '') + ' o-we-many2one',
    events: Object.assign({}, SelectUserValueWidget.prototype.events, {
        'input .o-we-m2o-search input': '_onSearchInput',
        'keydown .o-we-m2o-search input': '_onSearchKeydown',
        'click .o-we-m2o-search-more': '_onSearchMoreClick',
    }),
    ...
    init(parent, title, options, $target) {
        ...
        if (!options.fields.includes('displayName')) {
            options.fields.push('displayName');
        }
    ...
    async start() {
        ...
        if (this.options.createMethod) {
            ...
            this.createWidget = _buildRowElement('', {
                classes: ['o-we-full-row', 'o-we-m2o-create', 'p-1'],
    ...
    async _search(needle) {
        ...
        records.forEach(record => {
            this.displayNameCache[record.id] = record.displayName;
        });
        ...
        await Promise.all(records.slice(0, this.options.limit).map(async record => {
            ...
            const buttonWidget = new ButtonUserValueWidget(this, undefined, {
                ...
                childNodes: [document.createTextNode(record.displayName)],
            }, this.$target);
     ...
     */
    _onClick(ev) {
        // Prevent dropdown from closing if you click on the search or hasMore
        if (ev.target.closest('.o-we-m2o-search-more, .o-we-m2o-search, .o-we-m2o-create') &&
...
const Many2manyUserValueWidget = UserValueWidget.extend({
    ...
    async willStart() {
        ...
        this.m2oName = modelData[m2oField].fieldDescription; // Use as string attr?

        const selectedRecords = await this._rpc({
            model: this.m2oModel,
            method: 'read',
            args: [selectedRecordIds, ['displayName']],
        });
     ...
     */
    async start() {
        this.el.classList.add('o-we-m2m');
...
const SnippetOptionWidget = Widget.extend({
    ...
    events: {
        'click .o-we-collapse-toggler': '_onCollapseTogglerClick',
    },
    customEvents: {
        'userValueUpdate': '_onUserValueUpdate',
        'userValueWidgetCritical': '_onUserValueWidgetCritical',
    },
    /**
    ...
    selectStyle: async function (previewMode, widgetValue, params) {
        ...
        this.$target[0].classList.add('o-we-force-no-transition');
        const _restoreTransitions = () => this.$target[0].classList.remove('o-we-force-no-transition');
    ...
    async selectColorCombination(previewMode, widgetValue, params) {
        if (params.colorNames) {
            ...
            if (widgetValue) {
                this.$target[0].classList.add('o-cc', `o-cc${widgetValue}`);
            }
        }
    },
     ...
     * editors. This can be done thanks to the 'optionUpdate' event, which
    ...
    updateUIVisibility: async function () {
        ...
        for (const el of this.$el.find('we-row')) {
            el.classList.toggle('d-none', !$(el).find('> div > .o-we-user-value-widget').not('.d-none').length);
        }
        for (const el of this.$el.find('we-collapse')) {
            ...
            el.querySelector('.o-we-collapse-toggler').classList.toggle('d-none', hasNoVisibleElInCollapseMenu);
        }
    },
    ...
    _computeWidgetVisibility: async function (widgetName, params) {
        if (widgetName === 'moveUpOpt' || widgetName === 'moveLeftOpt') {
            return !this.$target.is(':first-child');
        }
        if (widgetName === 'moveDownOpt' || widgetName === 'moveRightOpt') {
    ...
    _requestUserValueWidgets: function (...args) {
        ...
        for (const widgetName of widgetNames) {
            let widget = null;
            this.triggerUp('userValueWidgetRequest', {
                name: widgetName,
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.verpEditor.automaticStepUnactive('previewOption');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.verpEditor.automaticStepActive('previewOption');
        }
        ...
        ($applyTo || this.$target).trigger('contentChanged');
    },
    /**
    ...
    _toggleCollapseEl(collapseEl, show) {
        ...
        collapseEl.querySelector('we-toggler.o-we-collapse-toggler').classList.toggle('active', show);
    },
    ...
    _onUserValueUpdate: async function (ev) {
        ...
        this.triggerUp('snippetEditionRequest', {exec: async () => {
            ...
            await new Promise(resolve => setTimeout(() => {
                ...
                this.triggerUp('snippetOptionUpdate', {
                    onSuccess: () => resolve(),
                });
...
registry.sizing = SnippetOptionWidget.extend({
    ...
    start: function () {
        ...
        this.$handles = this.$overlay.find('.o-handle');

        var resizeValues = this._getSize();
        this.$handles.on('mousedown', function (ev) {
            ev.preventDefault();
            self.options.wysiwyg.verpEditor.automaticStepUnactive('resizing');
            ...
            var bodyMouseUp = function () {
                ...
                $handle.removeClass('o-active');

                // Highlights the previews for a while
                var $handlers = self.$overlay.find('.o-handle');
                $handlers.addClass('o-active').delay(300).queue(function () {
                    $handlers.removeClass('o-active').dequeue();
                });
                ...
                self.options.wysiwyg.verpEditor.automaticStepActive('resizing');
            };
    ...
    _onResize: function (compass, beginClass, current) {
        ...
        var resizeValues = this._getSize();
        var $handles = this.$overlay.find('.o-handle');
        _.each(resizeValues, function (resizeValue, direction) {
            ...
            $handle.toggleClass('o-handle-start', current === 0);
            $handle.toggleClass('o-handle-end', current === classes.length - 1);
        });

        // Adapt the handles to fit the left, top and bottom sizes
        var ml = this.$target.css('margin-left');
        this.$overlay.find('.o-handle.w').css({
            width: ml,
            left: '-' + ml,
        });
        this.$overlay.find('.o-handle.e').css({
            width: 0,
        });
        _.each(this.$overlay.find(".o-handle.n, .o-handle.s"), function (handle) {
            var $handle = $(handle);
 ...
 */
registry['sizingY'] = registry.sizing.extend({
...
registry['sizingX'] = registry.sizing.extend({
    /**
     * @override
     */
    onClone: function (options) {
...
registry.Box = SnippetOptionWidget.extend({
    ...
    async _computeWidgetVisibility(widgetName, params) {
        if (widgetName === 'fakeInsetShadowOpt') {
            return false;
        }
        return this._super(...arguments);
    },
...
registry.layoutColumn = SnippetOptionWidget.extend({
    /**
     * @override
     */
    start: function () {
        ...
        // TODO: improve this.
        this.$el.find('we-button[data-name="zeroColsOpt"]')
            .toggleClass('d-none', !this.$target.is('.s-allow-columns'));
        return this._super(...arguments);
    },
    ...
    selectCount: async function (previewMode, widgetValue, params) {
        ...
        // Yield UI thread to wait for event to bubble before activateSnippet is called.
        // In this case this lets the select handle the click event before we switch snippet.
        // TODO: make this more generic in activateSnippet event handler.
        await new Promise(resolve => setTimeout(resolve));
        if (nbColumns === 0) {
            $row.contents().unwrap().contents().unwrap();
            this.triggerUp('activateSnippet', {$snippet: this.$target});
        } else if (previousNbColumns === 0) {
            this.triggerUp('activateSnippet', {$snippet: this.$('> .row').children().first()});
        }
    },
        ...
        if (count > 0) {
            ...
            for (var i = 0; i < count; i++) {
                await new Promise(resolve => {
                    this.triggerUp('cloneSnippet', {$snippet: $lastColumn, onSuccess: resolve});
                });
            }
        } else {
            var self = this;
            for (const el of $row.children().slice(count)) {
                await new Promise(resolve => {
                    self.triggerUp('removeSnippet', {$snippet: $(el), onSuccess: resolve, shouldRecordUndo: false});
                });
            }
        }
...
registry.SnippetMove = SnippetOptionWidget.extend({
    ...
    start: function () {
        ...
        var $overlayArea = this.$overlay.find('.o-overlay-move-options');
    ...
    moveSnippet: function (previewMode, widgetValue, params) {
        ...
        if (!this.$target.is(this.data.noScroll)
                && (params.name === 'moveUpOpt' || params.name === 'moveDownOpt')) {
...
registry.ReplaceMedia = SnippetOptionWidget.extend({
     ...
     */
    onFocus() {
        core.bus.on('activateImageLinkTool', this, this._activateLinkTool);
        core.bus.on('deactivateImageLinkTool', this, this._deactivateLinkTool);
     ...
     */
    onBlur() {
        core.bus.off('activateImageLinkTool', this, this._activateLinkTool);
        core.bus.off('deactivateImageLinkTool', this, this._deactivateLinkTool);
    },
    ...
    setUrl(previewMode, widgetValue, params) {
        ...
        if (!url) {
            ...
            this.$target.trigger('hrefChanged');
            return;
        }
    ...
    _activateLinkTool() {
        if (this.$target[0].parentElement.tagName === 'A') {
            this._requestUserValueWidgets('mediaUrlOpt')[0].focus();
        } else {
            this._requestUserValueWidgets('mediaLinkOpt')[0].enable();
        }
    },
    /**
     * @private
     ...
     */
    async _computeWidgetVisibility(widgetName, params) {
        if (widgetName === 'mediaLinkOpt') {
            if (this.$target[0].matches('img')) {
                return isImageSupportedForStyle(this.$target[0]);
            }
            return !this.$target[0].classList.contains('media-iframe-video');
        }
        return this._super(...arguments);
    },
    /**
    ...
    async _renderCustomXML(uiFragment) {
        ...
        rowEl.insertAdjacentHTML('beforeend', qweb.render('web_editor.mediaLinkToolsButton'));
        rowEl.insertAdjacentHTML('afterend', qweb.render('web_editor.mediaLinkToolsFields'));
    },
});
...
const ImageHandlerOption = SnippetOptionWidget.extend({
    ...
    async start() {
        ...
        weightEl.classList.add('o-we-image-weight', 'o-we-tag', 'd-none');
    ...
    async _renderCustomXML(uiFragment) {
        ...
        const $select = $(uiFragment).find('we-select[data-name=widthSelectOpt]');
    ...
    async _applyOptions(update = true) {
        ...
        if (update) {
            img.classList.add('o-modified-image-to-save');
...
const _addAnimatedShapeLabel = function addAnimatedShapeLabel(containerEl) {
    ...
    labelEl.classList.add('o-we-shape-animated-label');
...
registry.ImageTools = ImageHandlerOption.extend({
     ...
     */
    start() {
        this.$target.on('imageChanged.ImageOptimization', this._onImageChanged.bind(this));
        this.$target.on('imageCropped.ImageOptimization', this._onImageCropped.bind(this));
        return this._super(...arguments);
    },
    /**
     * @override
     ...
     */
    async crop() {
        this.triggerUp('hideOverlay');
        this.triggerUp('disableLoadingEffect');
        new weWidgets.ImageCropWidget(this, this.$target[0]).appendTo(this.options.wysiwyg.verpEditor.document.body);

        await new Promise(resolve => {
            this.$target.one('imageCropperDestroyed', async () => {
                await this._reapplyCurrentShape();
                resolve();
            });
        });
        this.triggerUp('enableLoadingEffect');
    },
    /**
    ...
    async resetCrop() {
        ...
        await cropper.appendTo(this.options.wysiwyg.verpEditor.document.body);
        await cropper.reset();
        await this._reapplyCurrentShape();
    },
    /**
...
￼    */
    _isCropped() {
        return this.$target.hasClass('o-we-image-cropped');
    },
    /**
     * @override
     */
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in Verp).
        } else if (img.closest('.container, .o-container-small')) {
    ...
    _relocateWeightEl() {
        ...
        const weightEl = titleTextEl.querySelector('.o-we-image-weight');
        if (weightEl) {
            weightEl.remove();
        }
...
registry.BackgroundOptimize = ImageHandlerOption.extend({
     ...
     */
    start() {
        this.$target.on('backgroundChanged.BackgroundOptimize', this._onBackgroundChanged.bind(this));
        return this._super(...arguments);
    },
    /**
     * @override
    ...
    async cleanForSave() {
        const img = this._getImg();
        if (img.matches('.o-modified-image-to-save')) {
            this.$target.addClass('o-modified-image-to-save');
...
registry.BackgroundToggler = SnippetOptionWidget.extend({
    ...
    toggleBgImage(previewMode, widgetValue, params) {
        if (!widgetValue) {
            this.$target.find('> .o-we-bg-filter').remove();
            // TODO: use setWidgetValue instead of calling background directly when possible
            const [bgImageWidget] = this._requestUserValueWidgets('bgImageOpt');
            ...
            this._requestUserValueWidgets('bgImageOpt')[0].el.click();
        }
    },
    /**
    ...
    toggleBgShape(previewMode, widgetValue, params) {
        const [shapeWidget] = this._requestUserValueWidgets('bgShapeOpt');
    ...
    async selectFilterColor(previewMode, widgetValue, params) {
        ...
        let filterEl = this.$target[0].querySelector(':scope > .o-we-bg-filter');
        ...
        if (!filterEl) {
            ...
            filterEl.classList.add('o-we-bg-filter');
    ...
    _computeWidgetState(methodName, params) {
        switch (methodName) {
            ...
            case 'selectFilterColor': {
                const filterEl = this.$target[0].querySelector(':scope > .o-we-bg-filter');
                if (!filterEl) {
                    return '';
                }
                const obj = createPropertyProxy(this, '$target', $(filterEl));
...
registry.BackgroundImage = SnippetOptionWidget.extend({
    ...
    background: async function (previewMode, widgetValue, params) {
        ...
        if (previewMode !== 'reset') {
            ...
            this.$target.trigger('backgroundChanged', [previewMode]);
        }
    },
    /**
     ...
     */
    notify(name, data) {
        if (name === 'addSizeIndicator') {
            this._requestUserValueWidgets('bgImageOpt')[0].$el.after(data);
        } else {
            this._super(...arguments);
        }
    },
    ...
    _computeWidgetVisibility(widgetName, params) {
        ...
        } else if (widgetName === 'mainColorOpt') {
            const src = new URL(getBgImageURL(this.$target[0]), window.location.origin);
    ...
    _setBackground(backgroundURL) {
        ...
        if (backgroundURL) {
            ...
            this.$target.addClass('oe-img-bg o-bg-img-center');
        } else {
            delete parts.url;
            this.$target.removeClass('oe-img-bg o-bg-img-center');
        }
        const combined = backgroundImagePartsToCss(parts);
        this.$target.css('background-image', combined);
    },
...
registry.BackgroundShape = SnippetOptionWidget.extend({
    ...
    _computeWidgetState(methodName, params) {
        switch (methodName) {
            ...
            case 'flipX': {
                ...
                const hasFlipClass = this.$target.find('> .o-we-shape.o-we-flip-x').length !== 0;
                return hasFlipClass || this._getShapeData().flip.includes('x');
            }
            case 'flipY': {
                // Compat: flip classes are no longer used but may be present in client db
                const hasFlipClass = this.$target.find('> .o-we-shape.o-we-flip-y').length !== 0;
    ...
    _renderCustomXML(uiFragment) {
        ...
        uiFragment.querySelectorAll('we-select-pager we-button[data-shape]').forEach(btn => {
            ...
            btnContent.classList.add('o-we-shape-btn-content', 'position-relative', 'border-dark');
            const btnContentInnerDiv = document.createElement('div');
            btnContentInnerDiv.classList.add('o-we-shape');
            ...
            const shapeEl = btnContent.querySelector('.o-we-shape');
            shapeEl.classList.add(`o-${shape.replace(/\//g, '-')}`);
    ...
    _insertShapeContainer(newContainer) {
        ...
        const shapeContainer = target.querySelector(':scope > .o-we-shape');
        if (shapeContainer) {
    ...
    _createShapeContainer(shape) {
        ...
        shapeContainer.className = `o-we-shape o-${shape.replace(/\//g, '-')}`;
    ...
    _handlePreviewState(previewMode, computeShapeData) {
        ...
        let shapeContainer = target.querySelector(':scope > .o-we-shape');
        if (!shape) {
        ...
        shapeContainer.classList.remove('o-we-flip-x', 'o-we-flip-y');
    ...
    _getLastPreShapeLayerElement() {
        const $filterEl = this.$target.find('> .o-we-bg-filter');
        if ($filterEl.length) {
            return $filterEl[0];
        }
        return null;
    ...
    _getDefaultColors($target = this.$target) {
        const $shapeContainer = $target.find('> .o-we-shape')
            .clone()
            .addClass('d-none')
    ...
    _getShapeDefaultColors(shapeId) {
        const $shapeContainer = this.$el.find(".o-we-shape-menu we-button[data-shape='" + shapeId + "'] div.o-we-shape");
        const shapeContainer = $shapeContainer[0];
        const shapeSrc = shapeContainer && getBgImageURL(shapeContainer);
        const url = new URL(shapeSrc, window.location.origin);
        return Object.fromEntries(url.searchParams.entries());
    ...
    _toggleShape() {
        if (this._getShapeData().shape) {
            ...
            this.triggerUp('snippetEditionRequest', {exec: () => {
                ...
                this._requestUserValueWidgets('bgShapeOpt')[0].enable();
            }});
...
registry.BackgroundPosition = SnippetOptionWidget.extend({
    ...
    backgroundType: function (previewMode, widgetValue, params) {
        this.$target.toggleClass('o-bg-img-opt-repeat', widgetValue === 'repeat-pattern');
    ...
    selectStyle: function (previewMode, widgetValue, params) {
        if (params.cssProperty === 'background-size'
                && !this.$target.hasClass('o-bg-img-opt-repeat')) {
    ...
    _initOverlay: function () {
        this.$backgroundOverlay = $(qweb.render('web_editor.backgroundPositionOverlay'));
        this.$overlayContent = this.$backgroundOverlay.find('.o-we-overlay-content');
        this.$overlayBackground = this.$overlayContent.find('.o-overlay-background');

        this.$backgroundOverlay.on('click', '.o-btn-apply', () => {
            this.$target.css('background-position', this.$bgDragger.css('background-position'));
            this._toggleBgOverlay(false);
        });
        this.$backgroundOverlay.on('click', '.o-btn-discard', () => {
            this._toggleBgOverlay(false);
        });
    ...
    _dimensionOverlay: function () {
        if (!this.$backgroundOverlay.is('.oe-active')) {
            return;
        }
        ...
        this.$overlayContent.find('.o-we-overlay-buttons').css('top', `${topPos}px`);
    },
    /**
    ...
    _toggleBgOverlay: function (activate) {
        if (!this.$backgroundOverlay || this.$backgroundOverlay.is('.oe-active') === activate) {
            return;
        }

        if (!activate) {
            this.$backgroundOverlay.removeClass('oe-active');
            this.triggerUp('unblockPreviewOverlays');
            this.triggerUp('activateSnippet', {$snippet: this.$target});
        ...
        this.triggerUp('blockPreviewOverlays');
        ...
        this.$bgDragger.removeClass('o-editable');
        ...
        this.$backgroundOverlay.addClass('oe-active');
    ...
    _onDocumentClicked: function (ev) {
        if (!$(ev.target).closest('.o-we-background-position-overlay').length) {
            this._toggleBgOverlay(false);
        }
    },
});
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.verpEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o-colored-level');
        this.options.wysiwyg.verpEditor.observerActive('_markColorLevel');
    },
});
...
registry.ContainerWidth = SnippetOptionWidget.extend({
    ...
    cleanForSave: function () {
        this.$target.removeClass('o-container-preview');
    },
    ...
    selectClass: async function (previewMode, widgetValue, params) {
        ...
        if (previewMode === 'reset') {
            ...
            this.$target.addClass('o-container-preview');
        }
    },
});
...
registry.many2one = SnippetOptionWidget.extend({
    ...
    async _rerenderContacts(contactId, defaultText) {
        ...
        // copy the HTML of the current m2o on contentChanged. Not sure why we
        // only do this for contacts, or why we do this here instead of in the
        // wysiwyg like we do for replacing text on contentChanged
        const selector = [
...
registry.VersionControl = SnippetOptionWidget.extend({
     ...
     */
    start: function () {
        this.triggerUp('getSnippetVersions', {
            ...
            onSuccess: snippetVersions => {
                ...
                if (!isUpToDate) {
                    this.$el.prepend(qweb.render('web_editor.outdatedBlockMessage'));
                }
            },
        });
        return this._super(...arguments);
                ...
                buttons: [
                    {
                        ...
                        click: () => {
                            ...
                            let thumbnailURL;
                            this.triggerUp('snippetThumbnailUrlRequest', {
                                key: snippetKey,
                            ...
                            this.triggerUp('requestSave', {
                                ...
                                onSuccess: async () => {
                                    ...
                                    // By the time onSuccess is called after requestSave, the
                                    // current widget has been destroyed and is orphaned, so this._rpc
                                    // will not work as it can't triggerUp. For this reason, we need
                                    // to bypass the service provider and use the global RPC directly
                                    await rpc.query({
                                        model: 'ir.ui.view',
                                        method: 'saveSnippet',
                                        kwargs: {
                                            'label': defaultSnippetName,
...
registry.DynamicSvg = SnippetOptionWidget.extend({
     ...
     */
    start() {
        this.$target.on('imageChanged.DynamicSvg', this._onImageChanged.bind(this));
        return this._super(...arguments);
    },
    /**
     * @override
...
registry.SelectTemplate = SnippetOptionWidget.extend({
    customEvents: Object.assign({}, SnippetOptionWidget.prototype.customEvents, {
        'userValueWidgetOpening': '_onWidgetOpening',
    }),
