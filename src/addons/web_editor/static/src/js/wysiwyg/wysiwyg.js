verp.define('web_editor.wysiwyg', function (require) {
'use strict';
...
const customColors = require('web_editor.customColors');
...
const VerpEditorLib = require('@web_editor/../lib/verp-editor/src/VerpEditor');
...
const VerpEditor = VerpEditorLib.VerpEditor;
const getDeepRange = VerpEditorLib.getDeepRange;
const getInSelection = VerpEditorLib.getInSelection;
const isBlock = VerpEditorLib.isBlock;
const rgbToHex = VerpEditorLib.rgbToHex;
const preserveCursor = VerpEditorLib.preserveCursor;
const closestElement = VerpEditorLib.closestElement;
const setSelection = VerpEditorLib.setSelection;
const endPos = VerpEditorLib.endPos;

var id = 0;
const faZoomClassRegex = RegExp('fa-[0-9]x');
const basicMediaSelector = 'img, .fa, .o-image, .media-iframe-video';
...
const Wysiwyg = Widget.extend({
    ...
    start: async function () {
        ...
        this.options.isInternalUser = await session.userHasGroup('base.groupUser');
        ...
        this.$editable.data('oe-model', options.recordInfo.resModel);
        this.$editable.data('oe-id', options.recordInfo.resId);
        ...
        if (
            ...
            this.getSession()['notificationType']
        ) {
        ...
        this.verpEditor = new VerpEditor(this.$editable[0], Object.assign({
            _t: _t,
            ...
            filterMutationRecords: (records) => {
                return records.filter((record) => {
                    return !(record.target.classList && record.target.classList.contains('o-header-standard'));
                });
            },
            preHistoryUndo: () => {
                if (this.linkTools) {
            ...
            renderingClasses: ['o-dirty', 'o-transform-removal', 'oe-edited-link'],
        ...
        if (this.verpEditor.document !== document) {
            this.verpEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.verpEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.verpEditor.document.addEventListener("keyup", this._signalOnline, true);
        }
        this.verpEditor.addEventListener('contentChanged', function () {
            self.$editable.trigger('contentChanged');
            self.triggerUp('wysiwygChange');
        });

        this._initialValue = this.getValue();
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $wrapwrap[0].addEventListener('scroll', this.verpEditor.multiselectionRefresh, { passive: true });
        ...
        if (this._peerToPeerLoading) {
            ...
            // ready to join.
            this._peerToPeerLoading.then(() => this.ptp.notifyAllClients('ptpJoin'));
        }

        this._observeVerpFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.verpEditor.observerUnactive();
                const $field = $(this);
                if (($field.data('oe-type') === "datetime" || $field.data('oe-type') === "date")) {
                    ...
                    $linkedFieldNodes.addClass('o-editable-date-field-linked');
                    if (!$field.hasClass('o-editable-date-field-format-changed')) {
                        $linkedFieldNodes.text($field.data('oe-original-with-format'));
                        $linkedFieldNodes.addClass('o-editable-date-field-format-changed');
                        $linkedFieldNodes.filter('.oe-hide-on-date-edit').addClass('d-none');
                        setTimeout(() => {
                            // we might hide the clicked date, focus the one
                            // supposed to be editable
                            Wysiwyg.setRange($linkedFieldNodes.filter(':not(.oe-hide-on-date-edit)')[0]);
                        }, 0);
                    }
                }
                if ($field.attr('contenteditable') !== 'false') {
                    if ($field.data('oe-type') === "monetary") {
                        ...
                        const $currencyValue = $field.find('.oe-currency-value');
                ...
                self.verpEditor.observerActive();
            }
        );

        this.$editable.on('click', '.o-image, .media-iframe-video', e => e.preventDefault());
        ...
        if (options.snippets) {
            $(this.verpEditor.document.body).addClass('editor-enable');
        ...
        $(this.verpEditor.editable).on('click', this._updateEditorUI.bind(this));
        $(this.verpEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.verpEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.$root.on('click', (ev) => {
            ...
            if ($(ev.target).parents('.o-edit-menu-popover').length && !$target.length) {
                ev.preventDefault();
            }
            ...
            if ($target.is(this.customizableLinksSelector)
                    ...
                    && !$target[0].closest('.o-extra-menu-items')
                    && $target[0].isContentEditable) {
        ...
        this.verpEditor.document.addEventListener('selectionchange', this._onSelectionChange);

        this.verpEditor.addEventListener('preObserverActive', () => {
            // The setONotEditable will be called right after the
            // editor sanitization (to be right before the historyStep).
            // If any `.o-not-editable` is created while the observer is
            // unactive, now is the time to call `setONotEditable` before the
            // editor could register a mutation.
            this._setONotEditable(this.verpEditor.editable);
        });

        return _super.apply(this, arguments).then(() => {
            if (this.options.autohideToolbar) {
                if (this.verpEditor.isMobile) {
                    $(this.verpEditor.editable).before(this.toolbar.$el);
                } else {
    ...
    setupCollaboration(collaborationChannel) {
        ...
        const channelName = `editorCollaboration:${modelName}:${fieldName}:${resId}`;
        ...
        this.call('busService', 'onNotification', this, (notifications) => {
        ...
        this.call('busService', 'addChannel', this._collaborationChannelName);
        this.call('busService', 'startPolling');
        ...
        this._getRecordWriteDate = (record) => {
            const dateString = record.updatedAt.replace(/^(\d{4}-\d{2}-\d{2}) ((\d{2}:?){3})$/, '$1T$2Z');
        ...
        this._getNewPtp = () => {
            ...
            return new PeerToPeer({
                ...
                onRequest: {
                    getStartTime: () => this._startCollaborationTime,
                    getClientName: async () => {
                        if (!this._userName) {
                    ...
                    getMissingSteps: (params) => this.verpEditor.historyGetMissingSteps(params.requestPayload),
                    getHistoryFromSnapshot: () => this.verpEditor.historyGetSnapshotSteps(),
                    getCollaborativeSelection: () => this.verpEditor.getCurrentCollaborativeSelection(),
                },
                onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                    switch (notificationName) {
                        case 'ptpRemove':
                            this.verpEditor.multiselectionRemove(notificationPayload);
                            break;
                        case 'rtcSignalDescription':
                        ...
                        case 'ptpDisconnect':
                            ...
                            this.verpEditor.multiselectionRemove(fromClientId);
                            break;
                        case 'rtcDataChannelOpen': {
                            fromClientId = notificationPayload.connectionClientId;
                            this.ptp.clientsInfos[fromClientId].startTime = await this.ptp.requestClient(fromClientId, 'getStartTime', undefined, { transport: 'rtc' });
                            this.ptp.requestClient(fromClientId, 'getClientName', undefined, { transport: 'rtc' }).then((clientName) => {
                            ...
                            if (!historySyncAtLeastOnce) {
                                ...
                                const remoteSelection = await this.ptp.requestClient(fromClientId, 'getCollaborativeSelection', undefined, { transport: 'rtc' });
                                if (remoteSelection) {
                                    this.verpEditor.onExternalMultiselectionUpdate(remoteSelection);
                                }
                            }
                            break;
                        }
                        case 'oeHistoryStep':
                            ...
                            if (historySyncFinished) {
                                this.verpEditor.onExternalHistorySteps([notificationPayload]);
                            }
                            break;
                        case 'oeHistorySetSelection': {
                            ...
                            this.verpEditor.onExternalMultiselectionUpdate(selection);
                            break;
                        }
                    }
                }
        ...
        this._peerToPeerLoading = new Promise(async (resolve) => {
            ...
            let iceServers = await this._rpc({route: '/web_editor/getIceServers'});
        ...
        const editorCollaborationOptions = {
            ...
            onHistoryStep: (historyStep) => {
                if (!this.ptp) return;
                this.ptp.notifyAllClients('oeHistoryStep', historyStep, { transport: 'rtc' });
            },
            onCollaborativeSelectionChange: _.throttle((collaborativeSelection) => {
                if (!this.ptp) return;
                this.ptp.notifyAllClients('oeHistorySetSelection', collaborativeSelection, { transport: 'rtc' });
            }, 50),
            onHistoryMissingParentSteps: async ({ step, fromStepId }) => {
                ...
                if (missingSteps === -1 || !missingSteps.length) {
                    ...
                    console.warn('Editor getMissingSteps result is erroneous.');
                    return;
                }
                this.ptp && this.verpEditor.onExternalHistorySteps(missingSteps.concat([step]));
            },
            onHistoryNeedSync: async () => {
                ...
                if (firstClientId !== this._currentClientId) {
                    const historySteps = await this.ptp.requestClient(firstClientId, 'getHistoryFromSnapshot', undefined, { transport: 'rtc' });
                    this.verpEditor.historyResetFromSteps(historySteps);
                    const remoteSelection = await this.ptp.requestClient(firstClientId, 'getCollaborativeSelection', undefined, { transport: 'rtc' });
                    if (remoteSelection) {
                        this.verpEditor.onExternalMultiselectionUpdate(remoteSelection);
                    }
                }
    ...
    destroy: function () {
        ...
        if (this.verpEditor) {
            this.verpEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.verpEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.verpEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.verpEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            for (const observer of this._oNotEditableObservers.values()) {
                observer.disconnect();
            }
            this.verpEditor.destroy();
        }
        // If peer to peer is initializing, wait for properly closing it.
        if (this._peerToPeerLoading) {
            this._peerToPeerLoading.then(()=> {
                this.call('busService', 'deleteChannel', this._collaborationChannelName);
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.verpEditor.multiselectionRefresh, { passive: true });
        }
    ...
    renderElement: function () {
        ...
        if (this.options.resizable && !device.isMobile) {
            const $wrapper = $('<div class="o-wysiwyg-wrapper verp-editor">');
            this.$root = $wrapper;
            $wrapper.append(this.$editable);
            this.$resizer = $(`<div class="o-wysiwyg-resizer">
                <div class="o-wysiwyg-resizer-hook"></div>
                <div class="o-wysiwyg-resizer-hook"></div>
                <div class="o-wysiwyg-resizer-hook"></div>
            </div>`);
    ...
    saveToServer: async function (reload = true) {
        const defs = [];
        if (!this.__editionWillStoppedAlreadyDone) {
            // TODO remove in master
            this.triggerUp('editionWillStopped');
        }
        this.triggerUp('readyToSave', {defs: defs});
        ...
        this.triggerUp('editionWasStopped');
        window.removeEventListener('beforeunload', this._onBeforeUnload);
        if (reload) {
    ...
    cancel: function (reload) {
        ...
        return new Promise((resolve, reject) => {
            if (!this.verpEditor.historySize().length) {
                resolve();
            } else {
    ...
    saveModifiedImages: function ($editable = this.$editable) {
        const defs = _.map($editable, async editableEl => {
            ...
            const proms = [...editableEl.querySelectorAll('.o-modified-image-to-save')].map(async el => {
                const isBackground = !el.matches('img');
                el.classList.remove('o-modified-image-to-save');
                // Modifying an image always creates a copy of the original, even if
                // it was modified previously, as the other modified image may be used
                // elsewhere if the snippet was duplicated or was saved as a custom one.
                const newAttachmentSrc = await this._rpc({
                    route: `/web_editor/modifyImage/${el.dataset.originalId}`,
                    params: {
                        resModel: resModel,
                        resId: parseInt(resId),
                        data: (isBackground ? el.dataset.bgSrc : el.getAttribute('src')).split(',')[1],
                        mimetype: el.dataset.mimetype,
                        label: (el.dataset.fileName ? el.dataset.fileName : null),
                    },
                });
                if (isBackground) {
    ...
    setValue: function (value) {
        ...
        this.verpEditor.sanitize();
        this.verpEditor.historyStep(true);
    },
    /**
     * Undo one step of change in the editor.
     */
    undo: function () {
        this.verpEditor.historyUndo();
    },
    /**
     * Redo one step of change in the editor.
     */
    redo: function () {
        this.verpEditor.historyRedo();
    },
    /**
     ...
     */
    focus: function () {
        if (this.verpEditor && !this.verpEditor.historyResetLatestComputedSelection()) {
            ...
            const selection = this.verpEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    },
    getDeepRange() {
        return getDeepRange(this.verpEditor.editable);
    },
    closestElement(...args) {
        return closestElement(...args);
    },
    isSelectionInEditable: function () {
        return this.verpEditor.isSelectionInEditable();
    },
    cleanForSave: async function () {
        this.verpEditor.clean();
        this.$editable.find('.oe-edited-link').removeClass('oe-edited-link');
    ...
    /**
     * Start or resume the Verp field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeVerpFieldChanges: function () {
        ...
        if (this.verpFieldObservers) {
            for (let observerData of this.verpFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const verpFieldSelector = '[data-oe-model], [data-oe-translation-id]';
            const $verpFields = this.$editable.find(verpFieldSelector);
            this.verpFieldObservers = [];

            $verpFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.verpEditor.filterMutationRecords(mutations);
                    if (!mutations.length) {
                        return;
                    }
                    let $node = $(field);
                    let $nodes = $verpFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    this._pauseVerpFieldObservers();
                    // Tag the date fields to only replace the value
                    // with the original date value once (see mouseDown event)
                    if ($node.hasClass('o-editable-date-field-format-changed')) {
                        $nodes.addClass('o-editable-date-field-format-changed');
                    }
                ...
                this.verpFieldObservers.push({field: field, observer: observer});
            });
        }
    },
    /**
     ...
     */
    _pauseVerpFieldObservers: function () {
        for (let observerData of this.verpFieldObservers) {
    ...
    openLinkToolsFromSelection() {
        const targetEl = this.verpEditor.document.getSelection().getRangeAt(0).startContainer;
        ...
        if (targetEl instanceof HTMLElement
            ...
            core.bus.trigger('activateImageLinkTool');
            return;
        }
        this.toggleLinkTools();
    },
    ...
    toggleLinkTools(options = {}) {
        const linkEl = getInSelection(this.verpEditor.document, 'a');
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.linkTools) {
                const $btn = this.toolbar.$el.find('#createLink');
                if (!this.linkTools || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkTools.$link[0])) {
                    const { link } = Link.getOrCreateLink({
                        containerNode: this.verpEditor.editable,
                    ...
                    this.linkTools = new weWidgets.LinkTools(this, {wysiwyg: this, noFocusUrl: options.noFocusUrl}, this.verpEditor.editable, linkToolsData, $btn, link );
                }
                this.linkTools.noFocusUrl = options.noFocusUrl;
                const _onClick = ev => {
                    if (
                        !ev.target.closest('#createLink') &&
                        ...
                        this.verpEditor.document.removeEventListener('click', _onClick, true);
                    }
                };
                this.verpEditor.document.addEventListener('click', _onClick, true);
            ...
            const restoreSelection = preserveCursor(this.verpEditor.document);
            linkDialog.open();
            linkDialog.on('save', this, data => {
                ...
                this.verpEditor.historyStep();
                link = linkWidget.$link[0];
                this.verpEditor.setContenteditableLink(linkWidget.$link[0]);
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.snippetsMenu._mutex.exec(() => {
                // Wait for the editor panel to be fully updated.
                core.bus.trigger('deactivateImageLinkTool');
            });
        } else {
            this.verpEditor.execCommand('unlink');
        }
    },
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.verpEditor.document.getSelection();
        const fontawesomeIcon = getInSelection(this.verpEditor.document, '.fa');
        ...
        const $editable = $(VerpEditorLib.closestElement(range.startContainer, '.o-editable'));
        ...
        mediaDialog.on('save', this, function (element) {
            ...
            if (wasImageOrVideo || wasFontAwesome) {
                ...
                this.verpEditor.unbreakableStepUnactive();
                this.verpEditor.historyStep();
            } else if (element) {
                this.verpEditor.execCommand('insertHTML', element.outerHTML);
            }
        });
    ...
    getInSelection(selector) {
        return getInSelection(this.verpEditor.document, selector);
    },
    ...
    _createSnippetsMenuInstance: function (options={}) {
        return new snippetsEditor.SnippetsMenu(this, Object.assign({
            wysiwyg: this,
            selectorEditableArea: '.o-editable',
        }, options));
    },
    _configureToolbar: function (options) {
        ...
        if (!this.options.snippets) {
            $toolbar.find('#justify, #table, #mediaInsert').remove();
        }
        $toolbar.find('#mediaInsert, #mediaReplace, #mediaDescription').click(openTools);
        $toolbar.find('#createLink').click(openTools);
        $toolbar.find('#imageShape div, #faSpin').click(e => {
        ...
        const $imageWidthButtons = $toolbar.find('#imageWidth div');
        ...
        $toolbar.find('#imagePadding .dropdown-item').click(e => {
            if (!this.lastMediaClicked) {
                return;
            }
        ...
        $toolbar.find('#imageCrop').click(e => {
            if (!this.lastMediaClicked) {
                return;
            }
            new weWidgets.ImageCropWidget(this, this.lastMediaClicked).appendTo(this.verpEditor.document.body);
            this.verpEditor.toolbarHide();
            $(this.lastMediaClicked).on('imageCropperDestroyed', () => this.verpEditor.toolbarShow());
        });
        $toolbar.find('#imageTransform').click(e => {
            if (!this.lastMediaClicked) {
                return;
            }
            ...
            $image.transfo({document: this.verpEditor.document});
            const mouseup = () => {
                $('#imageTransform').toggleClass('active', $image.is('[style*="transform"]'));
            };
            $(this.verpEditor.document).on('mouseup', mouseup);
            const mousedown = mousedownEvent => {
                if (!$(mousedownEvent.target).closest('.transfo-container').length) {
                    $image.transfo('destroy');
                    $(this.verpEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                }
                if ($(mousedownEvent.target).closest('#imageTransform').length) {
                    $image.data('transfo-destroy', true).attr('style', ($image.attr('style') || '').replace(/[^;]*transform[\w:]*;?/g, ''));
                }
                $image.trigger('contentChanged');
            };
            $(this.verpEditor.document).on('mousedown', mousedown);
        });
        $toolbar.find('#imageDelete').click(e => {
            if (!this.lastMediaClicked) {
                return;
            }
        ...
        $toolbar.find('#faResize div').click(e => {
            if (!this.lastMediaClicked) {
                return;
            }
        ...
        setTimeout(() => {
            ...
            if (!options.snippets && scrollableContainer.length) {
                this.verpEditor.addDomListener(
                    scrollableContainer[0],
                    'scroll',
                    this.verpEditor.updateToolbarPosition.bind(this.verpEditor),
                );
            }
        }, 0);
    },
        ...
        for (const elem of $dropdownContent) {
            ...
            $dropdown.on('show.bs.dropdown', () => {
                ...
                mutex.exec(() => {
                    ...
                    // The colorLeave event will revert the mutations with
                    // `historyRevertCurrentStep`. We must stash the current
                    // mutations to prevent them from being reverted.
                    this.verpEditor.historyStash();
                    colorpicker = new ColorPaletteWidget(this, {
                        excluded: ['transparentGrayscale'],
                        $editable: $(this.verpEditor.editable), // Our parent is the root widget, we can't retrieve the editable section from it...
                        selectedColor: selectedColor,
                        selectedTab: weUtils.isColorGradient(selectedColor) ? 'gradients' : 'theme-colors',
                        withGradients: true,
                    });
                    this.colorpickers[eventName] = colorpicker;
                    colorpicker.on('customColorPicked colorPicked', null, ev => {
                        if (hadNonCollapsedSelection) {
                            this.verpEditor.historyResetLatestComputedSelection(true);
                        }
                        // Unstash the mutations now that the color is picked.
                        this.verpEditor.historyUnstash();
                        this._processAndApplyColor(eventName, ev.data.color);
                        this._updateEditorUI();
                    });
                    colorpicker.on('colorHover', null, ev => {
                        if (hadNonCollapsedSelection) {
                            this.verpEditor.historyResetLatestComputedSelection(true);
                        }
                        this.verpEditor.historyPauseSteps();
                        try {
                            this._processAndApplyColor(eventName, ev.data.color);
                        } finally {
                            this.verpEditor.historyUnpauseSteps();
                        }
                    });
                    colorpicker.on('colorLeave', null, ev => {
                        this.verpEditor.historyRevertCurrentStep();
                    });
                    colorpicker.on('enterKeyColorColorpicker', null, () => {
    ...
    _processAndApplyColor: function (eventName, color) {
        ...
        const fonts = this.verpEditor.execCommand('applyColor', color, eventName === 'foreColor' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        ...
        this.verpEditor.updateColorpickerLabels({
    ...
    _handleShortcuts: function (e) {
        ...
        if (e && e.key === 'a' && (e.ctrlKey || e.metaKey)) {
            e.preventDefault();
            const selection = this.verpEditor.document.getSelection();
            const containerSelector = '#wrap>*, [contenteditable], .oe-structure>*';
            let $deepestParent =
                selection ?
    ...
    _updateEditorUI: function (e) {
        ...
        this.toolbar.$el.find('#createLink').toggleClass('d-none', !range || spansBlocks);
        ...
        if (this.snippetsMenu && $target.is('img')) {
            this.toolbar.$el.find('#mediaReplace').toggleClass('d-none', true);
        }
        // Only show the imageTransform, image-crop and media-description
        ...
        this.toolbar.$el.find([
            ...
            '#table',
            '#createLink',
            '#mediaInsert', // "Insert media" should be replaced with "Replace media".
        ].join(',')).toggleClass('d-none', isInMedia);
        // Some icons are relevant for icons, that aren't for other media.
        this.toolbar.$el.find('#colorInputButtonGroup, #createLink').toggleClass('d-none', isInMedia && !$target.is('.fa'));
        this.toolbar.$el.find('.only-fa').toggleClass('d-none', !$target.is('.fa'));
        // Toggle the toolbar arrow.
        this.toolbar.$el.toggleClass('noarrow', isInMedia);
        // Unselect all media.
        this.$editable.find('.o-we-selected-image').removeClass('o-we-selected-image');
        if (isInMedia) {
            this.verpEditor.automaticStepSkipStack();
            // Select the media in the DOM.
            const selection = this.verpEditor.document.getSelection();
            const range = this.verpEditor.document.createRange();
            ...
            for (const button of this.toolbar.$el.find('#imageShape div, #faSpin')) {
                button.classList.toggle('active', $(e.target).hasClass(button.id));
            }
            for (const button of this.toolbar.$el.find('#imageWidth div')) {
                button.classList.toggle('active', e.target.style.width === button.id);
            }
            this._updateMediaJustifyButton();
            this._updateFaResizeButtons();
        }
        const link = getInSelection(this.verpEditor.document, this.customizableLinksSelector);
        if (isInMedia || (link && link.isContentEditable)) {
            ...
            setTimeout(() => {
                ...
                this.verpEditor.observerActive();
    ...
    _updateMediaJustifyButton: function (commandState) {
        ...
        // Change the ID to prevent VerpEditor from controlling it as this is
    ...
    _updateFaResizeButtons: function () {
        ...
        const $buttons = this.toolbar.$el.find('#faResize div');
        ...
        const commands = [
            {
                ...
                callback: () => {
                    this.verpEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                groupName: _t('Basic blocks'),
                ...
                fontawesome: 'fa-code',
                callback: () => {
                    this.verpEditor.execCommand('setTag', 'pre');
                },
            },
            {
            ...
            },
            {
                ...
                callback: () => {
                    ...
                    setTimeout(() => {
                        $(".o-link-dialog .link-style[value=primary]").click();
                    }, 150);
                },
            },
        ];
    ...
    editable: function () {
        return $('#wrapwrap [data-oe-model]')
            .not('.o-not-editable')
            .filter(function () {
                return !$(this).closest('.o-not-editable').length;
            })
            .not('link, script')
    ...
    _saveViewBlocks: function (context) {
        ...
        const $allBlocks = $((this.options || {}).savableSelector).filter('.o-dirty');

        const $dirty = $('.o-dirty');
        $dirty
            .removeAttr('contentEditable')
            .removeClass('o-dirty oe-carlos-danger o-is-inline-editable');

        $('.o-editable')
            .removeClass('o-editable o-is-inline-editable o-editable-date-field-linked o-editable-date-field-format-changed');
        ...
        const defs = _.map($allBlocks, (el) => {
            ...
            return this.savingMutex.exec(() => {
                ...
                .then(function () {
                    ...
                    var id = _.uniqueId('carlos-danger-');
                    $el.addClass('o-dirty o-editable oe-carlos-danger ' + id);
                    $('.o-editable.' + id)
                        .removeClass(id)
                        .popover({
                            trigger: 'hover',
    ...
    _getEscapedElement: function ($el) {
        var escapedElem = $el.clone();
        var toEscape = escapedElem.find('*').addBack();
    ...
    _saveElement: function ($el, context, withLang) {
        ...
        // remove ZeroWidthSpace from verp field value
        // ZeroWidthSpace may be present from VerpEditor edition process
    ...
    _reload: function () {
        ...
        if (window.location.search.indexOf('enableEditor') >= 0) {
            window.location.href = window.location.href.replace(/&?enableEditor(=[^&]*)?/g, '');
        } else {
    ...
    _onSelectionChange() {
        if (this.options.autohideToolbar) {
            ...
            if (isVisible && !this.verpEditor.document.getSelection().isCollapsed) {
                this.linkPopover.hide();
            }
        }
    },
    _onDocumentMousedown: function (e) {
        if (!e.target.classList.contains('o-editable-date-field-linked')) {
            this.$editable.find('.o-editable-date-field-linked').removeClass('o-editable-date-field-linked');
        }
        if (e.target.closest('.oe-toolbar')) {
            this._onToolbar = true;
        } else {
            if (this._pendingBlur && !e.target.closest('.o-wysiwyg-wrapper')) {
                this.triggerUp('wysiwygBlur');
    ...
    _signalOnline: async function () {
        ...
        try {
            ...
            if (currentContent !== dbContent && dbRecordDate !== currentRecordDate) {
                ...
                $dialogContent.append($(this.verpEditor.editable).clone());
    ...
    _resetEditor: function (value) {
        ...
        this._startCollaborationTime = new Date().getTime();
        this.ptp = this._getNewPtp();
        this.verpEditor.collaborationSetClientId(this._currentClientId);
        this.setValue(value);
        this.verpEditor.historyReset();
        this.ptp.notifyAllClients('ptpJoin');
    },
    /**
     * Set contenteditable=false for all `.o-not-editable` found within node if
     * node is an element.
     *
     * For all `.o-not-editable` element found, the attribute contenteditable
     * will be removed if the class is removed.
     *
     * @param {Node} node
     */
    _setONotEditable: function (node) {
        const nodes = (node && node.querySelectorAll && node.querySelectorAll('.o-not-editable:not([contenteditable=false])')) || [];
        for (const node of nodes) {
                ...
                observer = new MutationObserver((records) => {
                    for (const record of records) {
                        if (record.type === 'attributes' && record.attributeName === 'class') {
                            ...
                            // previsouly o-not-editable but are no longer
                            // o-not-editable.
                            if (!node.classList.contains('o-not-editable')) {
                                this.verpEditor.observerUnactive('_setONotEditable');
                                node.removeAttribute('contenteditable');
                                this.verpEditor.observerActive('_setONotEditable');
                                observer.disconnect();
...
verp.define('web_editor.widget', function (require) {
'use strict';
    return {
