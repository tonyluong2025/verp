verp.define('wysiwyg.widgets.MediaDialog', function (require) {
'use strict';
...
var MediaDialog = Dialog.extend({
    ...
    events: _.extend({}, Dialog.prototype.events, {
        'click #editorMediaImage-tab': '_onClickImageTab',
        'click #editorMediaDocument-tab': '_onClickDocumentTab',
        'click #editorMediaIconTab': '_onClickIconTab',
        'click #editorMediaVideoTab': '_onClickVideoTab',
    }),
    customEvents: _.extend({}, Dialog.prototype.customEvents || {}, {
        saveRequest: '_onSaveRequest',
        showParentDialogRequest: '_onShowRequest',
        hideParentDialogRequest: '_onHideRequest',
    }),
    ...
    init: function (parent, options, media) {
        ...
        } else if (this.documentWidget && $media.is('a.o-image')) {
            this.activeWidget = this.documentWidget;
        } else if (this.videoWidget && $media.is('.media-iframe-video, .o-bg-video-iframe')) {
    ...
    start: function () {
        ...
        this.$modal.find('.modal-dialog').addClass('o-select-media-dialog');

        if (this.imageWidget) {
            promises.push(this.imageWidget.appendTo(this.$("#editorMediaImage")));
        }
        if (this.documentWidget) {
            promises.push(this.documentWidget.appendTo(this.$("#editorMediaDocument")));
        }
        if (this.iconWidget) {
            promises.push(this.iconWidget.appendTo(this.$("#editorMediaIcon")));
        }
        if (this.videoWidget) {
            promises.push(this.videoWidget.appendTo(this.$("#editorMediaVideo")));
        }

        this.opened(() => this.$('input.o-we-search:visible:first').focus());
     ...
     *
     * The save event data `finalData` will be one Element in general, but it
    ...
    save: function () {
        ...
        return this.activeWidget.save().then(function (data) {
            ...
            $(data).trigger('contentChanged');
            $(data).trigger('replaceTarget', data);
        });
    },
