verp.define('wysiwyg.widgets.Link', function (require) {
'use strict';
...
const Link = Widget.extend({
    ...
    init: function (parent, options, editable, data, $button, link) {
        ...
        if (this.data.range) {
            this.$link = this.$link || $(VerpEditorLib.getInSelection(this.editable.ownerDocument, 'a'));
            ...
            while ($node.length && !$node.is('body')) {
                ...
                const m = className.match(/\b(o-cc\d+)\b/g);
                if (m) {
        ...
        if (!this.data.url) {
            const urls = this.data.content.match(VerpEditorLib.URL_REGEX_WITH_INFOS);
            if (urls) {
        ...
        // 'o-submit' class will force anchor to be handled as a button in linkdialog.
        if (/(?:s-website-form-send|o-submit)/.test(this.data.className)) {
            this.isButton = true;
        }
    },
    /**
    ...
    applyLinkToDom: function (data) {
        ...
        if (typeof data.classes === "string") {
            data.classes = data.classes.replace(/o-default-snippet-text/, '');
    ...
    _onURLInput: function () {
        ...
        var $linkUrlInput = this.$('#oLinkDialogUrlInput');
        let value = $linkUrlInput.val();
        let isLink = value.indexOf('@') < 0;
        this.$('input[name="isNewWindow"]').closest('.form-group').toggleClass('d-none', !isLink);
        this.$('.o-strip-domain').toggleClass('d-none', value.indexOf(window.location.origin) !== 0);
        this.options.wysiwyg && this.options.wysiwyg.verpEditor.historyPauseSteps('_onURLInput');
        this._adaptPreview();
        this.options.wysiwyg && this.options.wysiwyg.verpEditor.historyUnpauseSteps('_onURLInput');
    },
