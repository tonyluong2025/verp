...
const LinkPopoverWidget = Widget.extend({
    ...
    events: {
        'click .o-we-remove-link': '_onRemoveLinkClick',
        'click .o-we-edit-link': '_onEditLinkClick',
    },
     ...
     */
    start() {
        this.$urlLink = this.$('.o-we-url-link');
        this.$previewFaviconImg = this.$('.o-we-preview-favicon img');
        this.$previewFaviconFa = this.$('.o-we-preview-favicon .fa');
        this.$copyLink = this.$('.o-we-copy-link');
        this.$fullUrl = this.$('.o-we-full-url');
        ...
        this.$('[data-toggle="tooltip"]').tooltip({
            delay: 0,
            placement: 'bottom',
            container: this.options.wysiwyg.verpEditor.document.body,
        });
        const tooltips = [];
        ...
        .on('show.bs.popover.linkPopover', () => {
            this.options.wysiwyg.verpEditor.observerUnactive('show.bs.popover');
            this._loadAsyncLinkPreview();
            popoverShown = true;
        })
        .on('inserted.bs.popover', () => {
            this.options.wysiwyg.verpEditor.observerActive('show.bs.popover');
        })
        .on('hide.bs.popover.linkPopover', () => {
            this.options.wysiwyg.verpEditor.observerUnactive('hide.bs.popover');
            popoverShown = false;
        })
        .on('hidden.bs.popover.linkPopover', () => {
            this.options.wysiwyg.verpEditor.observerActive('hide.bs.popover');
            for (const tooltip of tooltips) {
                tooltip.hide();
            }
        })
        .on('inserted.bs.popover.linkPopover', () => {
            this.$target.data('bs.popover').tip.classList.add('o-edit-menu-popover');
        })
        .popover('show');


        this.popover = this.$target.data('bs.popover');
        this.$target.on('mousedown.linkPopover', (e) => {
            if (!popoverShown) {
                this.$target.popover('show');
            }
        });
        this.$target.on('hrefChanged.linkPopover', (e) => {
        ...
        $(document).on('mouseup.linkPopover', onClickDocument);
        if (document !== this.options.wysiwyg.verpEditor.document) {
            $(this.options.wysiwyg.verpEditor.document).on('mouseup.linkPopover', onClickDocument);
        }
    ...
    destroy() {
        ...
        this.$target.off('.linkPopover');
        $(document).off('.linkPopover');
        $(this.options.wysiwyg.verpEditor.document).off('.linkPopover');
    ...
    async _loadAsyncLinkPreview() {
        ...
        if (!protocol.startsWith('http')) {
            ...
            await this._dp.add($.get(this.target.href)).then(content => {
                ...
                this.$fullUrl.removeClass('d-none').addClass('o-we-webkit-box');
    ...
    _resetPreview(url) {
        ...
        this.$fullUrl.text(url).addClass('d-none').removeClass('o-we-webkit-box');
    },
...
LinkPopoverWidget.createFor = async function (parent, targetEl, options) {
    const noLinkPopoverClass = ".o-no-link-popover, .carousel-control-prev, .carousel-control-next, .dropdown-toggle";
    ...
    if (wysiwyg) {
        wysiwyg.verpEditor.observerUnactive('LinkPopoverWidget');
    }
    await popoverWidget.appendTo(targetEl)
    if (wysiwyg) {
        wysiwyg.verpEditor.observerActive('LinkPopoverWidget');
    }
