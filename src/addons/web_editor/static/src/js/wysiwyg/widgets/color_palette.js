verp.define('web_editor.ColorPalette', function (require) {
'use strict';
...
const ColorPaletteWidget = Widget.extend({
    ...
    events: {
        'click .o-we-color-btn': '_onColorButtonClick',
        'mouseenter .o-we-color-btn': '_onColorButtonEnter',
        'mouseleave .o-we-color-btn': '_onColorButtonLeave',
        'click .o-we-colorpicker-switch-pane-btn': '_onSwitchPaneButtonClick',
        'click .o-custom-gradient-editor .o-custom-gradient-btn': '_onGradientCustomButtonClick',
        'click .o-custom-gradient-editor': '_onPanelClick',
        'change .o-custom-gradient-editor input[type="text"]': '_onGradientInputChange',
        'keypress .o-custom-gradient-editor input[type="text"]': '_onGradientInputKeyPress',
        'click .o-custom-gradient-editor we-button:not(.o-remove-color)': '_onGradientButtonClick',
        'mouseenter .o-custom-gradient-editor we-button:not(.o-remove-color)': '_onGradientButtonEnter',
        'mouseleave .o-custom-gradient-editor we-button:not(.o-remove-color)': '_onGradientButtonLeave',
        'click .o-custom-gradient-scale': '_onGradientPreviewClick',
        // Note: _onGradientSliderClick on slider is attached at slider creation.
        'click .o-custom-gradient-editor .o-remove-color': '_onGradientDeleteClick',
    },
    customEvents: {
        'colorpickerSelect': '_onColorPickerSelect',
        'colorpickerPreview': '_onColorPickerPreview',
    },
    /**
     * @override
     *
    ...
    init: function (parent, options) {
        ...
        this.triggerUp('requestEditable', {callback: val => this.options.$editable = val});
    ...
    start: async function () {
        ...
        const switchPaneButtons = this.el.querySelectorAll('.o-we-colorpicker-switch-pane-btn');
        ...
        if (colorpickerArch) {
            ...
            sectionEl.classList.add('o-colorpicker-section');
            sectionEl.dataset.name = 'common';
            colorpickerEl.appendChild(sectionEl);
        }
        colorpickerEl.querySelectorAll('button').forEach(el => el.classList.add('o-we-color-btn'));

        // Populate tabs based on the tabs configuration indicated in this.tabs
        _.each(this.tabs, (tab, index) => {
            // Append pickers to section
            const sectionEl = this.el.querySelector(`.o-colorpicker-sections[data-color-tab="${tab.id}"]`);
            ...
            _.each(tab.pickers, pickerId => {
                ...
                switch (pickerId) {
                    ...
                    case 'custom':
                        ...
                        pickerEl.classList.add("o-colorpicker-section");
        ...
        if (this.options.withGradients && this.options.opacity !== 1) {
            this.pickers['predefinedGradients'].querySelectorAll('button').forEach(elem => {
        ...
        if (this.pickers['customGradient']) {
            this.gradientColorPicker = new ColorpickerWidget(this, {
                stopClickPropagation: true,
            });
            await this.gradientColorPicker.appendTo(this.sections['gradients']);
            const editor = this.pickers['customGradient'];
            this.gradientEditorParts = {
                'customButton': editor.querySelector('.o-custom-gradient-btn'),
                'customContent': editor.querySelector('.o-color-picker-inputs'),
                'linearButton': editor.querySelector('we-button[data-gradient-type="linear-gradient"]'),
                'angleRow': editor.querySelector('.o-angle-row'),
                ...
                'positionRow': editor.querySelector('.o-position-row'),
                'positionX': editor.querySelector('input[data-name="positionX"]'),
                'positionY': editor.querySelector('input[data-name="positionY"]'),
                'sizeRow': editor.querySelector('.o-size-row'),
                'scale': editor.querySelector('.o-custom-gradient-scale div'),
                'sliders': editor.querySelector('.o-slider-multi'),
                'deleteButton': editor.querySelector('.o-remove-color'),
            };
        ...
        this.el.querySelectorAll('.o-colorpicker-section').forEach(elem => {
        ...
        if (!this.options.excluded.includes('common')) {
            customColors.forEach((colorRow, i) => {
                ...
                colorRow.forEach(color => {
                    $div.append(this._createColorButton(color, ['o-common-color']));
                });
            });
        }
        ...
        this.el.querySelectorAll('button[data-color]:not(.o-custom-gradient-btn)').forEach(elem => {
            ...
            if (isCCName) {
                $color.find('.o-we-cc-preview-wrapper').addClass(`o-cc o-cc${colorName}`);
            } else {
    ...
    _buildCustomColors: function () {
        ...
        this.el.querySelectorAll('.o-custom-color').forEach(el => el.remove());
        const existingColors = new Set(Object.keys(this.colorToColorNames));
        this.triggerUp('getCustomColors', {
            onSuccess: (colors) => {
    ...
    _createColorButton: function (color, classes) {
        return $('<button/>', {
            class: 'o-we-color-btn ' + classes.join(' '),
    ...
    _selectColor: function (colorInfo, eventName) {
        ...
        if (eventName) {
            this.triggerUp(eventName, colorInfo);
        }
        this._buildCustomColors();
        if (this.colorPicker) {
    ...
    _selectGradient: function (gradient) {
        ...
        if (this.gradientColorPicker.$el) {
            this.gradientColorPicker.doHide();
        }
        const colorSplits = [];
        if (gradient) {
        ...
        this._activateGradientSlider($lastSlider || $(this.pickers['customGradient'].querySelector('.o-slider-multi input')));
    },
    /**
    ...
    _activateGradientSlider: function ($slider) {
        ...
        this.gradientColorPicker.doShow();
    ...
    _computeGradient: function () {
        ...
        const $picker = $(this.pickers['customGradient']);
        ...
        const type = $picker.find('.o-type-row we-button.active').data('gradientType');
        ...
        if (isLinear) {
            ...
            const size = $picker.find('.o-size-row we-button.active').data('gradientSize');
    ...
    _updateGradient: function (isPreview) {
        ...
        this.triggerUp(isPreview ? 'colorHover' : 'colorPicked', Object.assign(this.getSelectedColors(), {
            color: gradient,
    ...
    _selectTabFromButton(buttonEl) {
        this.el.querySelectorAll('.o-we-colorpicker-switch-pane-btn').forEach(el => {
            el.classList.remove('active');
        });
        buttonEl.classList.add('active');
        this.el.querySelectorAll('.o-colorpicker-sections').forEach(el => {
    ...
    _onColorButtonClick: function (ev) {
        ...
        this._selectColor(colorInfo, 'colorPicked');
    },
    /**
    ...
    _onColorButtonEnter: function (ev) {
        ev.stopPropagation();
        this.triggerUp('colorHover', Object.assign(this.getSelectedColors(), this._getButtonInfo(ev.currentTarget)));
    },
    /**
    ...
    _onColorButtonLeave: function (ev) {
        ev.stopPropagation();
        this.triggerUp('colorLeave', Object.assign(this.getSelectedColors(), {
            target: ev.target,
        }));
    },
    /**
    ...
    _onColorPickerPreview: function (ev) {
        if (ev.target === this.gradientColorPicker) {
            ...
            this.triggerUp('colorHover', Object.assign(this.getSelectedColors(), {
    ...
    _onGradientCustomButtonClick(ev) {
        ...
        if (!gradient) {
            ...
            gradient = this.pickers['predefinedGradients'].querySelector('button').dataset.color;
        }
        this._selectColor(Object.assign(this.getSelectedColors(), {
            color: gradient,
    ...
    _onGradientDeleteClick(ev) {
        ev.stopPropagation();
        const $activeSlider = $(this.pickers['customGradient'].querySelector('.o-slider-multi input.active'));
        $activeSlider.off();
        $activeSlider.remove();
        this.gradientEditorParts.deleteButton.classList.add('d-none');
        this.gradientEditorParts.deleteButton.classList.remove('active');
        this._updateGradient();
        this._activateGradientSlider($(this.pickers['customGradient'].querySelector('.o-slider-multi input')));
    },
});
