verp.define('wysiwyg.widgets.media', function (require) {
'use strict';
...
const {removeOnImageChangeAttrs} = require('web_editor.imageProcessing');
...
var MediaWidget = Widget.extend({
     ...
     * @param {Object} options: useful parameters such as resId, resModel,
     *  context, userId, ...
     */
...
var SearchableMediaWidget = MediaWidget.extend({
    events: _.extend({}, MediaWidget.prototype.events || {}, {
        'keydown .o-we-search': '_onSearchKeydown',
        'input .o-we-search': '_onSearchInput',
    }),
...
/**
 * Let users choose a file, including uploading a new file in verp.
 */
var FileWidget = SearchableMediaWidget.extend({
    events: _.extend({}, SearchableMediaWidget.prototype.events || {}, {
        'click .o-upload-media-button': '_onUploadButtonClick',
        'change .o-file-input': '_onFileInputChange',
        'click .o-upload-media-url-button': '_onUploadURLButtonClick',
        'input .o-we-url-input': '_onURLInputChange',
        'click .o-existing-attachment-cell': '_onAttachmentClick',
        'click .o-existing-attachment-remove': '_onRemoveClick',
        'click .o-load-more': '_onLoadMoreClick',
    }),
    ...
    start: function () {
        ...
        var self = this;
        this.$urlInput = this.$('.o-we-url-input');
        this.$form = this.$('form');
        this.$fileInput = this.$('.o-file-input');
        this.$uploadButton = this.$('.o-upload-media-button');
        this.$addUrlButton = this.$('.o-upload-media-url-button');
        this.$urlSuccess = this.$('.o-we-url-success');
        this.$urlWarning = this.$('.o-we-url-warning');
        this.$urlError = this.$('.o-we-url-error');
        this.$errorText = this.$('.o-we-error-text');
        ...
        } else if (this.$media.is('a.o-image')) {
        ...
        return this.search('').then(async () => {
            ...
            if (o.url) {
                self._selectAttachement(_.find(self.attachments, function (attachment) {
                    return o.url === attachment.imageSrc;
                }) || o);
            }
            return def;
        });
    ...
    fetchAttachments: function (number, offset) {
        return this._rpc({
            ...
            kwargs: {
                ...
                fields: ['label', 'mimetype', 'description', 'checksum', 'url', 'type', 'resId', 'resModel', 'isPublic', 'accessToken', 'imageSrc', 'imageWidth', 'imageHeight', 'originalId'],
     ...
     */
    _clear: function () {
        this.media.className = this.media.className && this.media.className.replace(/(^|\s+)(o-image)(?=\s|$)/g, ' ');
    },
    /**
     ...
     * @returns {Array} "ir.attachment" verp domain.
     */
    _getAttachmentsDomain: function (needle) {
        ...
        if (!this.options.resId) {
            ...
            attachedDocumentDomain.push(['createdUid', '=', this.options.userId]);
        }
        if (this.options.dataResModel) {
            var relatedDomain = ['&',
                ['resModel', '=', this.options.dataResModel],
                ['resId', '=', this.options.dataResId|0]];
            if (!this.options.dataResId) {
                relatedDomain.unshift('&');
                relatedDomain.push(['createdUid', '=', session.uid]);
            }
            domain = domain.concat(['|'], attachedDocumentDomain, relatedDomain);
        } else {
            domain = domain.concat(attachedDocumentDomain);
        }
        domain = ['|', ['isPublic', '=', true]].concat(domain);
        domain = domain.concat(this.options.mimetypeDomain);
        if (needle && needle.length) {
            domain.push(['label', 'ilike', needle]);
        }
        if (!this.options.useMediaLibrary) {
            domain.push('|', ['url', '=', false], '!', ['url', '=ilike', '/web_editor/shape/%']);
        }
        domain.push('!', ['label', '=like', '%.crop']);
    ...
    _highlightSelected: function () {
        var self = this;
        this.$('.o-existing-attachment-cell.o-we-attachment-selected').removeClass("o-we-attachment-selected");
        _.each(this.selectedAttachments, function (attachment) {
            self.$('.o-existing-attachment-cell[data-id=' + attachment.id + ']')
                .addClass("o-we-attachment-selected").css('display', '');
        });
    },
    /**
     * @private
    ...
    _loadMoreImages: function (forceSearch) {
        return this.fetchAttachments(10, this.numberOfAttachmentsToDisplay).then(() => {
            ...
            if (!forceSearch) {
                ...
                return this.search(this.$('.o-we-search').val() || '');
            }
        });
    },
    /**
    ...
    _renderThumbnails: function () {
        ...
        // Render menu & content
        this.$('.o-we-existing-attachments').replaceWith(
        ...
        // adapt load more
        this.$('.o-we-load-more').toggleClass('d-none', !this.hasContent());
        ...
        this.$('.o-load-done-msg').toggleClass('d-none', noLoadMoreButton || !noMoreImgToLoad);
        this.$('.o-load-more').toggleClass('d-none', noMoreImgToLoad);
    },
    /**
     * @private
     * @returns {Promise}
    ...
    _save: async function () {
        ...
        const toSave = Object.fromEntries(this.selectedMedia.map(media => [
            media.id, {
                query: media.query || '',
                isDynamicSvg: !!media.isDynamicSVG,
                dynamicColors: media.dynamicColors,
            }
        ]));
        let mediaAttachments = [];
        if (Object.keys(toSave).length !== 0) {
            mediaAttachments = await this._rpc({
                route: '/web_editor/saveLibraryMedia',
                params: {
                    media: toSave,
                },
            });
        ...
        const selected = this.selectedAttachments.concat(mediaAttachments).map(attachment => {
            ...
            if (attachment.imageSrc && attachment.imageSrc.startsWith('/web_editor/shape/')) {
                const colorCustomizedURL = new URL(attachment.imageSrc, window.location.origin);
                ...
                attachment.imageSrc = colorCustomizedURL.pathname + colorCustomizedURL.search;
            }
            return attachment;
        });
        ...
        if (!img || !img.id || this.$media.attr('src') === img.imageSrc) {
            return this.media;
        }

        if (!img.isPublic && !img.accessToken) {
            await this._rpc({
                model: 'ir.attachment',
                method: 'generateAccessToken',
                args: [[img.id]]
            }).then(function (accessToken) {
                img.accessToken = accessToken[0];
            });
        }

        if (img.imageSrc) {
            var src = img.imageSrc;
            if (!img.isPublic && img.accessToken) {
                src += _.str.sprintf('?accessToken=%s', img.accessToken);
            }
            if (!this.$media.is('img')) {
                ...
                this.$media = $('<img/>', {class: 'img-fluid o-we-custom-image'});
            ...
            if (!img.isPublic && img.accessToken) {
                href += _.str.sprintf('accessToken=%s&', img.accessToken);
            }
            href += 'unique=' + img.checksum + '&download=true';
            this.$media.attr('href', href);
            this.$media.addClass('o-image').attr('title', img.label);
        }
        ...
        this.media.classList.remove('o-modified-image-to-save');
        this.$media.trigger('imageChanged');
        return this.media;
    },
    /**
     * @param {object} attachment
     ...
     * saveRequest event to insert the attachment in the DOM.
     *
     * @private
     * @returns {Promise}
     */
        ...
        _.each(files, (file, index) => {
            ...
            uploadMutex.exec(async () => {
                ...
                const attachment = await this.uploader.rpcShowProgress({
                    route: '/web_editor/attachment/addData',
                    params: {
                        'label': file.name,
                ...
                if (!attachment.error) {
                    this.triggerUp('wysiwygAttachment', attachment);
    ...
    _onRemoveClick: function (ev) {
        ...
        Dialog.confirm(this, _t("Are you sure you want to delete this file ?"), {
            confirmCallback: function () {
                var $a = $(ev.currentTarget).closest('.o-existing-attachment-cell');
                ...
                var attachment = _.findWhere(self.attachments, {id: id});
                 return self._rpc({
                    ...
                    if (_.isEmpty(prevented)) {
                        ...
                        self.attachments.filter(at => at.originalId[0] === attachment.id).forEach(at => delete at.originalId);
                        if (!self.attachments.length) {
                            self._renderThumbnails(); //render the message and image if empty
                        } else {
                            $a.closest('.o-existing-attachment-cell').remove();
                        }
                        return;
                    }
    ...
    _onUploadURLButtonClick: function () {
        if (this.$urlInput.is('.o-we-horizontal-collapse')) {
            this.$urlInput.removeClass('o-we-horizontal-collapse');
    ...
    _addUrl: function () {
        var self = this;
        return this._rpc({
            route: '/web_editor/attachment/addUrl',
            params: {
...
/**
 * Let users choose an image, including uploading a new image in verp.
 */
var ImageWidget = FileWidget.extend({
    ...
    events: Object.assign({}, FileWidget.prototype.events, {
        'change input.o-we-show-optimized': '_onShowOptimizedChange',
        'change .o-we-search-select': '_onSearchSelect',
    }),
    MIN_ROW_HEIGHT: 128,
    ...
    async fetchAttachments(number, offset) {
        ...
        this.attachments.forEach(attachment => {
            if (attachment.imageSrc.startsWith('/')) {
                const newURL = new URL(attachment.imageSrc, window.location.origin);
                // Set the main colors of dynamic SVGs to o-color-1~5
                if (attachment.imageSrc.startsWith('/web_editor/shape/')) {
                ...
                attachment.thumbnailSrc = newURL.pathname + newURL.search;
            }
        });
        if (this.needle && this.options.useMediaLibrary) {
            try {
                const response = await this._rpc({
                    route: '/web_editor/mediaLibrarySearch',
                    params: {
                        'query': this.needle,
     ...
     */
    _renderThumbnails: function () {
        const alreadyLoaded = this.$('.o-existing-attachment-cell[data-loaded="true"]');
        this._super(...arguments);
        // Hide images until they're loaded
        this.$('.o-existing-attachment-cell').addClass('d-none');
        // Replace images that had been previously loaded if any to prevent scroll resetting to top
        alreadyLoaded.each((index, el) => {
            const toReplace = this.$(`.o-existing-attachment-cell[data-id="${el.dataset.id}"], .o-existing-attachment-cell[data-media-id="${el.dataset.mediaId}"]`);
            if (toReplace.length) {
                toReplace.replaceWith(el);
            }
        });
        this._toggleOptimized(this.$('input.o-we-show-optimized')[0].checked);
        // Placeholders have a 3:2 aspect ratio like most photos.
        const placeholderWidth = 3 / 2 * this.MIN_ROW_HEIGHT;
        this.$('.o-we-attachment-placeholder').css({
    ...
    _toggleOptimized: function (value) {
        this.$('.o-we-attachment-optimized').each((i, cell) => cell.style.setProperty('display', value ? null : 'none', 'important'));
    },
    /**
     * @override
     */
    _highlightSelected: function () {
        ...
        this.selectedMedia.forEach(media => {
            this.$(`.o-existing-attachment-cell[data-media-id=${media.id}]`)
    ...
    _onAttachmentImageLoad: async function (ev) {
        const img = ev.target;
        const cell = img.closest('.o-existing-attachment-cell');
        if (!cell) {
            return;
        }
    ...
    _onSearchSelect: function (ev) {
        ...
        this.$('.o-we-search').trigger('input');
    },
    /**
     * @private
     */
    _onSearchInput: function (ev) {
        this.libraryMedia = [];
        this._super(...arguments);
        this.$('.o-we-search-select').removeClass('d-none');
    },
    /**
     * @override
     */
    _clear: function (type) {
        ...
        var allImgClasses = /(^|\s+)(img|img-\S*|o-we-custom-image|rounded-circle|rounded|thumbnail|shadow|w-25|w-50|w-75|w-100|o-modified-image-to-save)(?=\s|$)/g;
...
/**
 * Let users choose a document, including uploading a new document in verp.
 */
...
var IconWidget = SearchableMediaWidget.extend({
    ...
    save: async function () {
        ...
        if (!this.$media.is('span, i')) {
            ...
            if (this.$media.length) {
                ...
                const data = { proms: [] };
                this.$media.trigger('beforeReplaceTarget', data);
    ...
    _highlightSelectedIcon: function () {
        var self = this;
        this.$icons.removeClass('o-we-attachment-selected');
...
var VideoWidget = MediaWidget.extend({
    ...
    events: _.extend({}, MediaWidget.prototype.events || {}, {
        'change .o-video-dialog-options input': '_onupdateVideoOption',
        'input textarea#oVideoText': '_onVideoCodeInput',
        'change textarea#oVideoText': '_onVideoCodeChange',
        'click .o-sample-video': '_onSampleVideoClick',
    }),
     ...
     */
    start: function () {
        this.$content = this.$('.o-video-dialog-iframe');

        if (this.media) {
            var $media = $(this.media);
            var src = $media.data('oe-expression') || $media.data('src') || ($media.is('iframe') ? $media.attr('src') : '') || '';
            this.$('textarea#oVideoText').val(src);

            this.$('input#oVideoAutoplay').prop('checked', src.indexOf('autoplay=1') >= 0);
            this.$('input#oVideoHideControls').prop('checked', src.indexOf('controls=0') >= 0);
            this.$('input#oVideoLoop').prop('checked', src.indexOf('loop=1') >= 0);
            this.$('input#oVideoHideFullscreen').prop('checked', src.indexOf('fs=0') >= 0);
            this.$('input#oVideoHideYtLogo').prop('checked', src.indexOf('modestbranding=1') >= 0);
            this.$('input#oVideoHideDmLogo').prop('checked', src.indexOf('ui-logo=0') >= 0);
            this.$('input#oVideoHideDmShare').prop('checked', src.indexOf('sharing-enable=0') >= 0);

            this._updateVideo();
        }

        // loads the thumbnail of vimeo video previews.
        this.$('.o-sample-video').each((index, node) => {
            const $node = $(node);
    ...
    save: function () {
        ...
        if (this.$('.o-video-dialog-iframe').is('iframe') && videoSrc) {
    ...
    getWrappedIframe: function (src) {
        return $(
            '<div class="media-iframe-video" data-oe-expression="' + src + '">' +
                '<div class="css-editable-mode-display">&nbsp;</div>' +
                '<div class="media-iframe-video-size" contenteditable="false">&nbsp;</div>' +
    ...
    _clear: function () {
        ...
        var allVideoClasses = /(^|\s)media-iframe-video(\s|$)/g;
    ...
    _createVideoNode: function (url, options) {
        ...
        const $video = $('<iframe>').width(1280).height(720)
            ...
            .addClass('o-video-dialog-iframe');
    ...
    _updateVideo: function () {
        // Reset the feedback
        this.$content.empty();
        this.$('#videoFormGroup').removeClass('o-has-error o-has-success').find('.form-control, .custom-select').removeClass('is-invalid is-valid');
        this.$('.o-video-dialog-options div').addClass('d-none');

        // Check video code
        var $textarea = this.$('textarea#oVideoText');
        ...
        var query = this._createVideoNode(url, {
            'autoplay': this.isForBgVideo || this.$('input#oVideoAutoplay').is(':checked'),
            'hideControls': this.isForBgVideo || this.$('input#oVideoHideControls').is(':checked'),
            'loop': this.isForBgVideo || this.$('input#oVideoLoop').is(':checked'),
            'hideFullscreen': this.isForBgVideo || this.$('input#oVideoHideFullscreen').is(':checked'),
            'hideYtLogo': this.isForBgVideo || this.$('input#oVideoHideYtLogo').is(':checked'),
            'hideDmLogo': this.isForBgVideo || this.$('input#oVideoHideDmLogo').is(':checked'),
            'hideDmShare': this.isForBgVideo || this.$('input#oVideoHideDmShare').is(':checked'),
        });

        var $optBox = this.$('.o-video-dialog-options');

        // Show / Hide preview elements
        this.$el.find('.o-video-dialog-preview-text, .media-iframe-video-size').add($optBox).toggleClass('d-none', !query.$video);
        // Toggle validation classes
        this.$el.find('#videoFormGroup')
            .toggleClass('o-has-error', !query.$video).find('.form-control, .custom-select').toggleClass('is-invalid', !query.$video)
            .end()
            .toggleClass('o-has-success', !!query.$video).find('.form-control, .custom-select').toggleClass('is-valid', !!query.$video);

        // Individually show / hide options base on the video provider
        $optBox.find('div.o-' + query.type + '-option').removeClass('d-none');
        ...
        if (query.type === 'youtube') {
            ...
            this.$('input#oVideoHideFullscreen, input#oVideoHideYtLogo').closest('div').toggleClass('d-none', this.$('input#oVideoHideControls').is(':checked'));
        }
        ...
        if (!$content) {
            switch (query.errorCode) {
                case 0:
                    $content = $('<div/>', {
                        class: 'alert alert-danger o-video-dialog-iframe mb-2 mt-2',
                        text: _t("The provided url is not valid"),
                    });
                    break;
                case 1:
                    $content = $('<div/>', {
                        class: 'alert alert-warning o-video-dialog-iframe mb-2 mt-2',
    ...
    _onSampleVideoClick(ev) {
        ...
        if (vimeoId) {
            this.$('#oVideoText').val(`https://player.vimeo.com/video/${vimeoId}`);
            this._updateVideo();
        }
    },
    /**
    ...
    _getVideoURLData: function (url, options) {
        ...
        const controls = options.hideControls ? '&controls=0' : '';
        const loop = options.loop ? '&loop=1' : '';

        let embedURL;
        let type;
        if (matches.youtube && matches.youtube[2].length === 11) {
            const fullscreen = options.hideFullscreen ? '&fs=0' : '';
            const ytLoop = loop ? loop + `&playlist=${matches.youtube[2]}` : '';
            const logo = options.hideYtLogo ? '&modestbranding=1' : '';
            ...
            const logo = options.hideDmLogo ? '&ui-logo=0' : '';
            const share = options.hideDmShare ? '&sharing-enable=0' : '';
