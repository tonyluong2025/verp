verp.define('wysiwyg.widgets.ImageCropWidget', function (require) {
'use strict';

const core = require('web.core');
const Widget = require('web.Widget');
const {applyModifications, cropperDataFields, activateCropper, loadImage, loadImageInfo} = require('web_editor.imageProcessing');
...
const ImageCropWidget = Widget.extend({
    ...
    events: {
        'click.cropOptions [data-action]': '_onCropOptionClick',
    ...
    async start() {
        if (this.uncroppable) {
            this.displayNotification({
              ...
              message: _t("This type of image is not supported for cropping.<br/>If you want to crop it, please first download it from the original source and upload it in Verp."),
            });
            return this.destroy();
        }
        const _super = this._super.bind(this);
        const $cropperWrapper = this.$('.o-we-cropper-wrapper');

        // Replacing the src with the original's so that the layout is correct.
        await loadImage(this.originalSrc, this.media);
        this.$cropperImage = this.$('.o-we-cropper-img');
    ...
    destroy() {
        ...
        this.$media.trigger('imageCropperDestroyed');
    ...
    async _save(cropped = true) {
        ...
        this.media.classList.add('o-modified-image-to-save');
        ...
        this.media.classList.toggle('o-we-image-cropped', cropped);
        this.$media.trigger('imageCropped');
        this.destroy();
    },
    /**
