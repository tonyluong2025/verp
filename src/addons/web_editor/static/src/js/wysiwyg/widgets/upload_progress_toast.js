...
export const UploadProgressToast = Widget.extend({
    ...
    template: 'wysiwyg.widgets.upload.progressToast',
    events: {
        'click .o-notification-close': '_onCloseClick',
    },
    ...
    start() {
        ...
        this.$el.find('.o-notification-content').append(this.$progress);
    },
    ...
    async rpcShowProgress(params, index) {
        let $progressBar = this.$el.find(`.js-progressbar-${index}`);
        try {
            ...
            if (attachment.error) {
                this.hasError = true;
                $progressBar.find('.js-progressbar-txt .text-danger').removeClass('d-none');
                $progressBar.find('.js-progressbar-txt .text-danger .o-we-error-text').text(attachment.error);
            } else {
                $progressBar.find('.js-progressbar-txt .text-success').removeClass('d-none');
            }
            return attachment;
        } catch (error) {
            this.hasError = true;
