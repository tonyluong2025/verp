...
checkoutForm.include({
    events: _.extend({}, checkoutForm.prototype.events || {}, {
        'change .o-wpayment-fee-impact': '_onFeeParameterChange',
    }),
     ...
     */
    start: function () {
        core.bus.on('updateShippingCost', this, this._updateShippingCost);
    ...
    _processPayment: function (provider, paymentOptionId, flow) {
        if ($('.o-donation-payment-form').length) {
            const errorFields = {};
    ...
    _prepareTransactionRouteParams: function (provider, paymentOptionId, flow) {
        ...
        return $('.o-donation-payment-form').length ? {
            ...transactionRouteParams,
            'partnerDetails': {
                'label': this.$('input[name="label"]').val(),
                'email': this.$('input[name="email"]').val(),
                'countryId': this.$('select[name="countryId"]').val(),
            },
            'donationComment': this.$('#donationComment').val(),
            'donationRecipientEmail': this.$('input[name="donationRecipientEmail"]').val(),
        } : transactionRouteParams;
    },
    ...
    _onFeeParameterChange: function (ev) {
        ...
        const acquirerIds = [];
        for (const card of this.$('.o-payment-option-card:has(.o-payment-fee)')) {
            const radio = $(card).find('input[name="oPaymentRadio"]');
            if (radio.data("paymentOptionType") === 'acquirer') {
                acquirerIds.push(radio.data("paymentOptionId"));
            }
        }
        const countryId = this.$('select[name="countryId"]').val();
            ...
            this._rpc({
                ...
                for (const card of this.$('.o-payment-option-card:has(.o-payment-fee)')) {
                    ...
                    if (radio.data("paymentOptionType") === 'acquirer') {
                        ...
                        const chunk = $(card).find('.o-payment-fee .oe-currency-value')[0];
