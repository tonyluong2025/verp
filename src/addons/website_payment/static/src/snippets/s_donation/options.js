...
options.registry.Donation = options.Class.extend({
    ...
    togglePrefilledOptions(previewMode, widgetValue, params) {
        ...
        this.$el.find('.o-we-prefilled-options-list').toggleClass('d-none', !widgetValue);
    ...
    renderListItems(previewMode, value, params) {
        ...
        _.each(valueList, value => {
            donationAmounts.push(value.displayName);
        });
    ...
    setMinimumAmount(previewMode, widgetValue, params) {
        ...
        const $rangeSlider = this.$('#sDonationRangeSlider');
        const $amountInput = this.$('#sDonationAmountInput');
    ...
    _renderCustomXML(uiFragment) {
        ...
        list.dataset.dependencies = "preFilledOpt";
        ...
        $(uiFragment).find('we-checkbox[data-name="preFilledOpt"]').after(list);
    },
    /**
    ...
    _buildDescriptionsList() {
        if (this.$target[0].dataset.descriptions) {
            const $descriptions = this.$target.find('#sDonationDescriptionInputs > input');
     ...
     */
    _updateDescriptions() {
        const descriptionInputs = this.$target.find('#sDonationDescriptionInputs');
        descriptionInputs.empty();
        const descriptions = this.$el.find('we-list input[type=text]');
        _.each(descriptions, description => {
            const inputEl = document.createElement('input');
            inputEl.type = 'hidden';
            inputEl.classList.add('o-translatable-input-hidden', 'd-block', 'mb-1', 'w-100');
            inputEl.name = 'donationDescriptions';
    ...
    _rebuildPrefilledOptions() {
        ...
        this.$target.find('.s-donation-prefilled-buttons').remove();
        const layout = this.$target[0].dataset.customAmount;
        const $slider = this.$target.find('.s-donation-range-slider-wrap');
        ...
        if (rebuild) {
            if (layout === "slider" && !$slider.length) {
                ...
                this.$target.find('.s-donation-donate-btn').before(sliderTemplate);
            }
            ...
            this.$target.find('#sDonationDescriptionInputs').after($prefilledButtons);
        }
    },
});
