...
class InsufficientCreditDialog extends Dialog {
    ...
    async willStart() {
        ...
        this.url = await this.orm.call("iap.account", "getCreditsUrl", [], {
        ...
        if (errorData.trial && isEnterprise) {
            this.buttonMessage = _t("Start a Trial at Verp");
        } else {
...
InsufficientCreditDialog.bodyTemplate = "iap.redirectToverpCredit";
...
function insufficientCreditHandler(env, error, originalError) {
    ...
    if (data && data.name === "verp.addons.iap.tools.iap_tools.InsufficientCreditError") {
