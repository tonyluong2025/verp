verp.define('barcodes.FormView', function (require) {
"use strict";
...
FormController.include({
    ...
    _barcodeRecordFilter: function (candidate, barcode, activeBarcode) {
        return candidate.data.productBarcode === barcode;
    },
    /**
     * @private
     */
    ...
    _barcodeStartListening: function () {
        core.bus.on('barcodeScanned', this, this._barcodeScanned);
    ...
    _barcodeStopListening: function () {
        core.bus.off('barcodeScanned', this, this._barcodeScanned);
    ...
    /**
     * The barcode is activate when at least one widget triggerUp 'activeBarcode' event
    ...
    _quantityOpenDialog: function (character, activeBarcode) {
        ...
        this.dialog.opened().then(function () {
            // This line set the value of the key which triggered the _setQuantity in the input
            var $input = self.dialog.$('.o-set-qty-input').focus().val(character);
...
FormRenderer.include({
    ...
    //--------------------------------------------------------------------------
    /**
     * triggerUp 'activeBarcode' to Add barcode event handler
     *
     * @private
    ...
    _barcodeButtonHandler: function ($button, node) {
        ...
        commands['O-BTN.' + node.attrs.barcodeTrigger] = function () {
            if (!$button.hasClass('o-invisible-modifier')) {
                $button.click();
            }
