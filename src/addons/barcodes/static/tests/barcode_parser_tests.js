verp.define('barcodes/static/tests/barcode_parser_tests.js', function (require) {
"use strict";
...
QUnit.module('Barcode Parser', {
    beforeEach: function () {
        this.data = {
            ...
            'barcode.rule': {
                fields: {
                    name: {type: 'char', string: 'Barcode Nomenclature'},
                    barcodeNomenclatureId: {type: 'many2one', relation: 'barcode.nomenclature'},
                ...
                records: [
                    {id: 1, name: "Product Barcodes", barcodeNomenclatureId: 1, sequence: 90, type: 'product', encoding: 'any', pattern: ".*"},
                ],
            }
        };
    }
    ...
    QUnit.test('Test check digit', async function (assert) {
        assert.expect(6);
        const barcodeNomenclature = new BarcodeParser({'nomenclatureId': 1});
