verp.define('web_tour.tour_manager_tests', async function (require) {
    "use strict";
     ...
     * @param {string[]} [params.consumedTours]
     ...
     */
    async function createTourManager({ consumedTours, disabled, template, tours, ...params }) {
        const parent = await testUtils.createParent(params);
        const tourManager = new TourManager(parent, consumedTours, disabled);
        tourManager.runningStepDelay = 0;
        ...
        await tourManager._registerAll(true);
        ...
        QUnit.test("kanban quick create VS tour tooltips", async function (assert) {
            ...
            const tourManager = await createTourManager({
                ...
                tours: [{
                    name: "Tour",
                    options: { rainbowMan: false },
                    steps: [{ trigger: "input[name='displayName']" }],
                }],
            });
