...
QUnit.module('calendar', {
    ...
    QUnit.test("many2manyattendee widget: basic rendering", async function (assert) {
        ...
        assert.hasClass(form.$('.o-field-widget[name="partner_ids"]'), 'o_field_many2manytags');
        assert.containsN(form, '.o-field-widget[name="partner_ids"] .badge', 2,
            "there should be 2 tags");
        assert.strictEqual(form.$('.o-field-widget[name="partner_ids"] .badge:first').text().trim(), "Jesus",
            "the tag should be correctly named");
        assert.hasClass(form.$('.o-field-widget[name="partner_ids"] .badge:first img'),'o_attendee_border_accepted',
            "Jesus should attend the meeting");
        assert.strictEqual(form.$('.o-field-widget[name="partner_ids"] .badge[data-id="2"]').text().trim(), "Mahomet",
            "the tag should be correctly named");
        assert.hasClass(form.el.querySelector('.o-field-widget[name="partner_ids"] .badge[data-id="2"] img'), 'o_attendee_border_tentative',
        ...
        assert.containsOnce(form, '.o_field_many2manytags.avatar.o-field-widget .badge:first img',
            "should have img tag");
        assert.hasAttrValue(form.$('.o_field_many2manytags.avatar.o-field-widget .badge:first img'),
            'data-src',
