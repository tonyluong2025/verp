verp.define('bus.CrossTab', function (require) {
"use strict";
 ...
 * - {LOCAL_STORAGE_PREFIX}.{sanitizedOrigin}.tabList : list of opened tab ids
 * - {LOCAL_STORAGE_PREFIX}.{sanitizedOrigin}.tabMaster : generated id of the master tab
 *
 * trigger:
 * - windowFocus : when the window is focused
 * - notification : when a notification is receive from the long polling
 * - becomeMaster : when this tab became the master
 * - noLongerMaster : when this tab is not longer the master (the user swith tab)
 */
var CrossTabBus = Longpolling.extend({
    ...
    init: function () {
        ...
        if (this._callLocalStorage('getItem', 'lastTs', 0) + 50000 < now) {
            this._callLocalStorage('removeItem', 'last');
        }
        this._lastNotificationID = this._callLocalStorage('getItem', 'last', 0);
        this.call('localStorage', 'onStorage', this, this._onStorage);
    },
    destroy: function () {
        this._super();
    ...
    _callLocalStorage: function (method, key, param) {
        return this.call('localStorage', method, this._generateKey(key), param);
    },
    /**
     ...
     * If this tab became the master, trigger 'becomeMaster' event
     *
     * @private
     */
    ...
    _onPoll: function (notifications) {
        ...
        if (this._isMasterTab && notifs.length) {
            ...
            this._callLocalStorage('setItem', 'lastTs', new Date().getTime());
