...
export const assetsWatchdogService = {
    ...
    start(env, { notification }) {
        ...
        env.bus.on("WEB_CLIENT_READY", null, async () => {
            ...
            legacyEnv.services.busService.onNotification(this, onNotification);
            legacyEnv.services.busService.startPolling();
        });
        ...
        function onNotification(notifications) {
            for (const { payload, type } of notifications) {
                if (type === 'bundleChanged') {
                    if (payload.serverVersion !== session.serverVersion) {
