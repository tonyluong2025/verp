...
tour.register('snippet_countdown', {
    ...
    wTourUtils.dragNDrop({id: 'sCountdown', name: 'Countdown'}),
    wTourUtils.clickOnSnippet({id: 'sCountdown', name: 'Countdown'}),
    ...
    // widgetsStartRequest is triggered.
    {
        content: "Hover an option which has a preview",
        trigger: '[data-select-class="o-half-screen-height"]',
    ...
    },
    {
        ...
        run: () => {
            ...
            // to make some sense.
            const $previousAnchor = $('[data-select-class="o-half-screen-height"]');
    ...
    wTourUtils.changeOption('countdown', 'we-select:has([data-end-action]) we-toggler', 'end action'),
    wTourUtils.changeOption('countdown', 'we-button[data-end-action="messageNoCountdown"]', 'end action'),
    {
        content: "Check that the countdown is not displayed",
        trigger: '.s-countdown:has(.s-countdown-canvas-wrapper:not(:visible))',
