...
function switchTo(type, _name) {
    ...
    return {
        trigger: `.o-we-colorpicker-switch-pane-btn[data-target="${target}"]`,
...
function addCheck(steps, checkX, checkNoX, xType, noSwitch = false) {
    ...
    const selectorCheckX = checkX && `.o-we-color-btn[data-color="${checkX}"].selected`;
    const selectorCheckNoX = checkNoX && `.o-we-color-btn[data-color="${checkNoX}"]:not(.selected)`;
    const step = {
    ...
    if (!selectorCheckX && selectorCheckNoX) {
        step.extraTrigger = selectorCheckNoX;
    }
...
function checkAndUpdateBackgroundColor({
    ...
    if (changeType) {
        ...
        steps.push(wTourUtils.changeOption('ColoredLevelBackground', `.o-we-color-btn[data-color="${change}"]`));
        steps.push({
...
function updateAndCheckCustomGradient({updateStep, checkGradient}) {
    const steps = [updateStep, {
        trigger: `#wrapwrap section.${snippets[0].id}.o-cc1`,
...
...checkAndUpdateBackgroundColor({
    changeType: 'cc',
    change: 3,
    finalSelector: `.${snippets[0].id}.o-cc.o-cc3:not([class*=bg-]):not([style*="background"])`,
}),
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'cc',
    change: 2,
    finalSelector: `.${snippets[0].id}.o-cc.o-cc2:not(.o-cc3):not([class*=bg-])`,
}),
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'bg',
    change: 'black-50',
    finalSelector: `.${snippets[0].id}.o-cc.o-cc2.bg-black-50`,
}),
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'bg',
    change: '800',
    finalSelector: `.${snippets[0].id}.o-cc.o-cc2.bg-800:not(.bg-black-50)`,
}),
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'cc',
    change: 4,
    finalSelector: `.${snippets[0].id}.o-cc.o-cc4:not(.o-cc2).bg-800`,
}),
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'gradient',
    change: gradients[0],
    finalSelector: `.${snippets[0].id}.o-cc.o-cc4:not(.bg-800)[style*="background-image: ${gradients[0]}"]`,
}),
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'gradient',
    change: gradients[1],
    finalSelector: `.${snippets[0].id}.o-cc.o-cc4[style*="background-image: ${gradients[1]}"]:not([style*="background-image: ${gradients[0]}"])`,
}),
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'cc',
    change: 1,
    finalSelector: `.${snippets[0].id}.o-cc.o-cc1:not(.o-cc4)[style*="background-image: ${gradients[1]}"]`,
}),
...
{
    trigger: '.o-existing-attachment-cell img',
    content: "Select an image in the media dialog",
},
{
    trigger: `.${snippets[0].id}.o-cc.o-cc1`,
    run: function () {
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'gradient',
    change: gradients[0],
    finalSelector: `.${snippets[0].id}.o-cc.o-cc1:not([style*="${gradients[1]}"])`,
    finalRun: function () {
        const parts = weUtils.backgroundImageCssToParts(this.$anchor.css('background-image'));
        if (!parts.url || !parts.url.startsWith('url(')) {
...
...updateAndCheckCustomGradient({
    updateStep: {
        trigger: '.colorpicker .o-custom-gradient-scale',
        content: 'Add step',
        run: 'click',
    },
...
...updateAndCheckCustomGradient({
    updateStep: {
        trigger: '.colorpicker .o-slider-multi input.active',
        content: 'Move step',
        run: () => {
            const slider = $('.colorpicker .o-slider-multi input.active');
            slider.val(45);
...
...updateAndCheckCustomGradient({
    updateStep: {
        trigger: '.colorpicker .o-color-picker-inputs .o-hex-div input',
...
...updateAndCheckCustomGradient({
    updateStep: {
        trigger: '.colorpicker .o-remove-color',
        content: 'Delete step',
        run: 'click',
    },
...
{
    trigger: `.o-we-color-btn[data-color="${gradients[0]}"]`,
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'bg',
    change: 'black-75',
    finalSelector: `.${snippets[0].id}.o-cc.o-cc1.bg-black-75[style^="background-image: url("]:not([style*="${gradients[0]}"])`
}),
...
...checkAndUpdateBackgroundColor({
    ...
    changeType: 'gradient',
    change: gradients[1],
    finalSelector: `.${snippets[0].id}.o-cc.o-cc1:not(.bg-black-75)`,
    finalRun: function () {
        const parts = weUtils.backgroundImageCssToParts(this.$anchor.css('background-image'));
        if (!parts.url || !parts.url.startsWith('url(')) {
...
wTourUtils.changeOption('ColoredLevelBackground', '[data-name="bg_image_toggle_opt"]'),
{
    trigger: `.${snippets[0].id}.o-cc.o-cc1[style*="background-image: ${gradients[1]}"]`,
    run: () => null,
},

// Now removing all colors via the 'None' button (note: colorpicker still opened)
{
    trigger: '.o-colorpicker-reset',
    content: "Click on the None button of the color palette",
},
{
    trigger: `.${snippets[0].id}:not(.o-cc):not(.o-cc1):not([style*="background-image"])`,
