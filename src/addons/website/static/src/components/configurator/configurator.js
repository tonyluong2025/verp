...
const WEBSITE_TYPES = {
    ...
    2: {id: 2, label: _lt("an online store"), name: 'onlineStore'},
...
const WEBSITE_PURPOSES = {
    1: {id: 1, label: _lt("get leads"), name: 'getLeads'},
    2: {id: 2, label: _lt("develop the brand"), name: 'developBrand'},
    3: {id: 3, label: _lt("sell more"), name: 'sellMore'},
    4: {id: 4, label: _lt("inform customers"), name: 'informCustomers'},
    5: {id: 5, label: _lt("schedule appointments"), name: 'scheduleAppointments'}
};
...
const SESSION_STORAGE_ITEM_NAME = 'websiteConfigurator' + session.websiteId;
...
class DescriptionScreen extends Component {
    ...
    mounted() {
        ...
        $(this.industrySelection.el).autocomplete({
            appendTo: '.o-configurator-industry-wrapper',
            delay: 400,
            minLength: 1,
            ...
            classes: {
                'ui-autocomplete': 'custom-ui-autocomplete shadow-lg border-0 o-configurator-show-fast',
            }
        });
    ...
    _customizeNoResultMenuStyle() {
        if (!this.autocompleteHasResults) {
            ...
            noResultLinkEl.classList.add('o-no-result');
        }
    }
...
class PaletteSelectionScreen extends Component {
    ...
    async updatePalettes() {
        ...
        const [color1, color2] = await this.rpc({
            ...
            method: 'extractImagePrimarySecondaryColors',
            args: [img],
            kwargs: {mitigate: 255},
        });
...
class FeaturesSelectionScreen extends Component {
    ...
    async buildWebsite() {
        ...
        const themes = await this.rpc({
            model: 'website',
            method: 'configuratorRecommendedThemes',
            kwargs: params,
        });
...
const actions = {
    selectWebsiteType({state}, id) {
        Object.values(state.features).filter((feature) => feature.moduleState !== 'installed').forEach((feature) => {
            feature.selected = feature.websiteConfigPreselection.includes(WEBSITE_TYPES[id].name);
        });
    ...
    selectWebsitePurpose({state}, id) {
        ...
        Object.values(state.features).filter((feature) => feature.moduleState !== 'installed').forEach((feature) => {
            ...
            feature.selected |= id && feature.websiteConfigPreselection.includes(WEBSITE_PURPOSES[id].name);
        });
    ...
    toggleFeature({state}, featureId) {
        ...
        const isModuleInstalled = feature.moduleState === 'installed';
...
async function getInitialState(services) {
    ...
    // Load values from javascript and iap
    var results = await services.rpc({
        model: 'website',
        method: 'configuratorInit',
    });
    const r = {
    ...
    if (localState) {
        ...
        if (localState.selectedIndustry && localState.selectedPalette) {
            const params = {
                industryId: localState.selectedIndustry.id,
    ...
    results.features.forEach(feature => {
        features[feature.id] = Object.assign({}, feature, {selected: feature.moduleState === 'installed'});
        const wtp = features[feature.id].websiteConfigPreselection;
        features[feature.id].websiteConfigPreselection = wtp ? wtp.split(',') : [];
    });
...
async function skipConfigurator(services) {
    await services.rpc({
        model: 'website',
        method: 'configuratorSkip',
    });
    window.sessionStorage.removeItem(SESSION_STORAGE_ITEM_NAME);
    window.location = '/web#action=website.themeInstallKanbanAction';
}
...
async function applyConfigurator(self, themeName) {
    ...
    async function attemptConfiguratorApply(data, retryCount = 0) {
        try {
            ...
            document.querySelector('.o-theme-install-loader-container').remove();
            throw error;
        }
    }
    ...
    if (themeName !== undefined) {
        ...
        const data = {
            ...
            industryId: self.state.selectedIndustry.id,
            selectedPalette: selectedPalette,
            themeName: themeName,
            websitePurpose: WEBSITE_PURPOSES[
                self.state.selectedPurpose || self.state.formerSelectedPurpose
            ].name,
            websiteType: WEBSITE_TYPES[self.state.selectedType].name,
            logoAttachmentId: self.state.logoAttachmentId,
        };
