verp.define('website.sDynamicSnippet', function (require) {
'use strict';
...
const DynamicSnippet = publicWidget.Widget.extend({
    ...
    init: function () {
        ...
        this.template_key = 'website.sDynamicSnippet.grid';
    },
    /**
     *
     * @override
    ...
    _clearContent: function () {
        const $templateArea = this.$el.find('.dynamic-snippet-template');
        ...
        if (this._isConfigComplete()) {
            ...
            const filterFragments = await this._rpc({
                ...
                'params': Object.assign({
                    ...
                    'searchDomain': this._getSearchDomain(),
    ...
    _render: function () {
        if (this.data.length > 0 || this.editableMode) {
            this.$el.removeClass('o-dynamic-empty');
            this._prepareContent();
        } else {
            this.$el.addClass('o-dynamic-empty');
            this.renderedContent = '';
        }
    ...
    _setupSizeChangedManagement: function (enable) {
        if (enable === true) {
            config.device.bus.on('sizeChanged', this, this._onSizeChanged);
        } else {
            config.device.bus.off('sizeChanged', this, this._onSizeChanged);
        }
    },
    /**
     *
    ...
    _toggleVisibility: function (visible) {
        this.$el.toggleClass('o-dynamic-empty', !visible);
    },
