verp.define('website.sShare', function (require) {
'use strict';

const publicWidget = require('web.public.widget');

const ShareWidget = publicWidget.Widget.extend({
    selector: '.s-share, .oe_share', // oe_share for compatibility
        ...
        this.$('a').each((index, element) => {
            ...
            $a.attr('href', (i, href) => {
                return href.replace(urlRegex, (match, a, b, c) => {
                    ...
                    if ($a.hasClass('s-share-whatsapp')) {
            ...
            if ($a.attr('target') && $a.attr('target').match(/_blank/i) && !$a.closest('.o-editable').length) {
                $a.on('click', function () {
