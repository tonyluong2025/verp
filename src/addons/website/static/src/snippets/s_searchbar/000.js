...
publicWidget.registry.searchBar = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        this.order = this.$('.o-search-order-by').val();
        ...
        const form = this.$('.o-search-order-by').parents('form');
    ...
    async _fetch() {
        const res = await this._rpc({
            ...
            params: {
                ...
                'order': this.order,
                'limit': this.limit,
                'maxNbChars': Math.round(Math.max(this.autocompleteMinWidth, parseInt(this.$el.width())) * 0.22),
    ...
    _render: function (res) {
        ...
        if (res && this.limit) {
            ...
            let template = 'website.sSearchbar.autocomplete';
            const candidate = template + '.' + this.searchType;
            if (qweb.hasTemplate(candidate)) {
                template = candidate;
            }
            this.$menu = $(qweb.render(template, {
                results: results,
                parts: res['parts'],
                hasMoreResults: results.length < res['resultsCount'],
                search: this.$input.val(),
                fuzzySearch: res['fuzzySearch'],
                widget: this,
            }));
            ...
            const megaMenuEl = this.el.closest('.o-mega-menu');
            if (megaMenuEl) {
            ...
            this.$el.find('button.extraLink').on('click', function (event) {
                event.preventDefault();
                window.location.href = event.currentTarget.dataset['target'];
            });
            this.$el.find('.s-searchbar-fuzzy-submit').on('click', (event) => {
                event.preventDefault();
                this.$input.val(res['fuzzySearch']);
                const form = this.$('.o-search-order-by').parents('form');
                form.submit();
            });
        }
