verp.define('website.sImageGallery', function (require) {
'use strict';
...
const GalleryWidget = publicWidget.Widget.extend({
    ...
    selector: '.s-image-gallery:not(.o-slideshow)',
    ...
    _onClickImg: function (ev) {
        ...
        var $images = $cur.closest('.s-image-gallery').find('img');
        var size = 0.8;
        var dimensions = {
            min_width: Math.round(window.innerWidth * size * 0.9),
            minHeight: Math.round(window.innerHeight * size),
        ...
        const milliseconds = $cur.closest('.s-image-gallery').data('interval') || false;
        ...
        this.$modal.find('.modal-content, .modal-body.o-slideshow').css('height', '100%');
        this.$modal.appendTo(document.body);

        this.$modal.one('shown.bs.modal', function () {
            self.triggerUp('widgetsStartRequest', {
                editableMode: false,
                $target: self.$modal.find('.modal-body.o-slideshow'),
            });
        });
    },
});
...
const GallerySliderWidget = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        this.$prev = this.$indicator.find('li.o-indicators-left').css('visibility', ''); // force visibility as some databases have it hidden
        this.$next = this.$indicator.find('li.o-indicators-right').css('visibility', '');
        ...
        this.$indicator.on('click.gallery_slider', '> li:not([data-slide-to])', function () {
            page += ($(this).hasClass('o-indicators-left') ? -1 : 1);
