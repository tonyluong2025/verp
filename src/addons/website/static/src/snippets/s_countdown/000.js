verp.define('website.sCountdown', function (require) {
'use strict';
...
const CountdownWidget = publicWidget.Widget.extend({
     ...
     */
    start: function () {
        this.$wrapper = this.$('.s-countdown-canvas-wrapper');
     ...
     */
    destroy: function () {
        this.$('.s-countdown-end-redirect-message').remove();
        this.$('.s-countdown-end-message').addClass('d-none');
        this.$('.s-countdown-text-wrapper').remove();
        this.$('.s-countdown-canvas-wrapper').removeClass('d-none');
        this.$('.s-countdown-canvas-flex').remove();
    ...
    _handleEndCountdownAction: function () {
        if (this.endAction === 'redirect') {
            ...
            if (this.hereBeforeTimerEnds) {
                ...
                if (!this.$('.s-countdown-end-redirect-message').length) {
                    const $container = this.$('> .container, > .container-fluid, > .o-container-small');
                    $container.append(
                        $(qweb.render('website.sCountdown.endRedirectMessage', {
                            redirectUrl: redirectUrl,
                        }))
                    );
                }
            }
        } else if (this.endAction === 'message' || this.endAction === 'messageNoCountdown') {
            this.$('.s-countdown-end-message').removeClass('d-none');
        }
    },
    /**
    ...
    _initTimeDiff: function () {
        ...
        if (this._isUnitVisible('d') && !(this.onlyOneUnit && delta < 86400)) {
            this.diff.push({
                canvas: $('<div class="s-countdown-canvas-flex"><canvas class="w-100"/></div>').appendTo(this.$wrapper)[0],
    ...
    _render: function () {
        ...
        if (this.layout === 'text') {
            this.$('.s-countdown-canvas-flex').addClass('d-none');
            if (!this.$textWrapper) {
                this.$textWrapper = $('<span/>').attr({
                    class: 's-countdown-text-wrapper d-none',
                });
                this.$textWrapper.text(_t("Countdown ends in"));
                this.$textWrapper.append($('<span/>').attr({
                    class: 's-countdown-text ml-1',
                }));
            ...
            this.$('.s-countdown-text').text(countdownText.toLowerCase());
        } else {
            for (const val of this.diff) {
                ...
                this.$('.s-countdown-canvas-flex').toggleClass('mx-1', this.layout === 'boxes');
            }
        }
