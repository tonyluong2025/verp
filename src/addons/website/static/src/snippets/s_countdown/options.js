verp.define('website.sCountdownOptions', function (require) {
'use strict';

const core = require('web.core');
const options = require('web_editor.snippets.options');
const CountdownWidget = require('website.sCountdown');
...
options.registry.countdown = options.Class.extend({
    ...
    cleanForSave: async function () {
        this.$target.find('.s-countdown-canvas-wrapper').removeClass("s-countdown-none");
        this.$target.find('.s-countdown-end-message').removeClass("s-countdown-enable-preview");
    },
    ...
    endAction: function (previewMode, widgetValue, params) {
        ...
        if (widgetValue === 'message' || widgetValue === 'messageNoCountdown') {
            if (!this.$target.find('.s-countdown-end-message').length) {
                const message = this.endMessage || qweb.render('website.sCountdown.endMessage');
                this.$target.append(message);
            }
            this.$target.toggleClass('hide-countdown', widgetValue === 'messageNoCountdown');
        } else {
            const $message = this.$target.find('.s-countdown-end-message').detach();
    ...
    */
    ...
    updateUIEndMessage: function () {
        this.$target.find('.s-countdown-canvas-wrapper')
            .toggleClass("s-countdown-none", this.showEndMessage === true && this.$target.hasClass("hide-countdown"));
        this.$target.find('.s-countdown-end-message')
            .toggleClass("s-countdown-enable-preview", this.showEndMessage === true);
    },
