verp.define('website.sPopupOptions', function (require) {
'use strict';
...
options.registry.SnippetPopup = options.Class.extend({
    ...
    start: function () {
        ...
        this.$target.on('click.SnippetPopup', '.js-close-popup:not(a, .btn)', ev => {
            ev.stopPropagation();
            this.onTargetHide();
            this.triggerUp('snippetOptionVisibilityUpdate', {show: false});
        });
        this.$target.on('shown.bs.modal.SnippetPopup', () => {
            ...
            this.$target[0].querySelectorAll('.media-iframe-video').forEach(media => {
    ...
    onTargetHide: async function () {
        return new Promise(resolve => {
            const timeoutID = setTimeout(() => {
                this.$target.off('hidden.bs.modal.popupOnTargetHide');
                resolve();
            }, 500);
            this.$target.one('hidden.bs.modal.popupOnTargetHide', () => {
    ...
    cleanForSave: function () {
        this.$target.removeClass("s-popup-overflow-page");
    },
    ...
    moveBlock: function (previewMode, widgetValue, params) {
        ...
        this.$target.closest('.s-popup').prependTo($container.find('.oe-structure:o-editable').first());
    },
    /**
     * @see this.selectClass for parameters
     */
    ...
    _assignUniqueID: function () {
        this.$target.closest('.s-popup').attr('id', 'sPopup' + Date.now());
    },
    /**
     * @override
     */
