verp.define('website.sChartOptions', function (require) {
'use strict';
...
options.registry.InnerChart = options.Class.extend({
    customEvents: _.extend({}, options.Class.prototype.customEvents, {
        'getCustomColors': '_onGetCustomColors',
    }),
    events: _.extend({}, options.Class.prototype.events, {
        'click we-button.add-column': '_onAddColumnClick',
        'click we-button.add-row': '_onAddRowClick',
        'click we-button.o-we-matrix-remove-col': '_onRemoveColumnClick',
        'click we-button.o-we-matrix-remove-row': '_onRemoveRowClick',
     ...
     */
    start: function () {
        this.backSelectEl = this.el.querySelector('[data-name="chartBgColorOpt"]');
        this.borderSelectEl = this.el.querySelector('[data-name="chartBorderColorOpt"]');
    ...
    updateUI: async function () {
        ...
        this.tableEl.classList.toggle('o-we-matrix-five-col', this.tableEl.querySelectorAll('tr:first-child th').length > 5);
    ...
    _computeWidgetVisibility: function (widgetName, params) {
        switch (widgetName) {
            case 'stackedChartOpt': {
                return this._getColumnCount() > 1;
            }
            case 'chartBgColorOpt':
            case 'chartBorderColorOpt': {
    ...
    _makeDeleteButton: function (...classes) {
        const rmbuttonEl = options.buildElement('we-button', null, {
            classes: ['o-we-text-danger', 'o-we-link', 'fa', 'fa-fw', 'fa-minus', ...classes],
        });
        rmbuttonEl.title = classes.includes('o-we-matrix-remove-col') ? _t("Remove Serie") : _t("Remove Row");
    ...
    _addColumn: function (title, values, heardeBackgroundColor, headerBorderColor, cellBackgroundColors = [], cellBorderColors = []) {
        ...
        const removeButton = this._makeDeleteButton('o-we-matrix-remove-col');
    ...
    _addRow: function (tilte) {
        ...
        trEl.appendChild(this._makeDeleteButton('o-we-matrix-remove-row'));
    ...
    _displayRemoveColButton: function (colIndex) {
        if (this._getColumnCount() > 1) {
            this._displayRemoveButton(colIndex, 'o-we-matrix-remove-col');
        }
    },
    /**
    ...
    _displayRemoveRowButton: function (rowIndex) {
        ...
        if (rowCount > 1) {
            this._displayRemoveButton(rowIndex, 'o-we-matrix-remove-row');
        }
    },
    /**
     * @private
     * @param {Int} tdIndex Can be undefined, if so the last remove button will be shown
     * @param {String} btnClass Either o-we-matrix-remove-col or o-we-matrix-remove-row
     */
     ...
     * and apply it to the onSuccess method provided by the triggerUp.
     *
     * @private
     */
