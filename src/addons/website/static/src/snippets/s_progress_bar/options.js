verp.define('website.sProgressBarOptions', function (require) {
'use strict';
...
options.registry.progress = options.Class.extend({
    ...
    display: function (previewMode, widgetValue, params) {
        ...
        if (this.$target.hasClass('progress')) {
            ...
            this.$target.find('.progress-bar span').addClass('s-progress-bar-text');
        }

        let $text = this.$target.find('.s-progress-bar-text');
        if (!$text.length) {
            $text = $('<span/>').addClass('s-progress-bar-text').html(_t('80% Development'));
        }
    ...
    progressBarValue: function (previewMode, widgetValue, params) {
        ...
        const $progressBarText = this.$target.find('.s-progress-bar-text');
    ...
    _computeWidgetState: function (methodName, params) {
        switch (methodName) {
            case 'display': {
                const isInline = this.$target.find('.s-progress-bar-text')
