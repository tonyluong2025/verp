verp.define('website.newMenu', function (require) {
'use strict';
...
var NewContentMenu = websiteNavbarData.WebsiteNavbarActionWidget.extend({
    ...
    start: function () {
        ...
        this.$newContentMenuChoices = this.$('#oNewContentMenuChoices');

        var $modules = this.$newContentMenuChoices.find('.o-new-content-element');
        _.each($modules, function (el, index) {
            var $el = $(el);
            $el.data('original-index', index);
            if ($el.data('module-id')) {
                $el.appendTo($el.parent());
                $el.find('a i, a p').addClass('o-uninstalled-module');
            }
        });
        ...
        if ($.deparam.querystring()[enableFlag] !== undefined) {
            Object.keys(tour.tours).forEach(
                el => {
                    ...
                    if (element.steps[0].trigger == '#newContentMenu > a'
                        && !element.steps[0].extraTrigger) {
        ...
        this.$loader = $(qweb.render('website.newContentLoader'));
    ...
    _createNewPage: function () {
        return wUtils.prompt({
            ...
            init: function () {
                ...
                var $add = $('<div/>', {'class': 'form-group mb0 row'})
                            .append($('<span/>', {'class': 'offset-md-3 col-md-9 text-left'})
                                    .append(qweb.render('website.components.switch', {id: 'switchAddToMenu', label: _t("Add to menu")})));
            ...
            const res = wUtils.sendRequest(url, {
                addMenu: $dialog.find('input[type="checkbox"]').is(':checked') || '',
            });
    ...
    _hideMenu: function () {
        this.shown = false;
        this.$newContentMenuChoices.addClass('o-hidden');
        $('body').removeClass('o-new-content-open');
    },
    /**
     * Install a module
     *
    ...
    _showMenu: function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            self.firstTab = true;
            self.shown = true;
            self.$newContentMenuChoices.removeClass('o-hidden');
            $('body').addClass('o-new-content-open');
            self.$('> a').focus();
        });
    },
    /**
    ...
    _addLoader(moduleName) {
        ...
        this.$loader.find('#newContentLoaderText').replaceWith(newContentLoaderText);
    ...
    _onBackgroundClick: function (ev) {
        if (this.$newContentMenuChoices.hasClass('o-hidden')) {
            this._showMenu();
        } else {
            this._hideMenu();
        }
            ...
            buttons = [{
                ...
                click: function () {
                    ...
                    var $finalPosition = self.$newContentMenuChoices
                        .find('.o-new-content-element:not([data-module-id])')
                        .filter(function () {
                    ...
                    if ($finalPosition) {
                        $el.fadeTo(400, 0, function () {
                            ...
                            if (!$el.hasClass('o-new-content-element-once')) {
                            ...
                            $p.removeClass('o-uninstalled-module')
...
registry.category("websiteNavbarWidgets").add("NewContentMenu", {
    Widget: NewContentMenu,
    selector: '.o-new-content-menu',
});
