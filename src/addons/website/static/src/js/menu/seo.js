verp.define('website.seo', function (require) {
'use strict';
...
var Suggestion = Widget.extend({
    template: 'website.seoSuggestion',
    xmlDependencies: ['/website/static/src/xml/website.seo.xml'],
    events: {
        'click .o-seo-suggestion': 'select',
    },
...
var SuggestionList = Widget.extend({
    template: 'website.seoSuggestionList',
    xmlDependencies: ['/website/static/src/xml/website.seo.xml'],
    ...
    refresh: function () {
        ...
        this._rpc({
            route: '/website/seoSuggest',
            params: {
                keywords: self.root,
                lang: language,
            },
        }).then(function (keywordList) {
            self.addSuggestions(JSON.parse(keywordList));
        });
    },
...
var Keyword = Widget.extend({
    ...
    init: function (parent, options) {
        ...
        this.language = options.language;
        this.htmlPage = options.htmlPage;
        this.usedH1 = this.htmlPage.isInHeading1(this.keyword);
        this.usedH2 = this.htmlPage.isInHeading2(this.keyword);
        this.usedContent = this.htmlPage.isInBody(this.keyword);
        this._super(parent);
    },
    start: function () {
        var self = this;
        this.$('.o-seo-keyword-suggestion').empty();
        ...
        return this.suggestionList.appendTo(this.$('.o-seo-keyword-suggestion')).then(function() {
    ...
    _updateTitle: function () {
        var $title = this.$('.js-seo-keyword-title');
    ...
    _updateDescription: function () {
        var $description = this.$('.js-seo-keyword-description');
...
var KeywordList = Widget.extend({
    ...
    keywords: function () {
        var result = [];
        this.$('.js-seo-keyword').each(function () {
...
var HtmlPage = Class.extend(mixins.PropertiesMixin, {
    init: function () {
        ...
        this.defaultTitle = $('meta[name="defaultTitle"]').attr('content');
    ...
    bodyText: function () {
        return $('body').children().not('.oe-seo-configuration').text();
    },
    heading1: function () {
        return $('body').children().not('.oe-seo-configuration').find('h1').text();
    },
    heading2: function () {
        return $('body').children().not('.oe-seo-configuration').find('h2').text();
    },
...
var MetaTitleDescription = Widget.extend({
    ...
    template: 'website.seoMetaTitleDescription',
    xmlDependencies: ['/website/static/src/xml/website.seo.xml'],
    events: {
        'input input[name=websiteMetaTitle]': '_titleChanged',
        'input input[name=websiteSeoName]': '_seoNameChanged',
        'input textarea[name=websiteMetaDescription]': '_descriptionOnInput',
        'change textarea[name=websiteMetaDescription]': '_descriptionOnChange',
    },
     ...
     */
    start: function () {
        this.$title = this.$('input[name=websiteMetaTitle]');
        this.$seoName = this.$('input[name=websiteSeoName]');
        this.$seoNamePre = this.$('span.seo-name-pre');
        this.$seoNamePost = this.$('span.seo-name-post');
        this.$description = this.$('textarea[name=websiteMetaDescription]');
        this.$warning = this.$('div#websiteMetaDescriptionWarning');
        this.$preview = this.$('.js-seo-preview');
     ...
     * Get the potential new url with custom seoName as slug.
       I can differ after save if slug JS != slug Javascript, but it provide an idea for the preview
     */
    getUrl: function () {
...
var MetaKeywords = Widget.extend({
    ...
    template: 'website.seoMetaKeywords',
    xmlDependencies: ['/website/static/src/xml/website.seo.xml'],
    events: {
        'keyup input[name=websiteMetaKeywords]': '_confirmKeyword',
    ...
    start: function () {
        var self = this;
        this.$input = this.$('input[name=websiteMetaKeywords]');
    ...
    _addKeyword: function () {
        var $language = this.$('select[name=seoPageLanguage]');
    ...
    _getLanguages: function () {
        ...
        this._rpc({
            route: '/website/getLanguages',
        }).then(function (data) {
            self.$('#languageBox').html(core.qweb.render('Configurator.languagePromote', {
                'language': data,
                'defLang': context.lang
            }));
        });
    },
    /*
...
var MetaImageSelector = Widget.extend({
    template: 'website.seoMetaImageSelector',
    xmlDependencies: ['/website/static/src/xml/website.seo.xml'],
    events: {
        'click .o-meta-img-upload': '_onClickUploadImg',
        'click .o-meta-img': '_onClickSelectImg',
    },
    /**
     * @override
     * @param {widget} parent
    ...
    init: function (parent, data) {
        ...
        const imgField = data.hasSocialDefaultImage ? 'socialDefaultImage' : 'logo';
        data.pageImages.unshift(_.str.sprintf('/web/image/website/%s/%s', session.websiteId, imgField));
    ...
    */
    _updateTemplateBody: function () {
        ...
        this.$el.append(core.qweb.render('website.ogImageBody', {widget: this}));
    },
...
var SeoConfigurator = Dialog.extend({
    template: 'website.seoConfiguration',
    ...
    start: function () {
        ...
        this.$modal.addClass('oe-seo-configuration');
        ...
        this.disableUnsavableFields().then(function () {
            ...
            self.metaImageSelector.appendTo(self.$('.js-seo-image'));
            ...
            self.metaTitleDescription.appendTo(self.$('.js-seo-meta-title-description'));

            // keywords
            self.metaKeywords = new MetaKeywords(self, {htmlPage: self.htmlPage});
            self.metaKeywords.appendTo(self.$('.js-seo-meta-keywords'));
        });
    },
    /*
    ...
    disableUnsavableFields: function () {
        ...
        return this.loadMetaData().then(function (data) {
            ...
            // - no reload if we are not editing a view (condition: websiteId === undefined)
            // - reload if generic page (condition: websiteId === false)
            self.reloadOnSave = data.websiteId === undefined ? false : !data.websiteId;
            //If website.page, hide the google preview & tell user his page is currently unindexed
            self.isIndexed = (data && ('websiteIndexed' in data)) ? data.websiteIndexed : true;
            self.canEditTitle = data && ('websiteMetaTitle' in data);
            self.canEditDescription = data && ('websiteMetaDescription' in data);
            self.canEditKeywords = data && ('websiteMetaKeywords' in data);
            self.metaImg = data.websiteMetaOgImg;
            self.hasSocialDefaultImage = data.hasSocialDefaultImage;
            self.canEditUrl = data && ('seoName' in data);
            self.seoName = self.canEditUrl && data.seoName;
            self.seoNameDefault = self.canEditUrl && data.seoNameDefault;
    ...
    update: function () {
        ...
        var data = {};
        if (this.canEditTitle) {
            data.websiteMetaTitle = this.metaTitleDescription.$title.val();
        }
        if (this.canEditDescription) {
            data.websiteMetaDescription = this.metaTitleDescription.$description.val();
        }
        if (this.canEditKeywords) {
            data.websiteMetaKeywords = this.metaKeywords.keywordList.keywords().join(', ');
        }
        if (this.canEditUrl) {
            if (this.metaTitleDescription.$seoName.val() != this.metaTitleDescription.previousSeoName) {
                data.seoName = this.metaTitleDescription.$seoName.val();
                self.reloadOnSave = true;  // will force a refresh on old url and redirect to new slug
            }
        }
        data.websiteMetaOgImg = this.metaImageSelector.activeMetaImg;
    ...
    getSeoObject: function () {
        var seoObject;
        this.triggerUp('seoObjectRequest', {
            callback: function (value) {
                seoObject = value;
            },
        });
    ...
    loadMetaData: function () {
        ...
        return new Promise(function (resolve, reject) {
            if (!obj) {
                // return Promise.reject(new Error("No mainObject was found."));
                resolve(null);
            } else {
                rpc.query({
                    route: "/website/getSeoData",
                    params: {
                        'resId': obj.id,
                        'resModel': obj.model,
                    },
...
var SeoMenu = websiteNavbarData.WebsiteNavbarActionWidget.extend({
    actions: _.extend({}, websiteNavbarData.WebsiteNavbarActionWidget.prototype.actions || {}, {
        'promoteCurrentPage': '_promoteCurrentPage',
    }),

    init: function (parent, options) {
        this._super(parent, options);

        if ($.deparam.querystring().enableSeo !== undefined) {
...
registry.category("websiteNavbarWidgets").add("SeoMenu", {
    Widget: SeoMenu,
    selector: '#promoteMenu',
});
