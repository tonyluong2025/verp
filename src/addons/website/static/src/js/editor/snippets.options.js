/* globals google*/
verp.define('website.editor.snippets.options', function (require) {
'use strict';
...
const {isImageSupportedForStyle} = require('web_editor.imageProcessing');
require('website.sPopupOptions');
...
const UrlPickerUserValueWidget = InputUserValueWidget.extend({
    customEvents: _.extend({}, InputUserValueWidget.prototype.customEvents || {}, {
        'websiteUrlChosen': '_onWebsiteURLChosen',
    }),
    events: _.extend({}, InputUserValueWidget.prototype.events || {}, {
        'click .o-we-redirect-to': '_onRedirectTo',
    }),
    ...
    start: async function () {
        ...
        linkButton.classList.add('o-we-redirect-to');
...
const FontFamilyPickerUserValueWidget = SelectUserValueWidget.extend({
    ...
    events: _.extend({}, SelectUserValueWidget.prototype.events || {}, {
        'click .o-we-add-google-font-btn': '_onAddGoogleFontClick',
        'click .o-we-delete-google-font-btn': '_ondeleteGoogleFontClick',
    }),
    ...
    start: async function () {
        ...
        _.times(nbFonts, fontNb => {
            ...
            fontEl.classList.add(`o-we-option-font-${realFontNb}`);
        ...
        if (this.googleLocalFonts.length) {
            ...
            googleLocalFontsEls.forEach((el, index) => {
                $(el).append(core.qweb.render('website.deleteGoogleFontBtn', {
                    index: index,
                    local: true,
                }));
            });
        ...
        $(this.menuEl).append($(core.qweb.render('website.addGoogleFontBtn', {
            variable: variable,
        })));
    },
    ...
    async setValue() {
        ...
        for (const className of this.menuTogglerEl.classList) {
            if (className.match(/^o-we-option-font-\d+$/)) {
                this.menuTogglerEl.classList.remove(className);
            }
        }
        const activeWidget = this._userValueWidgets.find(widget => !widget.isPreviewed() && widget.isActive());
        if (activeWidget) {
            this.menuTogglerEl.classList.add(`o-we-option-font-${activeWidget.el.dataset.font}`);
        }
    },
            ...
            buttons: [
                {
                    ...
                    classes: 'btn-primary',
                    click: async () => {
                        const inputEl = dialog.el.querySelector('.o-input-google-font');
                        ...
                        const googleFontServe = dialog.el.querySelector('#googleFontServe').checked;
                        if (googleFontServe) {
                            this.googleFonts.push(font);
                        } else {
                            this.googleLocalFonts.push(`'${font}': ''`);
                        }
                        this.triggerUp('googleFontsCustoRequest', {
...
const GPSPicker = InputUserValueWidget.extend({
    ...
    async willStart() {
        ...
        if (!this._gmapLoaded && !this._gmapErrorNotified) {
            this.triggerUp('userValueWidgetCritical');
            return;
        }
    },
    /**
    ...
    async start() {
        ...
        google.maps.event.addListener(this._gmapAutocomplete, 'placeChanged', this._onPlaceChanged.bind(this));
    },
    ...
    _notifyGMapError() {
        ...
        setTimeout(() => this.triggerUp('userValueWidgetCritical'));
    },
...
options.Class.include({
    xmlDependencies: (options.Class.prototype.xmlDependencies || [])
        .concat(['/website/static/src/xml/website.editor.xml']),
    customEvents: _.extend({}, options.Class.prototype.customEvents || {}, {
        'googleFontsCustoRequest': '_onGoogleFontsCustoRequest',
    }),
    ...
    async _customizeWebsiteData(value, params, isViewData) {
        ...
        if (params.name === 'headerSidebarOpt') {
            ...
            await new Promise(resolve => {
                this.triggerUp('actionDemand', {
                    actionName: 'togglePageOption',
                    params: [{name: 'headerOverlay', value: false}],
                    onSuccess: () => resolve(),
                });
            });
        }
        ...
        return this._rpc({
            route: '/website/themeCustomizeData',
            params: {
                'isViewData': isViewData,
                'enable': enableDataKeys,
                'disable': disableDataKeys,
                'resetViewArch': resetViewArch,
            },
        });
    },
    /**
    ...
    async _getEnabledCustomizeValues(possibleValues, isViewData) {
        ...
        const enabledValues = await this._rpc({
            route: '/website/themeCustomizeDataGet',
            params: {
                'keys': allDataKeys,
                'isViewData': isViewData,
            },
    ...
    _reloadBundles: async function () {
        const bundles = await this._rpc({
            route: '/website/themeCustomizeBundleReload',
        });
        let $allLinks = $();
...
function _getLastPreFilterLayerElement($el) {
    ...
    // color filters / shape
    const $bgVideo = $el.find('> .o-bg-video-container');
    if ($bgVideo.length) {
        return $bgVideo[0];
    }
    const $parallaxEl = $el.find('> .s-parallax-bg');
...
options.registry.BackgroundToggler.include({
    ...
    toggleBgVideo(previewMode, widgetValue, params) {
        if (!widgetValue) {
            ...
            const [bgVideoWidget] = this._requestUserValueWidgets('bgVideoOpt');
            ...
            this._requestUserValueWidgets('bgVideoOpt')[0].el.click();
        }
    },
    ...
    _computeWidgetState(methodName, params) {
        if (methodName === 'toggleBgVideo') {
            return this.$target[0].classList.contains('o-background-video');
        }
        return this._super(...arguments);
    },
    /**
...
options.registry.ReplaceMedia.include({
    ...
    async _computeWidgetVisibility(widgetName, params) {
        if (widgetName === 'mediaLinkAnchorOpt') {
            const parentEl = this.$target[0].parentElement;
    ...
    async _renderCustomXML(uiFragment) {
        ...
        const oldURLWidgetEl = uiFragment.querySelector('[data-name="mediaUrlOpt"]');
        ...
        selectEl.dataset.name = 'mediaLinkAnchorOpt';
        selectEl.dataset.dependencies = 'mediaUrlOpt';
...
options.registry.BackgroundVideo = options.Class.extend({
    ...
    _computeWidgetState: function (methodName, params) {
        if (methodName === 'background') {
            if (this.$target[0].classList.contains('o-background-video')) {
                return this.$('> .o-bg-video-container iframe').attr('src');
            }
            return '';
        }
        return this._super(...arguments);
    ...
    _setBgVideo: async function (previewMode, value) {
        this.$('> .o-bg-video-container').toggleClass('d-none', previewMode === true);
        ...
        target.classList.toggle('o-background-video', !!(value && value.length));
...
options.registry.OptionsTab = options.Class.extend({
    ...
    async updateUI() {
        ...
        this.$el.find(".o-we-gray-preview").each((_, e) => {
            e.style.removeProperty("background-color");
        });

        // If the gray palette has been generated by Verp standard option,
        ...
        // same hue by the Verp editor, there might be rounding errors
    ...
    async configureApiKey(previewMode, widgetValue, params) {
        return new Promise(resolve => {
            this.triggerUp('gmapApiKeyRequest', {
                editableMode: true,
                reconfigure: true,
                onSuccess: () => resolve(),
            });
    ...
    async openCustomCodeDialog(previewMode, widgetValue, params) {
        ...
        const dataProm = this._rpc({
            model: 'website',
            method: 'read',
            args: [[websiteId], ['customCodeHead', 'customCodeFooter']],
        }).then(websites => {
            website = websites[0];
        });
        ...
        await new Promise(resolve => {
            ...
            const aceEditor = this._renderAceEditor($content.find('.o-ace-editor-container')[0], website[fieldName] || '');
            const dialog = new Dialog(this, {
                title,
                $content,
                buttons: [
    ...
    async switchTheme(previewMode, widgetValue, params) {
        ...
        this.triggerUp('requestSave', {
            reload: false,
            onSuccess: () => window.location.href = '/web#action=website.themeInstallKanbanAction',
        });
    },
...
options.registry.ThemeColors = options.registry.OptionsTab.extend({
    ...
    async updateUIVisibility() {
        await this._super(...arguments);
        const oldColorSystemEl = this.el.querySelector('.o-old-color-system-warning');
    ...
    async _renderCustomXML(uiFragment) {
        ...
        for (const paletteName of allPaletteNames) {
            ...
            btnEl.classList.add('o-palette-color-preview-button');
            btnEl.dataset.customizeWebsiteVariable = `'${paletteName}'`;
            [1, 3, 2].forEach(c => {
                const colorPreviewEl = document.createElement('span');
                colorPreviewEl.classList.add('o-palette-color-preview');
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            ccPreviewEl.classList.add('text-center', `o-cc${i}`, 'o-we-collapse-toggler');
            collapseEl.appendChild(ccPreviewEl);
            const editionEls = $(qweb.render('website.colorCombinationEdition', {number: i}));
...
options.registry.menuData = options.Class.extend({
    /**
...
options.registry.companyData = options.Class.extend({
    /**
     ...
     */
    start: function () {
        var proto = options.registry.companyData.prototype;
        var prom;
        var self = this;
        if (proto.__link === undefined) {
            prom = this._rpc({route: '/web/session/getSessionInfo'}).then(function (session) {
                return self._rpc({
                    model: 'res.users',
                    method: 'read',
                ...
                proto.__link = '/web#action=base.actionResCompanyForm&viewType=form&id=' + (res && res[0] && res[0].companyId[0] || 1);
            });
        }
...
options.registry.Carousel = options.Class.extend({
    ...
    start: function () {
        ...
        this.$controls.addClass('o-we-no-overlay');

        let _slideTimestamp;
        this.$target.on('slide.bs.carousel.carouselOption', () => {
            _slideTimestamp = window.performance.now();
            setTimeout(() => this.triggerUp('hideOverlay'));
        });
        this.$target.on('slid.bs.carousel.carouselOption', () => {
            ...
            setTimeout(() => {
                ...
                this.$target.trigger('activeSlideTargeted');
            }, 0.2 * _slideDuration);
        });
    ...
    destroy: function () {
        ...
        this.$target.off('.carouselOption');
    },
    /**
     * @override
     */
...
options.registry.CarouselItem = options.Class.extend({
    ...
    destroy: function () {
        this._super(...arguments);
        this.$carousel.off('.carouselItemOption');
    },
    ...
    removeSlide: function (previewMode) {
        ...
        if (!this.removing && newLength > 0) {
            ...
            this.$carousel.one('activeSlideTargeted.carouselItemOption', () => {
                $toDelete.remove();
                this.$indicators.find('li:last').remove();
                this.$controls.toggleClass('d-none', newLength === 1);
                this.$carousel.trigger('contentChanged');
                this.removing = false;
            });
            this.removing = true;
...
options.registry.Parallax = options.Class.extend({
     ...
     */
    async start() {
        this.parallaxEl = this.$target.find('> .s-parallax-bg')[0] || null;
        this._updateBackgroundOptions();

        this.$target.on('contentChanged.ParallaxOption', this._onExternalUpdate.bind(this));
    ...
    async selectDataAttribute(previewMode, widgetValue, params) {
        ...
        this.$target.toggleClass('s-parallax-is-fixed', widgetValue === '1');
        this.$target.toggleClass('s-parallax-no-overflow-hidden', (widgetValue === '0' || widgetValue === '1'));
        if (isParallax) {
            if (!this.parallaxEl) {
                this.parallaxEl = document.createElement('span');
                this.parallaxEl.classList.add('s-parallax-bg');
    ...
    async _computeVisibility(widgetName) {
        return !this.$target.hasClass('o-background-video');
    },
    /**
     * @override
     */
    ...
    _onExternalUpdate(ev) {
        ...
        if (!bgImage || bgImage === 'none' || this.$target.hasClass('o-background-video')) {
            // The parallax option was enabled but the background image was
            // removed: disable the parallax option.
            const widget = this._requestUserValueWidgets('parallaxNoneOpt')[0];
            widget.enable();
...
options.registry.collapse = options.Class.extend({
    ...
    start: function () {
        ...
        this.$target.on('shown.bs.collapse hidden.bs.collapse', '[role="tabpanel"]', function () {
            ...
            self.$target.trigger('contentChanged');
        });
        return this._super.apply(this, arguments);
    },
    /**
...
const VisibilityPageOptionUpdate = options.Class.extend({
    ...
    async start() {
        ...
        this.triggerUp('snippetOptionVisibilityUpdate', {show: shown});
    },
    /**
     * @override
     */
    ...
    async visibility(previewMode, widgetValue, params) {
        ...
        this.triggerUp('snippetOptionVisibilityUpdate', {show: show});
    },
...
options.registry.TopMenuVisibility = VisibilityPageOptionUpdate.extend({
    pageOptionName: 'headerVisible',
    showOptionWidgetName: 'regularHeaderVisibilityOpt',
    ...
    async _changeVisibility(widgetValue) {
        ...
        await new Promise((resolve, reject) => {
            this.triggerUp('actionDemand', {
                actionName: 'togglePageOption',
                params: [{name: 'headerOverlay', value: transparent}],
                onSuccess: () => resolve(),
                onFailure: reject,
            });
        });
    ...
    _computeWidgetVisibility(widgetName, params) {
        if (widgetName === 'headerVisibilityOpt') {
            return this.$target[0].classList.contains('o-header-sidebar') ? '' : 'true';
        }
        return this._super(...arguments);
    },
    /**
    ...
    _renderCustomXML(uiFragment) {
        ...
        const weSelectEl = uiFragment.querySelector('we-select#optionHeaderVisibility');
        if (weSelectEl) {
            weSelectEl.dataset.name = 'headerVisibilityOpt';
        }
    },
});
...
options.registry.topMenuColor = options.Class.extend({
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        await new Promise((resolve, reject) => {
            this.triggerUp('actionDemand', {
                actionName: 'togglePageOption',
                params: [{name: 'headerColor', value: className}],
                onSuccess: resolve,
                onFailure: reject,
            });
        });
...
options.registry.HideFooter = VisibilityPageOptionUpdate.extend({
    pageOptionName: 'footerVisible',
    showOptionWidgetName: 'hideFooterPageOpt',
    shownValue: 'shown',
});
...
options.registry.anchor = options.Class.extend({
    ...
    openAnchorDialog: function (previewMode, widgetValue, params) {
        ...
        var buttons = [{
            ...
            classes: 'btn-primary',
            click: function () {
                var $input = this.$('.o-input-anchor-name');
                ...
                this.$('.o-anchor-already-exists').toggleClass('d-none', !alreadyExists);
...
options.registry.CookiesBar = options.registry.SnippetPopup.extend({
    ...
    selectLayout: function (previewMode, widgetValue, params) {
        ...
        const $template = $(qweb.render(`website.cookiesBar.${widgetValue}`, {
            websiteId: websiteId,
        }));

        const $content = this.$target.find('.modal-content');
        const selectorsToKeep = [
            '.o-cookies-bar-text-button',
            '.o-cookies-bar-text-policy',
            '.o-cookies-bar-text-title',
            '.o-cookies-bar-text-primary',
            '.o-cookies-bar-text-secondary',
        ];
...
/**
 * Allows edition of 'coverProperties' in website models which have such
...
options.registry.CoverProperties = options.Class.extend({
    ...
    init: function () {
        ...
        this.$image = this.$target.find('.o-record-cover-image');
        this.$filter = this.$target.find('.o-record-cover-filter');
    },
    /**
     * @override
     */
    ...
    background: async function (previewMode, widgetValue, params) {
        if (widgetValue === '') {
            ...
            this.$target.removeClass('o-record-has-cover');
        } else {
            this.$image.css('background-image', `url('${widgetValue}')`);
            this.$target.addClass('o-record-has-cover');
            const $defaultSizeBtn = this.$el.find('.o-record-cover-opt-size-default');
    ...
    filterValue: function (previewMode, widgetValue, params) {
        ...
        this.$filter.toggleClass('oe-black', parseFloat(widgetValue) !== 0);
    ...
    _updateSavingDataset(colorValue) {
        const [colorPickerWidget, sizeWidget, textAlignWidget] = this._requestUserValueWidgets('bgColorOpt', 'sizeOpt', 'textAlignOpt');
        // TODO: `o-record-has-cover` should be handled using model field, not
        // resizeClass to avoid all of this.
        ...
        const filterEl = this.$target[0].querySelector('.o-record-cover-filter');
...
options.registry.ScrollButton = options.Class.extend({
    ...
    start: async function () {
        await this._super(...arguments);
        this.$button = this.$('.o-scroll-button');
    },
    ...
    _renderCustomXML(uiFragment) {
        ...
        if (this.$target[0].dataset.snippet !== 'sImageGallery') {
            return;
        }
        let minHeightEl = uiFragment.querySelector('[data-name="minheightAutoOpt"]');
        if (!minHeightEl) {
            return;
        }
        ...
        heightEl.dataset.dependencies = 'minheightAutoOpt';
...
options.registry.ConditionalVisibility = options.Class.extend({
     ...
     */
    async onTargetHide() {
        this.$target[0].classList.add('o-conditional-hidden');
    },
    /**
     * @override
     */
    async onTargetShow() {
        this.$target[0].classList.remove('o-conditional-hidden');
    },
    /**
     * @override
     */
    cleanForSave() {
        ...
        this.triggerUp('snippetOptionVisibilityUpdate', { show: true });
    },
    ...
    _updateCSSSelectors() {
        ...
        for (const attribute of onlyAttributes) {
            ...
            }, 'html') + ` body:not(.editor-enable) [data-visibility-id="${visibilityId}"]`;
            selectors += selector + ', ';
        }
        for (const attribute of hideAttributes) {
            ...
            const selector = attribute.records.reduce((acc, record, i, a) => {
                acc += `html[${attribute.name}="${record.value}"] body:not(.editor-enable) [data-visibility-id="${visibilityId}"]`;
...
options.registry.ImageTools.include({
    ...
    async _computeWidgetVisibility(widgetName, params) {
        ...
        if ('transform' in params.optionsPossibleValues) {
            ...
            return result && !this.$target.hasClass('o-animate');
        }
        return result;
    },
});
...
options.registry.WebsiteAnimate = options.Class.extend({
    ...
    async start() {
        await this._super(...arguments);
        this.isAnimatedText = this.$target.hasClass('o-animated-text');
     ...
     */
    async onBuilt() {
        this.$target[0].classList.toggle('o-animate-preview', this.$target[0].classList.contains('o-animate'));
    },
    /**
     * @override
     */
    ...
    async selectClass(previewMode, widgetValue, params) {
        ...
        if (params.isAnimationTypeSelection) {
            ...
            this.$target.toggleClass('o-animate-preview o-animate', !!widgetValue);
        }
    },
    ...
    _forceAnimation() {
        ...
        this.$target.addClass('o-animating');
        $scrollingElement[0].classList.add('o-wanim-overflow-x-hidden');
        this.$target.css('animation-name', '');
        this.$target.one('webkitAnimationEnd oanimationend msAnimationEnd animationend', () => {
            $scrollingElement[0].classList.remove('o-wanim-overflow-x-hidden');
            this.$target.removeClass('o-animating');
        });
    },
    /**
     * @override
     */
    _computeWidgetVisibility(widgetName, params) {
        if (widgetName === 'noAnimationOpt') {
            return !this.isAnimatedText;
        }
        if (widgetName === 'animationLaunchOpt') {
...
options.registry.MegaMenuLayout = options.registry.SelectTemplate.extend({
    ...
    init() {
        this._super(...arguments);
        this.selectTemplateWidgetName = 'megaMenuTemplateOpt';
    },
    ...
    _getCurrentTemplateXMLID: function () {
        const templateDefiningClass = this.containerEl.querySelector('section')
            .classList.value.split(' ').filter(cl => cl.startsWith('sMegaMenu'))[0];
