verp.define("website.tourUtils", function (require) {
"use strict";
...
function changeBackground(snippet, position = "bottom") {
    return {
        trigger: ".o-we-customize-panel .o-we-bg-success",
...
function changeBackgroundColor(position = "bottom") {
    return {
        trigger: ".o-we-customize-panel .o-we-color-preview",
...
function selectColorPalette(position = "left") {
    return {
        trigger: ".o-we-customize-panel .o-we-so-color-palette we-selection-items",
        altTrigger: ".o-we-customize-panel .o-we-color-preview",
        content: Markup(_t(`<b>Select</b> a Color Palette.`)),
        position: position,
        run: 'click',
        location: position === 'left' ? '#oeSnippets' : undefined,
    };
}

function changeColumnSize(position = "right") {
    return {
        trigger: `.oe-overlay.ui-draggable.o-we-overlay-sticky.oe-active .o-handle.e`,
...
function changeIcon(snippet, index = 0, position = "bottom") {
    return {
        ...
        extraTrigger: "body.editor-enable",
...
function selectNested(trigger, optionName, altTrigger = null, optionTooltipLabel = '', position = "top") {
    const option_block = `we-customizeblock-option[class='snippet-option-${optionName}']`;
    return {
        trigger: trigger,
        content: Markup(_.str.sprintf(_t("<b>Select</b> a %s."), optionTooltipLabel)),
        altTrigger: altTrigger == null ? undefined : `${option_block} ${altTrigger}`,
        position: position,
        run: 'click',
        location: position === 'left' ? '#oeSnippets' : undefined,
    };
...
function changePaddingSize(direction) {
    ...
    return {
        trigger: `.oe-overlay.ui-draggable.o-we-overlay-sticky.oe-active .o-handle.${paddingDirection}`,
...
function clickOnSnippet(snippet, position = "bottom") {
    return {
        ...
        extraTrigger: "body.editor-enable #oeSnippets.o-loaded",
...
function clickOnSave(position = "bottom") {
    return [{
        ...
        extraTrigger: "#oeSnippets:not(:has(.o-we-already_dragging))",
        inModal: false,
...
function dragNDrop(snippet, position = "bottom") {
    return {
        trigger: `#oeSnippets .oe-snippet[name="${snippet.name}"] .oe-snippet-thumbnail:not(.o-we-already_dragging)`,
        extraTrigger: "body.editor-enable.editor-has-snippets",
        moveTrigger: '.oe-drop-zone',
        ...
        run: "dragAndDrop #wrapwrap > footer",
    };
}
...
function clickOnExtraMenuItem(stepOptions) {
    return Object.assign({}, {
        ...
        trigger: '#topMenu',
        run: function () {
            const extraMenuButton = this.$anchor[0].querySelector('.o-extra-menu-items a.nav-link');
            if (extraMenuButton) {
