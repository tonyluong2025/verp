verp.define("website.configuratorTour", function (require) {
"use strict";
...
let isTitleTextImage = $(titleSelector).hasClass('s-text-image');
...
if ($(shapeSelector).first().length) {
    ...
    shapeStep.push(wTourUtils.selectNested('we-select-page', 'BackgroundShape', ':not(.o-we-pager-controls)', _t('Background Shape')));
}
...
wTourUtils.registerThemeHomepageTour('configuratorTour', steps);
