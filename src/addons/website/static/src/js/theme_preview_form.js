verp.define('website.themePreviewForm', function (require) {
"use strict";
...
const ThemePreviewControllerCommon = {
     ...
     * @param {String} action
     */
    _handleThemeAction(resId, action) {
        this.$loader = $(qweb.render('website.ThemePreview.Loader', {
            'showTips': action !== 'buttonRefreshTheme',
        }));
        let actionCallback = undefined;
        this._addLoader();
        switch (action) {
            case 'buttonChooseTheme':
                actionCallback = result => this.doAction(result);
                break;
            case 'buttonRefreshTheme':
...
var ThemePreviewController = FormController.extend(ThemePreviewControllerCommon, {
    events: Object.assign({}, FormController.prototype.events, {
        'click .o-use-theme': '_onStartNowClick',
        'click .o-switch-theme': '_onSwitchThemeClick',
     ...
     */
    start: function () {
        this.$el.addClass('o-view-form-theme-preview-controller');
    ...
    _updateControlPanelProps: async function () {
        ...
        this.controlPanelProps.cpContent.$pager = $switchModeButton;
        return props;
    },
    ...
    _onSwitchButtonChange: function () {
        this.$('.o-preview-frame').toggleClass('isMobile');
    },
    /**
    ...
    _onStartNowClick: function () {
        this._handleThemeAction(this.getSelectedIds()[0], 'buttonChooseTheme');
    },
});
...
viewRegistry.add('themePreviewForm', ThemePreviewFormView);
