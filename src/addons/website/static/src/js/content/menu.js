verp.define('website.content.menu', function (require) {
'use strict';
...
const BaseAnimatedHeader = animations.Animation.extend({
    ...
    start: function () {
        ...
        this.isOverlayHeader = !!this.$el.closest('.o-header-overlay, .o-header-overlay-theme').length;
        ...
        this.$navbarCollapses.on('show.bs.collapse.BaseAnimatedHeader', function () {
            if (config.device.sizeClass <= config.device.SIZES.SM) {
        ...
        // transitionstart, so we listen to a custom verp event.
        this._transitionCount = 0;
        this.$el.on('verp-transitionstart.BaseAnimatedHeader', () => {
            this.el.classList.add('o-transitioning');
    ...
    destroy: function () {
        ...
        this.$el.removeClass('o-header-affixed o-header-is-scrolled o-header-no-transition o-transitioning');
    ...
    _adaptToHeaderChange: function () {
        this.options.wysiwyg && this.options.wysiwyg.verpEditor.observerUnactive();
        ...
        this.el.classList.toggle('o-top-fixed-element', this._isShown());

        for (const callback of extraMenuUpdateCallbacks) {
            callback();
        }
        this.options.wysiwyg && this.options.wysiwyg.verpEditor.observerActive();
    },
    /**
     * @private
    ...
    _adaptToHeaderChangeLoop: function (addCount = 0) {
        ...
        if (this._transitionCount > 0) {
            ...
            // setTimeout fallback.
            clearTimeout(this._changeLoopTimer);
            this.el.classList.remove('o-transitioning');
        }
    },
    /**
     * @private
    ...
    _toggleFixedHeader: function (useFixed = true) {
        ...
        this._adaptToHeaderChange();
        this.el.classList.toggle('o-header-affixed', useFixed);
     ...
     * 'o-header-is-scrolled' class. If so, we do not add this class if the
    ...
    _scrollHeightTooShort() {
        ...
        clonedHeader.classList.add('o-header-is-scrolled', 'o-header-affixed', 'o-header-no-transition');
    ...
    _updateHeaderOnScroll: function (scroll) {
        ...
        if (!this.hasScrolled) {
            ...
            if (scroll > 0) {
                this.$el.addClass('o-header-no-transition');
            }
        } else {
            this.$el.removeClass('o-header-no-transition');
        ...
        if (this.headerIsScrolled !== headerIsScrolled) {
            ...
            if (!this.scrollHeightTooShort) {
                this.el.classList.toggle('o-header-is-scrolled', headerIsScrolled);
                this.$el.trigger('verp-transitionstart');
    ...
    _updateHeaderOnResize: function () {
        ...
        if (document.body.classList.contains('overflow-hidden')
                && config.device.sizeClass > config.device.SIZES.SM) {
...
publicWidget.registry.StandardAffixedHeader = BaseAnimatedHeader.extend({
    selector: 'header.o-header-standard:not(.o-header-sidebar)',
...
publicWidget.registry.FixedHeader = BaseAnimatedHeader.extend({
    selector: 'header.o-header-fixed:not(.o-header-sidebar)',
    ...
    _updateHeaderOnScroll: function (scroll) {
        ...
        if (scroll > (this.scrolledPoint + this.topGap)) {
            if (!this.$el.hasClass('o-header-affixed')) {
...
publicWidget.registry.DisappearingHeader = BaseDisappearingHeader.extend({
    selector: 'header.o-header-disappears:not(.o-header-sidebar)',
...
publicWidget.registry.FadeOutHeader = BaseDisappearingHeader.extend({
    selector: 'header.o-header-fade-out:not(.o-header-sidebar)',
...
publicWidget.registry.hoverableDropdown = animations.Animation.extend({
    selector: 'header.o-hoverable-dropdown',
    disabledInEditableMode: false,
    effects: [{
        startEvents: 'resize',
...
publicWidget.registry.HeaderMainCollapse = publicWidget.Widget.extend({
    ...
    disabledInEditableMode: false,
    events: {
        'show.bs.collapse #topMenuCollapse': '_onCollapseShow',
        'hidden.bs.collapse #topMenuCollapse': '_onCollapseHidden',
    },
    ...
    start() {
        ...
        if (this.$target[0].querySelector('.o-offcanvas-menu-toggler')) {
            this.navbarEl = this.$target[0].querySelector('#topMenu');
            this.languageSelectorEl = this.$target[0].querySelectorAll('#topMenuCollapse .js-language-selector');
            // As there are 2 language selectors in 'templateHeaderVertical',
            ...
            if (this.$target[0].querySelector('.o-header-hamburger-full-toggler')) {
                const callToActionEl = this.$target[0].querySelector('.oe-structure-solo');
                if (callToActionEl) {
                    this.options.wysiwyg && this.options.wysiwyg.verpEditor.observerUnactive();
     ...
     */
    _onCollapseShow() {
        this.el.classList.add('o-top-menu-collapse-shown');
     ...
     */
    _onCollapseHidden() {
        this.el.classList.remove('o-top-menu-collapse-shown');
        if (this.languageSelectorMustBeMoved) {
            this.options.wysiwyg && this.options.wysiwyg.verpEditor.observerUnactive();
