verp.define('website.content.snippets.animation', function (require) {
'use strict';
...
publicWidget.Widget.include({
    ...
    init: function (parent, options) {
        ...
        var extraEvents = this.editableMode ? this.editEvents : this.readEvents;
        if (extraEvents) {
...
registry.Parallax = Animation.extend({
    ...
    start: function () {
        this._rebuild();
        $(window).on('resize.animationParallax', _.debounce(this._rebuild.bind(this), 500));
        return this._super.apply(this, arguments);
    },
    /**
     * @override
...
const MobileYoutubeAutoplayMixin = {
    ...
    _setupAutoplay: function (src) {
        ...
        this.isMobileEnv = config.device.sizeClass <= config.device.SIZES.LG && config.device.touch;
...
registry.mediaVideo = publicWidget.Widget.extend(MobileYoutubeAutoplayMixin, {
    ...
    start: function () {
        ...
        // integrated with Verp (this refactoring should be done in master).
        ...
        // videos added before bug fixes or new Verp versions where the
    ...
    _generateIframe: function () {
        ...
        this.$target.append(
            ...
            '<div class="media-iframe-video-size">&nbsp;</div>'
        );
...
registry.backgroundVideo = publicWidget.Widget.extend(MobileYoutubeAutoplayMixin, {
    ...
    start: function () {
        ...
        this.iframeID = _.uniqueId('o-bg-video-iframe_');
        ...
        if ($modal.length) {
            $modal.on('show.bs.modal', () => {
                const videoContainerEl = this.$target[0].querySelector('.o-bg-video-container');
    ...
    _appendBgVideo: function () {
        var $oldContainer = this.$bgVideoContainer || this.$('> .o-bg-video-container');
        this.$bgVideoContainer = $(qweb.render('website.background.video', {
            videoSrc: this.videoSrc,
            iframeID: this.iframeID,
        }));
        this.$iframe = this.$bgVideoContainer.find('.o-bg-video-iframe');
        this.$iframe.one('load', () => {
            this.$bgVideoContainer.find('.o-bg-video-loading').remove();
        });
...
registry.socialShare = publicWidget.Widget.extend({
    ...
    _bindSocialEvent: function () {
        this.$('.oe-social-facebook').click($.proxy(this._renderSocial, this, 'facebook'));
        this.$('.oe-social-twitter').click($.proxy(this._renderSocial, this, 'twitter'));
        this.$('.oe-social-linkedin').click($.proxy(this._renderSocial, this, 'linkedin'));
    },
    /**
     * @private
     */
    _render: function () {
        this.$el.popover({
            content: qweb.render('website.socialHover', {medias: this.socialList}),
            placement: 'bottom',
            container: this.$el,
            html: true,
            trigger: 'manual',
...
registry.FooterSlideout = publicWidget.Widget.extend({
    selector: '#wrapwrap:has(.o-footer-slideout)',
    disabledInEditableMode: false,
    ...
    async start() {
        ...
        this.el.classList.toggle('o-footer-effect_enable', slideoutEffect);
    ...
    destroy() {
        this._super(...arguments);
        this.el.classList.remove('o-footer-effect_enable');
        this.__pixelEl.remove();
    },
});

registry.HeaderHamburgerFull = publicWidget.Widget.extend({
    selector: 'header:has(.o-header-hamburger-full-toggler):not(:has(.o-offcanvas-menu-toggler))',
    events: {
        'click .o-header-hamburger-full-toggler': '_onToggleClick',
    },
...
registry.BottomFixedElement = publicWidget.Widget.extend({
    ...
    async start() {
        ...
        this.$scrollingElement.on('scroll.bottom-fixed-element', this.__hideBottomFixedElements);
        $(window).on('resize.bottom-fixed-element', this.__hideBottomFixedElements);
        return this._super(...arguments);
    },
    /**
     * @override
    ...
    destroy() {
        this._super(...arguments);
        this.$scrollingElement.off('.bottom-fixed-element');
        $(window).off('.bottom-fixed-element');
        $('.o-bottom-fixed-element').removeClass('o-bottom-fixed-element-hidden');
    },
    ...
    _hideBottomFixedElements() {
        ...
        const $bottomFixedElements = $('.o-bottom-fixed-element');
        if (!$bottomFixedElements.length) {
            return;
        }

        $bottomFixedElements.removeClass('o-bottom-fixed-element-hidden');
        if ((this.$scrollingElement[0].offsetHeight + this.$scrollingElement[0].scrollTop) >= (this.$scrollingElement[0].scrollHeight - 2)) {
            ...
            for (const el of $bottomFixedElements) {
                if (buttonEls.some(button => dom.areColliding(button, el))) {
                    el.classList.add('o-bottom-fixed-element-hidden');
                }
            }
        }
    },
...
registry.WebsiteAnimate = publicWidget.Widget.extend({
    ...
    start() {
        ...
        $(window).on('resize.o-animate, shown.bs.modal.o-animate, slid.bs.carousel.o-animate, shown.bs.tab.o-animate, shown.bs.collapse.o-animate', () => {
    ...
    destroy() {
        this._super(...arguments);
        this.$target.find('.o-animate')
            .removeClass('o-animating o-animated o-animate-preview o-animate-in-dropdown')
            .css({
                'animation-name': '',
                'animation-play-state': '',
                'visibility': '',
            });
        $(window).off('.o-animate');
        this.$scrollingElement[0].removeEventListener('scroll', this.__onScrollWebsiteAnimate, {capture: true});
        this.$scrollingElement[0].classList.remove('o-wanim-overflow-x-hidden');
    },
    ...
    _startAnimation($el) {
        ...
        setTimeout(() => {
            ...
            .addClass("o-animating")
            .one('webkitAnimationEnd oanimationend msAnimationEnd animationend', () => {
                $el.addClass("o-animated").removeClass("o-animating");
    ...
    _resetAnimation($el) {
        ...
        $el.css({"animation-name": "dummy-none", "animation-play-state": ""})
           .removeClass("o-animated o-animating");
    ...
    _toggleOverflowXHidden(add) {
        if (add) {
            this.$scrollingElement[0].classList.add('o-wanim-overflow-x-hidden');
        } else if (!this.$scrollingElement.find('.o-animating').length) {
            this.$scrollingElement[0].classList.remove('o-wanim-overflow-x-hidden');
        }
    },
    /**
    ...
    _scrollWebsiteAnimate(el) {
        ...
        _.each(this.$target.find('.o-animate:not(.o-animate-in-dropdown)'), el => {
            const $el = $(el);
            ...
            } else if (!visible && $el.hasClass("o-animate-both-scroll") && state === "running") {
                $el.removeClass("o-visible");
                this._resetAnimation($el);
            }
        });
    },
