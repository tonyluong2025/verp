verp.define('website.utils', function (require) {
'use strict';
    ...
    $input.urlcomplete({
        source: function (request, response) {
            if (request.term[0] === '#') {
                ...
                // avoid useless call to /website/getSuggestedLinks
                response();
            } else {
                return self._rpc({
                    route: '/website/getSuggestedLinks',
                    params: {
                        needle: request.term,
                        limit: 15,
                    }
                }).then(function (res) {
                    let choices = res.matchingPages;
        ...
        select: function (ev, ui) {
            // choose url in dropdown with arrow change ev.target.value without triggerUp
            // so cannot check here if value has been updated
            ev.target.value = ui.item.value;
            self.triggerUp('websiteUrlChosen');
            ev.preventDefault();
        },
    });
}
...
function websiteDomain(self) {
    var websiteID;
    self.triggerUp('contextGet', {
        callback: function (ctx) {
            websiteID = ctx['websiteId'];
        },
    });
    return ['|', ['websiteId', '=', false], ['websiteId', '=', websiteID]];
}
...
function sendRequest(route, params) {
    ...
    if (core.csrfToken) {
        _addInput(form, 'csrfToken', core.csrfToken);
    }
