...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/verp" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -dfx
cp -a /home/pi/verp/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/verp/* /home/pi/verp/
rm -r /home/pi/verp/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service verp restart) &
