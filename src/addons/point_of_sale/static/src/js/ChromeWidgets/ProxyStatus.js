verp.define('point_of_sale.ProxyStatus', function(require) {
    'use strict';
    ...
    class ProxyStatus extends PosComponent {
        ...
        async onClick() {
            try {
                await this.env.pos.connectToProxy();
            } catch (error) {
        ...
        _setStatus(newStatus) {
            if (newStatus.status === 'connected') {
                var warning = false;
                var msg = '';
                if (this.env.pos.config.ifaceScanViaProxy) {
                ...
                if (
                    this.env.pos.config.ifacePrintViaProxy ||
                    this.env.pos.config.ifaceCashdrawer
                ) {
                ...
                if (this.env.pos.config.ifaceElectronicScale) {
