verp.define('point_of_sale.Chrome', function(require) {
    'use strict';
    ...
    const { verpExceptionTitleMap } = require("@web/core/errors/error_dialogs");
    ...
    class Chrome extends PopupControllerMixin(PosComponent) {
        constructor() {
            ...
            this.previousTouchYCoordinate = -1;
        }
        ...
        get clientScreenButtonIsShown() {
            return this.env.pos.config.ifaceCustomerFacingDisplay;
        }
        ...
        async start() {
            try {
                ...
                const posModelDefaultAttributes = {
                    ...
                    doAction: this.props.webClient.doAction.bind(this.props.webClient),
                ...
                this.env.pos.set(
                    'selectedCategoryId',
                    this.env.pos.config.ifaceStartCategId
                        ? this.env.pos.config.ifaceStartCategId[0]
                        : 0
                );
                this.state.uiState = 'READY';
                this.env.pos.on('change:selectedOrder', this._showSavedScreen, this);
                this._showStartScreen();
                if (_.isEmpty(this.env.pos.db.productByCategoryId)) {
                    this._loadDemoData();
                }
                setTimeout(() => {
        ...
        shouldShowCashControl() {
            return this.env.pos.config.cashControl && this.env.pos.posSession.state == 'opening';
        }
        ...
        async _loadDemoData() {
            ...
            if (confirmed) {
                await this.rpc({
                    'route': '/pos/loadOnboardingData',
                });
                this.env.pos.loadServerData();
            }
        }

        _preloadImages() {
            for (let product of this.env.pos.db.getProductByCategory(0)) {
                const image = new Image();
                image.src = `/web/image?model=product.product&field=image128&id=${product.id}&updatedAt=${product.updatedAt}&unique=1`;
            }
            for (let category of Object.values(this.env.pos.db.categoryById)) {
                if (category.id == 0) continue;
                const image = new Image();
                image.src = `/web/image?model=pos.category&field=image128&id=${category.id}&updatedAt=${category.updatedAt}&unique=1`;
            }
        ...
        _buildChrome() {
            if ($.browser.chrome) {
                var chromeVersion = $.browser.version.split('.')[0];
                if (parseInt(chromeVersion, 10) >= 50) {
                    loadCSS('/point_of_sale/static/src/css/chrome50.css');
                }
            }

            if (this.env.pos.config.ifaceBigScrollbars) {
        ...
        _closeOtherTabs() {
            ...
            localStorage['message'] = JSON.stringify({
                message: 'closeTabs',
                session: this.env.pos.posSession.id,
            });
            ...
            window.addEventListener(
                ...
                (event) => {
                    if (event.key === 'message' && event.newValue) {
                        ...
                        if (
                            msg.message === 'closeTabs' &&
                            msg.session == this.env.pos.posSession.id
                        ) {
                            console.info(
        ...
        showCashMoveButton() {
            return this.env.pos && this.env.pos.config && this.env.pos.config.cashControl;
        }
         ...
         * the `errorHandlers` service when this component is properly mounted.
        ...
        _errorHandler(error, errorToHandle) {
            if (errorToHandle instanceof RPCError) {
                ...
                if (verpExceptionTitleMap.has(errorToHandle.exceptionName)) {
                    const title = verpExceptionTitleMap.get(errorToHandle.exceptionName).toString();
