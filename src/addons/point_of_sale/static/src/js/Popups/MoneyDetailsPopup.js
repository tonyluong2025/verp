verp.define('point_of_sale.MoneyDetailsPopup', function(require) {
    'use strict';
    ...
    class MoneyDetailsPopup extends PosComponent {
        ...
        updateMoneyDetailsAmount() {
            ...
            this.state.total = this.env.pos.roundDecimalsCurrency(total);
        }
        _closePopup() {
        ...
        confirm() {
            ...
            this.env.pos.bills.forEach(bill => {
                if (this.state.moneyDetails[bill.value]) {
                    moneyDetailsNotes += `  - ${this.state.moneyDetails[bill.value]} x ${this.env.pos.formatCurrency(bill.value)}\n`;
                }
            })
