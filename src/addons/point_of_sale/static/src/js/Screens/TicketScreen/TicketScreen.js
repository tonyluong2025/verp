verp.define('point_of_sale.TicketScreen', function (require) {
    ...
    class TicketScreen extends IndependentToOrderScreen {
        ...
        async _onDeleteOrder({ detail: order }) {
            ...
            if (['ProductScreen', 'PaymentScreen'].includes(screen.label) && order.getOrderlines().length > 0) {
        ...
        getFilteredOrderList() {
            ...
            const filterCheck = (order) => {
                if (this._state.ui.filter && this._state.ui.filter !== 'ACTIVE_ORDERS') {
                    const screen = order.getScreenData();
                    return this._state.ui.filter === this._getScreenToStatusMap()[screen.label];
                }
                return true;
            };
        ...
        getStatus(order) {
            if (order.locked) {
                ...
                const screen = order.getScreenData();
                return this._getOrderStates().get(this._getScreenToStatusMap()[screen.label]).text;
            }
        }
        /**
