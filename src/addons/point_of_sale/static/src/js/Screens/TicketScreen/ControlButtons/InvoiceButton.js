verp.define('point_of_sale.InvoiceButton', function (require) {
    'use strict';
    ...
    class InvoiceButton extends PosComponent {
        ...
        get isAlreadyInvoiced() {
            ...
            return Boolean(this.props.order.accountMove);
        }
        get commandName() {
            if (!this.props.order) {
        ...
        async _downloadInvoice(orderId) {
            try {
                const [orderWithInvoice] = await this.rpc({
                    method: 'read',
                    model: 'pos.order',
                    args: [orderId, ['accountMove']],
                    kwargs: { load: false },
                });
                if (orderWithInvoice && orderWithInvoice.accountMove) {
                    await this.env.pos.doAction('account.accountInvoices', {
                        additionalContext: {
        ...
        async _invoiceOrder() {
            ...
            if (!order.getClient()) {
                ...
                await this.rpc({
                    model: 'pos.order',
                    method: 'write',
                    args: [[orderId], { partnerId: newClient.id }],
