verp.define('point_of_sale.ReceiptScreen', function (require) {
    'use strict';

    const { Printer } = require('point_of_sale.Printer');
    const { isEmail } = require('web.utils');
    const { useRef, useContext } = owl.hooks;
    const { useErrorHandlers, onChangeOrder } = require('point_of_sale.customHooks');
    ...
    const ReceiptScreen = (AbstractReceiptScreen) => {
        class ReceiptScreen extends AbstractReceiptScreen {
            ...
            async onSendEmail() {
                if (!isEmail(this.orderUiState.inputEmail)) {
            ...
            get orderAmountPlusTip() {
                ...
                const tipProductId = this.env.pos.config.tipProductId && this.env.pos.config.tipProductId[0];
                const tipLine = order
                    .getOrderlines()
                    .find((line) => tipProductId && line.product.id === tipProductId);
                const tipAmount = tipLine ? tipLine.getAllPrices().priceWithTax : 0;
                const orderAmountStr = this.env.pos.formatCurrency(orderTotalAmount - tipAmount);
                if (!tipAmount) return orderAmountStr;
                const tipAmountStr = this.env.pos.formatCurrency(tipAmount);
            ...
            _shouldAutoPrint() {
                return this.env.pos.config.ifacePrintAuto && !this.currentOrder._printed;
            }
            _shouldCloseImmediately() {
                var invoicedFinalized = this.currentOrder.isToInvoice() ? this.currentOrder.finalized : true;
                return this.env.pos.proxy.printer && this.env.pos.config.ifacePrintSkipScreen && invoicedFinalized;
            }
            async _sendReceiptToCustomer() {
                ...
                const orderClient = { email: this.orderUiState.inputEmail, label: client ? client.label : this.orderUiState.inputEmail };
                const orderServerId = this.env.pos.validatedOrdersNameServerIdMap[orderName];
                if (!orderServerId) {
                ...
                await this.rpc({
                    model: 'pos.order',
                    method: 'actionReceiptToCustomer',
                    args: [[orderServerId], orderName, orderClient, ticketImage],
                });
            }
        }
