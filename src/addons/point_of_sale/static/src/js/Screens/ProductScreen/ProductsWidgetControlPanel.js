verp.define('point_of_sale.ProductsWidgetControlPanel', function(require) {
    'use strict';
    ...
    class ProductsWidgetControlPanel extends PosComponent {
        ...
        get displayCategImages() {
            return this.env.pos.config.ifaceDisplayCategImages && !this.env.isMobile;
        }
        updateSearch(event) {
        ...
        async loadProductFromDB() {
            ...
            try {
                let ProductIds = await this.rpc({
                    ...
                    method: 'search',
                    args: [['&',['availableInPos', '=', true], '|','|',
                     ['label', 'ilike', this.searchWordInput.el.value],
                     ['defaultCode', 'ilike', this.searchWordInput.el.value],
