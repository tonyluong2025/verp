verp.define('point_of_sale.NumpadWidget', function (require) {
    'use strict';
    ...
    class NumpadWidget extends PosComponent {
        ...
        get hasPriceControlRights() {
            const cashier = this.env.pos.get('cashier') || this.env.pos.getCashier();
            return (
                (!this.env.pos.config.restrictPriceControl || cashier.role == 'manager') &&
                !this.props.disabledModes.includes('price')
            );
        }
        get hasManualDiscount() {
            return this.env.pos.config.manualDiscount && !this.props.disabledModes.includes('discount');
        }
        changeMode(mode) {
        ...
        get decimalSeparator() {
            return this.env._t.database.parameters.decimalPoint;
        }
    }
