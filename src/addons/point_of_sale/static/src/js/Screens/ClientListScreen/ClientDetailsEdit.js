verp.define('point_of_sale.ClientDetailsEdit', function(require) {
    'use strict';
    ...
    class ClientDetailsEdit extends PosComponent {
        constructor() {
            super(...arguments);
            this.intFields = ['countryId', 'stateId', 'propertyProductPricelist'];
            const partner = this.props.partner;
            this.changes = {
                'countryId': partner.countryId && partner.countryId[0],
                'stateId': partner.stateId && partner.stateId[0],
            };
            if (!partner.propertyProductPricelist)
                this.changes['propertyProductPricelist'] = this.env.pos.defaultPricelist.id;
        }
        mounted() {
        ...
        get partnerImageUrl() {
            // We prioritize image1920 in the `changes` field because we want
            // to show the uploaded image without fetching new data from the server.
            const partner = this.props.partner;
            if (this.changes.image1920) {
                return this.changes.image1920;
            } else if (partner.id) {
                return `/web/image?model=res.partner&id=${partner.id}&field=avatar128&updatedAt=${partner.updatedAt}&unique=1`;
            } else {
                return false;
            }
        }
        ...
        saveChanges() {
            ...
            if ((!this.props.partner.label && !processedChanges.label) ||
                processedChanges.label === '' ){
        ...
        async uploadImage(event) {
            ...
            if (!file.type.match(/image.*/)) {
                ...
                if (loadedImage) {
                    ...
                    this.changes.image1920 = resizedImage.toDataURL();
