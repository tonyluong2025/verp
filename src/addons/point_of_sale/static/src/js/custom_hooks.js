verp.define('point_of_sale.customHooks', function (require) {
    'use strict';
    ...
    function onChangeOrder(prevOrderCB, newOrderCB) {
        ...
        onMounted(() => {
            ...
            newOrderCB(current.env.pos.getOrder());
        });
        onWillUnmount(() => {
            current.env.pos.off('change:selectedOrder', null, current);
            prevOrderCB(current.env.pos.getOrder());
        });
    }

    function useBarcodeReader(callbackMap, exclusive = false) {
        const current = Component.current;
        const barcodeReader = current.env.pos.barcodeReader;
        ...
        onMounted(() => {
            if (barcodeReader) {
                for (let key in callbackMap) {
                    if (exclusive) {
                        barcodeReader.setExclusiveCallback(key, callbackMap[key]);
                    } else {
                        barcodeReader.setActionCallback(key, callbackMap[key]);
                    }
                }
            }
        });
        onWillUnmount(() => {
            if (barcodeReader) {
                for (let key in callbackMap) {
                    if (exclusive) {
                        barcodeReader.removeExclusiveCallback(key, callbackMap[key]);
                    } else {
                        barcodeReader.removeActionCallback(key, callbackMap[key]);
                    }
                }
            }
        });
    ...
    function useValidateCashInput(inputRef, startingValue) {
        ...
        const current = Component.current;
        const decimalPoint = current.env._t.database.parameters.decimalPoint;
        const thousandsSep = current.env._t.database.parameters.thousandsSep;
