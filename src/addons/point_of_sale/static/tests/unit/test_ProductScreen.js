verp.define('point_of_sale.tests.ProductScreen', function (require) {
    'use strict';
    ...
    QUnit.test('NumpadWidget', async function (assert) {
        ...
        pos.config.restrictPriceControl = true;
    ...
    QUnit.test('ProductsWidgetControlPanel', async function (assert) {
        ...
        const old_config = pos.config;
        // set dummy config
        pos.config = { ifaceDisplayCategImages: false };
        ...
        pos.config.ifaceDisplayCategImages = true;
    ...
    QUnit.test('ProductList, ProductItem', async function (assert) {
        ...
        const dummyProducts = [
            { id: 0, displayName: 'Burger', price: '$10' },
            { id: 1, displayName: 'Water', price: '$2' },
            { id: 2, displayName: 'Chair', price: '$25' },
        ];
        ...
        class Parent extends PosComponent {
            ...
            _clickProduct({ detail: product }) {
                assert.step(product.displayName);
            }
        }
        Parent.env = makePosTestEnv();
        Parent.template = xml/* html */ `
        ...
        const product1el = parent.el.querySelector(
            'article.product[aria-labelledby="articleProduct_1"]'
        );
    ...
    QUnit.test('Orderline', async function (assert) {
        ...
        class Parent extends PosComponent {
            ...
            get line() {
                return this.env.pos.getOrder().getOrderlines()[0];
            }
            _selectLine() {
            ...
            willUnmount() {
                this.order.removeOrderline(this.line);
            }
        }
        Parent.env = makePosTestEnv();
        Parent.template = xml/* html */ `
        ...
        const [chair1, chair2] = Parent.env.pos.db.searchProductInCategory(0, 'Office Chair');
    ...
    QUnit.test('OrderWidget', async function (assert) {
        ...
        const order1 = parent.env.pos.getOrder();
        order1.addProduct(chair1);
        await testUtils.nextTick();
        ...
        const order2 = parent.env.pos.addNewOrder();
        await testUtils.nextTick();
        assert.notOk(parent.el.querySelector('.summary'));
        assert.ok(parent.el.querySelector('.order-empty'));

        // add line to the current order
        order2.addProduct(chair2);
        await testUtils.nextTick();
        assert.ok(parent.el.querySelector('.summary'));
        assert.notOk(parent.el.querySelector('.order-empty'));

        parent.env.pos.deleteCurrentOrder();
        parent.env.pos.deleteCurrentOrder();
