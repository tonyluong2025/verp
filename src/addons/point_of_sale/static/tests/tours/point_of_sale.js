/* global posmodel */
verp.define('point_of_sale.tour.pricelist', function (require) {
    "use strict";
    ...
    function compare_backend_frontend (product, pricelist_name, quantity) {
        return function () {
            ...
            var frontend_price = product.getPrice(pricelist, quantity);
            // ORM applies digits= on non-stored computed field when
            // reading. It does not however truncate like it does when
            // storing the field.
            frontend_price = roundDi(frontend_price, posmodel.dp['Product Price']);
    ...
    var steps = [{ // Leave category displayed by default
        ...
        extraTrigger: 'body .pos:not(:has(.loader))', // Pos has finished loading
        trigger: '.o_loading_indicator:not(.o_loading)', // WebClient has finished Loading
        run: function () {
            var product_wall_shelf = posmodel.db.searchProductInCategory(0, 'Wall Shelf Unit')[0];
            var product_small_shelf = posmodel.db.searchProductInCategory(0, 'Small Shelf')[0];
            var product_magnetic_board = posmodel.db.searchProductInCategory(0, 'Magnetic Board')[0];
            var product_monitor_stand = posmodel.db.searchProductInCategory(0, 'Monitor Stand')[0];
            var product_desk_pad = posmodel.db.searchProductInCategory(0, 'Desk Pad')[0];
            var product_letter_tray = posmodel.db.searchProductInCategory(0, 'Letter Tray')[0];
            var product_whiteboard = posmodel.db.searchProductInCategory(0, 'Whiteboard')[0];
            ...
            compare_backend_frontend(product_letter_tray, 'Public Pricelist', 0, undefined)()
                ...
                .then(compare_backend_frontend(product_wall_shelf, 'minQuantity ordering', 1, undefined))
                .then(compare_backend_frontend(product_wall_shelf, 'minQuantity ordering', 2, undefined))
    ...
    steps = steps.concat([{
        ...
        content: "select fixed pricelist",
        trigger: ".selection-item:contains('minQuantity ordering')",
    }, {
        ...
        extraTrigger: ".order-container .orderlines .orderline.selected .product-name:contains('Wall Shelf') ~ .info-list .info em:contains('2.0')",
        run: function() {},
    }, {
        ...
        extraTrigger: ".order-container .orderlines .orderline.selected .product-name:contains('Small Shelf') ~ .info-list .info em:contains('1.0')",
        run: function() {}
    }, {
        ...
        trigger: ".order-container .orderlines .orderline.selected .product-name:contains('Small Shelf')",
        extraTrigger: ".order-container .orderlines .orderline.selected .product-name:contains('Small Shelf') ~ .price:contains('5.0')",
    }, {
...
verp.define('point_of_sale.tour.acceptance', function (require) {
    "use strict";

    var Tour = require("web_tour.tour");

    function add_product_to_order(productName) {
        return [{
            content: 'buy ' + productName,
            trigger: '.product-list .product-name:contains("' + productName + '")',
        }, {
            content: 'the ' + productName + ' have been added to the order',
            trigger: '.order .product-name:contains("' + productName + '")',
            run: function () {},
        }];
    }
    ...
    function selected_payment_has(name, val) {
        return [{
            ...
            extraTrigger: `.paymentlines .paymentline.selected .payment-name:contains("${name}") ~ .payment-amount:contains("${val}")`,
            run: function () {},
        }]
    }
