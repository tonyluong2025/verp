 ...
 * 1 year          month
 * 1 year          quarter
 * 1 year          year    # we are not using a greater time period in Verp (yet)
 ...
 * @param {string} minGranularity granularity of the smallest time interval used in Verp for this
 *                                type
 */
 ...
 * field, in order to configure the _readGroupFillTemporal (see core models.js)
...
class FillTemporalPeriod {
     ...
     *                           this is the `resModel` from the action (i.e. `crm.lead`)
     * @param {Object} field a dictionary with keys "name" and "type".
    *                        name: Name of the field on which the fillTemporal should apply
    *                              (i.e. 'dateDeadline')
    *                        type: 'date' or 'datetime'
    ...
    /**
     * Compute the moment for the end of the fillTemporal period. This bound is exclusive.
     * The fillTemporal period is the number of [granularity] from [start] to the end of the
     ...
     *                                         constraint to limit readGroup results or not
     ...
     * @param {boolean} [forceFillingFrom=true] fillTemporal must apply from:
     *                                          true: this.start
     *                                          false: the first group with at least one record
     * @param {boolean} [forceFillingTo=!this.computedEnd] fillTemporal must apply until:
     *                                          true: this.end
    ...
    getContext({context, forceFillingFrom=true, forceFillingTo=!this.computedEnd}) {
        ...
        if (forceFillingFrom) {
            fillTemporal.fillFrom = this._getFormattedServerDate(this.start);
        }
        if (forceFillingTo) {
            fillTemporal.fillTo = this._getFormattedServerDate(
                moment(this.end).subtract(1, FIELD_TYPE_TABLE[this.field.type].minGranularity)
            );
        }
        context = {...context, fillTemporal: fillTemporal};
        return context;
    }
    /**
     * @param {integer} minGroups minimum amount of groups to display, regardless of other
    ...
    /**
     * Adds one "granularity" period to [this.end], to expand the current fillTemporal period
     */
    expand() {
...
/**
 * fillTemporal Service
 *
 * This service will be used to generate or retrieve fillTemporal periods
 *
 * A specific fillTemporal period configuration will always refer to the same instance
 ...
 */
 const FillTemporalService = AbstractService.extend({
    ...
    /**
     * Get a fillTemporal period according to the configuration.
     * The default initial fillTemporal period is the number of [granularity] from [start]
     ...
     * Once created, a fillTemporal period for a specific configuration will be stored
     ...
     *                             this is the `resModel` from the action (i.e. `crm.lead`)
     * @param {Object} [field] a dictionary with keys "name" and "type".
     * @param {string} [field.name] name of the field on which the fillTemporal should apply
     *                              (i.e. 'dateDeadline')
     ...
     * @param {boolean} [forceRecompute=false] optional whether the fillTemporal period should be
     *                                         reinstancied
