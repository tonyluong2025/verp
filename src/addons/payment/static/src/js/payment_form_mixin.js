verp.define('payment.paymentFormMixin', require => {
    'use strict';
    ...
    return {
        ...
        _displayError: function (title, description = '', error = '') {
            ...
            if ($checkedRadios.length !== 1) { // Cannot find selected payment option, show dialog
                ...
                let errorHtml = `<div class="alert alert-danger mb4" name="oPaymentError">
                ...
                const $inlineForm = this.$(`#oPayment_${formType}_inlineForm_${paymentOptionId}`);
                $inlineForm.removeClass('d-none'); // Show the inline form even if it was empty
                $inlineForm.append(errorHtml).find('div[name="oPaymentError"]')[0]
        ...
        _enableButton: function () {
            if (this._isButtonReady()) {
                ...
                $submitButton.find('span.o-loader').remove();
                return true;
            }
            return false;
        },
         ...
         */
        _hideError: () => this.$('div[name="oPaymentError"]').remove(),
         ...
         * @return {undefined}.
         */
        _hideInlineForms: () => this.$('[name="oPaymentInlineForm"]').addClass('d-none'),
        ...
        _hideInputs: function () {
            const $submitButton = this.$('button[name="oPaymentSubmitButton"]');
            const $tokenizeCheckboxes = this.$('input[name="oPaymentSaveAsToken"]');
        ...
        _prepareTransactionRouteParams: function (provider, paymentOptionId, flow) {
            return {
                ...
                'referencePrefix': this.txContext.referencePrefix !== undefined
                ...
                'invoiceId': this.txContext.invoiceId
                    ? parseInt(this.txContext.invoiceId) : null,
                'flow': flow,
                'tokenizationRequested': this.txContext.tokenizationRequested,
                'landingRoute': this.txContext.landingRoute,
                'isValidation': this.txContext.isValidation,
                'accessToken': this.txContext.accessToken
        ...
        _processRedirectPayment: (provider, acquirerId, processingValues) => {
            ...
            const $redirectForm = $(processingValues.redirectFormHtml).attr(
                'id', 'oPaymentRedirectForm'
            );
        ...
        _setPaymentFlow: function (flow = 'redirect') {
            if (flow !== 'redirect' && flow !== 'direct' && flow !== 'token') {
                console.warn(
                    `paymentFormMixin: method '_setPaymentFlow' was called with invalid flow: 
        ...
        _onClickLessPaymentIcons: ev => {
            ...
            $itemList.find('a[name="oPaymentIconMore"]').parents('li').removeClass('d-none');
        },
        ...
        _onClickPaymentOption: function (ev) {
            ...
            this.$('input[name="oPaymentRadio"]').prop('checked', false);
            // Check radio button linked to selected payment option
            const checkedRadio = $(ev.currentTarget).find('input[name="oPaymentRadio"]')[0];
