verp.define('payment.manageForm', require => {
    'use strict';
    ...
    publicWidget.registry.PaymentManageForm = publicWidget.Widget.extend(paymentFormMixin, {
        selector: 'form[name="oPaymentManage"]',
        events: Object.assign({}, publicWidget.Widget.prototype.events, {
            ...
            'click button[name="oPaymentSubmitButton"]': '_onClickSaveToken',
            'click button[name="oPaymentDeleteToken"]': '_onClickDeleteToken',
            'submit': '_onSubmit',
        }),
        ...
        _deleteToken: function (tokenId) {
            const execute = () => {
                this._rpc({
                    route: '/payment/archiveToken',
                    params: {
                        'tokenId': tokenId,
                    },
                }).then(() => {
                    const $tokenCard = this.$(
                        `input[name="oPaymentRadio"][data-payment-option-id="${tokenId}"]` +
                        `[data-payment-option-type="token"]`
                    ).closest('div[name="oPaymentOptionCard"]');
                    $tokenCard.siblings(`#oPaymentTokenInlineForm${tokenId}`).remove();
                    $tokenCard.remove();
        ...
        _onClickDeleteToken: function (ev) {
            ...
            const linkedRadio = $(ev.currentTarget).siblings().find('input[name="oPaymentRadio"]')[0];
